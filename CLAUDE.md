...
| [docs/plans/HANDOFF_UX_UI_AUDIT_2026-09-11.md](docs/plans/HANDOFF_UX_UI_AUDIT_2026-09-11.md) | 🔍 **ผลตรวจ UX/UI ทั้งเว็บ + แผนส่งต่อ** · วัดจริงบน production ทุกตัวเลข · ✅ **P0 ปิดครบ 3/3 แล้วใน PR #416** (สายด่วน 1323/1669 คอนทราสต์ 3.11/2.81 → **6.51/6.58** · หน้าแรกได้ landmark คืน · `h1` กลับมาเป็นหัวข้อแรก) · พร้อมด่าน CI `test-a11y-critical.ts` คุ้มกัน · 🔴 **เหลือ 17 ข้อ** — งานด่วนที่สุดคือ **`test-palette-drift.ts` ก่อนเริ่มกวาดสี** · หนี้ที่วัดได้: สีฮาร์ดโค้ด 4,232 จุด (**โตขึ้น 81 จุดในวันเดียว** เพราะไม่มีด่านกัน) · `<Button>` กลางใช้แค่ 12/67 ไฟล์ · 17 ช่องกรอกไร้ label · 5/7 หน้าต่างไม่มี Esc/focus trap · 49 เส้นทางไม่มี `loading.tsx` · ด่านกันไทยรั่วในหน้า EN มีอยู่สองตัวแต่**ตกไม่ได้ทั้งคู่** · มีภาคผนวก "ตรวจแล้วผ่าน" กันรอบหน้าเสียเวลาซ้ำ |
...
13. **Auto-Merge Enforcement**: เปิด PR ต้องใช้ `npm run pr:auto` เสมอ เพื่อให้ CI ตรวจ 48 ด่าน ➔ Auto-Merge (Squash) ➔ Auto-Deploy Cloudflare Workers
...
- `npm run repo:verify` — ตรวจครบทั้ง 48 ด่าน (ใช้หลัก)
