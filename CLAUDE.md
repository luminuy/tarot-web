...
| 🚧 | [🧠 แผนยกระดับ "ความแม่น" ของคำอ่าน + "ภาษาไทยที่ถูกต้อง" ของแม่หมอ AI](docs/plans/HANDOFF_AI_ACCURACY_THAI_2026-09-07.md) | คลื่น A + B ลงโค้ดครบ · ผลวัด ai:judge ของ `20260911-2` ได้คะแนนแค่ 6/27 เคส (4.83/5) ยังเทียบกับรุ่นก่อนไม่ได้ (ดู HANDOFF_AI_JUDGE_BASELINE) · 📌 **อ่านก่อนแตะ**: prompt / ภาษาไทยของแม่หมอ — กติกาภาษาไทยฉบับบ้านนี้ + rubric ของ LLM Judge · _ตรวจ 2026-09-24_ |
| 🚧 | [🧠 แผนแม่บทยกระดับความฉลาดของแม่หมอ AI](docs/plans/AI_INTELLIGENCE_PLAN.md) | คลื่น 1 ลงแล้ว (#245) · งานต่อยอดย้ายไปแผน HANDOFF_AI_ACCURACY_THAI · ผลวัด ai:judge ของ prompt รุ่นล่าสุดได้คะแนนบางส่วน (6 เคส) ยังต้องรันให้ครบ · _ตรวจ 2026-09-24_ |
| ⏸️ | [🔑 ชีทส่งต่อ — รัน `ai:judge` เก็บ baseline `20260911-1` แล้วลุยคลื่น B ต่อ](docs/plans/HANDOFF_AI_JUDGE_BASELINE_2026-09-11.md) | รันรอบ 2026-09-24 (หลังโควตารีเซ็ต) ได้คำอ่านจากโมเดลจริง 27/30 เคส (คำอ่านสำรอง 3 เคสถูกตัดออกถูกต้อง) แต่ผู้ตัดสินให้คะแนนได้แค่ 6 เคส (4.83/5 · ไทยจากโค้ด 95/100) — ผู้ตัดสิน `gemini-3.6-flash` ชน 429 โควตารายวันซึ่งใช้ร่วมกับเว็บจริง · ทับกับ baseline `20260911-1` 0 เคส ➔ ยังสรุปไม่ได้ว่าดีขึ้น/แย่ลง · ต้องการ: โควตาผู้ตัดสินแยกจากเว็บจริง (เปิด billing Gemini) หรือให้ผู้ตัดสินสลับไปโมเดลอื่นเมื่อโดน 429 แล้วรันใหม่ · _ตรวจ 2026-09-24_ |
...
| ✅ | [✦ แผนทำให้เว็บ "สมูทและไว" ระดับโลก — ทุกหน้า ทุกจังหวะ (2026-09-06)](docs/plans/HANDOFF_SMOOTH_FAST_2026-09-06.md) | เงาแยกชั้น · View Transition (เฉพาะจอที่ใช้เมาส์ — มือถือปิดเพราะทำจอว่างแวบ INC-0244) · สำรับออกจากบันเดิล (#338 · #345) · S-03 ปิดด้วย `card-tile-cv` (ไพ่ครบ 78 ใบใน HTML ตามข้อห้าม SEO แต่ข้ามการวาดนอกจอ — `/cards` TBT ~4,000 ➔ ~100 ms · HANDOFF_CARD_TILE_CV) · _ตรวจ 2026-09-24_ |
...
| ✅ | [💸 แผนคุมต้นทุน AI + รหัสทดสอบข้าม rate limit — handoff ให้ทีม Gemini](docs/plans/AI_COST_CONTROL_PLAN.md) | เพดานงบ AI รวมทั้งระบบ (`src/lib/security/ai-budget.ts`) + เพดานถี่ข้าม isolate แบบ atomic บน D1 `edge_rate_buckets` (migrations/0018 · `consumeEdgeRateLimits`) ครบทุกเส้นเปิดไพ่ `/start` · `/shuffle` · `/read` · `/chat` · `/clarify` · `checkRateLimit` ในหน่วยความจำเหลือเป็นแค่ด่านเร็ว/กันกดซ้อน · _ตรวจ 2026-09-23_ |
...
13. **Auto-Merge Enforcement**: เปิด PR ต้องใช้ `npm run pr:auto` เสมอ เพื่อให้ CI ตรวจ 82 ด่าน ➔ Auto-Merge (Squash) ➔ Auto-Deploy Cloudflare Workers
...
- `npm run repo:verify` — ตรวจครบทั้ง 82 ด่าน (ใช้หลัก)
