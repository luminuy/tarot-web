...
// ───────────────────────────────────────────────────────────────
// 2.6 ลิ้นชักนำทาง: ปิดอยู่ต้องเป็นหน่วยคงที่ · เปิดอยู่ค่อยเป็น `dvh` (INC-0209 ➜ INC-0210)
//
...
// ความสูงต้องบอกด้วยหน่วยจอเอง — และ "หน่วยไหน" ขึ้นกับว่าเปิดอยู่หรือปิดอยู่:
//
//   ปิดอยู่ (`-base`) ➔ ต้อง `100lvh` เท่านั้น
//     กล่องสองใบนี้อยู่ใน DOM ของ *ทุกหน้า* ตลอดเวลา (ปิดอยู่ก็แค่ `visibility: hidden`
//     ซึ่งยังถูกคิด layout) ถ้าความสูงผูกกับ `dvh` ทุกครั้งที่แถบ URL ของ iOS เลื่อน
//     เข้าออกระหว่างสกรอลล์ เบราว์เซอร์ต้องคิด layout กล่องเต็มจอสองใบใหม่ทั้งที่
//     ไม่มีใครเปิดเมนู = ทั้งเว็บหน่วง (เจ้าของรายงาน "แลคมาก" หลัง deploy INC-0209)
//     `lvh` เป็นค่าคงที่ตามสเปกและเผื่อสูงสุดไว้แล้ว จึงไม่มีวันรีโฟลว์
//     `svh` ก็คงที่แต่เตี้ยไป ➔ กลับไปเป็นบั๊ก INC-0209 (เมนูจบกลางอากาศ)
//
//   เปิดอยู่ (`-entering`) ➔ ต้อง `100dvh`
//     ตอนเปิดหน้าไม่เลื่อน แถบ URL จึงไม่ขยับ ใช้ค่าที่พอดีจอจริงได้โดยไม่มีรีโฟลว์
//     ถ้าปล่อยเป็น `lvh` ท้ายลิ้นชัก (ปุ่มบัญชีของฉัน) จะมุดใต้แถบ URL ตอนแถบกางเต็ม
//
// Chrome เดสก์ท็อปให้ lvh = svh = dvh เท่ากันหมด ทั้งสองอาการจึงไม่มีทางโผล่ตอนรีวิว
const DRAWER_HEIGHT_RULES: ReadonlyArray<readonly [string, string]> = [
  ["nav-drawer-panel-base", "100lvh"],
  ["nav-drawer-scrim-base", "100lvh"],
  ["nav-drawer-panel-entering", "100dvh"],
  ["nav-drawer-scrim-entering", "100dvh"],
];
for (const [cls, unit] of DRAWER_HEIGHT_RULES) {
  const rule = css.match(new RegExp(`\\.${cls}\\s*\\{([^}]*)\\}`));
  if (!rule) {
    failures.push(`globals.css: ไม่พบบล็อก \`.${cls} { ... }\` — ลิ้นชักนำทางต้องมีกฎนี้ (INC-0210)`);
    continue;
  }
  const height = rule[1].match(/height:\s*([^;]+);/);
  if (!height) {
    failures.push(
      `globals.css: \`.${cls}\` ต้องกำหนด \`height: ${unit}\` ให้ชัด (INC-0209 ➜ INC-0210)`,
    );
  } else if (height[1].trim() !== unit) {
    failures.push(
      `globals.css: \`.${cls}\` ต้องสูง \`${unit}\` (พบ: "${height[1].trim()}") — ` +
        (unit === "100lvh"
          ? "หน่วยที่ไม่คงที่ (`dvh`) ที่กฎตอนปิด ทำให้ทั้งเว็บรีโฟลว์ทุกครั้งที่แถบ URL ของ iOS ขยับ (INC-0210)"
          : "ตอนเปิดต้องพอดีจอจริง ไม่งั้นท้ายลิ้นชักมุดใต้แถบ URL (INC-0209)"),
    );
  }
}
...
//       และตามการ import ลึกไม่เกิน 3 ชั้น (หน้าแรกเป็นของ Astro · TarotFlow เรนเดอร์หัวเว็บเอง)
//
...
const INTENTIONALLY_BARE: { route: string; reason: string }[] = [
  ...
  {
    route: "en/reading/chat",
    reason: "ห้องแชทภาษาอังกฤษ — มีแถบหัวบางของตัวเองพร้อมปุ่มกลับหน้าคำทำนาย (INC-0109) หัวเว็บเต็มจะกินพื้นที่จอสนทนา",
  },
 ...
 * (เช่น page ➔ คอมโพเนนต์หน้า ➔ SiteHeader)
 *
...
if (outputDirs.every((dir) => fs.existsSync(dir))) {
  ...
  for (const page of renderedPages) {
    ...
    //
    // ⚠️ เกิดจริงบนหน้าแรก (INC-0136b): บล็อกเนื้อหา SEO ถูกวางไว้ท้าย fragment ของ
