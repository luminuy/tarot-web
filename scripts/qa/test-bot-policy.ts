 ...
 *  6. `fight_mode` ต้องเป็น `false` — Bot Fight Mode ฉีดสคริปต์ตรวจบอตเข้า HTML ทุกหน้า
 *     ซึ่งกินเธรดหลัก 3.3–5.4 วินาที และทำให้คะแนน perf ตก 25–32 จุดทุกหน้า (INC-0177)
 *     ของที่กันค่าใช้จ่ายจริงอยู่ที่ rate limit + กฎ WAF ทั้งหมด ไม่ได้อยู่ในสวิตช์นี้เลย
...
async function run(): Promise<void> {
  ...
  // ── กฎ 6: Bot Fight Mode ต้องปิดอยู่ (INC-0177) ─────────────────────────
  // เปิดเมื่อไหร่ Cloudflare จะฉีด jsd/main.js เข้า HTML ทุกหน้าที่ผู้ใช้จริงได้รับ
  // วัดจริงบน production 2026-09-15: กินเธรดหลัก 3,271–5,394 ms · คะแนน perf ตก 25–32 จุดทุกหน้า
  const fightModes = [...cfCodeOnly.matchAll(/fight_mode:\s*(true|false)/g)].map((m) => m[1]);

  if (fightModes.length === 0) {
    errors.push(
      "ไม่พบการตั้ง `fight_mode` ใน scripts/cloudflare-phase1.ts\n" +
        "    💡 ต้องตั้งเป็น false ไว้อย่างชัดเจน — ถ้าปล่อยว่าง สคริปต์จะไม่ไปปิดของที่เปิดค้างไว้บนโซนจริง",
    );
  }

  for (const value of fightModes) {
    if (value !== "false") {
      errors.push(
        "`fight_mode` ถูกตั้งเป็น true ใน scripts/cloudflare-phase1.ts\n" +
          "    Bot Fight Mode ฉีดสคริปต์ /cdn-cgi/challenge-platform/scripts/jsd/main.js เข้า HTML ทุกหน้า\n" +
          "    วัดจริงบน production 2026-09-15: perf / 52➔81 · /cards 56➔81 · /blog 52➔84 เมื่อบล็อกสคริปต์นั้นทิ้ง\n" +
          "    และของที่กันค่าใช้จ่ายจริง (rate limit · บล็อก UA เครื่องมือสคริปต์ · บล็อก URL ขยะ · บอตเทรนโมเดล)\n" +
          "    ไม่มีข้อไหนอยู่ใน Bot Fight Mode เลยสักข้อ\n" +
          "    💡 ถ้าจะเปิดกลับจริง ๆ ต้องแนบตัวเลขสองด้าน: บอตที่กันได้เพิ่มกี่คำขอ (Security Events)\n" +
          "       เทียบกับคะแนนที่เสีย (Lighthouse บน production) แล้วแก้ทั้งสคริปต์และด่านนี้พร้อมกัน",
      );
    }
  }
