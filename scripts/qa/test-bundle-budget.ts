...
export const BUDGETS: RouteBudget[] = [
  ...
  },
  {
    ...
    expectLcpImage: true,
    /*
     * 🪶 **ไม่โหลด React เลยสักไบต์แล้ว** — รัดเพดานลงตามของจริงทันที (2026-09-18)
     *
     * ⚠️ เพดานเดิมคือ 119 KB ซึ่งเป็นตัวเลขของยุคที่หน้านี้ยัง hydrate อยู่ · หลังถอด
     *    island ออกแล้วของจริงเหลือ 5.1 KB เพดานเดิมจึงกลายเป็นด่านที่ "ผ่านเสมอ"
     *    แม้มีคนเผลอใส่ `client:*` กลับเข้าไปจนหน้ากลับไปหนัก 100 KB ก็ยังขึ้น ✅
     *    — ตระกูลเดียวกับบทเรียนที่เขียนไว้ข้างล่างในส่วนวัด JS
     *
     * ⚠️ ตัวเลขนี้คือ "ของจริงที่วัดได้ + ระยะหายใจ" ตามหลัก ratchet ของบ้านนี้
     *    ห้ามขยับขึ้นเพื่อให้ผ่าน ให้ไปลดของจริงก่อนเสมอ
     */
    maxJsGzipKb: 8, // วัดจริง 2026-09-18: 5.1 KB
    maxHtmlGzipKb: 18, // วัดจริง 2026-09-18: 14.2 KB
  },
  {
    route: "/blog",
    expectLcpImage: true,
  ...
  },
  {
     ...
     * ไม่มี React runtime + router + เพย์โหลด RSC ติดมากับทุกหน้าอีกต่อไป
     *
     * 🔁 **2026-09-17 (R-02): เลิก hydrate คลังผังทั้งก้อน** — การ์ดทั้ง 25 ใบเรนเดอร์เป็น
     * HTML ตั้งแต่ตอนบิลด์ · แท็บทำงานด้วยสคริปต์ ~1 KB (`astro/scripts/spreads-tabs.ts`)
     * ปุ่มขยายใช้ `<details>` ของเบราว์เซอร์
     *
     * **นี่คือการแลกที่ตั้งใจ และวัดมาแล้วทั้งสองฝั่ง** (บิลด์จริงบนเครื่อง dev):
     *   HTML  20.0 ➔ 33.8 KB gzip  (+13.8 — เพราะเรนเดอร์ครบ 25 ใบ ไม่ใช่ 6 ใบ)
     *   JS   124.4 ➔ 100.1 KB gzip (−24.3 — ข้อมูลผัง 17.2 KB + island หายไปทั้งก้อน)
     *   รวมที่ผู้ใช้ต้องโหลด: **−10.5 KB** และงาน hydration ก้อนยาว 687 ms หายไปด้วย
     *   ของแถม: ลิงก์ผังใน HTML ดิบ 6 ➔ 25 เส้น (เนื้อหาทุกผังถึงบอตตั้งแต่ไบต์แรก)
     *
     * ⚠️ เพดาน HTML ที่ขยับขึ้นข้อนี้ **ห้ามใช้เป็นข้ออ้างให้ขยับข้ออื่น** — ขยับได้เพราะ
  ...
  },
  {
     ...
     * 🪶 **ไม่โหลด React เลยสักไบต์แล้ว** — รัดเพดานลงตามของจริงทันที (2026-09-18)
     *
     * ⚠️ เพดานเดิมคือ 136 KB ซึ่งเป็นตัวเลขของยุคที่หน้านี้ยัง hydrate อยู่ · หลังถอด
     *    island ออกแล้วของจริงเหลือ 4.7 KB เพดานเดิมจึงกลายเป็นด่านที่ "ผ่านเสมอ"
     *    แม้มีคนเผลอใส่ `client:*` กลับเข้าไปจนหน้ากลับไปหนัก 100 KB ก็ยังขึ้น ✅
     *    — ตระกูลเดียวกับบทเรียนที่เขียนไว้ข้างล่างในส่วนวัด JS
     *
     * ⚠️ ตัวเลขนี้คือ "ของจริงที่วัดได้ + ระยะหายใจ" ตามหลัก ratchet ของบ้านนี้
     *    ห้ามขยับขึ้นเพื่อให้ผ่าน ให้ไปลดของจริงก่อนเสมอ
     */
    maxJsGzipKb: 8, // วัดจริง 2026-09-18: 4.7 KB
    maxHtmlGzipKb: 16, // วัดจริง 2026-09-18: 12.5 KB
  },
  {
    /*
     * หน้านโยบายความเป็นส่วนตัว — ตัวแทนของ `/privacy` กับ `/en/privacy`
     *
     * ⚠️ เพิ่มเข้ามาเพราะ **ไม่เคยมีด่านคุมสองหน้านี้เลย** จึงไม่มีใครเห็นว่ามันโหลด
     *    React ทั้งก้อนอยู่หลายเดือนเพื่อปุ่มลบข้อมูล PDPA ปุ่มเดียว (72.9 KB gzip)
     *    ตอนนี้ปุ่มเป็น HTML ล้วน + สคริปต์ ~1 KB แล้ว ด่านนี้มีไว้กันมันกลับไปเป็นเหมือนเดิม
     */
    route: "/privacy",
    maxJsGzipKb: 8, // วัดจริง 2026-09-18: 5.6 KB (เดิม 72.9 KB)
    maxHtmlGzipKb: 16, // วัดจริง 2026-09-18: 12.9 KB
  },
  {
    /*
     * หน้าผังตามหมวดชีวิต — ตัวแทนของ 12 หน้า (`/spreads/topic/*` และ `/en/spreads/topic/*`)
     *
     * ⚠️ เพิ่มเข้ามาด้วยเหตุผลเดียวกับ `/privacy` — กลุ่มนี้ไม่เคยถูกวัดเลยทั้งที่มี 12 หน้า
     *    เคยโหลด 102.6 KB gzip เพื่อหีบเพลง "ความหมายตำแหน่งไพ่" ซึ่งตอนนี้เป็น `<details>`
     *    ของเบราว์เซอร์ที่ไม่ใช้ JS สักบรรทัด
     *
     * ⚠️ งบ HTML สูงกว่าหน้าอื่นโดยตั้งใจ: เนื้อหาทั้งสองสถานะของหีบเพลงถูกเรนเดอร์ลง HTML
     *    ครบทุกใบ (นั่นคือสิ่งที่แลกมากับการไม่ต้องใช้ JS) · หมวดที่ผังเยอะสุดคือ `love`
     */
    route: "/spreads/topic/love",
    maxJsGzipKb: 8, // วัดจริง 2026-09-18: 4.7 KB (เดิม 102.6 KB)
    maxHtmlGzipKb: 21, // วัดจริง 2026-09-18: 16.7 KB
  },
  {
    route: "/cards/all",
    /*
  ...
  {
    /*
     * หน้าเลือกกองไพ่ — ตัวแทนของ `/pick-a-card`, `/en/pick-a-card` และหน้าหัวข้ออีก 16 หน้า
     *
     * ⚠️ เพิ่มเข้ามาในคลื่นที่ 2 (2026-09-18) เพราะกลุ่มนี้ **ไม่เคยถูกวัดเลย** ทั้งที่มี 18 หน้า
     *    และเพิ่งได้ท่อ AI + กล่องสิทธิ์เข้าไปในหน้า (ทั้งคู่โหลดแบบ lazy)
     */
    route: "/pick-a-card",
    /*
     * ⚠️ ตัวเลขนี้คือ "ของจริงที่วัดได้ + ระยะหายใจ" ตามหลัก ratchet ของบ้านนี้
     *    ห้ามขยับขึ้นเพื่อให้ผ่าน ให้ไปลดของจริงก่อนเสมอ
     *
     * ✅ **ISSUE-050 ปิดแล้ว (2026-09-21)** — คลังคำอ่านที่เขียนไว้ล่วงหน้าของทั้ง 8 หัวข้อ
     *    (ไทย+อังกฤษ ≈1,600 บรรทัด) ย้ายไป `src/data/pick-a-card-readings.ts` ซึ่ง
     *    **ฝั่งเซิร์ฟเวอร์เท่านั้นที่โหลด** · เซิร์ฟเวอร์ประกอบย่อหน้าแล้วส่งมาใน `derived.script`
     *    ➔ วัดจริง **143 ➔ 101 KB** (−42 KB · −29%) จึงรัดเพดานลงตามของจริงทันที
     */
    maxJsGzipKb: 115, // dev วัดได้ 101 KB · เผื่อส่วนต่าง CI ~9 KB + ระยะหายใจ
    maxHtmlGzipKb: 20, // วัดจริง 2026-09-18: 16 KB
  },
  {
