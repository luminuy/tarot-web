...
if (!lines.includes("เคสที่มีคะแนน judge ทั้งสองรอบ (ใช้เทียบจริง): 1 เคส")) {
...
// เคสจริงที่เจอจากการรันบน GitHub: baseline มี 7 จาก 16 เคสที่ ok แต่ judge ว่าง
// (ผู้ตัดสินล้มแยกจากผู้ผลิต) ถ้านับเข้ามาเทียบจะได้ "0.00 → 5.00 🟢 ดีขึ้น" ทั้งกระดาน
const prevUnscored = mkReport("v-old", [
  { ...mkCase("gold-002", true, 0), judge: {} },
  mkCase("gold-009", true, 4),
]);
const currScored = mkReport("v-new", [mkCase("gold-002", true, 5), mkCase("gold-009", true, 4)]);
const linesUnscored = buildComparison(currScored, prevUnscored).join("\n");

if (linesUnscored.includes("0.00 → 5.00")) {
  fail("นับเคสที่ผู้ตัดสินไม่ได้ให้คะแนนเข้ามาเทียบ ➔ ได้ 0.00 → 5.00 ซึ่งเป็นผลลวง");
} else {
  pass("เคสที่ ok แต่ judge ว่าง ➔ ไม่ถูกนับเข้าการเทียบ");
}
if (!linesUnscored.includes("สำเร็จแต่ผู้ตัดสินไม่ได้ให้คะแนน")) {
  fail("ไม่ได้บอกว่ามีเคสที่สำเร็จแต่ไม่มีคะแนน judge");
} else {
  pass("บอกจำนวนเคสที่สำเร็จแต่ผู้ตัดสินไม่ได้ให้คะแนนไว้ชัด");
}
if (!linesUnscored.includes("เคสที่มีคะแนน judge ทั้งสองรอบ (ใช้เทียบจริง): 1 เคส")) {
  fail("นับเคสที่ใช้เทียบจริงผิด")
} else {
  pass("นับเฉพาะเคสที่มีคะแนน judge ทั้งสองรอบ")
}
...
if (!noOverlap.includes("ไม่มีเคสที่มีคะแนน judge ทั้งสองรอบเลย")) {
