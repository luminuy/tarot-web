...
export function bumpCounter(key: string, ttlSec: number, amount = 1): void {
  ...
  //
  // การเช็กว่าเปิด Upstash อยู่ไหมต้อง `await` เพราะต้องอ่าน env จาก
  // `getCloudflareContext()` (ดูเหตุผลในหัวไฟล์ `redis.ts`) จึงย้ายมาไว้ในงานเบื้องหลัง
  // ทั้งก้อน — ตัวฟังก์ชันนี้ยังเป็น sync เหมือนเดิมสำหรับผู้เรียก
  const task = (async () => {
    if (await isRedisEnabled()) {
      const ok = await redisIncrBy(key, amount, ttlSec);
      if (ok !== null) return;
      // Redis ล่มกลางทาง — ตกไปสะสมลง buffer ของ KV แทน จะได้ไม่นับหาย
    }
