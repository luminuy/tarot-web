...
/**
 * คุณภาพ AVIF ที่เลือกใช้ — **ค่าปัจจุบันคือ 52**
 *
 * 📌 ตัวเลขนี้ต้องตรงกับที่เขียนในคอมเมนต์ก้อนนี้เสมอ — ด่าน `test-image-paths.ts`
 *    (กฎ E) ตรวจให้อัตโนมัติแล้ว เพราะเคยเพี้ยนมาแล้วหนึ่งรอบ (ดูหัวข้อท้ายก้อนนี้)
 *
 * ที่มาของตัวเลข — วัดจริงรอบเดียวที่มี (2026-09-11)
 * ---------------------------------------------------------------------------
 * วัดด้วยไพ่ 4 ใบที่ขนาด w512 เทียบกับ WebP ที่ใช้อยู่จริง (PSNR เทียบภาพต้นฉบับย่อ):
 *
 *   | รูปแบบ           | ขนาดรวม 4 ใบ | PSNR เฉลี่ย |
 *   | WebP (ของเดิม)   | 408.7 KB     | 30.03 dB   |
 *   | AVIF q60         | 220.2 KB     | 29.76 dB   |
 *   | AVIF q65         | 279.8 KB     | 30.89 dB   |  ← ค่าที่ใช้อยู่เดิม
 *   | **AVIF q52**     | **ยังไม่ได้วัด** | **ยังไม่ได้วัด** |  ← ค่าที่ใช้จริงตอนนี้
 *
 * ⚠️ **ตรงไปตรงมา: q52 ไม่มีตัวเลขวัดรองรับ**
 * PR #476 (2026-09-14) เปลี่ยน 65 ➔ 52 โดยไม่แตะคอมเมนต์และไม่แนบผลวัดใด ๆ
 * ทั้งที่คอมเมนต์เดิมเขียนห้ามลดโดยไม่วัดใหม่ไว้ชัด ๆ — กฎที่เป็นแค่ตัวหนังสือ
 * จึงถูกข้ามไปเงียบ ๆ ตามบทเรียนประจำบ้านนี้ (หลักการข้อ 0.8)
 *
 * เจ้าของโปรเจกต์ตัดสินใจเมื่อ **2026-09-15 ว่าคง q52 ไว้** หลังดูภาพบนเว็บจริง
 * นี่คือการตัดสินด้วยสายตาของเจ้าของ ไม่ใช่ผลวัด PSNR — บันทึกไว้ให้ตรงตามจริง
 * เพื่อไม่ให้คนอ่านรอบหน้าเข้าใจว่ามีตัวเลขรองรับอยู่แล้ว
 *
 * ⚠️ ถ้าจะขยับค่านี้อีก ต้องวัดใหม่ก่อนเสมอ — ภาพชุดนี้เป็นลายเส้นละเอียดของปี 1909
 *    การไล่ระดับสีน้อยแต่ความถี่สูงเยอะ ตัวเลขจากภาพถ่ายทั่วไปใช้แทนกันไม่ได้
 *    และ **ต้องแก้ตัวเลขในคอมเมนต์ก้อนนี้ให้ตรงกันด้วย** ไม่งั้นด่าน E ตก
 */
const IMAGEKIT_AVIF_QUALITY = 52;
