...
/**
 * ✦ อุ่นเครื่อง chunk ล่วงหน้า — เรียกตอนผู้ใช้ "ส่อแวว" ว่ากำลังจะเปิด (hover / focus / กดปุ่มที่นำไปสู่มัน)
 *
 * ทำไมต้องมี: `withMotionScope()` แลกน้ำหนักบันเดิลตั้งต้นมาด้วย "ค่าหน่วงตอนกดครั้งแรก"
 * เพราะต้องดาวน์โหลดทั้ง chunk ของคอมโพเนนต์และไลบรารี `motion` (40 KB) ก่อนถึงจะเรนเดอร์ได้
 * ระหว่างนั้นจอไม่ขยับเลยสักพิกเซล ผู้ใช้จึงเห็นเป็น "กดแล้วเงียบ แล้วเด้งพรึ่บ"
 * โหลดล่วงหน้าตั้งแต่ตอนเมาส์แตะปุ่ม = พอกดจริงของอยู่ในแคชแล้ว อนิเมชันขาเข้าได้เล่นครบจังหวะ
 *
 * ⚠️ เรียกได้เฉพาะหลังผู้ใช้ลงมือเท่านั้น ห้ามเรียกตอนหน้าโหลด —
 *    ไม่งั้นเท่ากับลาก `motion` กลับเข้าเส้นทางโหลดแรกที่อุตส่าห์ถอดออกไป
 */
export function prefetchMotionScope(load: () => Promise<unknown>): void {
  if (typeof window === "undefined") return;
  void load().catch(() => {
    /* noop — ของเสริม ล้มแล้วปล่อยให้ตอนเปิดจริงโหลดใหม่ */
  });
  void import("motion/react").catch(() => {
    /* noop */
  });
}
