...
export const CardDetailView: React.FC<CardDetailViewProps> = ({
  ...
  return (
    <div className="space-y-8 w-full max-w-5xl mx-auto relative z-10">
      ...
      <div className="flex items-center justify-between border-b border-[#D6B48D]/30 pb-4 text-xs font-mono">
        <Link
          href="/cards"
          className="inline-flex items-center gap-1.5 text-[#5A432F] hover:text-[#CD9F5B] transition-colors py-1 px-3 rounded-full bg-[#FDF7F0] border border-[#D6B48D] hover:border-[#CD9F5B] font-serif-th shadow-xs"
        >
          <span>←</span> กลับหน้ารวมไพ่ 78 ใบ
        </Link>
        <span className="text-[#8C735D]">
          ลำดับที่ <strong className="text-[#CD9F5B]">{currentIndex + 1}</strong> / {totalCards}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        ...
        <div className="md:col-span-5 lg:col-span-4 flex flex-col items-center space-y-5">
          ...
          <div className="relative group">
            ...
            <div
              className="absolute -inset-4 rounded-3xl opacity-20 blur-2xl transition-all duration-500 group-hover:opacity-40"
            ...
            <motion.div
              ...
              className="relative w-64 sm:w-72 aspect-[7/12] rounded-[1.618rem] overflow-hidden border-2 border-[#D6B48D] p-1.5 bg-[#FFFFFF] shadow-md"
            >
              <div className="relative w-full h-full rounded-2xl overflow-hidden bg-[#FCF0E6]">
                <motion.div
                ...
                <div className="absolute top-2 left-2 right-2 flex items-center justify-between pointer-events-none">
                  <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-md bg-[#5A432F] text-[#FDF7F0] border border-[#D6B48D] shadow-xs">
                    {card.arcana === "major" ? `Major #${card.number}` : card.suit?.toUpperCase()}
                  </span>
                  <span className={`text-[10px] font-mono px-2 py-0.5 rounded-md border ${elem.border} ${elem.bg} ${elem.text} font-bold shadow-xs`}>
          ...
          <div className="flex items-center justify-center p-1.5 rounded-2xl bg-[#FCF0E6] border border-[#D6B48D] w-full max-w-xs select-none shadow-xs">
            <button
              type="button"
            ...
            <button
              ...
              className={`flex-1 py-2.5 text-xs font-serif-th font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                !isUpright
                  ? "bg-[#E4C09F] text-[#5A432F] shadow-xs font-bold"
                  : "text-[#8C735D] hover:text-[#5A432F]"
              }`}
            >
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-xs font-mono">
            ...
            {card.astrology && (
              <span className="px-3 py-1 rounded-full border border-[#D6B48D] bg-[#FFFFFF] text-[#5A432F]">
                🪐 {card.astrology}
              </span>
            )}
            {card.yesNo && (
        ...
        <div className="md:col-span-7 lg:col-span-8 space-y-6">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs font-mono text-[#CD9F5B]">
              <span className="px-2.5 py-0.5 rounded-full bg-[#FCF0E6] border border-[#D6B48D] font-semibold uppercase">
                {card.arcana === "major" ? "Major Arcana" : `${card.suit} Suit`}
              </span>
              <span className="text-[#8C735D]">|</span>
              <span className="text-[#5A432F] font-sans tracking-wide">{card.nameEn}</span>
            </div>
            <h1 className="font-serif-th text-3xl sm:text-4xl lg:text-5xl font-bold font-mystic-gold leading-tight">
              {card.nameTh}
            </h1>
            <p className="text-xs sm:text-sm text-[#8C735D] leading-relaxed pt-1">
              {card.numerology}
            </p>
          </div>
          ...
          <div className="space-y-2">
            <h4 className="text-[11px] font-mono text-[#CD9F5B] uppercase tracking-wider flex items-center gap-1.5 font-bold">
            ...
            <div className="flex flex-wrap gap-2">
              {currentKeywords.map((kw, i) => (
                <span
                  key={i}
                  className="text-xs px-3 py-1.5 rounded-xl border border-[#D6B48D] bg-[#FFFFFF] text-[#5A432F] font-serif-th font-semibold shadow-xs"
                >
                  ✦ {kw}
                </span>
              ))}
          ...
          <div className="space-y-3.5 pt-2">
            <h3 className="font-serif-th text-base font-bold text-[#5A432F] flex items-center gap-2">
              <span className="text-[#CD9F5B]">✦</span> ความหมายและการทำนาย 5 ด้าน ({isUpright ? "หัวตั้ง" : "หัวกลับ"})
            </h3>
              ...
              >
                {categories.map((cat) => {
                  ...
                  return (
                    <div
                      key={cat.id}
                      className="rounded-2xl border border-[#D6B48D] bg-[#FFFFFF] p-4 sm:p-5 space-y-2 hover:border-[#CD9F5B] transition-all shadow-xs group"
                    >
                      ...
                      <p className="font-serif-th text-xs sm:text-sm text-[#5A432F] leading-relaxed pl-4 border-l-2 border-[#D6B48D] group-hover:border-[#CD9F5B] transition-colors">
          ...
          <div className="pt-4 flex items-center gap-4 flex-wrap">
            <Link
              href="/"
              className="px-7 py-3 rounded-2xl text-xs sm:text-sm font-serif-th font-bold bg-[#CD9F5B] hover:bg-[#B8853E] text-[#FDF7F0] shadow-md hover:scale-105 transition-transform flex items-center gap-2"
            >
      ...
      <div className="pt-8 border-t border-[#D6B48D]/30 flex items-center justify-between gap-4">
        {prevCard ? (
          <Link
            href={`/cards/${prevCard.id}`}
            className="flex items-center gap-3 p-3.5 rounded-2xl border border-[#D6B48D] hover:border-[#CD9F5B] bg-[#FFFFFF] hover:bg-[#FDF7F0] transition-all group max-w-[48%] shadow-xs"
          >
            <div className="w-9 h-14 rounded-lg overflow-hidden border border-[#D6B48D] flex-shrink-0 bg-[#FCF0E6]">
            ...
            <div className="text-left overflow-hidden">
              <span className="text-[10px] font-mono text-[#8C735D] block">← ใบก่อนหน้า</span>
              <span className="font-serif-th text-xs sm:text-sm font-bold text-[#5A432F] group-hover:text-[#CD9F5B] truncate block">
                {prevCard.nameTh}
              </span>
            </div>
          </Link>
        ...
        {nextCard ? (
          <Link
            ...
            className="flex items-center gap-3 p-3.5 rounded-2xl border border-[#D6B48D] hover:border-[#CD9F5B] bg-[#FFFFFF] hover:bg-[#FDF7F0] transition-all group max-w-[48%] text-right shadow-xs"
          >
            <div className="text-right overflow-hidden">
              <span className="text-[10px] font-mono text-[#8C735D] block">ใบถัดไป →</span>
              <span className="font-serif-th text-xs sm:text-sm font-bold text-[#5A432F] group-hover:text-[#CD9F5B] truncate block">
                {nextCard.nameTh}
              </span>
            </div>
