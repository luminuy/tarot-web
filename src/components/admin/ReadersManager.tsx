  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-ink">จัดการแม่หมอตัวจริง (Marketplace)</h2>
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="altar-card-porcelain !rounded-xl fixed bottom-6 right-6 z-50 px-4 py-2.5 text-xs font-semibold text-ink">
          {toastMessage}
        </div>
      )}
      ...
      {/* Readers List */}
      {loading ? (
        <div className="altar-card-porcelain altar-panel flex h-48 items-center justify-center text-xs text-muted">
        ...
        <div className="altar-card-porcelain altar-panel flex flex-col items-center justify-center py-12 text-center">
          <p className="text-sm font-semibold text-ink">ยังไม่มีแม่หมอในหมวดนี้</p>
          <p className="mt-1 text-xs text-muted">คลิกปุ่ม &quot;+ เพิ่มแม่หมอใหม่&quot; เพื่อเริ่มต้นสร้างโปรไฟล์</p>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredReaders.map((r) => (
            <div
              key={r.id}
              className="altar-card-porcelain altar-panel flex flex-col justify-between p-5 transition hover:border-ink/40"
            >
              <div>
                {/* Header */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="glass-chip flex h-12 w-12 shrink-0 items-center justify-center text-lg font-bold text-ink overflow-hidden">
                      {r.avatarUrl ? (
                    ...
                    <div>
                      <h3 className="font-bold text-ink">{r.displayName}</h3>
                      <span
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {r.specialties.map((s, idx) => (
                    <span
                      key={idx}
                      className="glass-chip px-2 py-0.5 text-[12px] font-medium text-ink"
                    >
                      {s}
                    </span>
                  ))}
                ...
                {/* Contact & Meta */}
                <div className="altar-card-porcelain !rounded-xl mt-4 space-y-1 p-2.5 text-[13px] text-muted">
              ...
              {/* Action Buttons */}
              <div className="mt-5 border-t border-line pt-3 space-y-2">
            ...
            <Field label="สถานะโปรไฟล์">
              {(field) => (
                <select
                  ...
                  className="glass-field w-full rounded-xl border border-line-interactive px-3 py-2 text-xs text-ink outline-none focus:border-ink"
                >
