...
> สถานะรวม: ✅ **บริการภายนอกหลักตั้งค่าครบและใช้งานได้จริงทั้งหมด**
>
> ⚠️ **ยกเว้น 1 ค่าที่ยังว่างและมีแต่เจ้าของตั้งได้** — ดูหัวข้อ 0 ด้านล่าง
> (เดิมหัวไฟล์นี้เขียนว่า "ครบถ้วนสมบูรณ์ 100%" ทั้งที่ Meta Pixel ไม่เคยทำงานบน production เลย
> การเขียนเกินจริงในเอกสารทำให้ไม่มีใครไปตรวจซ้ำอยู่หลายเดือน)

---

## 🟠 0. ค่าที่ยังต้องให้เจ้าของตั้งเอง (AI ทำแทนไม่ได้)

| ค่า | ตั้งที่ไหน | ผลตอนนี้ | ท่อในโค้ด |
| :--- | :--- | :--- | :--- |
| `NEXT_PUBLIC_META_PIXEL_ID` | **GitHub ➔ Settings ➔ Secrets and variables ➔ Actions** (ใส่เป็น secret หรือ variable ก็ได้) · หรือใส่เป็น secret ของ Cloudflare Worker ก็ได้เหมือนกัน | Meta Pixel ไม่ทำงานบน production · `fbq` เป็น `undefined` · ยิงโฆษณา Facebook แล้ววัด conversion ไม่ได้ | ✅ ต่อครบแล้ว — `deploy.yml` / `pr.yml` อ่านจาก `secrets`/`vars` ตอน build และ `/api/config/analytics` อ่าน env ของ Worker ตอน runtime **เหลือแค่ใส่ค่า** |

> 💡 ตั้งค่าแล้วต้อง **deploy ใหม่หนึ่งรอบ** ถ้าใส่ฝั่ง GitHub (ค่า `NEXT_PUBLIC_*` ถูกฝังตอน build)
> ส่วนถ้าใส่ฝั่ง Cloudflare Worker จะมีผลทันทีผ่านทางสำรอง runtime โดยไม่ต้อง build ใหม่
>
> ตั้งเสร็จแล้วขั้น "📊 ตรวจว่าตั้งค่าวัดผลครบไหม" ใน `deploy.yml` จะขึ้น `✅ ตั้งค่าวัดผลครบทุกตัว` เอง
