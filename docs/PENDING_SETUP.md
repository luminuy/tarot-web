...
> ⚠️ **กับดักที่เหยียบมาแล้ว (INC-0141)**: กฎ WAF ข้อ 5 ของโซนนี้บล็อก User-Agent ที่เป็น
> `curl` / `python` / `Postman` / `Scrapy` ที่เส้น `/api/` ตั้งแต่ชั้น edge
> workflow จึงต้องส่ง `-A "SeerTarot-Cron/1.0"` เสมอ (มีด่านตรวจกำกับแล้ว)
> วัดจาก production จริง: UA `curl/8.5.0` → **403** · UA ที่ตั้งเอง → **200**
> ถ้าวันหน้าไปเพิ่มกฎ WAF ใหม่ ระวังอย่าเผลอบล็อก UA ตัวนี้
