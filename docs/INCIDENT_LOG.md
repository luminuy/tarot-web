...
### INC-0130 · 2026-09-11 21:42 · 🟠 High · เครื่องมือวัดคุณภาพ AI ยิงแค่ Groq ทั้งที่ production มี failover 2 ชั้น — รายงาน baseline จึงบอกว่าล้มเหลว 14/30 เคสทั้งที่ผู้ใช้จริงได้คำอ่านครบ
...
| **ผลกระทบ** | ตัวเลขคุณภาพคำอ่านที่ใช้ตัดสินใจพัฒนาต่อเอียงไปทางเคสที่ Groq บังเอิญตอบจบ และไม่มีใครรู้ว่าค่าเฉลี่ยมาจากกลุ่มตัวอย่างเล็กแค่ 9 เคส เพราะรายงานไม่เคยพิมพ์เลขนี้ออกมา |
| **สาเหตุราก** | scripts/qa/run-golden-judge.ts เรียก streamGroqReading ตัวเดียวจบ แต่ /api/reading/[id]/read มี 2 ชั้นคือ Groq ทุกโมเดลไม่จบแล้วตกไป Gemini เครื่องมือวัดจึงวัดคนละระบบกับที่ deploy อยู่จริง และ summary ก็นับแค่ succeeded ไม่เคยนับจำนวนเคสที่ผู้ตัดสินให้คะแนน |
| **การแก้ไข** | ให้ run-golden-judge.ts เดิน failover เหมือน production ครบสองชั้น บันทึก provider ที่ผลิตคำอ่านจริงและ providerNotes ของชั้นที่ล้ม เพิ่ม summary.judged แล้วให้ test-judge-baseline.ts เตือนเมื่อ judged น้อยกว่า succeeded |
| **🛡️ กฎป้องกันถาวร** | **เครื่องมือวัดคุณภาพทุกตัวต้องเดินเส้นทางเดียวกับ production เป๊ะ และด่านตรวจต้องยืนยันข้อนี้จากซอร์สจริง ไม่ใช่เชื่อคำอธิบายในคอมเมนต์ ทุกค่าเฉลี่ยที่รายงานออกมาต้องพิมพ์ขนาดกลุ่มตัวอย่างกำกับเสมอ ห้ามปล่อยให้คนอ่านเดาเองว่าเฉลี่ยจากกี่เคส** |
| **การพิสูจน์ว่าแก้ได้จริง** | npx tsx scripts/qa/test-judge-baseline.ts ต้องผ่านและขึ้นว่า run-golden-judge.ts มี failover ไป Gemini ครบ · npm run repo:verify ผ่าน 47/47 |
| **บันทึกโดย** | ไม่ระบุ · branch `claude/elegant-ride-nbwug1` · commit `5ecbf94` |
