...
### INC-0056 · 2026-09-02 15:20 · 🟠 High · ป้องกันการกุไพ่ The Fool เมื่อหาข้อมูลไม่เจอ — เปลี่ยนเป็นส่ง Error ให้โหลดใหม่แทน
...
| **การแก้ไข** | (1) ปรับ `cardByIndex` ให้คืน `TarotCard | undefined` โดยคืน `undefined` เสมอเมื่อ index ไม่ถูกต้อง ไม่กุ `DECK[0]` อีกต่อไป (2) ใน API routes (`shuffle`, `read`, `chat`) ตรวจสอบความถูกต้องของไพ่ทุกใบ หากไม่พบคืน Error 404/500 `"ไม่พบสำรับไพ่ที่เปิดไว้ในรอบนี้ กรุณารีเฟรชหน้าเว็บเพื่อเชื่อมต่อกับสำรับไพ่ของคุณอีกครั้ง"` โดยตัด fallback The Fool ใน chat route ออกทั้งหมด (3) ใน `page.tsx` และ `StreamReader.tsx` ลบ fallback The Fool ออก และแสดงข้อความแจ้งเตือนพร้อมปุ่ม `"✦ โหลดใหม่อีกครั้ง"` / รีเฟรช (4) ปรับ `GEMINI_FIRST_MODEL_TIMEOUT_MS = 4000` (4 วินาที) เร่งความเร็วแชทและคำทำนายลดเวลารอลงครึ่งหนึ่ง พร้อมเพิ่มชิปถามต่อด่วนในห้องแชท (5) เพิ่ม Gate 20 `test-no-fake-card.ts` และขยาย Gate 19 `test-chat-history-schema.ts` ใน CI |
| **🛡️ กฎป้องกันถาวร** | **ห้ามเขียน fallback คืนไพ่ใบใดใบหนึ่ง (โดยเฉพาะ The Fool) เมื่อข้อมูลสูญหายเด็ดขาด — ฟังก์ชัน resolve ไพ่ต้องคืน undefined และ UI/API ต้องแจ้งเตือนให้ผู้ใช้โหลดใหม่เท่านั้น และใช้เพดานเวลาโมเดลแรก 4 วิเพื่อตัดเข้า flash-lite เร็วขึ้น** |
| **การพิสูจน์ว่าแก้ได้จริง** | รัน `scripts/qa/test-no-fake-card.ts` ผ่าน 19/19 ข้อ และ `npm run repo:verify` ผ่านครบ 20/20 ด่าน |
