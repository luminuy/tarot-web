...
| **การแก้ไข** | เขียนกฎข้อ 13 ใน CLAUDE.md ใหม่ให้ระบุตรง ๆ ว่า push แล้วจบ = งานยังไม่เสร็จ และกฎของโปรเจกต์นี้อยู่เหนือแนวปฏิบัติทั่วไปของ AI ทุกตัว · เพิ่มหมายเหตุว่าถ้า environment ไหนไม่มี gh CLI (เช่น Claude Code บนเว็บ) ให้เปิด PR ผ่าน GitHub API/MCP แทน ห้ามใช้เป็นข้ออ้างข้ามขั้นตอน |
| **🛡️ กฎป้องกันถาวร** | **จบงาน = PR ถูกเปิดแล้ว ไม่ใช่ push เสร็จแล้ว — ห้าม AI ตัวไหนอ้างว่า 'ผู้ใช้ไม่ได้สั่งให้เปิด PR' เพราะ CLAUDE.md ข้อ 12/13 คือคำสั่งยืนของเจ้าของโปรเจกต์อยู่แล้ว · ถ้ารัน pr:auto ไม่ได้เพราะ environment ขาด gh ต้องเปิด PR ด้วยวิธีอื่นให้สำเร็จแล้วรายงานข้อจำกัดนั้น ห้ามหยุดเงียบ ๆ ที่ push** |
| **การพิสูจน์ว่าแก้ได้จริง** | เปิด PR ของ branch claude/login-bug-check-k66uny สำเร็จหลังบันทึกบทเรียนนี้ และแก้ถ้อยคำกฎข้อ 13 ใน CLAUDE.md แล้ว |
| **บันทึกโดย** | ไม่ระบุ · branch `claude/login-bug-check-k66uny` · commit `9aaa55c` |
