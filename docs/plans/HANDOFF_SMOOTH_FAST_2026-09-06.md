...
> **สถานะ**: ✅ เสร็จแล้ว — เงาแยกชั้น · View Transition (เฉพาะจอที่ใช้เมาส์ — มือถือปิดเพราะทำจอว่างแวบ INC-0244) · สำรับออกจากบันเดิล (#338 · #345) · S-03 ปิดด้วย `card-tile-cv` (ไพ่ครบ 78 ใบใน HTML ตามข้อห้าม SEO แต่ข้ามการวาดนอกจอ — `/cards` TBT ~4,000 ➔ ~100 ms · HANDOFF_CARD_TILE_CV) · **ตรวจล่าสุด**: 2026-09-24 · **หลักฐาน**: PR #338 · #345 · `src/components/encyclopedia/CardsExplorer.tsx` · `CardGroupView.tsx`

> **สถานะตอนเขียนแผน**: แผน (ยังไม่ลงมือ) · **ฐานที่วัด**: `7f2dab2` (main หลัง #327) · **ผู้ตรวจ**: Claude Opus 5
...
> ✅ **ปิดแล้ว (ตรวจ 2026-09-23)**: ทำด้วยคลาส `card-tile-cv` (`content-visibility: auto` + ความสูงจองด้วยสูตร `calc(vw)`)
> บนกริด `CardsExplorer` / `CardGroupView` — ทุกใบยังอยู่ใน HTML เบราว์เซอร์แค่ข้ามการวาดใบที่อยู่นอกจอ
> ตรงตามข้อห้ามด้านล่าง ไม่ต้องทยอยเรนเดอร์เพิ่ม (รายละเอียด: `HANDOFF_CARD_TILE_CV_2026-09-15.md`)
>
...
> ⛔ **แก้ภายหลัง 2026-09-24 (INC-0244)** — โค้ดขั้นที่ 1 ข้างล่างทำ **จอว่างแวบบนมือถือ** ทุกครั้งที่เปิดหน้า
> (เฟดเริ่มก่อนหน้าใหม่มีเนื้อหา) ตอนนี้เปิดเฉพาะใต้ `@media (hover: hover) and (pointer: fine)`
> และด่านกฎ 14 ของ `test-motion-quality.ts` บังคับไว้แล้ว — **ห้ามคัดลอกโค้ดข้างล่างไปใช้ตรง ๆ**
