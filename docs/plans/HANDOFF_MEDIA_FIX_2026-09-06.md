...
> **สถานะ**: ✅ เสร็จแล้ว — M-01 ถึง M-06 ปิดครบ (M-06 ต่อ UI แล้วที่ `SemanticSearchPanel`) · **ตรวจล่าสุด**: 2026-09-23 · **หลักฐาน**: PR #328 · #330 · #333 · #338

> **สถานะตอนเขียนแผน**: 🟢 **ลงมือไปแล้ว 5/6 ข้อ** (อัปเดต 2026-09-06 หลัง PR #328 · #330 · #333)
> **ฐานที่ตรวจครั้งแรก**: `a72968b` (main หลัง #325) · **ผู้ตรวจ**: Claude Opus 5
>
> | งาน | สถานะ | ยืนยันอย่างไร |
> | :-- | :-- | :--- |
> | **M-01** ภาพแชร์ตัวหนังสือทับกัน | ✅ **แก้แล้ว** (#328) | เจนภาพจริงแล้วเปิดดู — ข้อความไม่ทับกัน · ตัวไทยครบสระ/วรรณยุกต์ · ตัดบรรทัดถูก · ไพ่ย้ายไปชิดขวาด้วย `c_pad,g_east` |
> | **M-02** Cloudinary แย่งที่ภาพไพ่จริง | ✅ **แก้แล้ว** (#328) | `s/[id]/page.tsx:44` ใช้ `hasRealShareImage` มาก่อนตรงตามแผน |
> | **M-03** ImageKit ไม่มีทางถอย | ✅ **แก้แล้ว** (#330) | `CardImage.tsx:59` มี `dataset.fellBack` + `forceLocal` · ถอด hardcode default ออกจาก `deploy.yml` แล้ว |
> | **M-04** `.env.example` สอนผิด | ✅ **แก้แล้ว** (#330) | ไม่มี `wrangler secret put NEXT_PUBLIC_*` เหลืออยู่แล้ว |
> | **M-05** `CACHE_VERSION` ฮาร์ดโค้ด | ✅ **แก้แล้ว** (#333) | `scripts/bump-sw-version.ts` ผูก `CACHE_VERSION` กับ git SHA อัตโนมัติใน build/deploy pipeline |
> | **M-06** `/api/search` กำพร้า | ✅ **ตัดสินแล้ว → "ทาง ข · ต่อ UI จริง"** | แผนลงมืออยู่ที่ [`HANDOFF_SEMANTIC_SEARCH_2026-09-06.md`](HANDOFF_SEMANTIC_SEARCH_2026-09-06.md) |
>
> ➔ **แผนนี้ปิดครบทุกข้อแล้ว** — M-01 ถึง M-05 ลงมือเสร็จ · M-06 เจ้าของเลือก "ต่อ UI จริง"
> และแยกออกไปเป็นแผนของตัวเองแล้ว ไม่ต้องกลับมาอ่านเอกสารนี้อีก
>
> **ผลตรวจครั้งแรก**: `repo:verify` 34/34 ผ่าน · build prerender 167 routes · เฟส 1 Cloudflare ครบ ·
...
## 6. M-05 · `CACHE_VERSION` ใน `sw.js` ฮาร์ดโค้ด 🟡 (✅ แก้ไขแล้วใน PR #333)

> **สถานะ**: แก้ไขเรียบร้อยแล้ว โดยสร้าง `scripts/bump-sw-version.ts` และเชื่อมเข้ากับสคริปต์ `build:worker` และ `deploy` ใน `package.json` ทำให้ทุก build อัปเดต `CACHE_VERSION` ตาม Git Commit SHA หรือ Timestamp โดยอัตโนมัติ
