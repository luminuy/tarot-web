...
### 11.2 จุดที่ *ยัง* ล้มเงียบจริง (ยังไม่แก้)

```ts
// entitlement.ts — ท้าย consumeReading()
} catch {
  // ชน UNIQUE(reading_id) = เคยหักไปแล้ว → ถือว่าผ่าน ไม่หักซ้ำ
  return true;
}
...
`catch` นี้เหมาเอาว่า **error ทุกชนิด = ชน UNIQUE** ซึ่งจริงเฉพาะกรณีที่ตั้งใจ
ถ้า INSERT ล้มด้วยเหตุอื่น (D1 throttle, schema drift, คอลัมน์ไม่ตรง, โควตาเต็ม)
จะคืน `true` = **ได้เปิดไพ่ฟรีโดยไม่ถูกหักสิทธิ์ และไม่มีร่องรอยใด ๆ**

เช่นเดียวกับ `refundReading()` ที่ `catch {}` เปล่า — ถ้าคืนสิทธิ์ไม่สำเร็จ ผู้ใช้เสียสิทธิ์เงียบ ๆ

### 11.3 ทางป้องกัน เรียงตามผลลัพธ์ต่อความเสี่ยง

| # | มาตรการ | แก้อะไร | แรง |
| :-- | :--- | :--- | :-- |
| 1 | **ให้ `deploy.yml` รัน d1 migrations อัตโนมัติ** | ตัดต้นเหตุทิ้งทั้งหมด — ตารางหายไม่ได้อีก | เล็ก |
| 2 | **แยกชนิด error ใน `catch`** — เฉพาะ `/UNIQUE\|constraint/i` เท่านั้นที่คืน `true` นอกนั้น `recordEvent("entitlement_db_error")` แล้ว throw ต่อ | ปิดช่องล้มเงียบใน §11.2 | เล็ก |
| 3 | **ยิง metric ทุกครั้งที่ D1 ล้ม** แล้วโชว์ในการ์ด "สถิติระบบสิทธิ์" | ทำให้มองเห็นแทนที่จะเงียบ | เล็ก |
| 4 | **ตัดสินนโยบายให้ชัดว่า DB ล่มแล้วจะ allow หรือ deny** แล้วเขียนเป็นโค้ดตรง ๆ พร้อมข้อความที่ผู้ใช้อ่านรู้เรื่อง (ไม่ใช่ 500 ดิบ) | เปลี่ยน 500 ที่เกิดโดยบังเอิญ เป็นพฤติกรรมที่ตั้งใจ | กลาง |
| 5 | **`/api/health` ตรวจว่าตารางมีจริง** ให้ uptime monitor ยิงเป็นระยะ | รู้ก่อนผู้ใช้เจอ | กลาง |

**ข้อ 1 คุ้มที่สุด** — ที่เหลือคือตาข่ายรองรับกรณีที่ยังหลุดไปได้อยู่ดี

> ⚠️ ข้อ 4 ต้องตัดสินใจเชิงธุรกิจก่อนเขียนโค้ด: ถ้าระบบสิทธิ์ล่ม
> จะยอมให้เปิดไพ่ฟรีไปก่อน (เสียเงินค่า AI แต่ผู้ใช้ไม่เดือดร้อน)
> หรือปิดกั้นไว้ (ปลอดภัยกับกระเป๋า แต่ผู้ใช้ที่จ่ายเงินมาแล้วใช้ไม่ได้)
> — **ห้ามปล่อยให้เป็นผลพลอยได้ของ `try/catch` เหมือนตอนนี้**
