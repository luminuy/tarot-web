...
> **สถานะ**: ✅ เสร็จแล้ว — ระบบสิทธิ์และโควตาใช้งานจริง (`entitlement.enforced` = เปิด) · **ตรวจล่าสุด**: 2026-09-23 · **หลักฐาน**: HANDOFF_ROUND2_CLOSEOUT ข้อ 9

> **สถานะ:** 🟢 **ใช้งานจริงบน production แล้ว** — PR A–F เสร็จ + เปิดธงเป็นค่าเริ่มต้นตั้งแต่ PR #101
> **ฐานอ้างอิง:** commit `75bc1f9` (ตรวจซ้ำ 2026-09-01)
> **ธง `entitlement.enabled`:** **เปิด (default-on)** — fail-closed · ปิดฉุกเฉินได้ที่ `/admin` → แท็บ "สิทธิ์เปิดไพ่"
> **โควตาสมาชิก:** **1 ครั้ง/วัน** (2026-09-12 · เดิม 3 ครั้ง/วัน · ก่อนหน้านั้น 3 ครั้ง/สัปดาห์ — ดู [§0 บันทึกการเปลี่ยนสเปก](#0-บันทึกการเปลี่ยนสเปก))

---

## 0. บันทึกการเปลี่ยนสเปก

เอกสารนี้เขียนตอนออกแบบ (3 ครั้ง/**สัปดาห์**) แต่ของจริงตอนนี้คือ **1 ครั้ง/วัน**
ส่วนที่เหลือของเอกสารบางจุดยังใช้คำว่า "สัปดาห์" ตามบริบทการออกแบบเดิม — **ยึดตารางนี้เป็นความจริง**

| หัวข้อ | แผนเดิม | ของจริงตอนนี้ | เหตุผล |
| :--- | :--- | :--- | :--- |
| โควตาสมาชิก | 3 / สัปดาห์ | **1 / วัน** (`DAILY_LIMIT`) | เคยเป็น 3/วัน (เหตุผลเดิม: 3/สัปดาห์ สร้างนิสัยรายวันไม่ได้) · ลดเหลือ **1/วัน ตามคำสั่งเจ้าของโปรเจกต์ 2026-09-12** — ยังได้เปิดทุกวันเหมือนเดิม แค่วันละครั้ง · อยากได้มากกว่านั้นเติมรอบจากแพ็กเกจได้ |
| รอบรีเซ็ต | จันทร์ 00:00 ไทย | **เที่ยงคืนทุกวัน เวลาไทย** | ตามข้อบน |
| ไพ่ประจำวัน | ไม่ได้ออกแบบแยก | **นับเข้าโควตารายวันเหมือนผังอื่น + มี streak** (`lib/entitlement/daily.ts`) | ⚠️ แก้ข้อความให้ตรงโค้ดจริง (2026-09-12): `consumeReading()` หักสิทธิ์ผัง `daily` ด้วย — โมดูล `daily.ts` เก็บแค่ streak ไม่ได้ยกเว้นโควตา (ด่าน QA ข้อ 12 ยืนยัน) |
| ค่าเริ่มต้นของธง | ปิด | **เปิด (fail-closed)** | KV ล่ม/คีย์หาย → ยังบังคับสิทธิ์ต่อ ไม่เปิดฟรีไม่จำกัด |
| สิทธิ์ผู้เยี่ยมชม | ทดลองฟรี 1 ครั้งโดยไม่ต้องสมัคร | **0 ครั้ง — ต้องสมัคร/เข้าสู่ระบบก่อนเปิดไพ่** (`GUEST_LIMIT = 0`) | คำสั่งเจ้าของโปรเจกต์ 2026-09-09: เว็บยังฟรีเหมือนเดิม แต่ย้ายเส้นเริ่มต้นมาที่การสมัครสมาชิก เพื่อให้ทุกคนที่ใช้ AI มีบัญชีผูกกับคำทำนายของตัวเอง |
| `WEEKLY_LIMIT` | เพดานจริง | **alias ของ `DAILY_LIMIT`** (compat) | อย่าใช้ในโค้ดใหม่ — ใช้ `DAILY_LIMIT` |
| รหัสแลกสิทธิ์ | ไม่ได้ออกแบบไว้ | **2 ชนิด: `gift_*` (รอบเปิดไพ่) · `purchase_*` (ปลดพรีเมียม)** ออกจาก `/admin` เท่านั้น | 2026-09-12 (INC-0134): รหัสที่ seed ไว้ในรีโปแลกได้ไม่จำกัดคนและทำให้ทุกคนเป็นลูกค้าที่จ่ายเงิน · ตอนนี้บังคับเพดานจำนวนคน + วันหมดอายุทุกใบ |

**ทำไมไม่กลับไป 3/สัปดาห์** (ตัดสิน 2026-09-01): ตัวคุมต้นทุนจริงคือ `AI_DAILY_CALL_CAP` (2000/วัน)
กับ `GUEST_CAP_RATIO` (0.7) ซึ่งทำงานแยกจากโควตาอยู่แล้ว · และยัง **ไม่มีระบบขายเครดิต/สมาชิก**
การบีบโควตาจึงไม่แปลงเป็นรายได้ แค่ทำให้คนที่สมัครแล้วใช้ไม่ได้ · อีกทั้งเพิ่งลดสิทธิ์
จาก "ไม่จำกัด" มาแล้วรอบหนึ่ง การลดซ้ำในสัปดาห์เดียวกันกระทบความรู้สึกผู้ใช้หนัก

**เพดานรองรับที่ `AI_DAILY_CALL_CAP = 2000`:** 1/วัน ≈ 2,000 สมาชิกถ้าใช้เต็มทุกคน
(~8,000 ถ้าใช้จริงราว 25% · เดิม 3/วัน รองรับ ~660 คน) · **ถ้าชนเพดาน ทุกคนถูกบล็อกรวมถึงผู้เยี่ยมชมที่กำลังจะสมัคร**
→ เฝ้า metric `aiCapHit` · ถ้าเริ่มขึ้น ให้**เพิ่ม `AI_DAILY_CALL_CAP` ก่อน** ไม่ใช่ลดโควตาผู้ใช้
...
| 2 | รอบโควตา | ~~สัปดาห์ปฏิทิน เริ่มจันทร์~~ → **รายวัน รีเซ็ตเที่ยงคืนเวลาไทย** ([§0](#0-บันทึกการเปลี่ยนสเปก)) |
...
| 7 | ผู้ใช้เดิมที่เคยใช้ไม่จำกัด | ~~โบนัสเปลี่ยนผ่าน 10 ครั้ง + ประกาศ 7 วัน~~ → **ข้ามแล้ว: ยังไม่มีผู้ใช้จริงให้ริบสิทธิ์** (เครื่องมือยังอยู่ ใช้กับการลดสิทธิ์ครั้งหน้า) |
...
> ⚠️ **อัปเดต 2026-09-09:** ย่อหน้าข้างบนเป็นสเปกยุคที่ยังมีสิทธิ์ทดลองฟรี 1 ครั้ง
> ตอนนี้ `GUEST_LIMIT = 0` แล้ว — ผู้เยี่ยมชมถูกกั้นตั้งแต่ครั้งแรก **ที่หน้าเลือกผังเหมือนเดิม**
> (ยังยึดหลักเดิมคือกั้นขั้น 1 ไม่ใช่ขั้น 5 — ห้ามปล่อยให้จับไพ่จนครบแล้วค่อยเจอกำแพง)

**ข้อ 2 — ทำไมเปลี่ยนเป็นรายวัน และทำไมกุญแจเป็น "วันที่" ไม่ใช่เลขสัปดาห์ ISO**
เหตุผลที่เปลี่ยนจากรายสัปดาห์เป็นรายวัน อยู่ใน [§0](#0-บันทึกการเปลี่ยนสเปก)

ส่วนรูปแบบกุญแจยังยึดหลักเดิม: เก็บเป็น **วันที่ `YYYY-MM-DD` เวลาไทย** ไม่ใช่ `2026-W01`
เพราะเลขสัปดาห์ ISO มีกับดักคาบปี (สัปดาห์แรกของปีอาจเริ่มในเดือนธันวาคมของปีก่อน)
และไลบรารีแต่ละตัวคำนวณไม่ตรงกัน · การเก็บเป็นวันที่ตรง ๆ ให้ผลเหมือนกันทุกที่
และมนุษย์อ่านออกทันทีตอนดีบัก

> ฟังก์ชัน `weekKey()` ยังมีอยู่เพื่อความเข้ากันได้กับข้อมูลเก่า แต่ **โค้ดใหม่ให้ใช้
> `todayDateKey()`** จาก `lib/entitlement/daily.ts` — ชื่อ `weekKey` เป็นมรดกจากสเปกเดิม
...
> 🛡️ **ระบบกันโกงเชิงลึก + threat model:** ดู [`ENTITLEMENT_ABUSE_MODEL.md`](ENTITLEMENT_ABUSE_MODEL.md)
> — P0 (server-authoritative gid marker) + P1 (guest IP/subnet quota) ลงแล้ว · P2 (PoW) / P3 (velocity alert) เลื่อนแบบ metric-driven
...
| สถานะผู้ใช้ | เปิดไพ่ | ไพ่ประจำวัน | คำทำนาย | คุยกับแม่หมอ | บันทึกดวง | ข้ามเครื่อง |
| :--- | :-- | :-- | :-- | :-- | :-- | :-- |
| ผู้เยี่ยมชม (ยังไม่ล็อกอิน) | ❌ 0 — ต้องสมัครก่อน | — | ⚠️ ดูของเดิมในเครื่องนี้ | ❌ ต้องสมัคร | ⚠️ เครื่องนี้ | ❌ |
| สมาชิกใหม่ · วันแรก | 3 + โบนัส 3 | ✅ ฟรี วันละใบ | ✅ เต็มรูปแบบ | ✅ | ✅ | ✅ |
| สมาชิก · โควตาเหลือ | **3 ต่อวัน** | ✅ ฟรี ไม่กินโควตา | ✅ เต็มรูปแบบ | ✅ | ✅ | ✅ |
| สมาชิก · โควตาหมด | ❌ รอเที่ยงคืน | ✅ ยังเปิดได้ (ถ้ายังไม่ใช้วันนี้) | ⚠️ ดูของเดิมทั้งหมด | ✅ กับดวงเดิม | ✅ | ✅ |

**ไพ่ประจำวันฟรี** (`lib/entitlement/daily.ts`) เปิดได้วันละ 1 ใบโดยไม่กินโควตา 3 ครั้ง
และนับ streak วันต่อเนื่อง — นี่คือตัวสร้างนิสัยรายวัน ส่วนโควตา 3 ครั้งสงวนไว้ให้ผังใหญ่

**ใช้ได้เสมอโดยไม่กินโควตาและไม่ต้องล็อกอิน:** สารานุกรมไพ่ 78 ใบ · คลังผัง ·
หน้าเนื้อหา/บทความ · ไพ่ประจำวันฝั่งเบราว์เซอร์ · นโยบายความเป็นส่วนตัว

> 🔐 **ตั้งแต่ 2026-09-09 การเปิดไพ่กับแม่หมอ AI ต้องล็อกอินก่อนเสมอ** (`GUEST_LIMIT = 0`)
> ผู้เยี่ยมชมที่ยังไม่ล็อกอินจะถูกกั้นตั้งแต่ขั้น 1 ด้วยเหตุผล `signup_required`
> (ไม่ใช่ `guest_used` — คนกลุ่มนี้ยังไม่เคยได้เปิดไพ่เลย ถ้อยคำต้องไม่บอกว่าใช้สิทธิ์ครบแล้ว)
> ปุ่มเดียวที่เสนอคือ "สมัครสมาชิกฟรี" · กลับไปเปิดสิทธิ์ทดลองได้ด้วยการตั้ง `GUEST_LIMIT = 1` ที่ `lib/entitlement/limits.ts` ที่เดียว
...
| **สิทธิ์การใช้งานระดับผลิตภัณฑ์** | ✅ ใช้งานจริง (PR A #87) | `src/lib/entitlement/entitlement.ts` |
| **โควตารายวันของสมาชิก** | ✅ ใช้งานจริง (PR A #87) | `migrations/0006_reading_entitlement.sql` |
| **สิทธิ์ฟรีของผู้เยี่ยมชม** | ✅ ใช้งานจริง (PR C #90 · กันโกง P0/P1 #98) | `src/lib/entitlement/guest.ts` |
| **หน้าชวนสมัครหลังอ่านจบ** | ✅ ใช้งานจริง (PR E #92) | `src/components/entitlement/PostReadingSignup.tsx` |
| **ไพ่ประจำวันฟรี + streak** | ✅ ใช้งานจริง | `src/lib/entitlement/daily.ts` |

> ตารางนี้เคยเป็น "⬜ ต้องสร้าง" ตอนเขียนแผน · อัปเดตหลัง PR A–F merge และเปิดธงจริง (2026-09-01)
...
### PR F · เปิดใช้งานจริง — 🟢 **เปิดแล้ว (default-on ตั้งแต่ PR #101)**

> ✅ **ข้ามพิธีเปิดตัวได้ — ตัดสินโดยเจ้าของ 2026-09-01**
> ขั้น "โบนัสเปลี่ยนผ่าน" กับ "ประกาศล่วงหน้า 7 วัน" มีไว้เพื่ออย่างเดียว คือ
> **ไม่ริบสิทธิ์จากคนที่เคยใช้ได้ไม่จำกัด** · เว็บยังใหม่และ **ยังไม่มีผู้ใช้จริง**
> จึงไม่มีใครให้ริบ ทั้งสองขั้นจึงไม่จำเป็นและข้ามไปแล้ว
>
> เครื่องมือยังอยู่ครบ (`npm run entitlement:grandfather`, `AnnouncementBanner`,
> แท็บแอดมิน) ไม่ได้ถอดออก — เก็บไว้ใช้กับ **การลดสิทธิ์ครั้งหน้า** ซึ่งจะเกิดตอนมีผู้ใช้จริงแล้ว
>
> ⚠️ **เงื่อนไขที่ทำให้ข้อสรุปนี้หมดอายุ:** ทันทีที่มีผู้ใช้จริงใช้งานอยู่
> การเปลี่ยนโควตาให้แคบลงกว่าเดิม (เช่น 3/วัน → 2/วัน) หรือย้ายฟีเจอร์ฟรีไปเป็นของสมาชิก
> **ต้องกลับมาทำสองขั้นนี้เสมอ** — อย่าอ้างเอกสารนี้เพื่อข้ามในอนาคต
...
**ขั้นตอนที่เหลือจริง (เว็บใหม่ ไม่มีผู้ใช้เดิม) — จาก `/admin` → แท็บ "สิทธิ์เปิดไพ่":**

1. ✅ **เปิดระบบสิทธิ์** — เปิดเป็นค่าเริ่มต้นแล้วตั้งแต่ PR #101 ไม่ต้องกดอะไร
2. ✅ **เตรียมฐานข้อมูล** — **ยืนยันแล้วบน production 2026-09-01**
   การ์ดแสดง **"✓ พร้อม"** และปุ่มเปลี่ยนเป็น "เตรียมซ้ำ" → ตาราง `reading_usage` / `user_bonus`
   ถูกสร้างบน D1 เรียบร้อย · `ADMIN_PASSWORD` ก็ตั้งบน production แล้ว (ล็อกอิน `/admin` ได้จริง)
3. ⬜ เฝ้าดู metric ในการ์ด "สถิติระบบสิทธิ์" — `blockedRead` / `aiCapHit` / `signupClicked`
   · ถ้า `aiCapHit` เริ่มขึ้น → **เพิ่ม env `AI_DAILY_CALL_CAP` ก่อน** ไม่ใช่ลดโควตาผู้ใช้ (ดู [§0](#0-บันทึกการเปลี่ยนสเปก))

> 🟢 **สรุปสถานะติดตั้ง ณ 2026-09-01 — ระบบสิทธิ์ทำงานเต็มรูปแบบบน production แล้ว**
> ธงเปิด ✓ · ตารางพร้อม ✓ · แอดมินเข้าได้ ✓ · พิธีเปิดตัวข้ามแล้ว (ไม่มีผู้ใช้เดิม)
> เหลือแค่ข้อ 3 ซึ่งเป็นงานเฝ้าดูต่อเนื่อง ไม่ใช่งานติดตั้ง
>
> ⚠️ **ถ้าอนาคตย้าย D1 หรือสร้างฐานข้อมูลใหม่ ต้องกลับมากดปุ่มนี้อีกครั้ง**
> ดูพฤติกรรมจริงเมื่อตารางหายที่ [§11 โหมดล้มเหลว](#11-โหมดล้มเหลวเมื่อฐานข้อมูลมีปัญหา)

**ข้ามไปแล้ว (ไม่มีผู้ใช้เดิมให้ริบสิทธิ์):** โบนัสเปลี่ยนผ่าน · แบนเนอร์ประกาศ 7 วัน
เครื่องมือยังอยู่ครบ ใช้เมื่อจะลดสิทธิ์ครั้งหน้าตอนมีผู้ใช้จริงแล้ว
...
## 11. โหมดล้มเหลวเมื่อฐานข้อมูลมีปัญหา

> **วัดจริงแล้ว 2026-09-01** ด้วยการ stub `globalThis.__tarot_d1_shim__` ให้ throw
> `D1_ERROR: no such table` แล้วเรียกฟังก์ชันจริง — ไม่ใช่การอนุมานจากการอ่านโค้ด

### 11.1 เกิดอะไรขึ้นจริงถ้าตาราง `reading_usage` หาย

| ฟังก์ชัน | ผู้ใช้ | ผลจริง |
| :--- | :--- | :--- |
| `consumeReading()` | สมาชิก | **throw** → route ตอบ 500 · เปิดไพ่ไม่ได้เลย |
| `getEntitlement()` | สมาชิก | **throw** → `/api/entitlement` 500 · QuotaBadge/หน้าเว็บพัง |
| `consumeReading()` | ผู้เยี่ยมชม | คืน `true` ปกติ — **ไม่แตะ D1 เลย** (นับด้วยคุกกี้/ticket ตามออกแบบ) |

**สรุป:** ไม่ใช่ "ทุกคนใช้ฟรีไม่จำกัดแบบเงียบ ๆ" อย่างที่เอกสารเคยเขียนผิดไว้
ของจริงคือ **สมาชิกพังดัง ๆ (500) ส่วนผู้เยี่ยมชมไม่กระทบ** — fail-closed แต่หน้าตาแย่

> 📌 บันทึกไว้เพราะเอกสารฉบับก่อนหน้า (PR #216) เขียนคำอธิบายนี้ผิด
> และคำอธิบายผิดนั้นอันตรายกว่าตัวบั๊ก เพราะทำให้คนประเมินความเสี่ยงผิดทาง

### 11.2 จุดที่เคยล้มเงียบ — 🟢 **แก้แล้ว (PR #218)**

ของเดิม `catch` ท้าย `consumeReading()` เหมาว่า **error ทุกชนิด = ชน UNIQUE**
ถ้า INSERT ล้มด้วยเหตุอื่น (D1 throttle, schema drift, คอลัมน์ไม่ตรง) จะคืน `true`
= ได้เปิดไพ่ฟรีโดยไม่ถูกหักสิทธิ์ **และไม่มีร่องรอยใด ๆ** · `refundReading()` ที่ `catch {}` เปล่าก็เช่นกัน

**แก้แล้วเป็น:**

```ts
} catch (e) {
  if (isUniqueViolation(e)) return true;          // ชน UNIQUE จริง → ผ่าน ไม่หักซ้ำ
  return onDbFailure("consumeReading/insert", e); // อย่างอื่น → metric + นโยบาย
}
```

### 11.3 นโยบายเมื่อฐานข้อมูลสิทธิ์ล่ม

ประกาศไว้เป็นค่าคงที่เดียวใน `entitlement.ts` — **ไม่ใช่ผลพลอยได้ของ `try/catch`**

```ts
const DB_FAILURE_POLICY: "allow" | "deny" = "allow";
```

**เลือก `"allow"` เพราะ** ต้นทุนถูกคุมด้วย `AI_DAILY_CALL_CAP` ซึ่งเป็นเพดานแยกที่ทำงานอยู่แล้ว
ความเสียหายด้านเงินจึงมีขอบเขตจำกัดอยู่ดี · ขณะที่ `"deny"` จะบล็อก **เฉพาะสมาชิก**
(ผู้เยี่ยมชมนับด้วยคุกกี้ ไม่แตะ D1) — คือลงโทษผู้ใช้ที่ดีที่สุดเพราะระบบเราเองพัง

เปลี่ยนเป็น `"deny"` ได้ด้วยการแก้บรรทัดเดียว ถ้าวันหนึ่งต้นทุนสำคัญกว่าประสบการณ์ผู้ใช้

### 11.4 พฤติกรรมหลังแก้ (วัดจริงอีกครั้ง)

| ฟังก์ชัน | ผู้ใช้ | ก่อนแก้ | หลังแก้ |
| :--- | :--- | :--- | :--- |
| `consumeReading()` | สมาชิก | throw → 500 | คืน `true` + metric (ตามนโยบาย allow) |
| `getEntitlement()` | สมาชิก | throw → 500 | คืนสิทธิ์แบบ degrade `remaining: 3` + metric |
| `refundReading()` | ทุกคน | กลืนเงียบ | ไม่ throw แต่ยิง `entitlement_refund_failed` |
| `consumeReading()` | ผู้เยี่ยมชม | คืน `true` | เหมือนเดิม (ไม่แตะ D1) · ใช้หมดแล้วยังคืน `false` ถูกต้อง |

### 11.5 มาตรการที่เหลือ

| # | มาตรการ | สถานะ |
| :-- | :--- | :-- |
| 1 | ให้ `deploy.yml` รัน d1 migrations อัตโนมัติ | ✅ **มีอยู่แล้ว** — ขั้น "🗄️ Apply D1 Migrations (remote)" ก่อน deploy |
| 2 | แยกชนิด error ใน `catch` | ✅ แก้แล้ว (PR #218) |
| 3 | ยิง metric ทุกครั้งที่ D1 ล้ม | ✅ `entitlement_db_error` · `entitlement_refund_failed` |
| 4 | ตัดสินนโยบาย allow/deny ให้ชัด | ✅ `DB_FAILURE_POLICY = "allow"` |
| 5 | `/api/health` ตรวจว่าตารางมีจริง | ⬜ ยังไม่ทำ — การ์ด `dbReady` ใน `/admin` ทำหน้าที่นี้อยู่แต่ต้องเข้าไปดูเอง |

> 📊 **ต้องเฝ้า:** ถ้า `entitlement_db_error` ขึ้นในการ์ด "สถิติระบบสิทธิ์"
> แปลว่าช่วงนั้น **โควตาไม่ได้ถูกบังคับจริง** (ทุกคนเปิดไพ่ได้โดยไม่ถูกหัก) — ต้องรีบดู

---

## 12. วิวัฒนาการการออกแบบแถบแจ้งสิทธิ์ (Entitlement Gate Evolution)

ประวัติการออกแบบ UI/UX ขั้นเลือกผัง (บันทึกเพื่อความต่อเนื่องทางสถาปัตยกรรมตาม ISSUE-021):
- **รุ่นแรก:** เมื่อสิทธิ์หมด ระบบลบหน้าเลือกผังทิ้งทั้งหน้า เหลือกล่องแจ้งเตือนเล็ก ๆ เพียงกล่องเดียว (สร้างความสับสนและสูญเสียโอกาสให้ผู้ใช้สำรวจผัง)
- **รุ่นสอง:** นำการ์ดกั้นใบใหญ่มาวางคาไว้ด้านบน แม้เลือกผังได้แต่บดบังเนื้อหาไปกว่าครึ่งจอมือถือ และแจ้งเตือนซ้ำซ้อนกับหน้าต่างสิทธิ์ (`AccessDialog`) ตอนกดเริ่ม
- **รุ่นสาม (Value-first + Just-in-time — สถาปัตยกรรมปัจจุบัน):**
  1. เลือกดูผังทั้ง 20 แบบได้ตามปกติ 100% (การสำรวจและศึกษาผังไม่ใช่การใช้สิทธิ์)
  2. ปุ่ม "เริ่มเปิดไพ่" เปลี่ยนข้อความและสถานะล่วงหน้าเมื่อสิทธิ์หมด เพื่อป้องกันความประหลาดใจ
  3. ปิดการขายและนำเสนอแพ็กเกจ/การอัปเกรดที่ `AccessDialog` ในจังหวะที่ผู้ใช้มีความตั้งใจสูงสุด (Just-in-time conversion)
  4. การบังคับสิทธิ์จริงอยู่ที่ระดับ Server-side (`/api/reading/start` และ `consumeReading()`) เสมอ การซ่อนปุ่มไม่ใช่ระบบความปลอดภัย
- **การตัดรหัสผ่านคอมโพเนนต์:** เดิมมีการหุ้มด้วย `<EntitlementGate>` แต่เนื่องจาก props ไม่ได้ถูกใช้งานจริงและคืนค่า `{children}` เปล่า จึงทำการ inline children ที่ `TarotFlow.tsx` เพื่อความสะอาดของโค้ดและขจัด Dead Code ตามมาตรฐานวิศวกรรม
