...
> **สถานะ**: ✅ พัฒนาและตรวจสอบเสร็จสมบูรณ์ 100% (PR #250, #251 และการปรับปรุงข้อ 6.8)
> **ผู้วางแผนและพัฒนา**: Claude & Antigravity — 2026-09-05
...
### 6.7 ชุดทดสอบ QA และการขยายระบบตรวจเป็น 27 ด่าน
- สร้าง `scripts/qa/test-quick-fortune.ts` ตรวจสอบทั้ง 4 หัวข้อ, ความปลอดภัยของคำถาม default, การแมปกับ Spread `quick`, และ Server Draw Logic (42 เคส)
- เชื่อมต่อเข้ากับ `scripts/github-auto.ts` เป็น Gate 27 ป้องกันความเสี่ยงจากการถอยหลัง (Regression Guard)

### 6.8 การบันทึกประวัติการทำนายด่วน (Reading Journal & History) และขอบเขตห้องแชท

ตรวจโค้ดจริงพบสถานะปัจจุบัน 2 ส่วนที่แยกกันชัดเจน:

1. **คำทำนายหลัก**:
   - ระบบมีที่เก็บอยู่แล้ว (`SavedReadingItem` ใน `src/lib/utils/history.ts`) มีฟิลด์ `question` (ข้อความเต็ม) + `category` (หัวข้อ) พร้อมอยู่แล้ว
   - งานที่ต้องทำ: ส่งคำถามเต็มจริง ๆ ที่ auto-fill ไว้เข้าไปตอนบันทึก ไม่ใช่ชื่อหัวข้อสั้น ๆ — ผู้ใช้เปิดประวัติย้อนหลังจะเห็นชัดว่าตอนนั้น "ถามเรื่องไหน ถามอย่างไร"
   - ใน `src/app/TarotFlow.tsx`: ส่ง overrides `{ question, category, spread, nickname }` เข้า `startAIStreaming` โดยตรง ป้องกันปัญหา stale closure ของ React state ตอนทำนายด่วน
2. **ห้องแชทกับแม่หมอ**:
   - ตรวจแล้วพบว่า **ไม่เคยถูกบันทึกที่ไหนเลย ทั้งฝั่ง client และ server** (ตั้งใจออกแบบไว้แบบนี้เพื่อความเป็นส่วนตัวตาม PDPA)
   - ระบุในแผนชัดว่า **ไม่แตะส่วนนี้ คงพฤติกรรมเดิมไว้** ถ้าจะเก็บแชทในอนาคตต้องเปิดเป็นโปรเจกต์แยกที่ผ่านกระบวนการยินยอม PDPA ใหม่

ทั้งหมดรวมเป็นงานเดียวกับ state machine ในข้อ 6.4 ตามที่ขอ ไม่ต้องแยกทำทีหลัง — อัปเดต acceptance criteria, ไฟล์ที่เกี่ยวข้อง และ WORK_LOG ให้ตรงกันแล้ว
...
- [x] ลิงก์ "เลือกผังแบบเต็ม" ยังพาไปยัง flow 20 ผังเดิมได้ครบทุกฟีเจอร์ (ไม่มีอะไรพัง)
- [x] Provably-Fair proof (SHA-256 commit-reveal) ยังตรวจสอบได้ปกติแม้ไม่มี `pickedIndices`/`clientSeed` จากผู้ใช้
- [x] คำทำนายหลักบันทึกประวัติ (`SavedReadingItem`) ด้วยคำถามเต็มจริง ๆ ที่ auto-fill ไว้ (ไม่ใช่ชื่อหัวข้อสั้น ๆ) พร้อม category ถูกต้อง
- [x] ห้องแชทกับแม่หมอคงพฤติกรรมเดิมตาม PDPA (ไม่ถูกบันทึกทั้ง client และ server)
...
| ไฟล์ | ทำอะไร |
| :--- | :--- |
| `src/app/TarotFlow.tsx` | state machine หลัก + เชื่อม Quick Fortune + ส่งคำถามเต็มลงประวัติ |
| `src/data/spreads.ts` | ผูก spread `"quick"` ไพ่ 1 ใบ |
| `src/lib/entitlement/limits.ts` | ยืนยัน spread `"quick"` ใน `STANDARD_SPREAD_IDS` |
| `src/components/reading/QuickFortunePicker.tsx` (ใหม่) | UI การ์ดหัวข้อ 4 ใบ สไตล์วิหารพยากรณ์พร้อมภาพไพ่ 1909 Rider-Waite |
| `src/lib/utils/history.ts` | โครงสร้าง `SavedReadingItem` สำหรับบันทึกประวัติการทำนาย |
| `scripts/qa/test-quick-fortune.ts` | ชุดทดสอบ QA ครอบคลุม 42 เคส (Gate 27) |
