...
| :-- | :-- | :--- | :--- | :-- |
| **044** | 🟡 Medium | **หน้าต่างลอยอีก 4 บานยังไม่มีอนิเมชันขาออก — `return null` อยู่เหนือ `AnimatePresence` เหมือนกันหมด** | `src/components/reading/ShareModal.tsx`, `src/components/history/ReadingHistoryModal.tsx`, `src/components/encyclopedia/TarotEncyclopediaModal.tsx`, `src/components/card/CardZoomModal.tsx` | 🟠 **รอทำ** — บั๊กชุดเดียวกับ INC-0126 ที่แก้ไปแล้วที่ `AuthModal`: เขียน `exit={{...}}` ไว้ครบทุกบรรทัดแต่ไม่เคยทำงาน เพราะคอมโพเนนต์คืน `null` ทั้งก้อนตอนปิด `AnimatePresence` จึงหายไปพร้อมลูกในเฟรมเดียวกัน (วัดที่ `AuthModal` ก่อนแก้: หลังกดปิดอยู่ใน DOM 41ms · opacity ค่าเดียว — หลังแก้ 270ms · 12 ขั้น) · ขึ้นทะเบียนไว้ใน `ALLOWLIST` ของ **กฎข้อ 9 ของด่าน `test-motion-quality.ts`** แล้ว ปลดออกทีละบานเมื่อแก้ · ⚠️ `CardZoomModal` ต้องแยก `!card` ออกจาก `!isOpen` ตอนแก้ (เงื่อนไขปนกันอยู่ในบรรทัดเดียว) |
