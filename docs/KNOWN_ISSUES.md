...
| :-- | :-- | :--- | :--- | :-- |
| **049** | 🟡 Open | **สี่เรื่องสุดท้ายของผลตรวจรอบ 2 — ปิดไปแล้ว 3 เหลือเรื่องประสิทธิภาพที่วัดใหม่แล้วและเปลี่ยนรูป** | `astro/pages/spreads/index.astro`, `astro/layouts/BaseLayout.astro` | 🟡 **เหลือเฉพาะ R-02 · R-11/R-12 (2026-09-17)** — ✅ ข้อ 9 ปิด (KV บอกว่า `entitlement.enforced` = **เปิด** · มี `npm run flags:show`) · ✅ R-26 ปิด (`useState` 28 ➔ 16 · ด่านใหม่ 37 ข้อ) · ✅ R-31 ปิด (`knip.jsonc` + ลบของตาย + ต่อสายฮุกกำพร้า) · ✅ R-10 ปิด (unused-js เหลือ 20–270 ms และเป็น runtime ของ Astro) · 🟡 เหลือ **สไตล์ชีตก้อนเดียวถ่วงทุกหน้า (เรื่องเดียวกับ T-23)** และ **hydration ของ `/spreads`** · ตัวเลขวัดใหม่ทั้งชุดที่ [`HANDOFF_ROUND2_CLOSEOUT_2026-09-17.md`](plans/HANDOFF_ROUND2_CLOSEOUT_2026-09-17.md) |
...
### ISSUE-049 · สี่เรื่องสุดท้ายของผลตรวจรอบ 2 (พบ 2026-09-17 · ปิดไป 3 เมื่อ 2026-09-17)

เงื่อนไขภายนอกที่ค้างอยู่ทั้งสี่เรื่อง **ถูกปลดหมดแล้ว** ในรอบ `HANDOFF_ROUND2_CLOSEOUT_2026-09-17`
บันทึกเต็มพร้อมตัวเลขทุกตัวอยู่ที่ [`plans/HANDOFF_ROUND2_CLOSEOUT_2026-09-17.md`](plans/HANDOFF_ROUND2_CLOSEOUT_2026-09-17.md)

| ข้อ | ผลสรุป |
| :--- | :--- |
| **ข้อ 9** — ธง `entitlement.enforced` | ✅ **ปิด** — อ่านจาก KV ของ production ตรง ๆ ได้ `{"value":true}` = **ระบบสิทธิ์ถูกบังคับใช้อยู่จริง** ไม่มีใครปิดไว้ · ถามซ้ำได้เองด้วย `npm run flags:show` (อ่านอย่างเดียว) |
| **R-26 (ส่วนที่เหลือ)** | ✅ **ปิด** — ยุบครบสามกลุ่มที่เหลือ (สำรับ · เซสชัน provably-fair · สตรีมคำอ่าน) เป็นตัวลดสามตัว · `useState` **28 ➔ 16** · ด่านใหม่ `test-flow-state.ts` 37 ข้อ พิสูจน์ด้วย mutation test ว่าจับของพังได้จริงทั้ง 5 แบบ |
| **R-31 (ส่วนที่เหลือ)** | ✅ **ปิด** — commit `knip.jsonc` ที่นับ `scripts/**` เป็นจุดเริ่ม (ต้นตอของ "รายงานผิด 10 จุด") · ลบ `export default` ที่ไม่มีใครใช้ 4 จุด + alias `WEEKLY_LIMIT` + ถอด export ตัวอ่านความลับ 5 ตัว · **เจอของใหญ่กว่าที่รายงาน**: ฮุก `use-admin-resource.ts` เป็นไฟล์กำพร้า ไม่มีแผงไหนเรียกเลย (ต่อสายครบ 6 แผงแล้ว · INC-0195) |
| **R-10** `unused-javascript` | ✅ **ปิด** — วัดใหม่เหลือ **20–270 ms** (เดิมรายงาน 310–450) และไฟล์เดียวที่ถูกฟ้องคือ runtime React ของ Astro ซึ่งตัดเองไม่ได้ |

**ที่ยังเปิดอยู่ — และเปลี่ยนรูปจากที่ผลตรวจเขียนไว้**

| เรื่อง | ตัวเลขล่าสุด (Lighthouse 13.4.1 · มือถือ · production · 2026-09-17) | ทำไมยังไม่ลงมือ |
| :--- | :--- | :--- |
| **R-11 · R-12** สไตล์ชีตถ่วงเรนเดอร์ | ไฟล์เดียว `_astro/BaseLayout.*.css` (145 KB ดิบ / 24 KB zstd) ประหยัดได้ **100–400 ms** แล้วแต่หน้า | **เป็นเรื่องเดียวกับ T-23** (Tailwind v4 ออกสไตล์ชีตก้อนเดียวทั้งเว็บ) · ทางที่เหลือมีสองทาง: critical CSS (แลกความเสี่ยง FOUC ถาวร) หรือ **Early Hints ของ Cloudflare = สวิตช์ของเจ้าของ** |
| **R-02** `/spreads` ช้าที่สุดในเว็บ | TBT **926 ms** (ปกติ) · **701 ms** (บล็อกสคริปต์บอตแล้ว) · Style & Layout **1,357 ms** ทั้งที่มี element **น้อยที่สุด** ในสี่หน้า (667) · งานก้อนยาวสุด 687 ms = hydration ของคลังผัง 25 แบบ | ต้นตอครึ่งหนึ่งคือสไตล์ชีตก้อนเดียวข้างบน (ทุกหน้าเสีย 0.9–1.4 วินาทีเท่ากันหมด) อีกครึ่งคือ `client:idle` ที่ hydrate ทั้งคลังผังเพื่อแท็บกรอง · **แก้ให้ถูกต้องเปลี่ยนโครงหน้า ต้องวัดก่อน-หลังบน production** และผู้ตรวจลดระดับข้อนี้เป็น LOW เองแล้ว |

> 📌 หนี้ที่ตั้งใจเหลือไว้อีกข้อจาก R-31: **UI ที่ไม่มีใครเรนเดอร์แล้วจริง ๆ**
> (`RitualStepProgress` 132 บรรทัด · โครงร่างตอนโหลด 7 ตัว · `MiniRwsCard`)
> ลบได้ทันทีถ้าเจ้าของยืนยันว่าไม่เอากลับมา — เป็นการตัดสินใจเรื่องหน้าตาเว็บ ไม่ใช่งานทำความสะอาด
