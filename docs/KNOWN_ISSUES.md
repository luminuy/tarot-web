...
| **047** | 🟡 Medium | **สคริปต์ตรวจบอตของ Cloudflare (`/cdn-cgi/challenge-platform/scripts/jsd/main.js`) กินเธรดหลักมากกว่าโค้ดทั้งเว็บรวมกัน** — ถูกฉีดเข้า HTML ทุกหน้าที่ผู้ใช้จริงได้รับ | `scripts/cloudflare-phase1.ts` (สวิตช์ `fight_mode`) | 🟠 **แก้ในโค้ดแล้ว · เหลือรันคำสั่งเดียวด้วยโทเคน (2026-09-15)** — เจ้าของสั่ง "ปิดไปเลย" ➔ ตั้ง `fight_mode: false` ในสคริปต์แล้ว พร้อมด่านที่ 37 กฎ 6 กันเปิดกลับเงียบ ๆ · **ขั้นตอนที่เหลือ**: `export CLOUDFLARE_API_TOKEN=<token> && npm run cf:phase1` (เซสชันนี้ไม่มีโทเคนในเครื่อง จึงรันแทนไม่ได้) · **ตัวเลขที่จะได้คืน** (วัดบน production 2026-09-15 · mobile · simulate): หน้าแรก perf **52 ➔ 81** · `/cards` **56 ➔ 81** · `/blog` **52 ➔ 84** · TBT ลด 1,414–2,376 ms ทุกหน้า · **ของที่ยังกันอยู่ครบหลังปิด**: rate limit 20 ครั้ง/10 วิ บนเส้นเปิดไพ่ · บล็อก `curl`/`python-requests`/`scrapy` บน `/api/*` · บล็อก `wp-*` `.php` `.env` `.git` · บล็อกบอตเทรนโมเดล + สแกนเนอร์ SEO · `ai_bots_protection: block` · ⚠️ **หลังรันแล้วต้องยืนยันสองอย่าง**: HTML บน production ไม่มี `challenge-platform` อีก และยิง Lighthouse ซ้ำให้เห็นตัวเลขจริง — ถ้าสคริปต์ยังถูกฉีดอยู่ แปลว่าต้องปิดสวิตช์ **JavaScript Detections** แยกที่ Security ➔ Bots ด้วย (ดู INC-0177) |
