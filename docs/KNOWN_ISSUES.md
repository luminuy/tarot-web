...
| **สถานะ** | 🔴 **ยังไม่ได้แก้ — ตรวจซ้ำแล้วเมื่อ 2026-08-31 15:xx บน `main` commit `4793cd4`** กดปุ่มแล้วยังคงค้างที่ `SpreadCardSelector` (`textarea` = 0 ช่อง) แม้จะผ่านงานปรับ ritual-flow และ transition ของ Antigravity AI มาหลายรอบแล้ว |

---

## 🟡 ระดับ Medium — ทำงานผิดแต่ไม่บล็อกผู้ใช้

### ISSUE-008 · `cache.ts` พรีโหลดภาพไพ่จาก path ที่ไม่มีอยู่จริง — ยิง 404 ทุกครั้งที่เปิดหน้า

| หัวข้อ | รายละเอียด |
| :--- | :--- |
| **อาการ** | ทุกหน้าที่โหลด ยิง request 404 จำนวน **9 รายการ** ไปที่ `/cards/variants/w320/major-XX.webp` |
| **ตรวจยืนยันแล้ว** | เกิดทั้งบน dev server และ **บน production จริง**<br>`curl -s -o /dev/null -w "%{http_code}" https://tarot-web.bankjack10452.workers.dev/cards/variants/w320/major-00.webp` ➔ **404** |
| **ผลกระทบ** | 1) เปลืองรีเควสต์เปล่า 9 ครั้งต่อการเปิดหน้าหนึ่งครั้ง<br>2) **ระบบ pre-decoding ไม่ได้ทำงานเลย** — ที่ตั้งใจว่าจะโหลดภาพล่วงหน้าให้ลื่น กลับไม่ได้โหลดอะไรสักภาพ ฟีเจอร์นี้จึงเป็นศูนย์<br>3) console เต็มไปด้วย error บดบังปัญหาจริงอื่น ๆ |
| **สาเหตุ** | `src/lib/utils/cache.ts` บรรทัด 43 เขียน path ตายตัวว่า<br>`` img.src = `/cards/variants/w320/${cardFile}` ``<br>แต่โฟลเดอร์จริงที่มีคือ `public/cards/w128/`, `w256/`, `w512/`, `w1024/` — **ไม่มีโฟลเดอร์ `variants/` และไม่มีขนาด `w320` เลย** |
| **เริ่มดูตรงไหน** | `src/lib/utils/cache.ts` (ตัวแปร `PRELOAD_MAJOR_CARDS` และฟังก์ชัน `warmupTarotAssets`) เรียกใช้จาก `src/components/performance/AssetWarmup.tsx` ที่ถูกวางไว้ใน `src/app/layout.tsx` |
| **แนวทางแก้** | ใช้ `getCardImageSrc()` / ค่าใน `CARD_IMAGE_VARIANTS` จาก `src/lib/tarot/card-image.ts` แทนการเขียน path ตายตัว — ตรงตามกฎเหล็กข้อ 9 ที่ห้ามเขียน path ภาพไพ่เอง (ดู INC-0002) |
| **เกณฑ์ว่าแก้สำเร็จ** | เปิดหน้าใดก็ได้แล้ว console ไม่มี 404 ของ `/cards/...` เหลือ และตรวจใน Network เห็นว่าภาพ Major Arcana ถูกโหลดล่วงหน้าจริง |
| **หลังแก้เสร็จต้องทำ** | ลบรายการ `src/lib/utils/cache.ts` ออกจาก `ALLOWLIST` ใน `scripts/qa/test-image-paths.ts` (ด่านตรวจจะเตือนให้เองเมื่อไม่มีการละเมิดเหลือแล้ว) |

### ISSUE-009 · `cache.ts` พรีโหลดไฟล์เสียงที่ไม่มีอยู่ในโปรเจกต์

| หัวข้อ | รายละเอียด |
| :--- | :--- |
| **อาการ** | ทุกหน้าที่โหลด ยิง request 404 อีก **3 รายการ**:<br>`/sounds/shuffle.mp3`, `/sounds/card-pick.mp3`, `/sounds/reveal.mp3` |
| **ตรวจยืนยันแล้ว** | `ls public/` มีแค่ `_headers` และ `cards/` — **ไม่มีโฟลเดอร์ `sounds/` เลย** และบน production ก็ตอบ **404** เช่นกัน |
| **ผลกระทบ** | เปลืองรีเควสต์เปล่าอีก 3 ครั้งต่อการเปิดหน้า รวมกับ ISSUE-008 เป็น **12 รีเควสต์ 404 ต่อผู้เข้าชมหนึ่งครั้ง** |
| **สาเหตุ** | `src/lib/utils/cache.ts` บรรทัด 22-24 ประกาศ `PRELOAD_SOUNDS` ชี้ไปยังไฟล์ mp3 ที่ไม่เคยมีในโปรเจกต์<br>ระบบเสียงจริงของโปรเจกต์นี้ใช้ **Web Audio API สังเคราะห์เสียงเอง** ใน `src/lib/utils/audio.ts` ไม่ได้ใช้ไฟล์ mp3 |
| **แนวทางแก้** | ลบบล็อกพรีโหลดเสียงออก (ตรงกับสถาปัตยกรรมจริง) หรือถ้าต้องการใช้ไฟล์เสียงจริง ต้องเพิ่มไฟล์ลง `public/sounds/` ให้ครบก่อน |
| **เกณฑ์ว่าแก้สำเร็จ** | เปิดหน้าใดก็ได้แล้วไม่มี 404 ของ `/sounds/` เหลือใน console |
| **ข้อควรระวัง** | ⚠️ ห้ามแตะ `soundManager` ใน `src/lib/utils/audio.ts` — เป็นระบบเสียงที่ใช้งานจริงอยู่ตามกฎเหล็กในคู่มือ |
...
| **สถานะ** | 🟡 **ยังไม่ได้แก้ — ตรวจซ้ำแล้วบน `main` commit `4793cd4`** ยังไม่มีการเรียก `.toFixed()` ใน `TarotArtIcons.tsx` เลย |
