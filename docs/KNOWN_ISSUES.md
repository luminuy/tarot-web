...
| หัวข้อ | ค่า |
| :--- | :--- |
| **วันที่** | 2026-09-09 (รอบตรวจรับงาน รอบ 6: ปิดงานค้างทั้งทะเบียน — แผนผังไพ่ทับกัน 4 แบบ · แถบยุบ/ขยาย · ธงสิทธิ์ที่มองไม่เห็น — INC-0118) |
| **commit ฐาน** | `main` — Production Live (`edda543`) |
| **วิธีตรวจ** | `npm run repo:verify` (43/43 ด่าน) + เปิดด้วย Chromium headless บน **production build** 5 หน้า (`/` · `/cards` · `/daily` · `/spreads` · `/cards/major-00`) ทั้งโหมดปกติและโหมดลดการเคลื่อนไหว แล้วอ่าน `getComputedStyle` ของทุก element หา `backdrop-filter` ที่ยังทำงาน · `transition` ที่แตะ layout · และ `document.getAnimations()` เพื่อนับว่าอนิเมชันที่วิ่งอยู่เป็น transform/opacity ล้วนหรือไม่ (มีด่านกันผลลวง — ถ้า stylesheet โหลดไม่ติด ผลตรวจถูกทิ้ง) + วัดขนาด CSS/JS เทียบกับ build ของ `origin/main` จริง + ทดสอบด่านใหม่ด้วยการทำให้พังจริง 7 เคส<br>(รอบก่อน) สแกน HTML ที่ build ออกมาทั้ง 309 ไฟล์หา `data-site-header="` + เปิดด้วย Chromium headless บน production build นับหัวเว็บ/ฟุตเตอร์รายหน้า (รวม URL ที่ไม่มีอยู่จริง) + ทดสอบด่านด้วยการทำให้พังจริง 5 เคส<br>(รอบก่อน) + วัดตำแหน่งหัวเว็บด้วย Chromium mobile 390px ตลอดทุกหน้า **บน production build** (ไม่ใช่ dev) + เทียบความสูงตัวกันที่กับหัวเว็บ + สแกนเนื้อหาที่ถูกหัวเว็บทับ + `LayerTree.compositingReasons` ผ่าน CDP + ทดสอบด่านด้วยการทำให้พังจริง 6 เคส |
| **ผลสรุป** | ✅ **ISSUE-001 ถึง ISSUE-030 ปิดครบสมบูรณ์ 100%** · ตรวจสอบ 43 ด่าน ผ่านครบ · ทุกหน้าที่ผู้ใช้เปิดได้มีหัวเว็บครบแล้ว (เหลือเฉพาะ 7 หน้าที่ตั้งใจไม่มี ซึ่งขึ้นทะเบียนไว้ในด่านตรวจแล้ว) |
