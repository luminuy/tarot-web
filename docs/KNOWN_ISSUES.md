...
| :-- | :-- | :--- | :--- | :-- |
| **049** | 🟡 Open | **ผลตรวจรอบ 2 — เหลือลูกบิดเดียวที่อยู่นอกรีโป (Early Hints)** | `astro/layouts/BaseLayout.astro` (สไตล์ชีตก้อนเดียว) | 🟡 **รอเจ้าของกดสวิตช์ (2026-09-17)** — ✅ ข้อ 9 · R-26 · R-31 · R-10 ปิดครบ · ✅ **R-02 แก้แล้ว**: `/spreads` เลิก hydrate คลังผัง (JS **124.4 ➔ 100.1 KB gzip** · HTML 20 ➔ 33.8 · รวม **−10.5 KB** · ลิงก์ผังใน HTML ดิบ 6 ➔ 25) · 🟡 เหลือ **R-11/R-12 = สไตล์ชีตก้อนเดียวถ่วงเรนเดอร์ 100–400 ms** ซึ่งตรวจซ้ำแล้วไม่มีไขมันให้ตัดในโค้ด (142 KB ดิบ · 1,593 กฎ · ไม่มีก้อนผิดปกติ) ➔ **ทางเดียวคือเปิด Early Hints ที่ Cloudflare Dashboard** (เครื่องนี้ไม่มีสิทธิ์แก้ค่าโซน) |
...
| **R-02** `/spreads` ช้าที่สุดในเว็บ | ✅ **แก้ส่วนที่อยู่ในโค้ดแล้ว** — เลิก hydrate คลังผังทั้งก้อน: การ์ด 25 ใบเรนเดอร์เป็น HTML ตั้งแต่ตอนบิลด์ · แท็บใช้สคริปต์ ~1 KB · ปุ่มขยายใช้ `<details>` ➔ JS **124.4 ➔ 100.1 KB gzip** · รวมที่ผู้ใช้โหลด **−10.5 KB** · งาน hydration ก้อนยาว 687 ms หายไป · ⚠️ **ยังต้องยิง `npm run perf:lh -- spreads` บน production หลัง deploy** เทียบฐาน TBT 926/701 ms |

**ที่ยังเปิดอยู่ — และเป็นสวิตช์ของเจ้าของ ไม่ใช่โค้ด**

**R-11 · R-12**: สไตล์ชีตก้อนเดียว `_astro/BaseLayout.*.css` (142 KB ดิบ / 22.8 KB gzip)
ถ่วงเรนเดอร์ **100–400 ms** แล้วแต่หน้า · ตรวจองค์ประกอบซ้ำแล้ว — ตัวแปร CSS 557 ตัว ·
`@font-face` 1 KB · `@keyframes` 2 KB · กฎ 1,593 ข้อ — **ไม่มีก้อนไหนผิดปกติให้ตัด**
(ยืนยันข้อสรุปของ T-23 ด้วยตัวเลขชุดใหม่) · ทางที่เหลือมีสองทาง:

1. สกัด critical CSS ➔ ต้องรันเบราว์เซอร์ตอนบิลด์ + แลกความเสี่ยง FOUC ถาวร (T-23 ปฏิเสธไว้แล้ว)
2. **เปิด Early Hints** ที่ Cloudflare Dashboard ➔ `seertarot.net` ➔ Speed ➔ Optimization
   — ปิดกลับได้ทันที · เครื่องที่ทำงานรอบนี้ **ไม่มีสิทธิ์แก้ค่าโซน** (`wrangler` ล็อกอินด้วย OAuth
   ที่มีเฉพาะสิทธิ์ Workers/KV และไม่มี `CLOUDFLARE_API_TOKEN`) จึงกดจากที่นี่ไม่ได้จริง ๆ

> เปิดเมื่อไหร่แล้วบอก — จะได้ตามด้วยการใส่ `Link: rel=preload` ของสไตล์ชีตลง `_headers`
> (สร้างตอนบิลด์เพราะชื่อไฟล์มีแฮช) แล้ววัดผลซ้ำด้วย `npm run perf:lh`
