...
| **043** | 🟡 Medium | **หน้า `/blog` มี TBT 660 ms · perf 59 แย่กว่าทุกหน้าในเว็บ 4.1 เท่า — ยังไม่รู้สาเหตุ** | `src/app/(th)/blog/BlogIndexClient.tsx` (ผู้ต้องสงสัย ยังไม่ยืนยัน) | 🟠 **รอทำ 2026-09-12 · รอโควตา PageSpeed รีเซ็ต** — แผนเต็มอยู่ที่ [`docs/plans/HANDOFF_BLOG_TBT_2026-09-12.md`](plans/HANDOFF_BLOG_TBT_2026-09-12.md) · Lighthouse ชี้ว่า `/blog` TBT 660 ms ขณะที่หน้าอื่น 30–160 ms · **ตัดผู้ต้องสงสัยไปแล้ว 6 ข้อ**: เนื้อบทความไม่รั่วเข้าไคลเอนต์ (`getArticleSummaries()` ตัด content/toc/faqs ครบ) · `article-helpers.ts` เป็น `import type` ล้วน · `soundManager` สร้าง AudioContext แบบ lazy · ก้อนข้อมูลไพ่ 446 KB ไม่โหลดในหน้าไหนเลยตอนเปิดครั้งแรก · บันเดิล `/blog` เบาเป็นอันดับสองของเว็บ (177 KB บนเน็ตจริง) · HTML 196 KB อยู่ในเกณฑ์ปกติ · ⚠️ **ห้ามรื้อโค้ดก่อนยิง Lighthouse อ่าน `bootup-time`** เพราะ Browser pane วัดเวลาแกว่งถึง 8 เท่า (โหลดหน้าเดียวกันได้ 3,844 ms แล้ว 481 ms) · ⚠️ **`resource-summary` ของ Lighthouse นับแบบคลายบีบ** รอบ 2026-09-11 อ่านได้ "52 คำขอ 675 KB" แล้วไล่แก้ผิดหน้า ของจริง `/cards` มี **18 สคริปต์ 183 KB** บนเน็ตและ TBT 140 ms ซึ่งผ่านสบาย |
