...
| :-- | :-- | :--- | :--- | :-- |
| **049** | 🟡 Open | **สี่เรื่องจากผลตรวจรอบ 2 (`HANDOFF_ROUND2_2026-09-17`) ที่รอเงื่อนไขภายนอก ไม่ใช่รอคนเขียนโค้ด** | `astro/pages/spreads/`, `src/components/home/TarotFlow.tsx`, KV `app:flag:entitlement.enforced` | 🟡 **เปิดอยู่ (2026-09-17)** — R-02/R-10/R-11/R-12 ต้องวัด Lighthouse ใหม่ **หลัง R-01 ขึ้น production** ไม่งั้นตัวเลขไม่สะท้อนการแก้ · R-26 ยุบ `useState` ครบทุกกลุ่มต้องทำทีละกลุ่มพร้อมวิธีพิสูจน์ · ข้อ 9 ของผลตรวจต้องอ่านค่าจริงใน KV ของ production · รายละเอียดที่ ISSUE-049 ด้านล่าง |
...
### ISSUE-049 · สี่เรื่องจากผลตรวจรอบ 2 ที่รอเงื่อนไขภายนอก (พบ 2026-09-17)

รอบปิดผลตรวจ `HANDOFF_ROUND2_2026-09-17` แก้ไป **27 จาก 31 ข้อ**
สี่เรื่องนี้เหลือไว้เพราะ **ติดเงื่อนไขที่ยังไม่เกิด** ไม่ใช่เพราะยังไม่ได้ลงมือ

| ข้อ | เรื่อง | ทำไมยังไม่ทำ | ต้องมีอะไรถึงจะทำได้ |
| :--- | :--- | :--- | :--- |
| **R-02 · R-10 · R-11 · R-12** | ประสิทธิภาพ: `/spreads` TBT 1,039 ms · `unused-javascript` 310–450 ms · สไตล์ชีตถ่วงเรนเดอร์ 156–165 ms | ผลตรวจกำหนดลำดับไว้เองว่า **"ยิง Lighthouse ใหม่ ดูว่าเหลืออะไรจริง ๆ ก่อนลงแรง"** และตัวเลขทั้งชุดวัดมา **ก่อนแก้ R-01** ซึ่งเป็นบั๊กที่ทำให้ทุกหน้าโหลดเอกสารสองรอบ · วัดตอนนี้จะได้ตัวเลขที่ไม่สะท้อนของจริง และอาจไล่แก้สิ่งที่ R-01 แก้ให้แล้ว | R-01 ขึ้น production ➔ ยิง Lighthouse ใหม่ทั้งสี่หน้า ➔ ค่อยตัดสินรายข้อ · **R-02 ผู้ตรวจลดระดับเป็น LOW เองแล้ว** เพราะ T-20 เป็นความคลาดเคลื่อนของวิธีวัด ไม่ใช่บั๊ก |
| **R-26 (ส่วนที่เหลือ)** | ยุบ `useState` ที่เหลือใน `TarotFlow.tsx` เป็น `FlowState` union เดียว | **ทำขั้นแรกไปแล้ว**: กลุ่มหน้าต่างลอย 8 สถานะยุบเป็นตัวลดเดียว (`flow-overlay.ts`) ➔ `useState` **37 ➔ 28** และตรึงเป็น ratchet แล้ว · กลุ่มที่เหลือ (สำรับ · สตรีม · provably-fair) คือกลุ่มที่กฎเหล็กข้อ 4 กับข้อ 14 คุ้มครองอยู่ ยุบผิดแปลว่าไพ่แสดงผลผิด ซึ่งพิสูจน์ด้วย typecheck อย่างเดียวไม่พอ | ทำทีละกลุ่มพร้อม **ชุดทดสอบตัวลดแบบเดียวกับ `test-flow-overlay.ts`** (ยิงลำดับการกระทำจริง ไม่ใช่ค้นข้อความในซอร์ส) · ผู้ตรวจเองก็เขียนว่า "ทำแค่ขั้นนี้ก่อนขั้นเดียว อย่าเพิ่งแยกคอมโพเนนต์" |
| **ข้อ 9 ของผลตรวจ** | ยืนยันว่า `entitlement.enforced` บน production เป็น `false` จริงหรือไม่ | **ตัดสินจากโค้ดไม่ได้** — `src/lib/entitlement/flag.ts` ตั้งค่าเริ่มต้นเป็น **เปิด** (KV ล่ม/คีย์หาย = ยังบังคับสิทธิ์ต่อ) ค่าที่เป็นปัญหาได้มีทางเดียวคือมีคนตั้ง `false` ไว้ในคีย์ `app:flag:entitlement.enforced` | เจ้าของเปิด `/admin` ➔ แท็บระบบสิทธิ์ ➔ อ่านค่าที่แสดง (หน้านั้นดึงจาก KV ของ production ตรง ๆ) · เป็นการ**ตัดสินใจของเจ้าของ** ไม่ใช่บั๊กที่ต้องแก้ |
| **R-31 (ส่วนที่เหลือ)** | export ที่ใช้เฉพาะในไฟล์ตัวเอง 9 จุด | ลบสำเนา `breakdown()` ที่ชัดที่สุดไปแล้ว · ที่เหลือ `knip` **รายงานผิด 10 จุด** เพราะไม่ได้นับ `scripts/qa/*.ts` เป็นจุดเริ่ม (ผลตรวจเตือนเรื่องนี้ไว้เอง) · `CARD_IMAGE_VARIANTS` · `boxOf` · `MAP_MIN_COLUMN_PX` · `generateClientSeed` ถูกใช้โดยด่าน CI จริง ลบแล้วบิลด์พัง | ไล่ตรวจทีละตัวว่าไม่มีด่านไหนใช้ก่อนลบ · มูลค่าต่ำและความเสี่ยงไม่เป็นศูนย์ จึงไม่คุ้มทำรวดเดียว |
