...
| หัวข้อ | ค่า |
| :--- | :--- |
| **วันที่** | 2026-09-06 (รอบตรวจรับงาน: SEO Wave 2–4 + ความจริงเชิงตัวเลขในเอกสารแม่บท) |
| **commit ฐาน** | `main` — Production Live (`249f491`) |
| **วิธีตรวจ** | `npm run repo:verify` (33/33 ด่าน) + ตรวจสอบสารานุกรม 7 หน้า + ผัง 25 แบบ + ด่านตัวเลขเอกสาร |
| **ผลสรุป** | ✅ **ISSUE-001 ถึง ISSUE-030 ปิดครบสมบูรณ์ 100%** · ตรวจสอบ 33 ด่าน ผ่าน 33/33 |
| **รอบก่อนหน้า (ประวัติ)** | 2026-09-04 · dev server + `npm run repo:verify` (24/24 ด่าน) · ตรวจสอบ 24 ด่าน ผ่าน 24/24 |
...
## 🟡 ระดับ Medium — พบจากการตรวจรับคลื่น SEO 2–4 เมื่อ 2026-09-06 (ยังไม่ได้แก้)

> 📦 **แผนแก้พร้อมขั้นตอนและเกณฑ์ผ่าน อยู่ที่ [`docs/plans/HANDOFF_QA_SPREADS_2026-09-06.md`](plans/HANDOFF_QA_SPREADS_2026-09-06.md)** — อ่านที่นั่นก่อนลงมือ

### ISSUE-031 · `guestAllowed` ประกาศไว้ในข้อมูลผังแต่ไม่มีผลบังคับจริง

| หัวข้อ | รายละเอียด |
| :--- | :--- |
| **อาการ** | ผัง 5 ตัว (`yes-no`, `three-card`, `situation-solution`, `mind-body-spirit`, `how-they-feel`) ประกาศ `guestAllowed: false` แต่**ผู้เยี่ยมชมเปิดได้จริงทั้งหมด** |
| **ต้นเหตุ** | ไม่มีโค้ดฝั่งรันไทม์อ่าน `guestAllowed` เลยแม้แต่บรรทัดเดียว — ยืนยันด้วย `grep -rn "guestAllowed" src/` ซึ่งไม่เจอนอก `data/spreads.ts` · ตัวที่บังคับจริงคือ `STANDARD_SPREAD_IDS` ใน [`limits.ts`](../src/lib/entitlement/limits.ts) ที่ถูกอ่านใน [`api/reading/start/route.ts:122`](../src/app/api/reading/start/route.ts) และ [`TarotFlow.tsx:478`](../src/app/TarotFlow.tsx) |
| **ผลกระทบ** | คนอ่านโค้ดถูกหลอก — ใครตั้ง `guestAllowed: false` ให้ผังใหม่จะเชื่อว่าล็อกแล้วทั้งที่ไม่ได้ล็อก (รูปแบบเดียวกับ ISSUE-021 `EntitlementGate` ที่รับ props แล้วไม่ใช้) · ซ้ำร้าย [`test-seo-wave3.ts:43,51`](../scripts/qa/test-seo-wave3.ts) ตรวจฟิลด์นี้อยู่ ทำให้**ด่านเขียวโดยไม่ได้พิสูจน์พฤติกรรมจริง** |
| **แนวทางแก้** | ทำให้ `guestAllowed` เป็นแหล่งความจริงเดียวแทน `STANDARD_SPREAD_IDS` (ปิดปัญหาลิสต์ซ้ำสองที่ตาม INC-0005) หรือลบฟิลด์ทิ้งแล้วให้ด่านตรวจ `STANDARD_SPREAD_IDS` แทน |
| ⚠️ **ข้อควรระวัง** | **ห้ามเปลี่ยนว่าผังไหนเปิดฟรีอยู่จริง** — งานนี้คือทำให้โค้ดพูดความจริง ไม่ใช่เปลี่ยนนโยบายธุรกิจ |

### ISSUE-032 · ผังใหม่ทั้ง 5ยังไม่เคยถูกใช้งานจริงสักครั้ง

| หัวข้อ | รายละเอียด |
| :--- | :--- |
| **อาการ** | `love-six` `monthly-ten` `family` `luck` `study` ขึ้น production แล้ว (PR #284) แต่**ยังไม่มีใครเดินครบ 5 ขั้นจนได้คำทำนาย** และยังไม่จับภาพมือถือ 375px/320px ตามเกณฑ์ผ่านของคลื่น 3 |
| **ต้นเหตุ** | คลื่น 3 ปิดงานด้วยการตรวจค่าจากโค้ด (`positions.length`, พิกัด, `credits`) และ `repo:verify` ซึ่งไม่ได้เปิดเบราว์เซอร์จริง |
| **จุดเสี่ยงที่สุด** | `monthly-ten` ลอกพิกัดจาก `celtic-cross` แต่เลื่อนขวา ทำให้ช่องว่างระหว่างไพ่ใบที่ 6 (x=0.55) กับคอลัมน์ขวา (x=0.72) **แคบกว่าเดิม 23%** (0.17 เทียบกับ 0.22) — และ celtic-cross เองเคยล้นกรอบมาแล้วใน INC-0058 · อีกจุดคือเลขตำแหน่งบนไพ่ใบที่ 2 ซึ่งหมุน 90° ต้องตั้งตรงตาม INC-0070 |
| **ความเสี่ยงจริง** | ถ้าล้นจริงจะกระทบผู้ใช้มือถือซึ่งเป็น 85% ของเว็บ และเป็นหน้าที่ตั้งใจดึงทราฟฟิกจาก Google โดยเฉพาะ |
