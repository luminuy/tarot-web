...
| **047** | 🟡 Medium | **สคริปต์ตรวจบอตของ Cloudflare (`/cdn-cgi/challenge-platform/scripts/jsd/main.js`) กินเธรดหลักมากกว่าโค้ดทั้งเว็บรวมกัน** — ถูกฉีดเข้า HTML ทุกหน้าที่ผู้ใช้จริงได้รับ | ไม่ใช่ไฟล์ในรีโป — เป็นสวิตช์ใน Cloudflare Dashboard (Security ➔ Bots ➔ Bot Fight Mode / JavaScript Detections) | 🔴 **รอเจ้าของตัดสินใจ (พบ 2026-09-15)** — วัดด้วย Lighthouse 12.8.2 บน production (mobile · simulate): สคริปต์นี้กิน scripting **3,707–4,953 ms** และสร้าง long task รวม **1,849–2,472 ms** ต่อหน้า · ยิงหน้าเดียวกันซ้ำโดยบล็อกมันทิ้ง TBT ลดลงทันที **/blog 2,206 ➔ 24 ms · /spreads 1,952 ➔ 335 ms · /cards 1,256 ➔ 232 ms** (perf 53 ➔ 85 · 56 ➔ 76 · 59 ➔ 79) · ยืนยันว่าผู้ใช้ทั่วไปได้รับจริงด้วย `curl --compressed` พร้อม UA มือถือ (พบอ้างอิงในหน้า) · **ทางเลือก**: ปิด Bot Fight Mode แล้วกันบอตด้วย WAF rule แทน (เป็นทางที่ `HANDOFF_CF_REQUEST_REVIEW_2026-09-08.md` แนะนำอยู่แล้ว) — แต่ ⚠️ **ห้ามเผลอไปเปิด "Block AI Scrapers"** เพราะเจ้าของตั้งใจเปิดรับบอต AI search ไว้ (ISSUE-019) · **ยังไม่ยืนยันว่ากระทบเครื่องผู้ใช้จริงเท่าไร** — ตัวเลขข้างบนมาจาก Lighthouse CLI บนเครื่อง dev ที่หน่วง CPU 4 เท่า ควรยิง PSI ยืนยันเมื่อโควตาคืนในวันถัดไป |
