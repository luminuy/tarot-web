...
| :-- | :-- | :--- | :--- | :-- |
| **045** | 🟢 Low | **`TarotEncyclopediaModal` เปิดไม่ได้เลยทั้งเว็บ — ไม่มีใครเรียก `setIsEncyclopediaOpen(true)` สักจุด** | `src/components/home/TarotFlow.tsx`, `src/components/encyclopedia/TarotEncyclopediaModal.tsx` | 🟠 **รอเจ้าของเคาะ** — เจอระหว่างเก็บกวาดอนิเมชันรอบ 2026-09-11 · สถานะ `isEncyclopediaOpen` คงเป็น `false` ตลอดอายุหน้า คอมโพเนนต์ 300+ บรรทัด (พร้อม `DECK` 78 ใบ) จึงเป็นโค้ดที่ผู้ใช้ไม่มีวันเห็น · อยู่หลัง `next/dynamic` จึงไม่กินบันเดิลตั้งต้น แต่ก็ไม่มีประโยชน์เช่นกัน · หน้าที่ของมันซ้ำกับหน้า `/cards` ที่เป็นสารานุกรมตัวจริงอยู่แล้ว · **เป็นการลบฟีเจอร์ จึงไม่แตะเองโดยไม่ได้สั่ง** — สั่งลบได้เลยถ้าไม่ใช้แล้ว |
...
| **038** | 🔴 High | **ธง `entitlement.enforced` บน production ถูกตั้งเป็น `false` ค้างไว้** — ระบบสิทธิ์ทั้งชุดไม่ทำงาน | KV `app:flag:entitlement.enforced` · `/admin` แท็บ "สิทธิ์เปิดไพ่" | 🟠 **รอเจ้าของกดเปิด (ต้องใช้สิทธิ์แอดมิน · AI ทำแทนไม่ได้)** — ด่านล็อกอินถูกย้ายออกมานอกธงแล้ว (INC-0111) จึงบังคับ "ต้องสมัครก่อนเปิดไพ่" ได้แม้ธงปิด · แต่ตราบใดที่ธงยังปิด **โควตาสมาชิก 1 ครั้ง/วันไม่ถูกบังคับ · ผังใหญ่ทั้งหมดและปรมาจารย์ลับ 2 ท่านไม่ถูกล็อก** · ตรวจได้ด้วย `curl -s https://seertarot.net/api/entitlement` (ถ้าขึ้น `"enabled":false` แปลว่ายังปิดอยู่) · ⚠️ **อัปเดต 2026-09-09**: สาเหตุที่ปิดค้างได้นานคือ *สถานะของธงซ่อนอยู่ในแท็บที่ต้องกดเข้าไปดูเอง* ส่วนหน้าแรกของแผงแอดมินที่เปิดทุกวันเงียบสนิท · แก้ส่วนที่ AI ทำได้แล้ว: `/api/admin/system-health` รายงานสถานะธงออกมาเป็น `services.entitlement` + `warnings` และหน้าแรกของ `/admin` ขึ้น**แถบเตือนสีเหลืองพร้อมปุ่มไปแท็บที่แก้ได้ทันที** ตราบใดที่ธงยังปิด · **เหลือขั้นตอนเดียวที่ต้องใช้สิทธิ์เจ้าของ**: เปิด `/admin` → แท็บ "สิทธิ์เปิดไพ่" → กดเปิดสวิตช์ |
