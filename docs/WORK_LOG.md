...
### 🗓️ 2026-09-06: เขียนแผน QA ผังใหม่ทั้ง 5 และพบช่องโหว่ `guestAllowed` ที่ไม่มีผลบังคับ (โดย Claude)

> **ขอบเขต**: เขียนแผน + บันทึกปัญหาที่พบ — **ไม่ได้แก้โค้ดใด ๆ** (หลักการข้อ 0.2 ข้อ 7 · ไม่ขยายขอบเขตเอง)

**ยืนยันสถานะแผนก่อนหน้า** — งาน A/B/C ใน [`HANDOFF_DOCS_TRUTH_2026-09-06.md`](plans/HANDOFF_DOCS_TRUTH_2026-09-06.md) ปิดครบแล้วโดย Antigravity AI (commit `0a6e881`):
- `repo:verify` = **33 ด่าน** · ด่าน `test-docs-numbers.ts` ทำงานอยู่ใน `CHECKS`
- บทนำหมวดไพ่วัดใหม่ได้ **365–395 คำ** ครบทั้ง 6 หมวด (เกณฑ์ ≥ 300)
- ⚠️ **แก้คำรายงานเดิมของตัวเอง**: ข้อสงสัยที่เคยบันทึกไว้ว่า `CHECKS.length` (33) ไม่ตรงกับเลขที่ตัวรันพิมพ์ (32) **ไม่ใช่บั๊ก** — เกิดจากวิธีนับด้วย `grep`/slice ของผมเองที่ไปโดนอาร์เรย์อื่น ตอนนี้ `CHECKS.length` = 33 ตรงกับที่พิมพ์ทุกประการ ทีมไม่ต้องเสียเวลาขุด

**🔴 ปัญหาที่พบใหม่ระหว่างเตรียมแผน (บันทึกเป็น ISSUE-031 · ISSUE-032)**
1. **`guestAllowed` ประกาศไว้ในข้อมูลผังแต่ไม่มีโค้ดฝั่งรันไทม์อ่านเลย** — `grep -rn "guestAllowed" src/` ไม่เจอนอก `data/spreads.ts` ตัวที่บังคับจริงคือ `STANDARD_SPREAD_IDS` · ผลคือ **5 ผัง** (`yes-no`, `three-card`, `situation-solution`, `mind-body-spirit`, `how-they-feel`) เขียนว่า `guestAllowed: false` แต่ guest เปิดได้จริงทั้งหมด · ซ้ำร้าย [`test-seo-wave3.ts`](../scripts/qa/test-seo-wave3.ts) ตรวจฟิลด์นี้อยู่ ทำให้**ด่านเขียวโดยไม่ได้พิสูจน์พฤติกรรมจริง**
2. **ผังใหม่ทั้ง 5ยังไม่เคยถูกใช้งานจริงสักครั้ง** — จุดเสี่ยงสูงสุดคือ `monthly-ten` ที่ลอกพิกัดจาก `celtic-cross` แต่เลื่อนขวา ทำให้ช่องว่างระหว่างไพ่ใบที่ 6 กับคอลัมน์ขวา **แคบกว่าเดิม 23%** (0.17 เทียบกับ 0.22) ขณะที่ celtic-cross เองเคยล้นกรอบมาแล้วใน INC-0058

**เรื่องที่ต้องให้เจ้าของโปรเจกต์เคาะ**
- เจตนา SEO ของคลื่น 3 คือ "ให้ผังฟรีเพื่อลด bounce" แต่ `GUEST_LIMIT = 1` เป็นโควตา **ตลอดชีพ ไม่ใช่ต่อวัน** → คนที่มาจาก Google เปิดได้ครั้งเดียวแล้วเจอกำแพง ไม่ว่าจะเป็นผังฟรีหรือไม่ · เสนอ 3 ทางเลือกพร้อมผลกระทบต่อ SEO และรายได้ไว้ในแผน

**🔓 อัปเดตตามนโยบายเจ้าของ: เปิดให้ใช้ฟรีเต็มที่ 3 เดือน (ระบบสิทธิ์ปิดจากแผงแอดมิน)**
- ยืนยันจาก production: `GET /api/entitlement` คืน `{"enabled": false, ...}` — ธง KV `app:flag:entitlement.enforced` ถูกปิดโดยตั้งใจ (ค่าเริ่มต้นของโค้ดคือเปิด)
- **สิ่งที่ปิดตามไปด้วย**: โควตา guest/สมาชิก · ล็อกผังใหญ่ (`love-six`, `monthly-ten` เปิดได้ทุกคน) · ล็อกปรมาจารย์ลับ · เพดาน guest ต่อ IP — ทั้งหมดอยู่ในบล็อก `if (await isEntitlementEnabled())` ที่ [`api/reading/start/route.ts:100`](../src/app/api/reading/start/route.ts)
- ✅ **เกราะต้นทุนยังทำงาน**: `checkPerIpReadQuota()` และ `isAiCapReached()` อยู่ **นอก** บล็อกของธงใน [`api/reading/[id]/read/route.ts`](../src/app/api/reading/[id]/read/route.ts) — ความเสี่ยงด้านค่า AI ถูกคุมไว้
- 🔴 **พบเพิ่ม (ISSUE-033)**: `grep -rn "isEntitlementEnabled" scripts/qa/` เจอแค่ด่านเดียวที่ตรวจว่า *default = true* — **ไม่มีด่านไหนตรวจเคส "ธงปิด" เลย** ทั้งที่นั่นคือสภาพจริงของ production ไปอีก 3 เดือน และ **INC-0083 คือบั๊กในเส้นทางนี้พอดี**
- ⏰ **ISSUE-031 กลายเป็นระเบิดเวลา** — ไม่มีพิษภัยตอนนี้เพราะไม่มีอะไรถูกล็อก แต่วินาทีที่เปิดธงกลับ ค่าที่เขียนผิดจะกลายเป็นพฤติกรรมจริงทันที ต้องแก้ให้จบก่อนหมดโปร
- เขียนงาน C ใหม่ทั้งหมด: C.1 ทดสอบสภาพจริงธงปิด · C.2 เพิ่มด่านคุมเส้นทางธงปิด · C.3 เตรียมเส้นทางธงเปิดไว้ทดสอบในเครื่อง · พร้อม **เช็กลิสต์ "วันหมดโปร"**

**เอกสารที่จัดทำ**: [`docs/plans/HANDOFF_QA_SPREADS_2026-09-06.md`](plans/HANDOFF_QA_SPREADS_2026-09-06.md) — 3 งาน 2–3 PR พร้อมตารางเช็ก 5 ผัง × 3 ความกว้าง · สคริปต์วัดการล้นกรอบใน DevTools · ขั้นตอนทดสอบ guest แบบล้างคุกกี้ · กับดัก 9 ข้อ
...
- ยิง `curl` โดเมนจริง: `/cards/{major,minor,wands,cups,swords,pentacles,all,birth-card}` + ผังใหม่ทั้ง 5 + `/spreads/topic/love` ตอบ **200 ครบทุกตัว** และ `<title>` มีคำว่า "ยิปซี"
...
4. **หนี้ตกค้างจากคลื่น 3**: ยังไม่มีใครเดินพิธีกรรมครบขั้นบนผังใหม่ทั้ง 5 และยังไม่ได้จับภาพมือถือ 375px/320px ตามเกณฑ์ผ่านของคลื่น 3
...
- **เอกสารที่สร้าง**:
  ...
  - [`docs/plans/HANDOFF_SEO_WAVE2-4_2026-09-05.md`](plans/HANDOFF_SEO_WAVE2-4_2026-09-05.md) — แผนลงมือคลื่น 2-4: ยึด `/cards` (พบว่าตัวกรอง ชุดใหญ่/ดอกไพ่ ใน `CardsExplorer` เป็น client state ไม่มี URL แยก = มีเนื้อหาพอทำ 6 หน้าแต่ส่งแข่งได้หน้าเดียว), สร้างผังใหม่ทั้ง 5ชนกับ MyHora 1:1, ไพ่ประจำตัว, และเกณฑ์วัดผลจาก GSC
