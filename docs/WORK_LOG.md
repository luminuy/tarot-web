...
`nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free` เป็นผู้สมัครเดียวที่รอดรอบแรก แต่ตัวอย่างมีแค่ 1 ครั้ง (short probe ก็เจอ empty response 1/3 ครั้งแล้ว) — ยิงยืนยันซ้ำด้วย `--full-runs 3 --cases gold-004,gold-006,gold-021` (ผัง 1/5/10 ใบ คนละขนาด JSON) **ผลออกมา 0/3 — ล้มทุกรอบ** (`json_parse` พัง 1 ครั้ง, ตอบว่างเปล่า 2 ครั้ง) ยืนยันว่ารอบแรกที่ผ่านเป็นความบังเอิญ ไม่ใช่ของจริง

#### 🏁 สรุปสุดท้าย (2026-09-12): OpenRouter ฟรีไม่มีตัวไหนพร้อมใช้จริงตอนนี้

ทดสอบครบทั้ง 22 ตัวฟรีที่มี ณ วันนี้ — **0/22 ผ่านเกณฑ์คำอ่านไพ่เต็มรูปแบบซ้ำได้จริง** **ไม่ต่อเข้า production ในรอบนี้** เหตุผล: โมเดลฟรีของ OpenRouter (อย่างน้อยที่หาได้วันนี้) ไม่นิ่งพอสำหรับ path ที่ผู้ใช้จริงรอคำตอบอยู่ — ตอบว่างเปล่าแบบสุ่ม, ไม่รองรับ JSON mode, หรือ JSON พัง คนละครั้งกันในแต่ละรอบทดสอบของโมเดลเดียวกัน ต่างจาก Groq/Gemini ที่นิ่งพอจะวัดผลเป็นเปอร์เซ็นต์ที่ทำนายซ้ำได้

**ถ้าจะกลับมาทำต่อในอนาคต**: (1) รอ OpenRouter คัดโมเดลฟรีชุดใหม่ที่โตกว่านี้ แล้วรัน `npm run ai:probe-openrouter` ซ้ำ — ลิสต์เปลี่ยนบ่อยมาก หรือ (2) ยอมจ่ายเงินเลือกโมเดลแบบเสียเงินบน OpenRouter แทน (นอกขอบเขตคำถามเดิมที่ถามหา "ฟรี")
