...
### 🗓️ 2026-09-17 (รอบ 92): 🪶 ย้าย `/account` ออกจาก Worker ไป Astro (คลื่นที่ 4) + ปิดบันทึก ISSUE-047 ที่ค้างหลอกคนอ่าน

> ⚠️ **รอบนี้ยังไม่ deploy — เจ้าของสั่งให้รออนุมัติก่อน** (จึง commit + push ไว้ที่ branch เท่านั้น ยังไม่เปิด PR)

- **ต้นเรื่อง**: เจ้าของสั่งให้ลงมือ 2 เรื่องจากตารางเทียบทางเลือก — (2) Astro คลื่นที่ 4 และ (3) สไตล์ชีตถ่วงเรนเดอร์

- **1 · วัดของจริงก่อนว่าคลื่นที่ 4 เหลืออะไร** (ยิง production ดูส่วนหัวตอบกลับทีละเส้น)
  - `/` · `/daily` · `/spreads` · `/cards` · `/blog` · `/love/1-card` · `/pick-a-card` · `/cards/birth-card` ➔ **อยู่ที่ขอบหมดแล้ว** (ไม่มี `x-nextjs-cache`)
  - เหลือที่ Worker จริง ๆ แค่ `/account` · `/admin` · `/readers*` · `/s/*` · `/tester*` · `/api/*`
  - ในกลุ่มนี้ **มีแค่ `/account` ที่ย้ายได้** — ที่เหลือเป็นหน้าที่ต้องมีสถานะฝั่งเซิร์ฟเวอร์ (เอกสารระบุว่าตั้งใจคาไว้)

- **2 · ย้าย `/account` ไป Astro**
  - หน้านี้เป็น **เชลล์เปล่า** อยู่แล้วตั้งแต่อยู่ฝั่ง Next (T-28) — ข้อมูลผู้ใช้มาหลัง hydrate ทั้งหมด จึงย้ายได้โดยไม่เปลี่ยนพฤติกรรมและไม่มีข้อมูลใครรั่วผ่านแคชที่ขอบ
  - ไฟล์ใหม่: `astro/pages/account.astro` · `astro/islands/AccountRoot.tsx` (island ก้อนเดียว `client:load`) · `src/app/_shared/pages/account-th.ts` (metadata + `robots: noindex`)
  - ย้าย `AccountClient.tsx` ออกจากต้นไม้ของ Next ➔ `src/components/account/`
  - ถอด `/account` · `/account/*` ออกจาก `run_worker_first` ใน `wrangler.jsonc` และเติม `/account` เข้า `ASTRO_EXACT_ROUTES`
  - **แก้ด่านที่อ้างพาธเดิม 3 ด่าน**: `test-astro-routes` (เจ้าของเส้นทางเปลี่ยนข้าง) · `test-seo-wave4` (ตรวจ noindex ที่โมดูล metadata ตัวใหม่ + ตรวจว่าหน้า Astro ใช้จริง) · `test-digest` (พาธไฟล์หน้าบัญชี)
  - **ผลที่ได้**: `/account` ตอบจากขอบ ไม่ปลุก Worker อีกต่อไป · HTML 52 KB สร้างล่วงหน้าแล้ว · ไม่มีเพย์โหลด RSC/router ของ Next เหลือในหน้านี้

- **3 · ปิดบันทึก ISSUE-047 ที่ค้าง** — ตารางสรุปด้านบนของ `KNOWN_ISSUES.md` เขียนว่า Resolved ตั้งแต่ 2026-09-15 แต่ **บล็อกรายละเอียดด้านล่างยังเขียนว่า "ยังไม่ลงมือแก้ รอเจ้าของกดสวิตช์"** ➔ อัปเดตให้ตรงกัน พร้อมยืนยันซ้ำด้วย `curl` UA มือถือบนสามกลุ่มหน้า (`/` · `/daily` · `/account`) ว่า **ไม่มี `challenge-platform` เหลือแล้ว** และสรุปรายการของที่กันแทนอยู่ตอนนี้ + ความเสี่ยงที่ยอมรับไว้

- **4 · เรื่องสไตล์ชีต (ข้อ 3 ของเจ้าของ) — ยังไม่ลงมือ พร้อมเหตุผล**
  - ตรวจ production แล้วพบว่า **ของถูก ๆ ถูกใช้ไปหมดแล้วจริง**: ส่วนหัว `Link: rel=preload` มีอยู่ และโซนส่ง **Early Hints `103` จริง** (ยิงยืนยันแล้ว) — เอกสารปิดเคสรอบ 2 ที่ยังเขียนว่า "รอเจ้าของเปิด Early Hints" ล้าสมัยไปแล้ว
  - ที่เหลือคือ critical CSS (แลกกับ FOUC ถาวรทั้งเว็บ) หรือลดขอบเขต hydration ของคลังผัง — **ทั้งคู่เปลี่ยนโครงหน้าเว็บและต้องวัดก่อน-หลังด้วยการ profile จริง** ไม่ใช่งานที่ควรยัดรวมมากับรอบนี้
  - **อุปสรรคที่เจอระหว่างเตรียมวัด**: `npm run perf:lh` และ Lighthouse/Chromium ยิง `https://seertarot.net` จากเครื่องนี้ไม่ได้ (พร็อกซีของสภาพแวดล้อมใช้ใบรับรองของตัวเอง ➔ `ERR_CERT_AUTHORITY_INVALID`) ➔ ต้องวัดจากบิลด์ที่เสิร์ฟในเครื่องแทน หรือให้เจ้าของยิง PSI

- **การตรวจสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 75/75 ด่าน** · เปิดหน้า `/account` ที่ Astro สร้างด้วย Chromium จริง (จอ 1024 และ 390): h1 เดี่ยว · หัวเว็บ/ฟุตเตอร์ครบ · ตัวกันที่หัวเว็บ 69px ตรงกับความสูงจริง · ไม่มีล้นแนวนอน · ชื่อสมาชิกและป้าย "รอติดตามผล 44" ขึ้นถูก · สวิตช์ PDPA บันทึกได้ · ทางลัดเปิดบันทึกคำทำนายได้
