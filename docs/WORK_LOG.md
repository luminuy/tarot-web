...
| 3 | Meta Pixel ไม่ทำงาน — HTML จริง 274 KB ไม่มี `connect.facebook.net` เลย | 🟠 **รอเจ้าของใส่ค่า secret** (ท่อในโค้ดต่อครบแล้ว) |
...
**ข้อ 3 — Meta Pixel (AI ทำแทนไม่ได้)**
ตรวจแล้วพบว่า **ท่อในโค้ดต่อครบอยู่แล้ว** ทั้ง build-time (`deploy.yml` · `pr.yml` อ่านจาก `secrets`/`vars`)
และ runtime (`/api/config/analytics` อ่าน env ของ Worker) — **ที่ขาดคือ "ค่า" อย่างเดียว**
สิ่งที่ทำได้ในโค้ด: เพิ่มขั้น `📊 ตรวจว่าตั้งค่าวัดผลครบไหม` ใน `deploy.yml` ที่พิมพ์ชัด ๆ ว่าค่าไหนยังว่าง
พร้อมวิธีตั้ง (ตั้งใจให้ **เตือน ไม่ให้ล้ม** เพราะ deploy ไม่ควรพังด้วยเรื่องที่ไม่เกี่ยวกับโค้ด)
และแก้ `docs/PENDING_SETUP.md` ที่เขียนว่า "ครบถ้วนสมบูรณ์ 100%" ทั้งที่ไม่จริง ➔ เพิ่มหัวข้อ 0 ที่ระบุค่าที่ยังขาด
