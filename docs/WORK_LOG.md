...
**ที่มาและเหตุผล**:
เจ้าของโปรเจกต์แจ้งว่าหน้าตาเว็บ "ยังไม่สวย" และถามถึงการทำ UI แบบ Liquid Glass
รอบนี้เป็น **งานออกแบบและวางแผนล้วน ยังไม่แตะโค้ดจริงสักบรรทัด** — ผลลัพธ์คือเอกสารส่งต่อให้ทีมลงมือ

**สิ่งที่ตรวจพบจากโค้ดจริง**:
1. **ทุกกล่องมีน้ำหนักสายตาเท่ากัน** — `.altar-panel` · `.altar-card-porcelain` · `.altar-cloth`
   ใช้พื้น `#FFFFFF` เส้น `#D5CEC2` และ `--shadow-raised` ชุดเดียวกันทั้งหมด จึงไม่มีลำดับความสำคัญ
2. **เงาจางเกินไป** — `--shadow-raised` ใช้ alpha `0.04` / `0.06` มองแทบไม่เห็นบนมือถือ
3. **หน้าแรกมีทางเข้า 3 ทางซ้อนกัน และ `<h1>` อยู่ลำดับที่ 5** —
   `DailyCardStrip` ➔ `QuickFortunePicker` ➔ `<h1>` ➔ `SpreadCardSelector` (25 ผัง)
   ยืนยันว่าเป็นหนี้ที่รู้อยู่แล้วจากคอมเมนต์ `QuickFortunePicker.tsx:294` ที่ถูกบังคับให้ใช้ `<p>` แทน `<h2>` (INC-0130)
4. **Liquid Glass ของจริงใช้ไม่ได้** — `backdrop-filter` ถูกแบนถาวรจาก INC-0056 / INC-0060
   (วัดจริง: ถอดออกแล้ว fps 30 ➔ 58 · CSS 135,475 ➔ 124,134 ไบต์) และมีด่านที่ 43 `test-motion-quality.ts` เฝ้าอยู่

**สิ่งที่เจ้าของเคาะแล้ว**:
- **ธีม**: แบบ C · กระจกอุ่น (Warm Liquid Glass) — ได้ลุคกระจกโดยไม่ใช้ `backdrop-filter` เลย
- **โครงหน้าแรก**: แบบ 1 · ประตูเดียว — `<h1>` ขึ้นบนสุด ➔ ปุ่มเดียว ➔ ผัง 3 อัน ➔ ไพ่ประจำวัน ➔ ทำนายด่วน

**การพิสูจน์ก่อนส่งงาน (คำนวณจริง ไม่ได้ประมาณ)**:
- ไล่หาจุดมืดที่สุดของพื้นหลังไล่สีทั้งผืน (สุ่ม 51×51 จุด ตามสูตรผสมสี src-over) ได้ `#DBDCD0`
- คำนวณคอนทราสต์ตามสูตร WCAG 2.1 บนจุดนั้น: `ink` **10.90** · `muted` **4.84** · `gold-ink` **4.08**
- บนแผงกระจก (`--glass-fill: rgba(255,255,255,0.58)` ➔ ผิวจริง `#F0F0EB`):
  `ink` **13.23** · `muted` **5.87** · `gold-ink` **4.96** · ขอบตอนเลือก `#938265` **3.27**
- ปุ่มหลัก `#FDF7EC` บน `#33241A` = **13.99**
- ➔ ได้กฎถาวรของธีมนี้: **ตัวอักษรสีทองต้องอยู่บนแผงกระจกเสมอ ห้ามวางลอยบนพื้นหลังไล่สีโดยตรง**
  (`gold-ink` ได้แค่ 4.08 บนพื้นเปล่า ตกเกณฑ์ AA)

**สิ่งที่ส่งมอบ**:
- `docs/plans/HANDOFF_GLASS_HOME_2026-09-21.md` — ค่าทุกตัวคัดลอกไปวางได้ทันที · ตารางคอนทราสต์ที่วัดแล้ว ·
  กับดัก 7 ข้อ · เกณฑ์รับงาน 21 ข้อ · รายการงานที่ **ไม่อยู่** ในรอบนี้
- ตัวอย่างภาพที่เจ้าของอนุมัติ 2 ชุด (ธีม 4 แบบ · โครงหน้าแรก 3 แบบ) — ลิงก์อยู่ในเอกสารส่งต่อ

**กับดักที่บันทึกไว้เป็นพิเศษ**:
ด่าน `test-a11y-critical.ts` คำนวณคอนทราสต์จาก **ค่าโทเคน** ไม่ได้อ่านพิกเซลจริง
แผงกระจกเป็นพื้นโปร่ง สีใต้ตัวอักษรจึงเป็นผลของการผสม — **ด่านจะผ่านเขียวสนิทแม้คอนทราสต์จริงตก**
จึงกำหนด `--glass-fill` เป็น "พื้นความทึบขั้นต่ำ" ที่ห้ามลดลงเด็ดขาด

**สิ่งที่ค้างอยู่**: ทีมลงมือตามเอกสาร — 1 PR แตะ 4 ไฟล์ (`globals.css` · `TarotFlow.tsx` · `SpreadCardSelector.tsx` · `QuickFortunePicker.tsx`)

---

### 🗓️ 2026-09-21 (รอบ 109): 📉 ผ่าคลังคำอ่าน Pick A Card ออกจากบันเดิล — 143 ➔ 101 KB (ISSUE-050 ปิด)

**โจทย์**: `/pick-a-card` เป็นหน้าที่หนักที่สุดของเว็บ (JS 143 KB gzip) เพราะ `PICK_A_CARD_TOPICS`
เป็นก้อนเดียวที่มีทั้ง "ตัวตนของหัวข้อ/กอง" (ต้องใช้ทันที) และ "คลังคำอ่านไทย+อังกฤษ" ของ 8 หัวข้อ
(≈1,600 บรรทัด) ซึ่งผู้ใช้เห็นก็ต่อเมื่อ **เปิดกองแล้ว** เท่านั้น

**ที่ทำ**
- ผ่าไฟล์เป็นสองก้อน: `src/data/pick-a-card.ts` (451 บรรทัด — ตัวตนของหัวข้อ/กอง/เนื้อหา SEO)
  และ `src/data/pick-a-card-readings.ts` (1,647 บรรทัด — `PICK_A_CARD_POOLS` ที่**ฝั่งเซิร์ฟเวอร์เท่านั้นโหลด**)
- **เซิร์ฟเวอร์ประกอบบทเสริมให้เลย** แล้วส่งมากับคำตอบของ `/api/reading/[id]/shuffle` ใน `derived.script`
  (ประกอบตามภาษาของเซสชัน) ➔ เบราว์เซอร์ไม่ต้องรู้จักคลังคำอ่านอีกเลยสักบรรทัด
  · เลือกทางนี้แทน `import()` แบบ chunk แยก เพราะไม่ต้องยิงไฟล์เพิ่มตอนผู้ใช้กำลังรอเซิร์ฟเวอร์อยู่แล้ว
- `composeFromDerived()` เปลี่ยนพารามิเตอร์จาก "หัวข้อ" เป็น "คลังของหัวข้อนั้น" (ผู้เรียกตัวจริงคือเซิร์ฟเวอร์)
- หน้าเว็บยัง **เทียบรหัสไพ่ของบทเสริมกับไพ่ที่เปิดจริงทั้งสามใบก่อนแสดงเสมอ** — กฎเหล็กข้อ 14 ไม่ถูกลดทอน

**ผลวัดจริงจากบิลด์** (`npm run build` + ด่านงบบันเดิล)

| เส้นทาง | ก่อน | หลัง |
| :--- | ---: | ---: |
| `/pick-a-card` (ตัวแทน 18 หน้า) | 143 KB | **101 KB** (−42 KB · −29%) |
| HTML ของหน้าเดียวกัน | 16 KB | 16 KB (ไม่ขยับ) |

รัดเพดานด่านงบบันเดิลลงจาก 158 ➔ **115 KB** ทันทีตามหลัก ratchet

**ด่านตรวจ — กันความพังแบบใหม่ที่ "การแยกไฟล์" สร้างขึ้นมาเอง**
การแยกไฟล์ทำให้เกิดความพังที่ TypeScript จับไม่ได้เลย 3 แบบ จึงเพิ่มเข้า `test-pick-a-card.ts` (21 ➔ 24 ข้อ):
1. หัวข้อที่ไม่มีคลังคู่กัน — คอมไพล์ผ่านหมด แต่ผู้ใช้กดเปิดกองนั้นแล้วเปิดไม่ออกสักใบ
2. คลังกำพร้าที่ไม่มีหัวข้อรองรับ (เช่นพิมพ์รหัสหัวข้อผิด)
3. ไฟล์ฝั่งหน้าจอเผลอ `import` คลังคำอ่านกลับเข้าบันเดิล (สแกน 218 ไฟล์ใน `src/components` · `src/app/_shared` · `astro`)
**พิสูจน์แล้วว่าจับได้จริง** ด้วยการทำให้พังจริงทั้งสามแบบ ➔ ตกด่านทุกครั้ง

**ยิงจริงบนบิลด์ production ในเครื่อง**: `derived.script` มาครบทั้งภาษาไทยและอังกฤษ
และรหัสไพ่ในบทเสริมตรงกับไพ่ที่เซิร์ฟเวอร์เปิดจริงทั้งสามใบทั้งสองภาษา
...
**ปิดเคสครบแล้วในวันเดียวกัน** — เจ้าของเติมสิทธิ์ `Zone · Single Redirect · Edit` ให้ token
(เสียรอบไปหนึ่งรอบเพราะคู่มือเขียนชื่อสิทธิ์ผิดเป็น `Transform Rules` · INC-0204)
แล้วขั้นในสายพาน deploy ดันกฎขึ้นขอบสำเร็จ ยิงจริงยืนยันแล้วว่า `www` ทุกเส้นทางเด้ง 301
กลับโดเมนหลักพร้อม query string ครบ (รายละเอียดผลยิงอยู่ใน ISSUE-052)
อีกหนึ่งบทเรียนที่เก็บระหว่างทาง: ตัวตรวจผลหลังเขียนกฎยิงเร็วเกินไป 0.1 วินาที จนรายงานว่า
ล้มเหลวทั้งที่งานสำเร็จ — แก้เป็นยิงซ้ำแบบถอยหลังก่อนตัดสิน (INC-0205)
...
แต่ `pickedIndices` ของ API คือ *ตำแหน่งในพัด ไม่ใช่เลขไพ่* ฝั่งเว็บจึงสั่งไม่ได้ว่าให้ออกไพ่ใบไหน
และห้ามเจาะรูตรงนั้นเพราะเป็นหัวใจ Provably Fair ของทั้งเว็บ

**ทางที่เลือก — แยก "เส้นไพ่ที่คำนวณได้" ออกมาเป็นเส้นของตัวเอง**

`src/lib/reading/derived-draw.ts` (ใหม่) เป็นผู้คำนวณไพ่ทั้งหมดฝั่งเซิร์ฟเวอร์ โดยมีกติกา 4 ข้อ:
1. ไคลเอนต์ส่งได้แค่ **ข้อมูลตั้งต้น** (วันเกิด · รหัสหัวข้อ+กอง) ไม่ใช่คำตอบ — วันที่ของสำรับประจำวัน
   อ่านจากนาฬิกาเซิร์ฟเวอร์ (`bangkokDayKey()`) เท่านั้น ไม่งั้นไล่เลื่อนวันหาไพ่ที่ถูกใจได้
2. สเปกถูก **ตรึงลง record ตั้งแต่ `/start`** · `/shuffle` ไม่รับสเปกใหม่จาก body เลย
3. เซสชันไพ่คำนวณ **ไม่แตะ `drawCards()` และไม่รับ `pickedIndices`** (ตอบ 400 `DERIVED_SESSION_NO_PICK`)
4. คำนวณไม่ออก = ล้มพร้อมข้อความ "โหลดใหม่อีกครั้ง" ไม่มีไพ่สำรองใด ๆ (กฎเหล็กข้อ 14)

**หน้าที่เปลี่ยน**

| หน้า | เดิม | ใหม่ |
| :--- | :--- | :--- |
| `/pick-a-card` | เบราว์เซอร์จั่วเอง + คำอ่านสำเร็จรูป · ไม่ต้องล็อกอิน | เซิร์ฟเวอร์คำนวณสำรับประจำวันของกอง · แม่หมออ่านสด · ต้องเข้าสู่ระบบ |
| `/cards/birth-card` | คำนวณในเครื่องแล้วเฉลยไพ่ทันที · ไม่ต้องล็อกอิน | เซิร์ฟเวอร์คำนวณจากวันเกิดแล้วเปิดให้ · แม่หมออ่านสด · ต้องเข้าสู่ระบบ |

- คำอ่านที่เขียนไว้ล่วงหน้าของ Pick A Card **ไม่ได้ถูกทิ้ง** — ย้ายไปเป็น "บทเสริม" ใต้คำอ่านของแม่หมอ
  และแสดงได้ก็ต่อเมื่อรหัสไพ่ทั้งสามใบตรงกับไพ่ที่เซิร์ฟเวอร์เปิดจริง (ไม่ตรง = ทิ้งทั้งก้อน ห้ามเล่าถึงไพ่ที่ไม่ได้อยู่ตรงหน้า)
- หน้าไพ่วันเกิดยังคำนวณในเครื่องเพื่อโชว์รอยทางเลขศาสตร์ แต่ **เทียบรหัสไพ่กับของเซิร์ฟเวอร์ก่อนแสดงผลเสมอ**
- ลิงก์แชร์ `?d=&m=&y=` เปลี่ยนเป็น **เติมช่องให้เฉย ๆ ไม่เปิดไพ่เอง** — เดิมเปิดหน้าปุ๊บเสียโควตาของวันทันที
- `OneCardAiReading` ➔ `AiReadingPanel` (รองรับ 1–3 ใบ) ใช้ร่วมกันทั้งสี่หน้า ไม่ต้องมีสองตัวให้เลื่อนออกจากกัน

**ตัวจั่วฝั่งเบราว์เซอร์ถูกถอดออกหมด** (`draw-order.ts` · `drawPicks` · `initialDraw` · `dailyDraw`)
เหลือ `seededOrder()` ที่เซิร์ฟเวอร์เรียกอย่างเดียว — ไพ่ที่เบราว์เซอร์จั่วเองจะไม่ใช่ไพ่ใบเดียวกับที่แม่หมอกำลังอ่าน

**สูตรสำรับประจำวันใหม่: "ลำดับคงที่ต่อหัวข้อ + เข็มที่เดินทุกวัน"**
แต่ละตำแหน่งมีลำดับของตัวเอง (สับครั้งเดียวจากรหัสหัวข้อ) แล้วหยิบตามเข็มที่เดินวันละก้าว
ทำให้ได้คำสัญญาสองข้อ **ด้วยโครงสร้าง ไม่ใช่ด้วยความน่าจะเป็น**:
- วันเดียวกัน 4 กองได้คนละชุด และไพ่หลักไม่ซ้ำกัน
- เปิดกองเดิมพรุ่งนี้ **ไม่มีทาง** ได้ไพ่ใบเดิมในตำแหน่งเดิม (คำสัญญาเดิมของ INC-0198b ที่การสุ่มล้วนให้ได้แค่ "ส่วนใหญ่")
- กองหนึ่งเข้าถึงไพ่ได้ **64 ชุด** (วัดจริงจากการเดิน 400 วัน) และเข้าถึงคลังครบทุกชิ้น

**ด่านตรวจ**
- ด่านใหม่ `test-derived-draw.ts` — 20 ข้อ (ไพ่วันเกิดตรงสูตร 260 วันเกิด · ข้อมูลเสีย 11 กรณีต้องคืน undefined ·
...
· `/cards/birth-card` **116 ➔ 94 KB** (ทั้งที่เพิ่งได้ท่อ AI + กล่องสิทธิ์เข้าไปในหน้า)

**เส้นทางใหม่ในด่านงบบันเดิล**: `/pick-a-card` (ตัวแทนของ 18 หน้า ที่ไม่เคยถูกวัดเลย)
วัดได้ **JS 143 KB · HTML 16 KB** ➔ ตั้งเพดาน 158/20 KB · **นี่คือหน้าที่หนักที่สุดของเว็บตอนนี้**
ของหนักคือคลังคำอ่านสำเร็จรูปไทย+อังกฤษที่ถูกลากเข้าบันเดิลตั้งแต่ไบต์แรก ทั้งที่ใช้หลังเปิดกองแล้วเท่านั้น
➔ บันทึกเป็น **ISSUE-050** ไว้แล้ว (ไม่แก้ในรอบนี้ตามกฎข้อ 7 — คนละเรื่องกับงานคลื่นที่ 2 และต้องวัดผลแยก)

---

### 🗓️ 2026-09-18 (รอบ 101): 🔐🤖 ทุกทางเข้าเปิดไพ่ต้องใช้ AI และต้องเข้าสู่ระบบ — คลื่นที่ 1 (`/daily` · `/love/1-card`)

**คำสั่งเจ้าของโปรเจกต์**: ทุกหน้าที่เปิดไพ่ต้องให้คำอ่านจาก AI และต้องสมัครสมาชิกก่อน ไม่มีข้อยกเว้น
(โควตาคงเดิม **ฟรีวันละ 1 ครั้ง** ต่อสมาชิก — เจ้าของเคาะแล้วว่าไม่ขยับ)

**สิ่งที่พบตอนสำรวจ (ยิงจริงบน production 18 ก.ย. 2569 ในฐานะผู้ไม่ได้ล็อกอิน)**
- กำแพงสมัครสมาชิกมีอยู่ **หน้าเดียว** คือหน้าแรก (`/api/reading/start` ตอบ 403 `signup_required`)
- อีก 4 หน้าเปิดไพ่ได้จนจบโดยไม่ต้องล็อกอิน เพราะ **ไม่เคยเรียกเซิร์ฟเวอร์เลยสักครั้ง** —
  ใช้คำแปลไพ่ที่เขียนไว้ในสำรับ (`/daily` ได้อัศวินแห่งเหรียญ · `/love/1-card` ได้สี่แห่งดาบ จริง ๆ)

**ที่ทำในคลื่นนี้**
- `src/lib/reading/use-ai-reading.ts` (ใหม่) — ท่อ `start ➔ shuffle ➔ read` ที่ทุกหน้าใช้ร่วมกัน
  กำแพงสิทธิ์ถูกบังคับ **ที่เซิร์ฟเวอร์** ไม่ใช่ที่เบราว์เซอร์ · มีทางเลือก `resolveCards: false`
  ให้หน้าที่หวงบันเดิลเปิดสำรับไทยล้วนเอง ไม่ต้องลาก `@/data/cards` (พ่วงอังกฤษ ≈126 KB)
- `src/lib/tarot/client-seed.ts` (ใหม่) — ยกสูตรเมล็ดสุ่มออกจาก `TarotFlow` มาใช้ร่วมกัน
- `OneCardRitual` เลิกสุ่มไพ่เองในเครื่อง เปลี่ยนเป็นรับไพ่จากเซิร์ฟเวอร์ แล้วต่อ `AccessDialog` + `AuthModal`
- `OneCardAiReading` (ใหม่) — แสดงคำอ่านที่สตรีมมาบนหน้าไพ่ใบเดียว (ไม่ใช้ `StreamReader` ตัวเต็มที่ทำมาเพื่อผังหลายใบ)
- ผังภายใน 2 ผัง (`love-one` 1 ใบ · `birth-card` 2 ใบ) + ธง `internal` + `PUBLIC_SPREADS`
  ➔ ผังภายในไม่โผล่ในคลังผัง · sitemap · ตัวเลือกผังหน้าแรก **จำนวน "25 ผัง" ทั้งเว็บจึงไม่เพี้ยน**
- ด่าน `test-feature-gating` เปลี่ยนไปนับ `PUBLIC_SPREADS` แทน `SPREADS` (97/97 ผ่าน)

**ที่ยังไม่ได้ทำ — คลื่นที่ 2**
- `/pick-a-card` และ `/cards/birth-card` ยังไม่ได้ต่อ AI เพราะ **ไพ่สองหน้านี้ถูกกำหนดไว้ล่วงหน้า**
  (กองไพ่ตายตัว · ไพ่ที่คำนวณจากวันเกิด) แต่ `pickedIndices` ของ API คือ **ตำแหน่งในพัด ไม่ใช่เลขไพ่**
  (`deck[fanIndex]`) ฝั่งเว็บจึงสั่งไม่ได้ว่าให้ออกไพ่ใบไหน — และ **ห้ามเจาะรูตรงนี้** เพราะเป็นหัวใจ Provably Fair
  ➔ ทางที่ถูกคือให้ **เซิร์ฟเวอร์คำนวณไพ่เอง** จากวันเกิด / จากรหัสกอง แล้วเขียนลง `drawn` โดยตรง
  (ผัง `birth-card` เตรียมไว้ให้แล้วในคลื่นนี้ รอแค่ฝั่งเซิร์ฟเวอร์)

---

### 🗓️ 2026-09-18 (รอบ 100): 🫥 หัวลิ้นชักนำทางหายบน iOS Safari (INC-0200)

- **อาการ**: เจ้าของเปิดเมนูลิ้นชักบน iPhone แล้วหัวลิ้นชัก **ไม่มีชื่อ “วิหารพยากรณ์” และไม่มีบรรทัด “RIDER-WAITE TAROT”** เหลือแต่ป้าย `1909 RWS` ลอยอยู่ถัดจากโลโก้ · เปิดบน Chrome ทุกความกว้าง 320–1024px แสดงครบปกติ จึงไม่มีใครเห็นตอนรีวิว
- **สาเหตุราก**: คอลัมน์ที่ครอบสองบรรทัดนั้นเขียนไว้แค่ `flex flex-col min-w-0` **ไม่มีฐานความกว้าง** · ลูกทั้งคู่ใช้ `truncate` ซึ่งมี min-content เป็น 0 · Safari คิดความกว้างคอลัมน์แบบ shrink-to-fit จึงยุบเหลือเท่าป้าย `1909 RWS` (ป้ายเป็น `shrink-0` จึงรอด) ข้อความทั้งสองบรรทัดกว้าง 0 และหายไป ขณะที่ Chrome ใช้ max-content จึงไม่มีอาการ
- **เบาะแสที่ชี้ชัด**: แถวเมนูข้างล่างไม่เป็นอะไรเลย เพราะบล็อกข้อความของมันเขียน `flex-1 min-w-0` ไว้ครบ — ต่างกันแค่คำเดียว
- **การแก้ไข**: เติม `flex-1` ให้คอลัมน์หัวลิ้นชัก พร้อมคอมเมนต์อธิบายกับดักไว้ตรงจุด
- **กฎป้องกันถาวร**: ขยายด่าน “ของประดับต้องไม่ล่องหน” ให้ตรวจเพิ่มอีกข้อ — คอลัมน์ `flex-col` + `min-w-0` ที่มีลูกใช้ `truncate` **ต้องมีฐานความกว้าง** (`flex-1` · `grow` · `w-full` · `basis-*`) ไม่งั้นตกด่าน
- **พิสูจน์ว่าด่านจับได้จริง**: ย้อนมาร์กอัปกลับไปเป็นเวอร์ชันที่เป็นบั๊กแล้วรันด่าน ➔ ฟ้องตรงบรรทัดที่ผิด (`SacredNavDropdown.tsx:340`) · แก้แล้วผ่าน · `repo:verify` 76/76

### 🗓️ 2026-09-18 (รอบ 99): 🔐📐 ถอด React ออกจากหน้านโยบาย 2 หน้า และหน้าผังตามหมวด 12 หน้า — เหลือ 299/334 หน้าที่ไม่โหลด React เลย

**กวาดของที่เหลือหลังคลื่นถอด hydration สามรอบ** (รอบ 95 · 96 · 97) ตรวจไฟล์ `dist` ทั้งก้อน
แล้วพบว่าใน 45 หน้าที่ยัง hydrate อยู่ มี **14 หน้าที่จ่ายค่า React ฟรี ๆ** โดยไม่ได้อะไรกลับมา

| กลุ่มหน้า | จำนวน | JS ก่อน (gzip) | JS หลัง (gzip) | ลดลง |
| :--- | ---: | ---: | ---: | ---: |
| `/privacy` · `/en/privacy` | 2 | 72.9 KB · 11 ก้อน | **5.6 KB · 8 ก้อน** | **−92%** |
| `/spreads/topic/*` (ไทย+อังกฤษ) | 12 | 102.6 KB · 18 ก้อน | **4.7 KB · 5 ก้อน** | **−95%** |

**สิ่งที่ทำ**

1. **`TopicSpreadList` — ทิ้ง JS ทั้งหมด ไม่เหลือแม้แต่บรรทัดเดียว**
   `useState` ตัวเดียวของมันคือหีบเพลง "ความหมายตำแหน่งไพ่" เปลี่ยนเป็น `<details>`/`<summary>`
   ของเบราว์เซอร์ (ท่าเดียวกับ `ChangePasswordCard`) ได้คีย์บอร์ดและโปรแกรมอ่านหน้าจอครบกว่าเดิม
   *พฤติกรรมที่เปลี่ยนโดยตั้งใจ*: เดิมกางได้ทีละใบ ตอนนี้กางพร้อมกันหลายใบได้ — การ์ดแต่ละใบ
   มีความสูงของตัวเอง ไม่มีใบไหนดันใบอื่น และไม่เคยมีใครขอให้มันพับกันเอง

2. **ปุ่มลบข้อมูล PDPA — ย้ายตรรกะออกมาไว้ที่เดียว แล้วให้สองปลายทางเรียกใช้**
   `src/lib/account/delete-all-data.ts` (ใหม่) ถือขั้นตอนลบทั้งหมด · `/account` เรียกจาก `onClick`
   ของ island ที่ hydrate อยู่แล้ว · `/privacy` เรียกจาก `astro/scripts/delete-all-data.ts`
   (สคริปต์ 278 B gzip) ปุ่มตัวเดิมเรนเดอร์เป็น HTML ล้วนโดยไม่ hydrate
   **ห้ามคัดลอกตรรกะนี้ไปไว้สองที่** — เป็นขั้นตอนที่ลบข้อมูลผู้ใช้ถาวร

3. **รัดเพดานที่กลายเป็นด่านหลอกไปแล้ว + คุมกลุ่มที่ไม่เคยถูกวัด** (`test-bundle-budget`)
   `/cards/major-00` เพดาน 119 KB แต่ของจริง 5.1 KB · `/spreads/celtic-cross` เพดาน 136 KB
   แต่ของจริง 4.7 KB — เพดานยุค hydrate ที่ค้างอยู่หลังรอบ 96/97 แปลว่าถ้ามีคนเผลอใส่
   `client:*` กลับเข้าไปจนหน้าหนัก 100 KB ด่านก็ยังขึ้น ✅ · รัดทั้งคู่เหลือ 8 KB
   และเพิ่ม `/privacy` กับ `/spreads/topic/love` เข้าตาราง (สองกลุ่มนี้ไม่เคยมีด่านคุมเลย)

**ตรวจจริงด้วยเบราว์เซอร์** (Chromium ยิงไฟล์ `dist` จริง): หีบเพลงกาง/พับได้ · กางพร้อมกัน 2 ใบได้ ·
ป้ายสลับ ▼ ขยาย ↔ ▲ ย่อ · ตำแหน่งไพ่แสดงครบ 5 · ปุ่มลบกดยกเลิกแล้วปุ่มกลับมากดได้และข้อมูลไม่หาย ·
กดยืนยันแล้วยิง `DELETE /api/account` · ล้าง `localStorage`+`sessionStorage` · พากลับหน้าแรก ·
**ไม่มีคำขอสคริปต์ของ React สักเส้นทั้งสองหน้า**

**สถานะรวมหลังสี่รอบ**: **299 จาก 334 หน้าไม่โหลด React เลย** (เริ่มต้น 19 หน้า) · 35 หน้าที่เหลือ
ต้องใช้ React จริงทุกหน้า (หน้าแรก · Pick A Card · คลังไพ่ · ดัชนีบทความ · ไพ่วันเกิด · แชท · `/account` · `/daily` · `/love/1-card`)

ด่าน `repo:verify` ผ่านครบ 78/78 · `typecheck` 0 error

---

### 🗓️ 2026-09-18 (รอบ 99): 📐 จัดช่องไฟแถวปัดนิ้วของ Pick A Card ให้เป็นสัดส่วน

- **ที่มา**: เจ้าของบอกว่า “สวยแล้วแต่ชิดกันไป … อยากให้ปรับเป็นสัดส่วน ช่องใครช่องมันหน่อย”
- **สิ่งที่ทำ**:
  - แถบหัวข้อ: ช่องไฟจาก `gap-2` เป็น **`gap-4`** และการ์ดกว้างขึ้นเล็กน้อย (`33vw` ➔ `36vw`, สูงสุด 128 ➔ 136px) เห็น 2 ใบเต็ม ๆ พร้อมใบถัดไปโผล่ขอบ
  - แท่นกองไพ่ 4 กอง: ช่องไฟ `gap-3` ➔ **`gap-4`**
  - ไพ่ 3 ใบในแท่นเปิดไพ่: ช่องไฟ `gap-3` ➔ **`gap-4`** และ **ให้แต่ละใบมีช่องของตัวเอง** — กรอบบาง (`border-line/60`) + พื้นหลังอ่อน (`bg-surface/50`) + มุมโค้ง 16px + ระยะใน 12px จึงดูเป็นสัดเป็นส่วนแทนที่จะลอยติดกันเป็นพืด
- **พิสูจน์**: วัดบนจอ 375px — ช่องไพ่กว้าง 212px เท่ากันทุกใบ ช่องไฟ 16px เท่ากัน ไม่มีการล้นแนวตั้ง (ไพ่ 3D ไม่ถูกเฉือน) · `repo:verify` ผ่าน 76/76

### 🗓️ 2026-09-18 (รอบ 98): 🪶🔗 ปิดบัญชีแผนย้าย Astro — ตรวจว่าย้ายครบแล้ว · เจ้าของเคาะพักคลื่น 5 · แปลง 3 ต้นทุนที่เหลือให้มีเครื่องตรวจ

- **ต้นเรื่อง**: เจ้าของถามว่า "แผนที่เหลือเปลี่ยนเป็น React Astro Island ครบยัง" แล้วถามต่อว่า "ถ้าถอด Next ทิ้งมีผลกระทบอะไรบ้าง" และ "ไม่ย้ายก็ไม่มีผลใช่ไหม" ➔ สรุปเป็นคำสั่ง: **ไม่ย้ายต่อ · อัปเกรดเมื่อเริ่มขวาง · แก้เอกสาร · ปรับปรุงต้นทุนที่เหลือให้ดีที่สุด**

- **สิ่งที่พบเมื่อตรวจโค้ดจริง (ไม่ได้อ่านจากเอกสาร)**
  - **คลื่น 1–4 จบครบแล้ว** — วัดจาก `npm run build`: Astro **334 หน้า** · Next เหลือ **9 เส้นทาง** (หลังบ้าน/ต้องล็อกอิน 5 + ไดนามิกอ่าน D1 4) · ตัวเก็บกลางเห็น 339 หน้า (Astro 334 + Next 5) เพราะ 4 เส้นเป็น `force-dynamic` จึงไม่มีไฟล์ HTML
  - **หัวเอกสารแผน Astro ล้าสมัย** — ยังเขียน "Astro 305 หน้า · Next เหลือ 6 หน้า" และลิสต์ `/account` ว่าอยู่กับ Next ทั้งที่ย้ายไปแล้วใน PR #520 · และยังไม่นับ `/pick-a-card`
  - **การไม่ย้ายต่อไม่มีผลกับผู้ใช้ SEO หรือค่าใช้จ่ายเลย** — หน้าที่คนและ Google เข้าเป็นไฟล์สแตติกที่ขอบอยู่แล้ว · `/api/*` 68 เส้นต้องวิ่งผ่าน Worker ไม่ว่าจะเป็นเครื่องมือไหน ย้ายแล้วค่าคำขอเท่าเดิม
  - **ก้อนที่หนักที่สุดของคลื่น 5 ไม่ใช่ `/api/*`** — 68 เส้นใช้แค่ `request.headers` · `request.json` · `NextResponse.json` ซึ่งเกือบเป็น Web API มาตรฐาน · ตัวจริงคือ `cookies()` จาก `next/headers` ใน **16 ไฟล์** ที่พาดผ่านชั้น auth ทั้งชุด
  - **กับดักที่ผมเตือนผิดไปหนึ่งข้อแล้วไปเช็คเพิ่ม** — "เพิ่มหน้า Next แล้วลืมเติม `run_worker_first` = 404 เงียบ" มีด่าน `test-worker-first-routes.ts` จับให้อยู่แล้ว ตัดออกจากรายการความเสี่ยงได้

- **สิ่งที่ทำ — แปลง 3 ต้นทุนที่เหลือจาก "คอมเมนต์ที่คนต้องจำ" เป็น "เครื่องตรวจ"**

  | ต้นทุน | เดิม | ตอนนี้ |
  | :--- | :--- | :--- |
  | CI บิลด์สองเครื่องมือทุก PR | ไม่มีแคชเลย | ขั้น `actions/cache` ใน `pr.yml` เก็บ `.next/cache` + `node_modules/.vite` · **ห้ามแคช `.next` ทั้งก้อน** (นั่นคือผลลัพธ์ที่ด่านทั้งชุดเอาไปตรวจ — ตระกูลเดียวกับ R-06) · `deploy.yml` ไม่แคชโดยตั้งใจ |
  | อัปเกรด `next` ต้องรอ `@opennextjs/cloudflare` | ความจำคนล้วน ๆ | ด่าน `test-next-adapter-pair.ts` |
  | ห้ามแก้คอมโพเนนต์ร่วมให้เลิกใช้ `next/*` | คอมเมนต์ใน `astro.config.mjs` | ด่าน `test-astro-shim-coverage.ts` |

- **ด่านใหม่ 2 ด่าน (76 ➔ 78)**
  - `test-astro-shim-coverage.ts` — `next/*` ทุกตัวที่ถูกนำเข้าใน `astro/**` · `src/components/**` · `src/app/_shared/**` ต้องมี shim ผูก alias ไว้จริง และไฟล์ shim ต้องไม่ว่างเปล่า · **ความพังที่ด่านนี้กัน**: เติม `import Image from "next/image"` ลงคอมโพเนนต์ร่วม ➔ บิลด์ผ่าน · หน้าเว็บดูปกติ · แต่ island ลากรันไทม์ของ Next ติดไปด้วยเงียบ ๆ (ตระกูลเดียวกับ "เผลอ import ข้อมูล ➔ บันเดิล 24 KB ➔ 980 KB")
  - `test-next-adapter-pair.ts` — ตรวจสองชั้น: ช่วง `peerDependencies` ที่ตัวแปลงประกาศ **และ** คู่รุ่นใหญ่ที่พิสูจน์แล้ว (`PROVEN_PAIR`) · **ทำไมชั้นแรกไม่พอ**: ช่วงที่ตัวแปลงประกาศ (`>=15.5.24 <16 || >=16.3.3`) **เปิดปลายด้านบน** ➔ Next 17 ผ่าน `npm ci` ฉลุยทั้งที่ตัวแปลงยังไม่เคยเห็น แล้วไปล้มตอน `opennextjs-cloudflare build` บน main หลัง merge · **ด่านนี้คือสัญญาณเริ่มคลื่น 5**

- **การพิสูจน์ว่าด่านใหม่ไม่ใช่ด่านหลอก (mutation test)**
  - `test-astro-shim-coverage.ts`: แทรก `import NextImage from "next/image"` ลง `src/components/card/CardImage.tsx` ➔ ด่าน**ตก**พร้อมชี้ชื่อไฟล์และบอกทางแก้ (และเตือนห้ามแก้ด้วยการถอด `next/*` ออกจากคอมโพเนนต์ร่วม) แล้วคืนไฟล์กลับ
  - `test-next-adapter-pair.ts`: ยิงตัวประเมินช่วงเวอร์ชัน **15 เคส** ทั้งขอบล่าง/ขอบบน/รุ่น canary/`^`/`~`/ช่วงที่อ่านไม่ออก ➔ ถูกทุกเคส · เคสสำคัญคือ `17.0.0` **ผ่าน**ช่วงที่ประกาศ ซึ่งพิสูจน์ว่า `npm ci` จับไม่ได้จริงและชั้น `PROVEN_PAIR` จำเป็น · ช่วงที่อ่านไม่ออกคืน `null` ➔ ด่าน**ตก** ไม่ใช่ผ่านเงียบ
  - จับบั๊กของตัวเองระหว่างทาง: ตัวประเมินรุ่นแรกอ่าน `<16` (เลขไม่ครบสามหลัก) ไม่ออก ทำให้ด่านตกทั้งที่ของจริงถูกต้อง ➔ แก้ให้เติมเลขที่หายเป็น `16.0.0`

- **เอกสารที่แก้**
  - `docs/plans/HANDOFF_ASTRO_MIGRATION_2026-09-15.md` — เขียนหัวใหม่ทั้งบล็อกด้วยตัวเลขที่วัดวันนี้ · มาร์กคลื่น 4 เป็นเสร็จ · มาร์กคลื่น 5 เป็น ⏸️ พร้อมเงื่อนไขที่จะเริ่ม · เพิ่มหัวข้อ **3.5** ผลกระทบของการถอด Next ทิ้ง (ปริมาณงานรายก้อน · กับดัก 2 ข้อ · ต้นทุนที่เหลือกับสิ่งที่ทำไปแล้ว)
  - `CLAUDE.md` แถวดัชนีของแผน Astro — เดิมยังเล่าแค่คลื่นที่ 1
  - ตัวเลขด่าน 76 ➔ 78 ใน 8 ไฟล์ (ด่าน `test-docs-numbers` เป็นคนชี้จุดให้ทั้งหมด ไม่ได้ไล่หาเอง)

- **หมายเหตุกับดักที่ยังรอเจ้าของ**: วันที่ทำคลื่น 5 จริง กฎ `www ➔ apex` ย้ายไป `public/_redirects` **ไม่ได้** เพราะ Cloudflare `_redirects` ไม่รองรับเงื่อนไข host ➔ ต้องตั้ง Redirect Rule บน Dashboard ซึ่งต้องใช้สิทธิ์เจ้าของ (บันทึกไว้ในหัวข้อ 3.5 แล้ว)
### 🗓️ 2026-09-18 (รอบ 98): 🎴 การ์ดหัวข้อเปลี่ยนเป็นแนวตั้งตามสัดส่วนไพ่

- **ที่มา**: เจ้าของดูแถบหัวข้อแบบปัดนิ้วแล้วบอกว่า “ยังไม่สวย ยาวอย่างเดียว ไม่ได้สัดส่วน” — ของเดิมเป็นแถบนอนยาว 62vw แต่ภาพไพ่เล็กแค่ 30×48 จึงดูโล่งและเสียสัดส่วน
- **สิ่งที่ทำ**: เปลี่ยนการ์ดหัวข้อเป็น **แนวตั้งตามสัดส่วนไพ่จริง 2:3** — ภาพไพ่ 1909 เต็มความกว้างการ์ดอยู่ด้านบน ชื่อหัวข้ออยู่ล่าง ตามด้วยป้าย “4 กองไพ่”
  - จอเล็ก: การ์ดกว้าง `33vw` (สูงสุด 128px) เห็นประมาณ 2.5 ใบ + ใบถัดไปโผล่ขอบ
  - จอ `sm` ขึ้นไป: ตาราง 4 คอลัมน์ 2 แถว การ์ดกว้างสูงสุด 132px จัดกึ่งกลางช่อง
  - การ์ดที่เลือกอยู่: ขอบทอง + ขีดทองใต้การ์ด + ชื่อเป็นสีทอง
- **บั๊กที่เจอระหว่างทางและแก้แล้ว**: ใส่ `block` คู่กับ `line-clamp-2` ทำให้การตัดบรรทัดไม่ทำงาน (ทั้งคู่สั่ง `display` ชนกัน) ชื่อหัวข้อยาวจึงล้นเป็น 3 บรรทัดและการ์ดสูงไม่เท่ากัน — ถอด `block` ออกและใส่ `min-h-[38px]` ให้ทุกใบสูงเท่ากัน พร้อมคอมเมนต์เตือนไว้ในโค้ด
- **พิสูจน์**: ดูจริงทั้งจอ 375px และ 1024px — ชื่อตัดที่ 2 บรรทัดครบทุกใบ การ์ดสูงเท่ากันหมด · `repo:verify` ผ่าน 76/76

### 🗓️ 2026-09-18 (รอบ 97): 📐📰 ถอด React ออกจากหน้าคู่มือผัง 50 หน้า และหน้าบทความ 52 หน้า — เหลือ 285/326 หน้าที่ไม่โหลด React เลย

- **ต้นเรื่อง**: เจ้าของสั่ง "ลุยต่อ" หลังรอบ 96 (หน้าไพ่รายใบ) — เป้าที่เหลือใหญ่สุดคือคู่มือผังรายผังกับบทความ

- **สิ่งที่พบเมื่ออ่านโค้ดจริง**
  - `SpreadDetailClient` (307 บรรทัด) **ไม่มี `useState` · `useEffect` · `onClick` เลยสักตัว** — เป็นคอมโพเนนต์แสดงผลล้วน ๆ ที่ถูกสั่ง hydrate มาตลอดโดยไม่มีอะไรให้ทำ · ลูกของมัน (`SpreadPositionMap` · `LocaleLink`) ก็ไม่มีเช่นกัน ➔ **ถอด `client:idle` ทิ้งได้เลย ไม่ต้องเขียนสคริปต์แทนสักบรรทัด**
  - `ArticleReadingClient` มีของที่ต้องใช้ JS ชิ้นเดียว: ปุ่ม "คัดลอกลิงก์" (กับสถิติ `blog_read` หนึ่งครั้ง)

- **สิ่งที่ทำ**
  - ถอด `client:idle` ออกจาก `/spreads/[id]` ทั้งสองภาษา (50 หน้า) — ไม่มีตัวแทน เพราะไม่มีอะไรต้องแทน
  - ถอด `client:idle` ออกจาก `/blog/[slug]` ทั้งสองภาษา (52 หน้า) แล้วย้ายปุ่มคัดลอกลิงก์ + สถิติไปที่ `astro/scripts/article-share.ts` (< 1 KB) · ข้อความ "คัดลอกลิงก์สำเร็จ!" สลับกลับเองใน 2.5 วินาทีเหมือนเดิม
  - ค่าที่ส่งให้สถิติเป็นภาษาของหน้า (บทเรียนจากรอบ 96 — `slug` เป็นกุญแจกลางที่ไม่ขึ้นกับภาษาอยู่แล้ว)

- **ผลที่วัดได้ (บิลด์ production)**

  | หน้า | JS เดิม | JS ใหม่ | island |
  | :--- | ---: | ---: | :--- |
  | `/spreads/celtic-cross` | ~200 KB | **7.7 KB** | 1 ➔ **0** |
  | `/blog/tarot-career-change-spread` | ~200 KB | **7.7 KB** | 1 ➔ **0** |

  - **หน้าที่ไม่โหลด React เลย: 175 ➔ 285 จาก 326 หน้า**
  - ที่เหลือ 41 หน้าคือหน้าที่มี UI โต้ตอบจริง (หน้าแรก · พิธีเปิดไพ่ · Pick A Card · คลังไพ่ที่มีช่องค้นหา · หน้ารวมบทความ · เครื่องคำนวณไพ่ประจำตัว · บัญชีสมาชิก) ซึ่งสมควรโหลด React

- **การตรวจสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 76/76 ด่าน** · เปิดจริงด้วย Chromium: หน้าบทความกดคัดลอกลิงก์แล้วได้ URL ในคลิปบอร์ดจริง ข้อความเปลี่ยนเป็น "คัดลอกลิงก์สำเร็จ!" แล้วคืนค่าเดิมใน 2.5 วินาที · หน้าคู่มือผังเนื้อหาครบ ไม่มี island เหลือ · ไม่มี error ใน console สักหน้า

### 🗓️ 2026-09-18 (รอบ 97): 🃏 ไพ่ 3 ใบในแท่นเปิดไพ่ปัดนิ้วได้แล้ว

- **ที่มา**: เจ้าของสั่งต่อจากรอบ 96 ว่าอยากให้ไพ่ 3 ใบเป็นสไลด์ด้วย (เดิมเรียงลงล่างทีละใบ ต้องเลื่อนยาวมากกว่าจะครบสามใบ)
- **สิ่งที่ทำ**: แถวไพ่ 3 ใบใช้แพตเทิร์นเดียวกับแถบหัวข้อและแท่นกองไพ่ — จอเล็กเป็นแถว `overflow-x-auto` + `snap-x snap-mandatory` การ์ดละ `70vw` (สูงสุด 220px) · จอ `sm` ขึ้นไปยังเป็นตาราง 3 ช่องเหมือนเดิม (วัดแล้วได้ `grid` 3 คอลัมน์ · `overflow-x: visible` · เลื่อนไม่ได้)
- **ข้อความชี้ทางปรับใหม่**: “แตะที่ตัวไพ่เพื่อพลิกดูหน้าไพ่ และปัดไปด้านข้างเพื่อดูใบถัดไป”
- **กับดักที่ระวังเป็นพิเศษ**: ไพ่พลิกด้วย 3D (`perspective` + `rotateY`) ถ้ากล่องเลื่อนเตี้ยไปจะเฉือนเงาและมุมไพ่ตอนพลิก จึงเผื่อ `pt-1 pb-3` ไว้ และวัดยืนยันแล้วว่า **ไม่มีการล้นแนวตั้งเลย** (`scrollHeight` 294 = `clientHeight` 294) ไพ่จึงไม่ถูกตัดตามกฎเหล็กข้อ 3
- **พิสูจน์**: เปิดไพ่ทั้งสามใบในแถวเลื่อนแล้วหน้าไพ่ขึ้นครบ (`cups-12` · `wands-06` · `cups-10`) · แถวเลื่อนได้จริง (`scrollWidth` 716 > `clientWidth` 343) · `repo:verify` ผ่าน 76/76

### 🗓️ 2026-09-18 (รอบ 96): 🃏 ถอด React ออกจากหน้าไพ่รายใบ 174 หน้า — เหลือ JS 7.7 KB และเครื่องมือค้นหาเห็นความหมาย "หัวกลับ" เป็นครั้งแรก

- **ต้นเรื่อง**: ต่อจากรอบ 95 ที่ถอด island ของหัวเว็บและของล่องหนออกไปแล้ว เหลือหน้าที่ยังโหลด React เพราะ island เนื้อหาของตัวเอง — กลุ่มใหญ่สุดคือหน้าไพ่รายใบ 174 หน้า (JS 203 KB ต่อหน้า)

- **สิ่งที่พบเมื่ออ่านโค้ดจริง**: `CardDetailView` ทั้งไฟล์ 373 บรรทัดมี `useState` **ตัวเดียว** คือสวิตช์ "ไพ่หัวตั้ง/ไพ่หัวกลับ" กับ `useEffect` ที่ยิงสถิติหนึ่งครั้ง — นั่นคือทั้งหมดที่ React ถูกโหลดมาทำ

- **สิ่งที่ทำ**
  - เรนเดอร์เนื้อหา **ทั้งสองหัวไพ่** ลง HTML (คีย์เวิร์ด + ความหมาย 5 ด้าน + หัวข้อ) แล้วซ่อนฝั่งที่ไม่ได้เลือกด้วย CSS ที่ผูกกับ `data-orientation` บนกล่องนอกสุด
  - ภาพไพ่หมุน 180° ด้วย CSS เดิม (`.card-orientation-flip`) แต่เปลี่ยนตัวขับจาก prop ของ React เป็นแอตทริบิวต์เดียวกัน
  - สคริปต์ `astro/scripts/card-orientation.ts` (< 1 KB) สลับแอตทริบิวต์ + `aria-pressed` และยิงสถิติ `card_detail_view` ด้วยสัญญาเดิมทุกคีย์
  - ถอด `client:idle` ออกจาก `/cards/[id]` ทั้งไทยและอังกฤษ
  - เก็บกวาดของที่กำพร้าเพราะการแก้นี้: ฮุก `useHasMounted` (ไม่เหลือผู้ใช้เลย · บทเรียน INC-0195) และคลาส `.anim-pop-in` + `@keyframes popIn` ที่ไม่มีใครเรียกแล้ว

- **ผลที่วัดได้ (หน้า `/cards/major-00` · บิลด์ production)**

  | | เดิม | ใหม่ |
  | :--- | ---: | ---: |
  | JS ที่โหลด | 203 KB | **7.7 KB** |
  | island ที่ต้อง hydrate | 1 | **0** |
  | ขนาด HTML | 86.5 KB | **78.3 KB** |

  - HTML **เล็กลง** ทั้งที่เพิ่มเนื้อหาหัวกลับเข้าไป เพราะไม่ต้องฝัง props ของ island (ไพ่ทั้งใบถูก serialize ลง HTML) อีกต่อไป
  - **หน้าที่ไม่โหลด React เลย: 19 ➔ 175 จาก 326 หน้า**
  - 🔎 **ผลพลอยได้ด้าน SEO**: ความหมาย "ไพ่หัวกลับ" ของไพ่ทั้ง 78 ใบ **ไม่เคยอยู่ใน HTML มาก่อนเลย** (เดิมเรนเดอร์เฉพาะหัวที่เลือกอยู่) ตอนนี้เครื่องมือค้นหาเห็นครบทั้งสองหัว

- **บทเรียนระหว่างทาง (ด่านจับได้จริง)**: ครั้งแรกส่งค่าให้สถิติผ่าน `data-card-name={card.nameTh}` ตรง ๆ ➔ ด่านภาษาอังกฤษฟ้องทันทีว่ามีอักษรไทยหลุดเข้า HTML ของหน้าอังกฤษ (แม้อยู่ในแอตทริบิวต์) ➔ เปลี่ยนเป็นค่าตามภาษาของหน้า (`card_id` ยังเป็นกุญแจกลางที่ไม่ขึ้นกับภาษา รายงานจึงยังรวมกันได้)

- **การตรวจสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 76/76 ด่าน** · เปิดจริงด้วย Chromium ทั้งหน้าไทยและอังกฤษ: สลับหัวไพ่แล้วเนื้อหาเปลี่ยนครบ · ภาพไพ่หมุน 180° จริง (`matrix(-1, 0, 0, -1, 0, 0)`) · `aria-pressed` ถูกต้อง · ไม่มี error ใน console

### 🗓️ 2026-09-17 (รอบ 95): ⚡ ถอด React ออกจากหัวเว็บและ "ของล่องหน" ของทุกหน้า — หน้าที่ไม่มี island เหลือ JS 13.9 KB

- **ต้นเรื่อง**: เจ้าของเลือก "ลด hydration" หลังเทียบทางเลือกกับ critical CSS (ซึ่งแลกกับ FOUC ถาวร)

- **วัดก่อนลงมือ — และพบว่าเป้าที่เอกสารชี้ไว้ผิดตัว**
  - เอกสารปิดเคสรอบ 2 เขียนว่า `/spreads` มีงานก้อนยาว 687 ms จาก `client.js` (hydration ของคลังผัง)
  - ตรวจจริงพบว่า **คลังผัง 25 แบบถูกถอด hydration ไปแล้วตั้งแต่รอบก่อน** (`SpreadsLibraryRoot` ไม่มี `client:*` · แท็บใช้สคริปต์ 35 ไบต์)
  - ตัวที่ลาก React 184 KB มาทุกหน้าคือ **island สองตัวที่เหลือ**: หัวเว็บ (`client:idle`) และ `ClientChromeRoot` ซึ่งห่อ "ของล่องหน" 3 อย่าง (เกราะกันดูดเนื้อหา · Service Worker · เครื่องมือวัดผล + แบนเนอร์ PDPA) — **ไม่มี UI สักตัว**
  - ทดลองถอดทีละตัวแล้วยืนยัน: **ถอดอันเดียวไม่ช่วยเลย** (ถอดหัวเว็บอย่างเดียว JS เท่าเดิม 199 KB เพราะอีกตัวยังลาก React มาอยู่ดี)

- **สิ่งที่ทำ**
  1. **ยกตรรกะเครื่องมือวัดผลทั้งก้อนไปไว้โมดูลกลาง** `src/lib/analytics-bootstrap.ts` (เดิมฝังเป็นสตริงใน `<Script>` ของคอมโพเนนต์ React) — Consent Mode v2 · การกู้สถานะความยินยอม · Meta Pixel ที่กั้นตั้งแต่ตัวโหลด · จังหวะโหลดตอนผู้ใช้เริ่มมีปฏิสัมพันธ์ · และกติกา "ถามเส้น runtime เฉพาะตอนไม่มีรหัสติดมากับ build" (กันไม่ให้ทุกแท็บยิง `/api/config/analytics` เพิ่ม)
  2. **ทั้งสองเครื่องเรนเดอร์เรียกโมดูลเดียวกัน** — `AnalyticsTracker.tsx` (React) เรียกใน `useEffect` · `astro/scripts/site-chrome.ts` (สคริปต์ธรรมดา) เรียกตรง ๆ · การตัดสินใจที่แบนเนอร์ก็ผ่าน `applyConsentChoice()` ตัวเดียวกัน
  3. **เขียนหัวเว็บเป็นสคริปต์ธรรมดา** `astro/scripts/site-header.ts` — เปิด/ปิดลิ้นชัก (สลับคลาสชุดเดิม) · Esc · กักโฟกัส · ล็อกสกรอลล์ · คืนโฟกัส · ปุ่มสลับภาษา · `ResizeObserver` วัด `--site-header-h` · markup ทั้งหมดยังเรนเดอร์ฝั่งเซิร์ฟเวอร์เหมือนเดิม ไม่ได้เขียน HTML ใหม่สักบรรทัด
  4. ถอด `client:idle` ของหัวเว็บออกจากหน้า Astro ทั้ง **44 ไฟล์** และเปลี่ยน `ClientChromeRoot` เป็นแถบความยินยอมที่เรนเดอร์อย่างเดียว (ไม่ hydrate)
  5. **ย้ายด่านวัดผลตามไปตรวจที่โมดูลกลาง** (`test-analytics-integrity`) พร้อมเพิ่มด่านใหม่ 2 ข้อ: ทั้งสองฝั่งต้องเรียกตัวบูตตัวเดียวกัน · ต้องถามเส้น runtime เฉพาะตอนไม่มีรหัสติดมากับ build

- **ผลที่วัดได้ (บิลด์ production จริง เสิร์ฟในเครื่อง · Lighthouse มือถือ · simulate)**

  | หน้า `/spreads` | เดิม | ใหม่ |
  | :--- | ---: | ---: |
  | JS ที่โหลด | 278.7 KB | **13.9 KB** |
  | bootup-time | 140 ms | **69 ms** |
  | script evaluation | 140 ms | **74 ms** |
  | island ที่ต้อง hydrate | 2 | **0** |

  - **19 จาก 318 หน้าไม่มี island เหลือเลย** (ไม่โหลด React สักไบต์) — `/spreads` · `/about` · `/contact` · หน้ารวมหมวดต่าง ๆ
  - อีก 299 หน้ายังโหลด React เพราะ **island ของเนื้อหาตัวเอง** (หน้าไพ่รายใบ 174 หน้า · บทความ · คู่มือรายผัง) แต่ลดจาก hydrate 3 ก้อนเหลือ 1 ก้อนทุกหน้า
  - 👉 **งานถัดไปที่คุ้มที่สุด**: ถอด hydration ของ island เนื้อหา 3 กลุ่มนั้น (หน้าไพ่รายใบใช้ JS 203 KB เพื่อฟีเจอร์ไม่กี่อย่าง)

- **การตรวจสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 76/76 ด่าน** · เปิดจริงด้วย Chromium บนบิลด์ production:
  - ลิ้นชัก: เปิด/ปิดได้ · `aria-hidden` และ `aria-expanded` ถูกต้อง · โฟกัสเข้าไปในลิ้นชัก · Esc ปิดแล้วคืนโฟกัสให้ปุ่มเดิม · สกรอลล์ถูกล็อกแล้วคืนค่าเดิม — ทดสอบทั้งหน้าไม่มี island (`/spreads`) และหน้าที่ยังมี island (`/cards` · `/account` · หน้าแรก)
  - ความยินยอม PDPA: แถบขึ้นสำหรับเครื่องใหม่ · **ไม่มีคุกกี้ใด ๆ ก่อนกดยินยอม** · กดแล้วบันทึก `granted` แถบหาย และถอด `data-consent-ask`
  - ปุ่มภาษา: พาไป `/en/spreads` จริงและตั้งคุกกี้ให้ · หน้าที่ไม่มีฝาแฝดจะจำภาษาไว้เฉย ๆ ไม่รีโหลดทิ้งเปล่า
  - `--site-header-h` = 69px ตรงกับความสูงจริง · ไม่มี error ใน console สักหน้า

### 🗓️ 2026-09-17 (รอบ 94): 🪶 ย้าย `/account` ออกจาก Worker ไป Astro (คลื่นที่ 4) + ปิดบันทึก ISSUE-047 ที่ค้างหลอกคนอ่าน

- **ปัญหาเชิง SEO ที่แก้**: 4 หัวข้อกองรวมอยู่ในหน้าเดียว ทั้งที่แต่ละหัวข้อเป็นคีย์เวิร์ดค้นหาคนละตัว (“pick a card เขาคิดยังไง” · “pick a card การงาน” · “ข้อความจากจักรวาล”) หน้าเดียวจึงแย่งอันดับกับตัวเองและไม่ตรงคำค้นสักคำ
- **สิ่งที่เพิ่ม**: **8 หน้าใหม่** — `/pick-a-card/<slug>` 4 หน้า และฝาแฝดอังกฤษ `/en/pick-a-card/<slug>` อีก 4 หน้า (รวมเว็บ 317 ➔ **325 หน้า**)
  - `src/app/_shared/pages/pick-a-card-topic.tsx` — ชื่อเรื่อง คำอธิบาย คีย์เวิร์ด บทความ ~200 คำ และ FAQ 3 ข้อ **ต่อหัวข้อ ต่อภาษา** พร้อม JSON-LD Breadcrumb + FAQPage
  - หน้าหัวข้อเปิดกองของหัวข้อนั้นทันที และการ์ดหัวข้ออื่นกลายเป็น **ลิงก์จริง** ไปหน้าของมัน (ลิงก์ภายในครบวง · หน้ารวมยังสลับในที่เดิมเหมือนเดิม)
  - ฝาแฝดอังกฤษเพิ่มที่ `EN_TWIN_DYNAMIC_PREFIXES` ที่เดียว ➔ sitemap + hreflang + `LocaleLink` ได้ครบทันที
- **ด่านที่ 76 เพิ่มเป็น 17 ข้อ**: ทุกหัวข้อต้องมีหน้าของตัวเอง มีเนื้อหา SEO ครบสองภาษา ชื่อเรื่องไม่เกินเพดาน SERP (48 ตัวอักษร + “ · SeerTarot”) และ canonical ต้องชี้มาที่หน้าหัวข้อจริง — หัวข้อใหม่ที่ลืมเขียนเนื้อหาจะตกด่านทันที
- **พิสูจน์**: บิลด์จริง 325 หน้า · `/pick-a-card/next-step-career-finance` และ `/en/pick-a-card/...` ตอบ 200 · `repo:verify` 76/76 (รวมด่านภาษาอังกฤษไม่มีไทยหลุด · sitemap ตรงกับไฟล์จริง · hreflang ชี้กันครบ)
- **เหลือจากแผน**: เพิ่มหัวข้อจาก 4 เป็น 8 (งานเขียนเนื้อหาล้วน — รอเจ้าของเคาะชื่อหัวข้อ)

### 🗓️ 2026-09-18 (รอบ 96): 🫱 Pick A Card เปลี่ยนแถบหัวข้อและแท่นกองไพ่เป็นแบบปัดนิ้ว

- **ที่มา**: เจ้าของสั่งให้ทำแบบสไลด์ปัดขวา โดยให้ดูตัวอย่างจากหน้าแรกแล้วปรับให้เข้ากับหน้านี้
- **แพตเทิร์นที่ยืมมา**: `QuickFortunePicker` (เปิดไพ่ด่วนบนหน้าแรก) — จอเล็กเป็นแถว `overflow-x-auto` + `snap-x snap-mandatory` + `no-scrollbar`, จอ `sm` ขึ้นไปกลับเป็นตารางเหมือนเดิม ไม่มีการเลื่อนแนวนอนเลย
- **สิ่งที่ทำ**:
  1. **แถบหัวข้อ 8 หัวข้อ** — เดิมเป็นตาราง 2 คอลัมน์ 4 แถวกินพื้นที่แนวตั้งเกือบครึ่งจอ ตอนนี้เป็นแถวปัดนิ้วการ์ดละ `62vw` (สูงสุด 240px) เห็นใบถัดไปโผล่ขอบเป็นสัญญาณว่าปัดได้
  2. **จุดบอกตำแหน่ง 8 จุด** เฉพาะจอเล็ก — ห่อด้วยปุ่ม 24px ตามแพตเทิร์นหน้าแรก (ผ่านเกณฑ์พื้นที่กดโดยที่จุดยังเล็กเท่าเดิม) กดแล้วเลื่อนไปหัวข้อนั้น
  3. **แท่นกองไพ่ 4 กอง** — เปลี่ยนเป็นแถวปัดนิ้วเหมือนกัน การ์ดใหญ่ขึ้นจากครึ่งจอเป็น `62vw`
  4. **เปิดหน้าหัวข้อแล้วเลื่อนหัวข้อที่กำลังอ่านมาอยู่กลางแถบอัตโนมัติ** (วัดแล้วคลาดจากกึ่งกลางไม่เกิน 24px)
- **กับดักที่เลี่ยงไว้**: ไม่ใช้ `scrollIntoView` ตรง ๆ เพราะบน iOS มันลากหน้าทั้งหน้าตามแนวตั้งไปด้วย — ใช้ `rail.scrollTo({left})` คำนวณกึ่งกลางเอง
- **พิสูจน์**: ยิงสกรอลล์ทดสอบ 5 ตำแหน่ง จุดบอกตำแหน่งขยับตรงทุกจุด (0 ➔ 2 ➔ 4 ➔ 6 ➔ 7) · กดจุดแล้วเลื่อนถูกใบ · จอ 1024px ยังเป็นตาราง 4 คอลัมน์และจุดถูกซ่อน · `repo:verify` ผ่าน 76/76 (รวมด่านพื้นที่กดขั้นต่ำและ a11y)

### 🗓️ 2026-09-18 (รอบ 95): 🆕 Pick A Card ครบ 8 หัวข้อ — ดวงการเงินเดือนนี้ · สิ่งที่คนรอบตัวคิดกับคุณ

- **ปิดแผนครบ**: หัวข้อ 4 ➔ **8 หัวข้อ** ตามที่เจ้าของเคาะ
- **หัวข้อใหม่รอบนี้**:
  | หัวข้อ | URL | ผังที่ส่งต่อ |
  |---|---|---|
  | ดวงการเงินเดือนนี้ | `/pick-a-card/money-luck-this-month` | `money` |
  | สิ่งที่คนรอบตัวคิดกับคุณ | `/pick-a-card/what-people-think-of-you` | `three-card` |
- **ตำแหน่งไพ่ของหัวข้อใหม่**: การเงิน = กระแสเงินตอนนี้ · รูรั่วที่มองไม่เห็น · โอกาสรายได้ · คนรอบตัว = ภาพที่คนอื่นเห็น · สิ่งที่พูดกันลับหลัง · สิ่งที่ควรทำต่อ
- **เนื้อหาที่เขียนใหม่**: 16 ชิ้นคลังคำอ่าน (ไทย+อังกฤษ = **192 สตริง**) + เนื้อหา SEO 2 หัวข้อสองภาษา · ไพ่หัวข้อละ 24 ใบไม่ซ้ำกันเลย
- **ผลรวมของหน้า Pick A Card ตอนนี้**: **8 หัวข้อ × 512 = 4,096 ชุดคำอ่าน** · หน้าหัวข้อ 16 หน้า (ไทย 8 · อังกฤษ 8) · เว็บ 330 ➔ **334 หน้า**
- **ปรับคำอธิบายหน้ารวม**: เดิมเขียนว่า 4 กองความรัก การงาน การเงิน ตอนนี้ครอบคลุมทั้ง 8 หัวข้อรวมคำค้นใหม่ (เขาจะกลับมาไหม · คนที่กำลังจะเข้ามา)
- **พิสูจน์**: หน้าไทยและอังกฤษของสองหัวข้อใหม่ตอบ 200 ทุกเส้น · บิลด์จริง 334 หน้า · `repo:verify` ผ่าน 76/76 (รวมด่านความยาว title/description และด่านที่ 76 ครบ 17 ข้อ)

### 🗓️ 2026-09-18 (รอบ 94): 🆕 Pick A Card เพิ่ม 2 หัวข้อใหม่ — “เขาจะกลับมาไหม” และ “คนที่กำลังจะเข้ามา”

- **ที่มา**: ข้อสุดท้ายของแผนที่เจ้าของเคาะ (เพิ่มหัวข้อจาก 4 เป็น 8) — รอบนี้ลงสองหัวข้อแรกของกลุ่มความรัก ซึ่งเป็นคำค้นที่แรงที่สุดสองคำ
- **หัวข้อใหม่**:
  | หัวข้อ | URL | ผังที่ส่งต่อ | คลัง |
  |---|---|---|---|
  | เขาจะกลับมาไหม? | `/pick-a-card/will-they-come-back` | `ex-reconciliation` | 8 ชิ้น (512 ชุด) |
  | คนที่กำลังจะเข้ามาในชีวิต | `/pick-a-card/who-is-coming-into-your-life` | `soulmate` | 8 ชิ้น (512 ชุด) |
- **เนื้อหาที่เขียนใหม่**: 16 ชิ้นคลังคำอ่าน (ไทย+อังกฤษ = **192 สตริง**) + เนื้อหา SEO 2 หัวข้อสองภาษา (ชื่อเรื่อง · คำอธิบาย · คีย์เวิร์ด · บทความ ~200 คำ · FAQ 3 ข้อ)
- **ไพ่ที่ใช้**: หัวข้อละ **24 ใบไม่ซ้ำกันเลย** ตามกติกาของด่าน (จั่วแล้วไม่มีทางเห็นไพ่ใบเดิมสองช่องพร้อมกัน) · ทุกย่อหน้าเอ่ยชื่อเฉพาะไพ่ของตำแหน่งตัวเอง
- **ผลรวมทั้งหน้า**: 6 หัวข้อ × 512 ชุด = **3,072 ชุดคำอ่าน** · เว็บ 325 ➔ **330 หน้า** (เพิ่มหน้าหัวข้อไทย 2 · อังกฤษ 2 · และหน้าเดิมที่ปรับ)
- **พิสูจน์**: เล่นบนหน้าจริง 8 รอบติด ได้ 8 ชุดไม่ซ้ำกันเลย · หน้าไทยและอังกฤษตอบ 200 · `repo:verify` ผ่าน 76/76 (รวมด่านที่ 76 ทั้ง 17 ข้อ)
- **เหลือ**: อีก 2 หัวข้อ (ดวงการเงินเดือนนี้ · สิ่งที่คนรอบตัวคิดกับคุณ)

### 🗓️ 2026-09-17 (รอบ 93): 🔗 Pick A Card คลื่นที่ 3 (ส่วนที่สอง) — แตกหัวข้อเป็น URL ของตัวเอง
### 🗓️ 2026-09-17 (รอบ 92): 📅 Pick A Card คลื่นที่ 3 (ส่วนแรก) — สำรับประจำวัน

- **สิ่งที่ทำ**: หน้า `/pick-a-card` เปิดมาเจอ **สำรับประจำวัน** ที่ทั้งเว็บเห็นชุดเดียวกัน เปลี่ยนใหม่ทุกเที่ยงคืนตามเวลากรุงเทพฯ (`bangkokDayKey()` ตัวเดียวกับที่ทั้งเว็บใช้) — สูตรเดียวกับ pick-a-pile สายคลิปสั้นที่สร้างนิสัยให้คนกลับมาทุกวัน
- **ยังสับใหม่ได้ทุกเมื่อ**: กด “เลือกกองอื่น” = จั่วสุ่มจากคลัง 512 ชุดเหมือนเดิม และป้ายใต้หัวข้อจะเปลี่ยนจาก “สำรับประจำวันที่ …” เป็น “สับไพ่ใหม่แล้ว”
- **เทคนิค**: `src/lib/pick-a-card/daily.ts` ใช้ FNV-1a + mulberry32 (เมล็ด = `<วันกรุงเทพฯ>:<หัวข้อ>`) ทำซ้ำได้ 100% · เรียกใน `useEffect` เท่านั้น เพราะหน้านี้เป็น HTML นิ่งที่บิลด์ไว้ล่วงหน้า ถ้าคำนวณตอนเรนเดอร์ วันของตอนบิลด์จะไม่ตรงกับวันที่ผู้ใช้เปิดหน้า
- **ด่านที่ 76 เพิ่มเป็น 16 ข้อ**: เมล็ดเดียวกันต้องได้ชุดเดิมเป๊ะ · ดัชนีชี้ในคลังจริงและทุกกองได้คนละชิ้น · 60 วันติดกันต้องได้ชุดต่างกันมากกว่า 20 ชุด (วัดจริงได้ 57) · ป้ายวันที่อ่านออกทั้งสองภาษา · หน้าเว็บต้องเรียก `dailyDraw` + `bangkokDayKey` จริง
- **พิสูจน์**: เปิดหน้าใหม่สองรอบได้สำรับเดิมเป๊ะ (`cups-13 · swords-02 · wands-03`) · กดสับใหม่แล้วได้ชุดอื่นทันที · ไม่มี hydration warning · `repo:verify` 76/76
- **เหลือในคลื่นที่ 3**: แตกหัวข้อเป็น URL ของตัวเอง (`/pick-a-card/<slug>`) + เพิ่มหัวข้อจาก 4 เป็น 8
### 🗓️ 2026-09-17 (รอบ 91): 🟢 แก้ตราสัญลักษณ์ LINE ในหน้าต่างเข้าสู่ระบบที่ตัวอักษรบิดจนอ่านไม่ออก

- **ต้นเรื่อง**: เจ้าของโปรเจกต์ส่งภาพปุ่ม `Google` / `LINE` มาพร้อมข้อความว่า *"สัญลักษณ์ line เป็นงี้"* — ในฟองคำพูดสีเขียวอ่านได้เป็นอักษรมั่ว ไม่ใช่คำว่า LINE
- **ต้นเหตุ**: `AuthModal.tsx` วาดตัวอักษรทั้งสี่ตัวด้วย `<path>` ก้อนเดียวที่คัดลอกมาผิด — ท่อน N เขียนว่า `s-.5.2-.5.5` แทนที่จะเป็น `s.5.2.5.5` เส้นโค้งจึงย้อนกลับทิศ ตัว N และ E เลยทับกันจนกลายเป็นรูปอื่น (typecheck และ lint มองไม่เห็น เพราะเป็นแค่สตริงที่ถูกไวยากรณ์)
- **การแก้**: เขียนตัวอักษรใหม่เป็นเส้น (`stroke`) ทรงเรขาคณิตตรงตามโลโก้จริง แยกทีละตัว (L · I · N · E) พิกัดอ่านรู้เรื่องและแก้ต่อได้ · ฟองคำพูดสีเขียว `#06C755` ของเดิมถูกต้องอยู่แล้วจึงไม่แตะ
- **พิสูจน์**: เรนเดอร์จริงด้วย Chromium ที่ 20px (ขนาดในปุ่ม) · 40px · 96px — อ่านออกว่า "LINE" ครบทุกขนาด
- **ที่อื่นในเว็บ**: ค้นทั้ง `src/` แล้ว ตราสัญลักษณ์ LINE มีอยู่จุดเดียวคือปุ่มนี้

### 🗓️ 2026-09-17 (รอบ 91): 🗂️ Pick A Card คลื่นที่ 2 — คลังคำอ่าน 8 ชิ้นต่อหัวข้อ ➔ 512 ชุด

- **ต้นเรื่อง**: ต่อจากคลื่นที่ 1 ตามแผนที่เจ้าของเคาะ (“เอาแผนที่เสนอมา”)
- **สิ่งที่ทำ**:
  1. **แยกโครงข้อมูลเป็น `slots` + `pool`** — `slots` คือกอง 4 กองที่ผู้ใช้เห็นและเลือก (เลข · คริสตัล) ส่วน `pool` คือคลังคำอ่านที่ขยายได้ไม่จำกัด · ตรวจความเท่ากันของเนื้อหาก่อน/หลังแปลงโครงแบบทีละตัวอักษร (4 หัวข้อ · ไม่ต่างสักจุด)
  2. **เขียนคำอ่านใหม่หัวข้อละ 4 ชิ้น** (16 ชิ้น · ไทย+อังกฤษ · 192 สตริง) ➔ คลังจาก 4 เป็น **8 ชิ้นต่อหัวข้อ** ➔ **8 × 8 × 8 = 512 ชุดต่อหัวข้อ** (รวมทั้งเว็บ 2,048 ชุด)
  3. **ตัวจั่วรองรับคลังใหญ่กว่าจำนวนกอง** (`drawAnchors`) — ในรอบเดียวกันทุกกองต้องได้คนละชิ้น และทุกกองต้องเปลี่ยนจากรอบก่อน
  4. เลือกไพ่ให้ **ไม่ซ้ำกันเลยภายในหัวข้อเดียว** (24 ใบต่อหัวข้อ) จึงไม่มีทางจั่วแล้วเห็นไพ่ใบเดิมสองช่องพร้อมกัน
- **ด่านที่ 76 เพิ่มเป็น 11 ข้อ**: เพิ่มข้อ “ไพ่ใบเดียวกันห้ามโผล่สองตำแหน่งในหัวข้อเดียว” และปรับข้อเดิมให้ทดสอบคลังขนาด 4–12 ชิ้น · ข้อ “เข้าถึงครบทุกชุด” ตอนนี้ยืนยัน **512 ชุด** จากการสุ่ม 6,000 รอบ
- **พิสูจน์ว่าได้ผลจริง**: เล่นบนหน้าจริง 15 รอบ ➔ **ได้ 15 ชุดไม่ซ้ำกันเลย** · ไม่มีรอบไหนซ้ำกับรอบก่อน · ไม่มีไพ่ซ้ำใบในรอบเดียวกัน · `repo:verify` ผ่าน 76/76 ด่าน
- **เหลือคลื่นที่ 3**: ล็อกชุดประจำวัน (เขตเวลากรุงเทพฯ) + แตกหัวข้อเป็น URL ของตัวเอง + เพิ่มหัวข้อจาก 4 เป็น 8

### 🗓️ 2026-09-17 (รอบ 90): 🧭 คืนปุ่มบัญชีให้เป็น "ไอคอนล้วน" และยกเครื่องหน้า `/account` ให้เป็นดีไซน์ชุดเดียวกันทั้งหน้า

- **ต้นเรื่อง (คำสั่งเจ้าของโปรเจกต์ พร้อมภาพหน้าจอ 5 รูป)**:
  1. ปุ่มบนหัวเว็บ (รูป 1-2) *"เอาแค่เป็นไอคอน แบบเดิม และในรูปที่ 1 ไม่ต้องมีแจ้งเตือน +9"*
  2. หน้า `/account` (รูป 3-5) *"ไม่สวย workflow ไม่ดี และเหมือนไม่ครบ ที่แรก code หายไปไหน"*
  3. *"ปรับปรุงให้ดีและได้มาตรฐาน"*

- **1 · ปุ่มบัญชีบนหัวเว็บ (`UserProfileBadge.tsx`) — กลับไปเป็นไอคอนกลมใบเดียวทั้งสองสถานะ**
  - ยังไม่ล็อกอิน: เดิมเป็นปุ่มเม็ดยาวมีคำว่า "เข้าสู่ระบบ" (มาจาก #513) ➔ กลับเป็น **ไอคอนคนในวงกลม** ขนาดเท่ากันเป๊ะกับตอนล็อกอินแล้ว หัวเว็บจึงไม่ขยับตอนเซสชันโหลดเสร็จ
  - ล็อกอินแล้ว: ถอด **ป้ายแดง "9+"** ทิ้งทั้งชุด เหลือไอคอนคน + จุดทองบอกสถานะสมาชิก
  - ถอด `fetch("/api/journal/pending-count")` ออกจากหัวเว็บ — เดิมทุกหน้าที่ผู้ใช้เปิดยิงคำขอนี้ **ทุกครั้ง** เพียงเพื่อวาดตัวเลขบนไอคอน · จำนวนที่รอติดตามผลไปอยู่บนการ์ด "บันทึกคำทำนาย" ในหน้า `/account` ซึ่งกดต่อได้จริง
  - ถอด prop `onOpenPlans` / `onBuyCredits` ที่ค้างมาจากยุคแผงลอย (ไม่มีใครใช้แล้วตั้งแต่ #513) พร้อมแก้จุดเรียกใน `TarotFlow.tsx`

- **2 · หน้า `/account` — ยกเครื่องทั้งหน้าให้เป็นระบบดีไซน์ชุดเดียว**
  - **ต้นเหตุที่ "ไม่สวย"**: หน้านี้มีการ์ดสองระบบปนกัน — การ์ดที่หน้านี้เขียนเอง (`rounded-xl` + `border-line` + เงาหนัก) กับการ์ดที่นำเข้ามา (`rounded-lg` + `border-line-warm` ไม่มีเงา) ➔ รวบเป็นเปลือกเดียว `CARD_SHELL` ใช้ทั้งหน้า หัวข้อทุกการ์ดใช้ทรงเดียวกัน
  - **คลาสที่ไม่มีอยู่จริง**: `bg-crimson` · `text-crimson` · `border-crimson/40` **ไม่ใช่โทเคนของเว็บนี้เลย** (ไม่มีใน `globals.css`) จึงไม่เคยให้สีอะไรออกมา · `text-amber-800 dark:text-amber-300` ก็เป็นสีนอกพาเลตและเว็บนี้ไม่มีโหมดมืด ➔ เปลี่ยนเป็นโทเคนจริงทั้งหมด (`err` · `err-wash` · `gold-ink` · `line-warm`)
  - **workflow ที่ซ้ำซ้อน**: เดิมมีปุ่มเปิดบันทึกคำทำนาย 2 ที่ และลิงก์ "กลับไปเปิดไพ่" 2 ที่ ➔ รวมเหลือที่ละหนึ่ง · จำนวนคำทำนายที่รอติดตามผลกลายเป็นป้ายบนทางลัด "ประวัติการเปิดไพ่" แทนการ์ดแยกอีกใบ
  - **ของที่หายไปจากแผงลอยเดิม (คำถาม "code หายไปไหน")**: ทางเข้า **"มีรหัสแลกสิทธิ์? กดใส่รหัสที่นี่"** หายไปตั้งแต่ #513 (ช่องกรอกรหัสยังอยู่ในหน้าต่างเติมรอบ แต่ไม่มีทางเข้าจากหน้าไหนเลย) ➔ คืนกลับมาบนการ์ดสิทธิ์
  - **หน้าต่างเติมรอบซ้อนสองบาน**: `EntitlementStatusCard` mount `BuyCreditsModal` ของตัวเองพร้อมกับที่หน้า `/account` mount อีกบาน ➔ เพิ่ม prop `onBuyCredits` ให้หน้าที่ครอบเป็นเจ้าของบานเดียว (ไม่ส่งมาก็ยังยืนได้เอง)
  - **ฟอร์มรหัสผ่านที่กางค้าง**: ผู้ใช้ Google/LINE เห็นช่องรหัสผ่าน 3 ช่องกลางหน้าบัญชีจนเข้าใจผิดว่าต้องกรอก ➔ `ChangePasswordCard` พับเป็น `<details>` (เปิดปิดด้วยคีย์บอร์ดได้ ไม่ต้องเขียน JS)
  - **สวิตช์ PDPA**: เดิมวาดด้วย `peer-*` + `bg-white` / `border-gray-300` (สีนอกพาเลต) และไม่มีสถานะกำลังบันทึก ➔ เขียนใหม่เป็น `role="switch"` จริง มี `aria-checked` ขนาดแตะ 44x24 ผ่าน WCAG 2.2 และกันกดรัวระหว่างบันทึก
  - **ปุ่มลบข้อมูลทั้งหมด** (`DeleteAllDataButton`): เดิม `bg-err/80` + `text-err` = **ตัวอักษรแดงบนพื้นแดง คอนทราสต์ ~1.2:1** อ่านแทบไม่ออกทั้งที่เป็นปุ่มอันตรายที่สุดในหน้า ➔ พื้นอ่อน `err-wash` + ตัวอักษร `err` ผ่าน AA
  - **สถานะกำลังโหลด**: เพิ่มโครงร่างรอ (skeleton) ความสูงใกล้ของจริง กันหน้ากระโดดตอนเซสชันมาถึง
  - **ลำดับใหม่ของหน้า**: ตัวตน ➔ สิทธิ์การใช้งาน ➔ ทางลัด (บันทึกคำทำนาย + เปรียบเทียบแพลน) ➔ การแจ้งเตือน PDPA ➔ ความปลอดภัย ➔ ความเป็นส่วนตัว/ลบข้อมูล

- **3 · ทางเข้าหน้าบัญชีที่หายไปทุกหน้ายกเว้นหน้าแรก (`SacredNavDropdown.tsx`)**
  - ปุ่มไอคอนบัญชีมีเฉพาะหัวเว็บของหน้าดูดวงหลัก (`variant="app"`) เท่านั้น คนที่อยู่หน้าไพ่ บทความ หรือหน้าบัญชีเอง จึงไม่มีทางไป `/account` เลย ➔ เพิ่มลิงก์ **"บัญชีของฉันและสิทธิ์การใช้งาน"** ไว้ท้ายลิ้นชักนำทางซึ่งมีอยู่ทุกหน้า

- **4 · ด่านหลอกที่ต้องปิด (`scripts/qa/test-digest.ts`)**
  - ด่าน "มีสวิตช์ให้ผู้ใช้กดสมัคร/ยกเลิกเองในหน้าบัญชี" อ่านไฟล์ `UserProfileBadge.tsx` ทั้งที่สวิตช์ย้ายไปหน้า `/account` ตั้งแต่ #513 — #513 จึงต้องทิ้งฟังก์ชันกำพร้าไว้ในไฟล์หัวเว็บเพียงเพื่อให้ด่านผ่าน ➔ ชี้ด่านไปที่ `AccountClient.tsx` ของจริง และเพิ่มการตรวจว่าส่งธง `digest` ไปที่ `/api/account/consent` จริง

- **การตรวจสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ ผ่านครบทุกด่าน · เปิดของจริงด้วย Chromium (บิลด์ production) ทั้งจอ 1024px และ 390px ทั้งสถานะสมาชิกและผู้เยี่ยมชม: สวิตช์บันทึกได้ · การ์ดรหัสผ่านพับ/กางได้ · ทางลัดเปิดบันทึกคำทำนายได้ · ลิงก์รหัสแลกสิทธิ์เปิดหน้าต่างที่มีช่องกรอกรหัสจริง
...
### 🗓️ 2026-09-17 (รอบ 89): 🃏 แก้หน้า Pick A Card — ไพ่ซ้ำทุกครั้ง · หลังไพ่คนละลายกับทั้งเว็บ · แถบหัวข้อไม่เข้าชุด (INC-0198b)
### 🗓️ 2026-09-17 (รอบ 90): 🧩 Pick A Card คลื่นที่ 1 — ปลดเพดานคำทำนายจาก 4 เป็น 64 ชุดต่อหัวข้อ (INC-0199b)

- **ต้นเรื่อง**: หลังรอบ 89 กองสลับชุดกันได้แล้วแต่คลังยังมี 4 ชุดต่อหัวข้อ เจ้าของสั่งให้เสนอแผน แล้วเลือก “เอาแผนที่เสนอมา” — นี่คือคลื่นที่ 1 ของแผนนั้น
- **ข้อค้นพบที่ทำให้แผนนี้เป็นไปได้**: ตรวจเนื้อหาด้วยเครื่องทั้ง 96 ย่อหน้า (4 หัวข้อ × 4 กอง × 3 ตำแหน่ง × 2 ภาษา) พบว่า **ทุกย่อหน้าอ้างถึงไพ่ของตำแหน่งตัวเองใบเดียวเท่านั้น** ไม่มีย่อหน้าไหนอ้างไพ่ข้ามตำแหน่งเลย ➔ แยกเป็นคลังรายตำแหน่งได้ทันทีโดยไม่ต้องเขียนย่อหน้าใหม่สักบรรทัด
- **สิ่งที่ทำ**:
  - `src/lib/pick-a-card/compose.ts` (**ใหม่**) — แยกคลังเป็น 3 ตำแหน่ง (ไพ่หลัก · สิ่งที่ซ่อนอยู่ · คำแนะนำ) ตำแหน่งละ 4 ใบ แล้วจั่วอิสระ ➔ **4 × 4 × 4 = 64 ชุดต่อหัวข้อ** (เดิม 4 ชุด)
  - เขียนข้อความกรอบใหม่ **68 สตริง** (`theme` · `overview` · `affirmation` ของ 16 กอง ทั้งไทยและอังกฤษ) ให้พูดถึง **ไพ่หลักใบเดียว** กรอบจึงเดินทางไปกับไพ่หลักได้โดยไม่ขัดกับไพ่ใบที่ 2–3 ที่จั่วมาจากกองอื่น
  - `PickACardClient.tsx` — ใช้คำอ่านที่ประกอบเสร็จ (`composeReading`) และไล่แสดงย่อหน้าตามไพ่ที่จั่วได้จริงของรอบนั้น
- **กฎป้องกันถาวร (ขยายด่านที่ 76 เป็น 10 ข้อ)**: จั่วรอบใหม่ต้องเปลี่ยนครบทั้งสามตำแหน่ง · สุ่ม 6,000 รอบต้องเข้าถึงครบ 64 ชุด (ถ้าเนื้อหาถูกมัดกลับเป็นกอง จะเหลือ 4 แล้วด่านตกทันที) · ทุกย่อหน้าต้องมากับไพ่ของตำแหน่งตัวเองเสมอ (4 หัวข้อ × 60 รอบ × 2 ภาษา) · **กฎเนื้อหา: ห้ามย่อหน้าไหนอ้างชื่อไพ่ที่ไม่ได้อยู่ในตำแหน่งของตัวเอง** ซึ่งกันต้นเหตุโดยตรง
- **พิสูจน์ว่าแก้ได้จริง**: เล่นบนหน้าจริง 12 รอบ ได้ **11 ชุดไม่ซ้ำกัน** และไม่มีรอบไหนซ้ำกับรอบก่อน · ตรวจคำอ่านที่ประกอบออกมาแล้วทุกย่อหน้าตรงกับไพ่ที่แสดงอยู่ · `npm run repo:verify` ผ่านครบ 76/76 ด่าน
- **คลื่นถัดไปตามแผน**: คลื่น 2 = ขยายคลังต่อหัวข้อจาก 12 เป็น 24 ใบ (➔ 512 ชุด) · คลื่น 3 = ล็อกชุดประจำวัน + แตกหัวข้อเป็น URL ของตัวเอง + เพิ่มหัวข้อเป็น 8

### 🗓️ 2026-09-17 (รอบ 89): 🃏 แก้หน้า Pick A Card — ไพ่ซ้ำทุกครั้ง · หลังไพ่คนละลายกับทั้งเว็บ · แถบหัวข้อไม่เข้าชุด (INC-0198b)

- **ต้นเรื่อง**: เจ้าของทัก 3 ข้อหลังหน้า `/pick-a-card` (PR #512) ขึ้น production
  1. เลือกกองเดิมซ้ำได้ไพ่ 3 ใบเดิมและคำทำนายเดิมทุกครั้ง ทั้ง 4 กอง
  2. หลังไพ่บนกองไม่ใช่หลังไพ่ที่ใช้ปกติทั้งเว็บ
  3. แถบเลือกหัวข้อหน้าตาไม่เข้ากับดีไซน์ที่เหลือ
- **สาเหตุราก**:
  - ไฟล์ข้อมูลผูกไพ่ 3 ใบไว้กับกองแบบตายตัว และ UI อ่านตรงจาก `activeTopic.piles[n]` จึงไม่มีการจั่วใหม่เลยสักจุด
  - หน้านี้วาดหลังไพ่เองด้วย `bg-[#1e1b18]` + ตัวหนังสือ `SEER 1909 / TAROT` แทนคลาสกลาง `card-back-pattern` ที่ `TarotCard` · `ShuffleRitual` · `InteractiveCardFan` ใช้ร่วมกันอยู่แล้ว
  - แถบหัวข้อเป็นปุ่มข้อความล้วน ขณะที่ภาษาการออกแบบของบ้านนี้ (ลิ้นชักนำทาง รอบ 87) ใช้ภาพไพ่ 1909 ขนาดเล็กประกอบเมนู
- **สิ่งที่แก้**:
  - `src/lib/pick-a-card/draw-order.ts` (**ใหม่**) — `drawContentOrder()` คืนลำดับแบบ derangement เทียบรอบก่อน
  - `PickACardClient.tsx` — แยก “ตัวตนของกอง” (เลข · คริสตัล) ออกจาก “ชุดไพ่+คำทำนายของรอบนี้” แล้วสลับใหม่ทุกครั้งที่กลับมาหน้าเลือกกองและตอนเปลี่ยนหัวข้อ (เรียกใน `useEffect` เท่านั้น เพราะหน้านี้เสิร์ฟ HTML นิ่งจากขอบ สุ่มตอนเรนเดอร์จะทำให้ hydration ไม่ตรง) · หลังไพ่บนกองเปลี่ยนมาใช้ `card-back-pattern` · แถบหัวข้อเป็นการ์ดพร้อมภาพไพ่ประจำหัวข้อ · เพิ่มบรรทัดบอกผู้ใช้ว่ากลับมาเลือกใหม่ไพ่จะถูกสับใหม่
- **กฎป้องกันถาวร (ด่านที่ 76)**: `scripts/qa/test-pick-a-card.ts` ตรวจ 6 ข้อ — ไม่มีช่องไหนถือชุดเดิมซ้ำรอบก่อน (ขนาด 2–6 ช่อง · 300 รอบต่อขนาด) · ผลลัพธ์เป็นการเรียงสับเปลี่ยนที่ถูกต้อง · เล่นยาว 400 รอบต้องเข้าถึงครบทุกชุด (กันอัลกอริทึมที่สลับอยู่แค่สองชุด) · หน้านี้ต้องยังเรียก `drawContentOrder` จริง · ทะเบียนไฟล์ที่วาดไพ่คว่ำหน้า 4 ไฟล์ต้องใช้ `card-back-pattern` (ratchet) · หน้านี้ห้ามมี `bg-[#...]` อีก
- **พิสูจน์ว่าแก้ได้จริง**: รันหน้าจริงบน Astro dev แล้วเล่นซ้ำ 10 รอบด้วยสคริปต์ — เจอครบทั้ง 4 ชุด **ไม่มีรอบไหนซ้ำกับรอบก่อนเลย** (เดิมได้ชุดเดิม 10/10 รอบ) · `npm run repo:verify` ผ่านครบ 76/76 ด่าน
- **หนี้ที่ยังเหลือ (แจ้งเจ้าของแล้ว)**: คลังคำทำนายมี 4 ชุดต่อหัวข้อ ผู้ใช้ที่เล่นเกิน 4 รอบจะเริ่มเห็นชุดเดิมวนกลับมา — ถ้าอยากให้ลึกกว่านี้ต้องเขียนคำทำนายเพิ่มต่อหัวข้อ (งานเขียนเนื้อหา ไม่ใช่งานโค้ด)
...
- **ต้นเรื่อง**: เจ้าของโปรเจกต์ทักหลัง PR #509 ว่าเมนูลิ้นชัก “ตัวอักษรขาด สระหาย ไ หัวหาย” และบรรทัดที่สองของรายการไม่ตรงกับบรรทัดแรก
- **วัดของจริงก่อนแก้** (production · seertarot.net · เบราว์เซอร์จริง):
  - หัวข้อในลิ้นชัก **9 รายการ + ชื่อ “วิหารพยากรณ์” บนหัวลิ้นชัก** มี `scrollHeight` 18px แต่ `clientHeight` 16px → ถูกเฉือนยอดทิ้งทุกบรรทัด
  - หมึกไทยพุ่งเหนือเส้นฐานสูงสุด **1.056em** (สระอี+ไม้เอก เช่น “ที่”) ขณะที่กล่องบรรทัดเผื่อให้แค่ 0.894em เพราะ `leading-tight` (1.25) + `truncate` (`overflow: hidden`)
  - ปุ่ม 2 รายการ (`ประวัติการดูดวง` · `แพ็กเกจเติมรอบ`) มี `text-align: center` ส่วนลิงก์อีก 7 รายการเป็น `start` → บรรทัดคำอธิบายเลื่อนไปกลางกล่อง
- **สาเหตุราก**: PR #509 ยุบคลาสของปุ่มกับลิงก์เป็นตัวแปรเดียว (`buttonClass`) แล้ว `text-left` ตกหล่น ปุ่มจึงกลับไปใช้ค่าเริ่มต้นของเบราว์เซอร์ · และเปลี่ยนหัวข้อเป็น `text-[13px] leading-tight truncate` ซึ่งเตี้ยกว่าหมึกไทยจริง (ฟอนต์ละตินไม่มีอาการนี้จึงไม่มีใครสะดุดตอนรีวิว)
- **สิ่งที่แก้**:
  - `SacredNavDropdown.tsx` — หัวข้อ/คำอธิบาย/ชื่อบนหัวลิ้นชักใช้ `leading-[1.7]` (ค่ากลางของธีม) และคืน `text-left` ให้ `buttonClass`
  - `InteractiveCardFan.tsx` — จุดเดียวกันที่ด่านใหม่กวาดเจอ (`leading-normal` 1.5 ก็ยังไม่พอ) แก้เป็น `leading-[1.7]`
- **กฎป้องกันถาวร (ด่านที่ 75)**: `scripts/qa/test-thai-glyph-clipping.ts` — ห้ามคลาสชุดเดียวกันมีทั้ง `truncate` (หรือ `overflow-hidden` + `text-ellipsis`) และ `leading-*` ที่ต่ำกว่า **1.6** เว้นแต่มี `py-`/`pt-` เป็นที่ว่างให้หมึกล้น · พื้น 1.6 มาจากการวัดฟอนต์จริง (ขั้นต่ำที่คำนวณได้คือ 1.573) · ตัวด่านมีเคสพิสูจน์ตัวเอง 8 เคสกันด่านหลอก
- **พิสูจน์ว่าแก้ได้จริง**: จำลองคลาสชุดใหม่ทับ DOM จริงบน production แล้วถ่ายภาพซ้ำ — หัวสระ/วรรณยุกต์ครบทุกบรรทัด `scrollHeight == clientHeight` ทั้ง 25 กล่องข้อความ และบรรทัดสองตรงแนวกับบรรทัดแรกแล้ว · `npm run repo:verify` ผ่านครบ 75/75 ด่าน
...
ต่อจากผลตรวจรอบ 2 `HANDOFF_ROUND2_2026-09-17.md` (31 ข้อ) — ล็อตแรกปิด 4 ข้อที่เป็นฐานของข้ออื่น

**R-01 (CRITICAL) — ผู้ใช้ใหม่ทุกคนโหลดทั้งหน้าสองรอบ**
- `sw.js` เรียก `self.clients.claim()` ตอน `activate` ➔ ยิง `controllerchange` ➔ `ServiceWorkerRegister.tsx`
  สั่ง `window.location.reload()` แบบไม่มีเงื่อนไข **ทุกคนที่เปิดเว็บครั้งแรก** จึงโหลดเอกสารซ้ำ
  (มองไม่เห็นด้วย `curl` และผู้ใช้เก่าไม่เจอ จึงรอดมาตลอด)
- แยกตรรกะออกเป็น `src/components/pwa/sw-register.ts` ที่ทดสอบได้โดยไม่ต้องมี jsdom
  จำค่า `container.controller` **ก่อน** ผูก listener แล้ว reload เฉพาะตอนเป็นการ "เปลี่ยนเวอร์ชัน" จริง
- คอมโพเนนต์เดิมเหลือหน้าที่เดียว: ต่อสภาพแวดล้อมเบราว์เซอร์เข้ากับตรรกะนั้น
- ด่านใหม่ `test-sw-reload.ts` (12 ข้อ) — ผู้เข้าครั้งแรก reload 0 ครั้ง · ผู้ใช้เก่า 1 ครั้งพอดี · ยิงซ้ำไม่เพิ่ม

**R-06 — CI ตรวจก่อน build ทำให้ครึ่งชุดตรวจไม่มีของให้ตรวจ**
- `pr.yml` และ `deploy.yml` ย้ายขั้น build มา **ก่อน** `repo:verify` แล้ว
- เดิมมีเพียงสามด่านที่รอดเพราะสั่ง build เอง ที่เหลือวนอาเรย์ว่างแล้วขึ้น ✅

**R-05 · R-07 — ด่านหลอก 21 ตัว**
- `scripts/qa/lib/corpus.ts` : `assertNonEmptyCorpus()` — คลังว่าง = ตกทันที
- เติมให้ครบ **21 ด่าน** ที่วนคลังไฟล์โดยไม่เคยตรวจว่าคลังว่างหรือไม่
- เปลี่ยน "ข้ามเงียบ" เป็น "ตก" อีก 6 จุด: `test-bundle-budget` (ไม่มี HTML = วัดได้ 0 KB แล้วผ่าน) ·
  `test-motion-quality` (ไม่มีโฟลเดอร์ astro = ครึ่งเว็บไม่ถูกตรวจ) · `test-docs-numbers` ×2 ·
  `test-sticky-header` ×2 (CSS/HTML ที่ build แล้ว — จุดเดียวที่จับ INC-0108/0109 ได้)
- `test-seo-wave3` · `test-seo-wave4` : ไฟล์เป้าหมายหาย = ตกด่าน ไม่ใช่ข้าม
- `test-bot-policy` · `test-og-images` : ใน CI ยิงไม่ได้/ไม่มีค่า env = ตก (ในเครื่องยังเตือน)
- **ด่านแม่บทใหม่ `test-gate-integrity.ts`** — ตรวจตัวด่านเองสามแพตเทิร์น
  (ข้ามเงียบเมื่อไฟล์หาย · วนคลังว่าง · พิมพ์ว่า "ข้าม" แล้วจบสำเร็จ) รายการยกเว้นทุกตัวต้องมีเหตุผลกำกับ

**หลักฐานว่าด่านใหม่ตรวจของจริง** (กติกาผู้ตรวจ: ตกก่อนแก้ ผ่านหลังแก้)
- รันด่านแม่บทกับต้นไม้ก่อนแก้ (`0c834d7`) ➔ **2/5 ผ่าน** ฟ้อง 21 ด่านคลังว่าง · 2 จุดข้ามเงียบ · 2 จุดพิมพ์ว่าข้าม
- รันกับต้นไม้หลังแก้ ➔ **5/5 ผ่าน**

**ด่านตรวจ**: 66 ➔ **67** · `npm run repo:verify` ผ่านครบ 67/67 ตอนปิดล็อตนี้

---

### 🗓️ 2026-09-17 (รอบ 85): 💰 ด่านเฝ้าเส้นทางเงิน + ปิดบั๊กคีย์กันจ่ายซ้ำของ webhook (R-08)

**บั๊กที่เจอระหว่างเขียนด่าน — ผู้ใช้ซื้อรอบสองแล้วไม่ได้เครดิต**
...
- ผลสองต่อ: คีย์ชน `UNIQUE(user_id, reason)` ➔ **ซื้อครั้งที่สองไม่ได้เครดิตเลย**
  และครั้งแรก **ได้สองเท่า** เพราะ `checkout/confirm` ใช้คีย์อีกแบบ (`purchase_<orderId>`)
- webhook ยังทิ้งค่าที่ `grantBonus` คืนมาแล้วตอบ 200 — "จ่ายแล้วแต่ของไม่ถึงมือ" เงียบสนิท

**สิ่งที่ทำ**
- `src/lib/entitlement/purchase.ts` — แยก **การตัดสินใจ** (บริสุทธิ์ ทดสอบได้) ออกจาก **การลงมือ**
  `purchaseGrantReason()` เป็นแหล่งความจริงเดียวของกุญแจกันจ่ายซ้ำ ·
  `decidePurchaseGrant()` คืนรหัสเหตุผลพร้อม status ที่ปลายทางต้องตอบ
- `checkout/confirm` และ `payments/webhook` เรียกก้อนเดียวกัน · webhook ดึง `order_id` มาด้วย
  และตอบ **500** เมื่อเขียนเครดิตไม่ลง (ให้เกตเวย์ยิงซ้ำ)

**ด่านใหม่ `test-money-path.ts` (34 ข้อ) — ยิงจริงลงฐานข้อมูล ไม่ใช่ค้นข้อความในซอร์ส**
- ทิศทางสำเร็จ: ยอดเครดิตเพิ่ม **เท่ากับขนาดแพ็กพอดี**
- ยิงซ้ำออร์เดอร์เดิมได้เครดิตครั้งเดียว · **ออร์เดอร์ที่สองต้องได้อีกชุด** (เคสที่บั๊กเดิมทำพัง)
- ทิศทางล้มเหลว 7 เคส (ไม่มีออร์เดอร์ · ของคนอื่น · ยอดไม่ตรง · ยังไม่จ่าย · จ่ายไม่สำเร็จ ·
  รายการค่าปรึกษาแม่หมอ · ตัวจำลองบน production) ยืนยันทั้ง **status code** และ **ยอดคงเหลือไม่ขยับ**
- เส้นทาง `/read`: หัก ➔ คืน ต้องกลับมาเท่าเดิม และคืนซ้ำต้องไม่กลายเป็นเปิดไพ่ฟรี

**หลักฐานว่าด่านตรวจของจริง**
...
### 🗓️ 2026-09-17 (รอบ 87): ♿ การเข้าถึง — ประกาศข้อความผิดพลาด · เส้นขอบ 3:1 · ด่าน a11y จาก 8 หน้าเป็น 313 หน้า (R-03 · R-21 ถึง R-24)

**R-24 — เจอเรื่องใหญ่กว่าที่ผู้ตรวจรายงาน**
- ผู้ตรวจเขียนว่าด่าน `test-a11y-critical.ts` "ตรวจเฉพาะหน้าแรก" — วัดจริงแล้ว **ตรวจได้ 8 หน้า**
  ทั้งที่ป้ายของด่านโฆษณาว่า "ทั้งเว็บ 309 หน้า"
- ต้นเหตุ: เรียก `primaryOutputDir()` ซึ่งคืน **รากแรกรากเดียว** (`.next/server/app`)
  หน้าเนื้อหา 305 หน้าที่ Astro เรนเดอร์ไว้ที่ `dist/` ไม่เคยถูกตรวจเลยสักหน้า
- แก้เป็นไล่ทุกรากจาก `renderedOutputDirs()` ➔ **8 ➔ 313 หน้า** และผ่านครบทั้ง 313

**R-21 — ข้อความผิดพลาดในฟอร์มไม่ถูกประกาศให้โปรแกรมอ่านหน้าจอ (7 จุด ใน 6 ไฟล์)**
- ใส่ `role="alert"` ที่กล่องข้อความ + `aria-invalid` และ `aria-describedby` ที่ช่องกรอก
  เพื่อให้รู้ทั้งว่า "ผิด" และ "ผิดตรงไหน" · ข้อความสำเร็จใช้ `role="status"` (ไม่ขัดจังหวะ)
- ไฟล์: `reset-password` · `ChangePasswordCard` · `BuyCreditsModal` · `BirthCardCalculator` ·
  `ReadersManager` · `RedeemCodesManager` · `QuickFortunePicker`

**R-22 — เส้นขอบตัวควบคุมตกเกณฑ์ WCAG 1.4.11 (วัดได้ 1.27–1.64 : 1 จากเกณฑ์ 3:1)**
- ⚠️ **ไม่แก้ `--color-line` / `--color-line-warm` ตรง ๆ** เพราะสองโทเคนนั้นเป็นเส้นขอบ 394 จุดทั่วเว็บ
  ส่วนใหญ่เป็นการ์ดและเส้นคั่นที่ตั้งใจให้จาง และเกณฑ์ไม่ได้บังคับของประดับ
- เพิ่มโทเคนเฉพาะทางแทน: `--color-line-interactive` **#938265** · `--color-line-interactive-warm` **#A07F49**
  (คงเฉดเดิม ไล่ความสว่างลงอย่างเดียว) วัดได้ **3.02–3.74 : 1** บนพื้นทุกสีที่ใช้คู่กันจริง
- ใช้กับช่องกรอก/ตัวเลือกทั้ง **34 จุดใน 19 ไฟล์**

**R-23** — ลิงก์ "อ่านต่อ" / "Read Codex" ในหน้ารวมบทความใส่ `aria-label` ที่มีชื่อบทความแล้ว
(คนที่มองเห็นยังเห็นคำสั้นเหมือนเดิม · ลูกศร `→` ใส่ `aria-hidden`)

**R-03** — `SpreadBoard.tsx` เลิกเขียน `matchMedia("(prefers-reduced-motion: reduce)")` เอง
เปลี่ยนมาใช้ `useMotionSafe()` เหมือน `StreamReader` — ตรรกะเดียวกันเหลือที่เดียวทั้งเว็บ

**ด่าน `test-a11y-critical.ts` ตรวจเพิ่ม 3 ข้อ**
- คอนทราสต์เส้นขอบตัวควบคุม (คำนวณใหม่ทุกครั้ง ไม่ฮาร์ดโค้ดผล · เทียบกับพื้นทั้ง 5 สี)
- ช่องกรอกทุกช่องต้องใช้โทเคนเส้นขอบของตัวควบคุม (กันเพิ่มโทเคนไว้แล้วไม่มีใครใช้)
- กล่องข้อความผิดพลาดในไฟล์ที่มี `<form` ต้องมี `role="alert"` หรือ `aria-live`
- ลิงก์ในรายการ (`.map()`) ที่ใช้คำเรียกซ้ำ ๆ ต้องมี `aria-label`

**หลักฐานว่าด่านตรวจของจริง**
- รันด่านที่ขยายแล้วกับต้นไม้ก่อนแก้ (`0c834d7`) ➔ **ตก 5 จุด** (ไม่มีโทเคน 2 ตัว · ช่องกรอก 34 จุด ·
  ข้อความผิดพลาด 5 จุด · ลิงก์ในรายการ 1 จุด) · รันกับต้นไม้หลังแก้ ➔ **ผ่าน**

**ด่านตรวจ**: คงที่ 69 ด่าน (ขยายด่านเดิมให้ตรวจมากขึ้น ไม่ได้เพิ่มด่านใหม่) · `repo:verify` 69/69


---
### 🗓️ 2026-09-17 (รอบ 88): 🧱 หนี้โค้ด — เส้นแบ่งวันที่เดียว · ทะเบียนคีย์ · ซองจดหมาย API · แผงแอดมิน (R-25 · R-27 ถึง R-31)

**R-25 — "วันนี้ที่กรุงเทพฯ" เขียนซ้ำ 6 ที่ 3 วิธี**
- ค่านี้ไม่ใช่การจัดรูปแบบวันที่ แต่เป็น **คีย์โควตาและคีย์สตรีค** เส้นแบ่งวันจึงเป็นกฎทางธุรกิจ
- สร้าง `src/lib/time/bangkok.ts` (`bangkokDayKey` · `bangkokWeekKey` · `bangkokYesterdayKey` ·
  `bangkokNextMidnightISO` · `APP_TIME_ZONE`) แล้วให้ทั้ง 6 จุดเรียกจากที่นั่น
- ตัวจัดรูปแบบใช้ `formatToParts()` ประกอบสตริงเองจากส่วนประกอบที่ระบุชื่อ —
  ผลจึงไม่ขึ้นกับ locale (ของเดิมพึ่งความบังเอิญที่ `en-CA` กับ `sv-SE` ให้ `YYYY-MM-DD` เหมือนกัน)

**R-27 — `catch` 448 จุด 4 แนวทาง และ 0 จุดที่ยิง telemetry**
- สร้าง `src/lib/observability/caught.ts` (`recordCaughtError` · `recordDegraded`)
  ที่ป้อนตัวนับเข้า `recordEvent()` ซึ่ง `/admin` อ่านอยู่แล้ว · **ไม่ส่งข้อความ error ดิบเข้า metric**
  (metric เขียนลง KV และข้อความ error มี PII ปนได้)
- `catch` ที่ว่างเปล่าจริง ๆ **11 จุด** — เติมคำอธิบายว่าทำไมกลืนได้ทุกจุด
  ส่วนเส้นทางเปิดไพ่ที่กฎเหล็กข้อ 14 คุ้มครองอยู่ เปลี่ยนเป็นรายงานเข้าท่อสถิติ

**R-30 — `localStorage` ถูกเรียกตรงจาก 18 ไฟล์**
- สร้าง `src/lib/storage/keys.ts` (`STORAGE_KEYS` + `STORAGE_KEY_BUILDERS`) แล้วย้ายทุกคีย์เข้าไป
- ⚠️ **ตรวจโค้ดจริงแล้วผลตรวจคลาดเคลื่อนหนึ่งข้อ**: ผลตรวจเขียนว่าความถูกต้องของปุ่ม PDPA
  ขึ้นกับการรู้ทุกคีย์ — ของจริง `DeleteAllDataButton.tsx` เรียก `localStorage.clear()` อยู่แล้ว
  ซึ่งลบทุกคีย์รวมคีย์ที่ทะเบียนยังไม่รู้จัก **ปลอดภัยกว่าวนจากทะเบียน** จึงคงไว้และให้ด่านเฝ้าว่าห้ามเปลี่ยน
- คีย์ที่มีผู้ใช้จริงอยู่แล้ว (`persona_ratings` · `rating_<id>` · `welcome_shown_<id>` ·
  `seertarot_*`) **คงค่าเดิมไว้ทุกตัว** — เปลี่ยนชื่อคีย์ = ข้อมูลเดิมของผู้ใช้หายทันที

**R-29 — ซองจดหมาย API 4 แบบ · ทำแบบเพิ่มเข้าไป ไม่ใช่เขียนใหม่ทั้งหมด**
- ผลตรวจเสนอให้ "ย้าย 8 + 2 จุดก่อน" — ของจริง `{ success }` มี **23 จุด** ไม่ใช่ 8
- เปลี่ยนรูปคำตอบของ route ที่มีผู้ใช้อยู่ต้องแก้ทั้งสองฝั่งพร้อมกัน พลาดจุดเดียว = หน้าเว็บพังเงียบ ๆ
  โดยที่ typecheck ไม่ฟ้อง จึงทำสองอย่างที่ปลดล็อก R-28 ได้จริงโดยไม่แตะ route ที่ทำงานอยู่:
  `apiOk()` / `apiFail()` สำหรับของใหม่ · `readEnvelope()` ที่เข้าใจทั้งสี่แบบสำหรับฝั่งเรียก
- จำนวน route ที่ใช้รูปเก่าถูกตรึงเป็น **ratchet ที่ 23** (ลดได้ ห้ามเพิ่ม)

**R-28 — แผงแอดมิน 6 แผงแยก "พัง" ออกจาก "ไม่มีข้อมูล" ไม่ได้**
- `AdminOverview` · `StatsDashboard` — API คืน 500 แล้วเรนเดอร์ตารางว่าง (ผู้ดูแลอ่านว่า "ไม่มีคนเข้าเว็บ")
  `StatsDashboard` ตั้งค่า `err` ไว้แต่**ไม่เคยเรนเดอร์มันเลย**
- `EntitlementAdmin` — โหลดไม่สำเร็จแล้ว `s` ยังเป็น null ➔ **ค้างที่คำว่า "กำลังโหลด…" ตลอดกาล**
  (แย่กว่าไม่มี UI โหลด เพราะผู้ดูแลนั่งรอสิ่งที่ไม่มีวันมา)
- สร้าง `AdminErrorBanner` + ฮุก `useAdminResource` แล้วเติมให้ครบทั้ง 6 แผงที่ขาด
- **ล้างข้อมูลเดิมทิ้งเมื่อโหลดล้มเหลว** — เก็บของเก่าไว้คู่ข้อความผิดพลาด ผู้ดูแลจะอ่านตัวเลขเก่าเป็นปัจจุบัน

**R-31 — โค้ดตาย**
- `breakdown()` ที่ `StatsDashboard.tsx` เป็นสำเนาของตัวใน `lib/stats/read.ts` — ลบสำเนา ใช้ตัวกลาง
- ⚠️ ที่เหลือที่ `knip` รายงานยังไม่แตะ ตามคำเตือนในผลตรวจเองว่ารายงานผิด 10 จุด
  เพราะไม่ได้นับ `scripts/qa/*.ts` เป็นจุดเริ่ม (`CARD_IMAGE_VARIANTS` · `boxOf` ฯลฯ ถูกใช้โดยด่าน CI จริง)

**ด่านใหม่ `test-code-debt.ts` (15 ข้อ)**
- รันกับต้นไม้ก่อนแก้ (`0c834d7`) ➔ **ตก 13 ข้อ** · รันกับต้นไม้หลังแก้ ➔ **ผ่าน 15/15**
- ระหว่างทางด่าน `test-analytics-integrity.ts` ตกเพราะมันค้นหาสตริงคีย์ตายตัวที่ย้ายเข้าทะเบียนไปแล้ว
  แก้ให้ตรวจสิ่งที่สำคัญกว่าแทน: **ค่าคีย์ในทะเบียนต้องยังเป็นค่าเดิม** (เปลี่ยน = ผู้ใช้ทุกคนถูกถามความยินยอมใหม่)

**ด่านตรวจ**: 69 ➔ **70 ด่าน** · `npm run repo:verify` ผ่านครบ 70/70

---
### 🗓️ 2026-09-17 (รอบ 89): 🎛️ ยุบสถานะหน้าต่างลอยของ TarotFlow เป็นตัวลดเดียว (R-26 ขั้นที่ 1)

**สิ่งที่ผู้ตรวจสั่งไว้ชัดเจน**: *"ขั้นแรกที่คุ้มที่สุดคือยุบ 37 `useState` เป็น `useReducer` ตัวเดียว
ที่มี `FlowState` เป็น union ชัดเจน — **ทำแค่ขั้นนี้ก่อนขั้นเดียว อย่าเพิ่งแยกคอมโพเนนต์**"*

**ทำไมเริ่มที่กลุ่มหน้าต่างลอย**
- หน้าต่างลอยทั้ง 8 สถานะเป็น modal ที่กินทั้งจอ — โดยความหมายแล้วเปิดพร้อมกันไม่ได้
- แต่ของเดิมเก็บเป็น boolean อิสระ 6 ตัว + object 2 ตัว ➔ สถานะ "เปิดสองบานพร้อมกัน"
  **เป็นไปได้ในโค้ดโดยไม่มีอะไรห้าม** · ยุบเป็น union แล้วสถานะนั้นหายไปจากประเภทข้อมูลเลย
- เป็นกลุ่มที่พิสูจน์ความถูกต้องได้ครบด้วยชุดทดสอบ ไม่ต้องพึ่งเบราว์เซอร์

**บั๊กที่การยุบนี้ปิดไปด้วย**
- `close` ต้องเทียบชนิดก่อนปิด — เคสจริง: ผู้ใช้กด "เติมสิทธิ์" จากในหน้าต่างอัปเกรด ➔
  หน้าต่างเติมสิทธิ์เปิดทับ ➔ `onClose` ของหน้าต่างอัปเกรดยิงตามมาทีหลัง
  ของเดิม (`setAccessReason(null)` + `setIsBuyCreditsOpen(true)` เป็นคนละ state)
  ไม่มีอะไรรับประกันลำดับ · ตอนนี้ปิดผิดบานไม่ได้แล้วโดยโครงสร้าง

**ผลลัพธ์**
- `useState` ใน `TarotFlow.tsx`: **37 ➔ 28** · ตรึงเป็น **ratchet ที่ 28** (ลดได้ ห้ามเพิ่ม)
- ตรรกะอยู่ที่ `src/components/home/flow-overlay.ts` — ไม่มี I/O ไม่พึ่ง React

**ด่านใหม่ `test-flow-overlay.ts` (12 ข้อ)** — ยิงลำดับการกระทำจริงใส่ตัวลด ไม่ได้ค้นข้อความในซอร์ส
- รันกับต้นไม้ก่อนแก้ (`0c834d7`) ➔ **ตก 2 ข้อ** (`useState` 36 > เพดาน 28 · ยังไม่ใช้ตัวลดกลาง)
- รันกับต้นไม้หลังแก้ ➔ **ผ่าน 12/12**

**ที่เหลือของ R-26 ขึ้นทะเบียนเป็น ISSUE-049** — กลุ่มสำรับ/สตรีม/provably-fair คือกลุ่มที่
กฎเหล็กข้อ 4 กับข้อ 14 คุ้มครองอยู่ ยุบผิด = ไพ่แสดงผลผิด ซึ่ง typecheck อย่างเดียวพิสูจน์ไม่ได้
ต้องทำทีละกลุ่มพร้อมชุดทดสอบตัวลดแบบเดียวกับรอบนี้

**ด่านตรวจ**: 70 ➔ **71 ด่าน** · `npm run repo:verify` ผ่านครบ 71/71 · `npm run build` ผ่าน

---


### 🗓️ 2026-09-17 (รอบ 90): 🧾 ปิดสี่เรื่องสุดท้ายของผลตรวจรอบ 2 (ISSUE-049) — ปิดได้ 3 · เรื่องที่ 4 วัดใหม่แล้วเปลี่ยนรูป

รอบก่อนขึ้นทะเบียน 4 เรื่องไว้ที่ ISSUE-049 ว่า "ติดเงื่อนไขภายนอก ไม่ใช่รอคนเขียนโค้ด"
รอบนี้ปลดเงื่อนไขครบทุกเรื่อง · บันทึกเต็มพร้อมตัวเลขทุกตัวที่
[`plans/HANDOFF_ROUND2_CLOSEOUT_2026-09-17.md`](plans/HANDOFF_ROUND2_CLOSEOUT_2026-09-17.md)

**ข้อ 9 — ธงสิทธิ์บน production (ปิด)**
- อ่านจาก KV จริง: `app:flag:entitlement.enforced` = `{"value":true}` ➔ **ระบบสิทธิ์บังคับใช้อยู่**
  ไม่มีใครปิดไว้ · ไม่ใช่บั๊ก · คำถามนี้ค้างมาเพราะ "ตัดสินจากโค้ดไม่ได้" เท่านั้น
- เพิ่ม `npm run flags:show` (อ่านอย่างเดียว · อ่าน namespace จาก `wrangler.jsonc`)
  รอบหน้าไม่ต้องรบกวนคนที่ล็อกอินแอดมินได้อีก

**R-26 ขั้นที่ 2 — ยุบสถานะที่เหลือของ `TarotFlow` (ปิด)**
- สามกลุ่มที่กฎเหล็กข้อ 4/14 คุ้มครองอยู่ ถูกยุบเป็นตัวลดสามตัว:
  `flow-deck.ts` (สำรับ) · `flow-session.ts` (เซสชัน provably-fair) · `flow-reading.ts` (สตรีมคำอ่าน)
- **`useState` 28 ➔ 16** (จากจุดตั้งต้นของผลตรวจ 37 = ลด 57%) · ratchet ขันตามทันที
- บั๊กที่ปิดไปด้วย: โทเคนเซสชันถูกทับด้วยค่าว่าง (เดิมต้องจำเขียน `if (data.sessionToken)` 4 จุด) ·
  หลักฐาน provably-fair ของรอบที่ทิ้งแล้วไหลเข้ารอบใหม่ · ไพ่ชุดใหม่หงายค้าง
- ด่านใหม่ `test-flow-state.ts` **37 ข้อ** — ยิงลำดับการกระทำจริง ไม่ค้นข้อความในซอร์ส
  **ทดสอบด่านด้วย mutation test 5 แบบ: จงใจทำให้ตัวลดพัง ด่านจับได้ทั้ง 5**

**R-31 ส่วนที่เหลือ — โค้ดตาย (ปิด)**
- แก้ที่ต้นตอ: commit `knip.jsonc` ที่นับ `scripts/**` เป็นจุดเริ่ม ➔ "รายงานผิด 10 จุด"
  ที่ผลตรวจเตือนไว้ (`CARD_IMAGE_VARIANTS` · `boxOf` · `MAP_MIN_COLUMN_PX` · `generateClientSeed`)
  หายจากรายงานทันที · เพิ่ม `npm run deadcode`
- ลบของตายจริง: `export default` ที่ไม่มีใครนำเข้า 4 จุด · alias `WEEKLY_LIMIT` ·
  ถอด `export` ของตัวอ่านความลับ/ตัวช่วยภายใน 5 ตัว (แคบพื้นที่ผิดพลาด ไม่ใช่แค่ความสะอาด)
- **เจอของใหญ่กว่าที่รายงาน (INC-0195)**: ฮุก `use-admin-resource.ts` ที่รอบก่อนเขียนเพื่อปิด R-28
  เป็น **ไฟล์กำพร้า ไม่มีแผงไหนเรียกเลยสักแผง** — และด่านที่ควรจับกลับเขียนว่า `fs.existsSync(...)`
  ซึ่ง **ตกไม่ได้ตราบใดที่ไม่มีใครลบไฟล์ทิ้ง** · ต่อสายจริงครบ 6 แผงแล้ว
  (`AdminOverview` · `AiHealthPanel` · `StatsDashboard` · `SystemHealthPanel` · `EntitlementAdmin` · `MarketingAudience`)
  แล้วเปลี่ยนด่านเป็นการวัดการถูกใช้จริง (นับแผงที่เรียกฮุก ratchet ≥ 6 · ห้ามแผงไหนยิงคำขอโหลดเอง)
  **ทั้งสองข้อตกแน่นอนถ้ารันกับต้นไม้ก่อนแก้** (ก่อนแก้: ใช้ฮุก 0 แผง · ยิงเอง 6 แผง)

**R-02 · R-10 · R-11 · R-12 — วัดใหม่หลัง R-01 ขึ้น production**
- ยืนยันก่อนว่า R-01 ขึ้นจริง: `sw.js` บน production คือ `v-e0d8093b` = commit `e0d8093`
- ยิง Lighthouse 13.4.1 เอง 4 หน้า × 2 รอบ (ปกติ / บล็อกสคริปต์ตรวจบอตของ Cloudflare)
  เพิ่ม `npm run perf:lh` ให้รอบหน้าวัดด้วยวิธีเดียวกันได้โดยไม่ต้องประกอบคำสั่งใหม่
- **R-10 ปิด** — `unused-javascript` เหลือ 20–270 ms และไฟล์เดียวที่ถูกฟ้องคือ runtime React ของ Astro
- **R-11/R-12 ยังจริง แต่เป็นเรื่องเดียวกับ T-23** — เหลือไฟล์เดียว `BaseLayout.*.css`
  (145 KB ดิบ / 24 KB zstd) ประหยัดได้ 100–400 ms · ทางที่เหลือคือ critical CSS หรือ Early Hints (สวิตช์เจ้าของ)
- **R-02 เปลี่ยนรูป** — `/spreads` TBT 926 ms (ปกติ) / 701 ms (บล็อกสคริปต์บอต) ยังช้าสุดจริง
  แต่ **Style & Layout 1,357 ms ทั้งที่มี element น้อยที่สุดในสี่หน้า (667)** ➔ ครึ่งหนึ่งของต้นทุน
  คือสไตล์ชีตก้อนเดียวที่ทุกหน้าจ่ายเท่ากัน (0.9–1.4 วินาที) ไม่ใช่โค้ดของหน้านี้
  อีกครึ่งคือ hydration คลังผัง 25 แบบ (งานก้อนเดียวยาว 687 ms)
  ➔ "ไปไล่ลดโค้ดในหน้า `/spreads`" คือการแก้ผิดที่ · บันทึกทิศทางไว้ใน ISSUE-049 แล้ว
- ⚠️ บันทึกวิธีวัดไว้ด้วย: ยิงซ้ำวันเดียวกันได้ TBT ต่างกัน 40–50 ms
  **อย่าตัดสินการแก้จากส่วนต่างต่ำกว่า ~100 ms** และห้ามเทียบข้ามเวอร์ชัน Lighthouse

**ด่านตรวจ**: 73 ➔ **74 ด่าน** · `npm run repo:verify` ผ่านครบ 74/74 · `npm run typecheck` ผ่าน

---


### 🗓️ 2026-09-17 (รอบ 91): 📐 `/spreads` เลิก hydrate คลังผังทั้งก้อน + กวาด UI ที่ไม่มีใครเรนเดอร์

เจ้าของสั่ง *"แก้ข้อ 1-2 เลย"* หลังรอบ 90 ปิดผลตรวจ — สองข้อนั้นคือ (1) R-02/R-11/R-12 เรื่องประสิทธิภาพ
และ (2) โค้ดตายกลุ่ม UI ที่ไม่มีใครเรนเดอร์

**R-02 — คลังผัง 25 แบบไม่ต้อง hydrate อีกต่อไป**
- เดิม `SpreadsLibrary` เป็น island `client:idle` ทั้งก้อน เพื่อความสามารถแค่สองอย่าง:
  สลับแท็บหมวด กับ กดขยายดูตำแหน่งไพ่ · ต้นทุน = React runtime + **ข้อมูลผังทั้งชุด 17.2 KB gzip**
- ตอนนี้: การ์ดทั้ง 25 ใบเรนเดอร์เป็น HTML ตั้งแต่ตอนบิลด์ (เดิม 6 ใบ) ·
  แท็บใช้สคริปต์ ~1 KB (`astro/scripts/spreads-tabs.ts`) สลับแค่แอตทริบิวต์ `hidden` ·
  ปุ่มขยายใช้ `<details>` ของเบราว์เซอร์ ไม่ใช้ JS เลยสักบรรทัด
- **วัดจากบิลด์จริง**: JS **124.4 ➔ 100.1 KB gzip** · HTML 20.0 ➔ 33.8 KB ·
  **รวมที่ผู้ใช้ต้องโหลด −10.5 KB** · ไฟล์ JS 21 ➔ 19 ก้อน · ลิงก์ผังใน HTML ดิบ **6 ➔ 25 เส้น**
- เพดานด่านงบบันเดิลแก้ทั้งสองฝั่งพร้อมเหตุผล: HTML 26 ➔ 38 (ขยับขึ้นเพราะมีของที่ลดลงมาแลก)
  **JS 136 ➔ 120 (ขันให้แน่นขึ้น)**
- พิสูจน์ในเบราว์เซอร์จริงกับบิลด์ในเครื่อง: สลับแท็บครบ 5 หมวด (6·5·5·5·25 ตรงตัวเลขบนแท็บ) ·
  ลูกศรซ้าย/ขวาใช้ได้ · `aria-labelledby` ตามแท็บ · `<details>` ย่อ/ขยาย 308 ➔ 2,952 px
- **ผลจริงบน production หลัง deploy** (Lighthouse 13.4.1 · มือถือ · ยิงสองรอบ):
  TBT ปกติ **926 ➔ 645 / 85 ms** · TBT เมื่อบล็อกสคริปต์ตรวจบอต (= โค้ดเราล้วน) **701 ➔ 107 / 72 ms**
  ➔ **ส่วนที่เป็นของเราลดลง 85–90%** · perf (บล็อกสคริปต์บอต) **67 ➔ 96**
- ตรวจใช้งานจริงบน production หลัง deploy: สลับแท็บครบ (6·5·5·25·6) · `<details>` 43 ➔ 112 px ·
  ทั้งหน้าเหลือแท็ก `<script src>` **1 ก้อน**
- ⚠️ บทเรียนการวัด: บนเครื่อง dev ยิงหน้าเดิมสามรอบได้ 769 · 1,560 · 1,826 ms — **ตัดสินอะไรไม่ได้เลย**
  ตัวเลขที่ใช้ตัดสินต้องมาจาก production เท่านั้น

**R-11/R-12 — ตรวจซ้ำแล้วยืนยันว่าไม่มีไขมันให้ตัดในโค้ด**
- แยกองค์ประกอบสไตล์ชีต: ตัวแปร CSS 557 ตัว · `@font-face` 1 KB · `@keyframes` 2 KB · กฎ 1,593 ข้อ
  ➔ ไม่มีก้อนผิดปกติ · ยืนยันข้อสรุปของ T-23 ด้วยตัวเลขชุดใหม่
- ลูกบิดที่เหลือคือ **Early Hints ที่ Cloudflare Dashboard** · ยืนยันแล้วว่ากดจากที่นี่ไม่ได้จริง
  (`wrangler` มีเฉพาะสิทธิ์ Workers/KV · ไม่มี `CLOUDFLARE_API_TOKEN` ในสภาพแวดล้อม)

**โค้ดตายกลุ่ม UI (เจ้าของเคาะให้ลบ)**
- `RitualStepProgress.tsx` **132 บรรทัด** — WORK_LOG 2026-09-03 บันทึกเองว่าเจ้าของสั่งเอาแถบขั้นตอน
  ออกจากหน้าแรก ตั้งแต่นั้นไม่มีหน้าไหนเรนเดอร์อีกเลย · ย้าย `type RitualStep` ไป `ritual-step.ts`
- โครงร่างตอนโหลด **7 ตัว** (`Cards*` · `Blog*` · `Spreads*` · `ReadersLoadingEn`) — หน้าพวกนั้นย้ายไป Astro
  หมดแล้วจึงไม่มี `loading.tsx` ของ Next ให้ใช้ · เหลือ `ReadersLoading` ตัวเดียวที่มีคนเรียกจริง
- `SearchTabIcon` ลบทิ้ง · `MiniRwsCard` **ถอดแค่ `export`** เพราะใช้ภายในไฟล์ตัวเองอยู่
  (ตัวอย่างว่าทำไมต้องไล่ดูทีละตัว ไม่ใช่ลบตามรายงาน `knip`)

**ด่านตรวจ**: คงที่ **74 ด่าน** · `npm run repo:verify` ผ่านครบ 74/74 · `npm run build` ผ่าน (307 หน้า)

---


### 🗓️ 2026-09-17 (รอบ 92): ⚡ Early Hints — ส่วนหัว `Link` ของสไตล์ชีตที่สร้างตอนบิลด์ (ปิด R-11/R-12)

เจ้าของเปิดสวิตช์ **Early Hints** ที่ Cloudflare Dashboard แล้ว (ส่งภาพยืนยันมา) —
ของที่ขาดคือฝั่งเรา: Cloudflare อ่าน **ส่วนหัว `Link`** ของหน้า HTML ไปส่งเป็น `103`
แต่หน้าเว็บของเราไม่เคยมีส่วนหัวนั้นเลยสักหน้า (มีแต่แท็ก `<link>` ใน HTML ซึ่ง Early Hints ใช้ไม่ได้)

**สิ่งที่ทำ**
- `scripts/merge-astro-assets.ts` เติมบล็อก `Link: </_astro/<ไฟล์ CSS>>; rel=preload; as=style`
  ลง `_headers` ของ Worker assets ให้หน้า HTML **22 เส้นทาง** ทุกครั้งที่บิลด์
- **ไม่เขียนมือ** เพราะชื่อไฟล์ CSS มีแฮช — เขียนมือ = ค้างชื่อเก่าทันทีที่แก้ CSS ครั้งถัดไป
  แล้วจะกลายเป็นการพรีโหลดไฟล์ที่ไม่มีอยู่จริง ซึ่งแย่กว่าไม่ทำอะไรเลย
- **ไม่เก็บรายชื่อเส้นทางไว้สองที่** — ตัวสร้างอ่านจากบล็อกที่มี `s-maxage` ใน `_headers` เอง
  (นั่นคือนิยามของ "หน้า HTML ที่ขอบเสิร์ฟเอง" ในไฟล์นั้นอยู่แล้ว) เพิ่มหน้าใหม่ก็ตามไปเอง
- หาไฟล์ CSS ไม่เจอ หรือหาเส้นทางไม่เจอเลย = **หยุดบิลด์** ไม่ข้ามเงียบ (บทเรียน R-07)

**ด่านที่เพิ่มเข้าไปในด่านเดิม** (`test-static-headers.ts` หัวข้อ 3): ตรวจว่าตัวสร้างยังอยู่จริง
และ `public/_headers` ยังมีบล็อกหน้า HTML ให้มันเจอ ≥ 20 เส้นทาง พร้อมเส้นทางหลัก 5 เส้น
**ทดสอบแล้วว่าตกจริง**: ตอนเขียนครั้งแรกเรกเอ็กซ์จับบรรทัด `/` (สแลชเดี่ยว) ไม่ได้ ด่านฟ้องทันที
ทั้งที่ตัวสร้างก็พลาดจุดเดียวกัน — แก้ทั้งสองที่พร้อมกัน

**พิสูจน์บน production แล้วว่าทำงานจริง** — `curl -v --http2` ได้ `HTTP/2 103` พร้อมส่วนหัว
`link: </_astro/BaseLayout.C4MwRXdL.css>; as=style; rel=preload` **ครบทุกกลุ่มหน้า**
(`/` · `/cards` · `/cards/major-00` · `/blog` · `/spreads` · `/daily` · `/en/spreads`)
และ URL ในส่วนหัวคือไฟล์ CSS ที่หน้านั้นใช้จริง (ตอบ 200)

⚠️ **กับดักตอนตรวจ**: ต้องอุ่นแคชก่อน — Cloudflare จำส่วนหัว `Link` จากคำตอบที่เคยผ่านขอบ
แล้วค่อยส่ง `103` ในคำขอถัดไป · ยิงครั้งแรกหลัง deploy เห็นแค่ `200` **ไม่ได้แปลว่าพัง**
(เสียเวลาไปหนึ่งรอบกับเรื่องนี้)

**ตัวเลข Lighthouse หลังเปิด** (`simulate`): `/spreads` perf 90 · `/blog` 84 — ไม่ต่างอย่างมีนัย
และตรงกับที่คาด เพราะโหมด `simulate` จำลองเครือข่ายเอง จึงไม่สะท้อนประโยชน์ของ `103`
ผลจริงต้องดูจาก CrUX/PSI ภาคสนามเมื่อข้อมูลสะสมพอ

**ด่านตรวจ**: คงที่ **74 ด่าน** · `npm run repo:verify` ผ่านครบ 74/74

---



### 🗓️ 2026-09-17 (รอบ 85): 🌍 ปิดรูภาษาต่างด้าวทั้งหมวด — `você` หลุดเข้าคำอ่านจริงถึงมือผู้ใช้

**อาการที่ผู้ใช้เจอ** (ถ่ายหน้าจอมาให้ดู): คำอ่านจากแม่หมอขึ้นว่า

> "ในบริบทที่ **você** เบื่อชีวิต ไพ่ใบนี้กำลังบอกเป็นนัยว่า..."

`você` = "คุณ" ในภาษาโปรตุเกส · เจ้าของบอกว่า **"อีกแล้ว"** แปลว่าเคยเกิดมาก่อน

**ต้นตอ**: [`src/lib/ai/language.ts`](../src/lib/ai/language.ts) `FOREIGN_SCRIPT_REGEX` จับเฉพาะอักษร
**ที่ไม่ใช่ละติน** — จีน ญี่ปุ่น เกาหลี รัสเซีย อาหรับ ฮินดี ฮีบรู · `você` เขียนด้วยอักษรละติน
(`ê` = U+00EA อยู่ใน Latin-1 Supplement) ด่านจึงนับได้ **0 ตัว** ไม่มีอะไรทำงานเลย
ไม่ตัดวงจร ไม่สลับไป Gemini ไม่ยิง metric

รันเรกเอ็กซ์ชุดเดิมกับตัวอย่างจริงแล้วยืนยันว่า **หลุดทั้งหมวด**: โปรตุเกส · สเปน · ฝรั่งเศส ·
เยอรมัน · เวียดนาม · อินโดนีเซีย (ส่วนจีนกับญี่ปุ่นที่ออกแบบมาจับ ยังจับได้ปกติ)

**ทำไมเดิมถึงออกแบบแบบนั้น**: เว็บนี้ใช้อักษรละตินจริงทั้งเว็บ — ชื่อไพ่ `The Lovers` ·
หน้า `/en` ทั้งชุด · คำทับศัพท์ จะแบนอักษรละตินทั้งหมดไม่ได้ ตัวจับเดิมจึงเลือกดูแค่
"อักษรที่ไม่มีทางถูกต้อง" ซึ่งปลอดภัยแต่มองไม่เห็นภาษาที่ใช้อักษรเดียวกับอังกฤษ

**เส้นแบ่งที่ใช้แทน**: **ตัวอักษรละตินที่มีเครื่องหมายกำกับ** ซึ่งภาษาอังกฤษแทบไม่ใช้
สแกน `src/data/` `src/app/_shared/` `src/components/` ทั้งหมดแล้วพบตัวในช่วงนี้แค่
`×` (U+00D7) กับ `ƒ` (U+0192) ซึ่งเป็นสัญลักษณ์ไม่ใช่ตัวอักษร — ยกเว้นสองตัวนี้แล้ว
ที่เหลือถือว่าต่างด้าวได้อย่างปลอดภัย

**สิ่งที่ทำ** (ทำตามปรัชญาเดิมของไฟล์: *"ถ้าแก้ถูก 100% ด้วย regex ได้ ให้แก้เงียบ ๆ
อย่า failover เพราะ failover แลกด้วยเวลาที่ผู้ใช้นั่งรอ"*):

1. `ACCENTED_LATIN_WORD_REGEX` — จับ **ทั้งคำ** ที่มีอักษรมีเครื่องหมายกำกับอยู่ข้างใน
   ⚠️ ถ้าตัดแค่ตัวอักษร `não` จะเหลือ `no` และ `você` จะเหลือ `voc` ซึ่งแย่กว่าเดิม
2. `LATIN_LEAK_MAP` — คลังคำต่างด้าวที่ไม่มีเครื่องหมายกำกับ (`usted` `vous` `anda` `kamu`
   `porque` ฯลฯ) แปลเป็นไทย แนวเดียวกับ `CHINESE_LEAK_MAP` ที่มีอยู่แล้ว
   · **ไม่ใส่คำที่ชนกับภาษาอังกฤษ** (ไม่มี `ban` ของเวียดนาม ไม่มี `Sie` ของเยอรมัน)
3. ถอดเครื่องหมายกำกับแล้วเทียบคลังคำ → `você` กลายเป็น **`คุณ`** ไม่ใช่ถูกลบจนประโยคขาดคำ
4. `countForeignCharacters` นับ **ความยาวทั้งคำ** ไม่ใช่นับเฉพาะตัวที่มีเครื่องหมาย
   เพราะตัวตัดวงจรนับเป็นจำนวนตัวอักษรและสลับโมเดลที่ 14 ตัว — ถ้านับ `você` เป็น 1
   ต่อให้หลุดเป็นโปรตุเกสทั้งย่อหน้าก็ไม่มีวันถึงเกณฑ์
   · ผลที่ได้: ประโยคเต็มได้ 21 (สลับโมเดล) · คำเดี่ยวได้ 4 (แก้เงียบ ๆ) — ตรงตามที่ควรเป็น
5. **ด่านใหม่** `scripts/qa/test-foreign-leak.ts` (ด่านที่ 67) — 27 ข้อ แบ่งเป็น
   ต้องจับได้ 14 เคส (6 ภาษา + จีน/ญี่ปุ่นของเดิม) · **ห้ามแตะอังกฤษ 8 เคส**
   (ชื่อไพ่ในประโยคไทย · คำอ่านอังกฤษเต็มประโยค · คำทับศัพท์) · ห้ามเหลือเศษคำ 3 เคส ·
   เกณฑ์ตัดวงจร 2 เคส

**ข้อจำกัดที่รู้ตัวและจดไว้ในหัวไฟล์ด่าน**: คำต่างด้าวที่ **ไม่มีเครื่องหมายกำกับและ
ไม่อยู่ในคลังคำ** (เช่น `necesita` · `harus`) ยังหลุดได้ · เส้นแบ่งนี้เลือกโดยตั้งใจ
เพราะจะจับให้ครบต้องเดาว่าคำละตินคำไหนไม่ใช่อังกฤษ ซึ่งเสี่ยงไปแตะชื่อไพ่และคำอ่านอังกฤษจริง
เจอคำใหม่หลุดมาให้เติมลง `LATIN_LEAK_MAP` แล้วเพิ่มบรรทัดในคลังทดสอบ

**พิสูจน์**: `typecheck` 0 errors · `repo:verify` ผ่าน **67/67** ·
ด่านใหม่รันกับโค้ดก่อนแก้ **ตก 16 จาก 27 ข้อ** รันกับโค้ดหลังแก้ **ผ่านครบ 27 ข้อ** ·
เคสจริงให้ผล `"ในบริบทที่ คุณ เบื่อชีวิต ไพ่ใบนี้กำลังบอกเป็นนัย"` ·
ภาษาอังกฤษทั้ง 8 เคสได้น้ำหนัก **0** และข้อความไม่ถูกแก้แม้แต่ตัวเดียว

---

### 🗓️ 2026-09-17 (รอบ 84): 🚪 URL ตายไม่ปลุก Worker อีกต่อไป — ปิด R-25 (T-25 เดิม · ISSUE-048)

**บริบท**: รอบตรวจก่อนหน้าตั้ง T-25 ไว้ว่า "หน้า 404 ปลุก Next Worker และเสิร์ฟ `no-store`"
(วัดจาก production: 7,978 B ต่อครั้ง · ไม่มี `cf-cache-status` เลย · บอตไล่ยิง URL ตายพันเส้น
= Worker invocation พันครั้งที่คิดเงิน) แล้ว**ถูกตีกลับ**เข้า ISSUE-048 เพราะทีมที่รับไปแก้พบว่า
ถ้าเปลี่ยน `not_found_handling` เป็น `"404-page"` ตรง ๆ ตามที่เอกสารเขียน `/account` `/admin`
`/s/[id]` `/api/**` จะ **404 ทั้งหมด** — ซึ่งถูกต้อง และคอมเมนต์เดิมใน `wrangler.jsonc`
ก็เตือนเรื่องนี้ไว้แล้วว่า "ห้ามใช้ `404-page` เด็ดขาด"

**สิ่งที่รอบนี้พบเพิ่ม**: `wrangler` 4.128 รองรับ `assets.run_worker_first` เป็น **array ของเส้นทาง**
(ยืนยันกับ `config-schema.json` ในเครื่อง) ซึ่งมีไว้แก้ปัญหานี้โดยเฉพาะ — ระบุว่าเส้นไหนต้องเข้า
Worker เสมอ ที่เหลือค่อยตอบจากขอบ **จึงไม่ต้องไปตั้งกฎ WAF บน Dashboard อย่างที่เคยสรุปไว้**

**วิธีหารายการเส้นทาง** (จดไว้ให้รันซ้ำพิสูจน์ได้): ยิง production จริงแล้วดูหัว `cf-cache-status`
— **มีหัวนี้ = ตอบจากขอบอยู่แล้ว ห้ามใส่** · **ไม่มีหัวนี้ = วิ่งเข้า Worker อยู่ ต้องใส่**
วิธีนี้จับได้ 4 เส้นที่การไล่อ่านโค้ดอย่างเดียวมองข้าม: `/sitemap.xml` · `/robots.txt` ·
`/manifest.webmanifest` (สร้างสดจาก `src/app/*.ts` ไม่ใช่ไฟล์ static) และ `/_next/image*`

**สิ่งที่ทำ**:

1. `wrangler.jsonc` — `not_found_handling` เป็น `"404-page"` · `run_worker_first` เป็นรายการ 14 เส้นทาง
   พร้อมคอมเมนต์วิธีหารายการนี้ เพื่อให้คนที่เพิ่มหน้า Next ใหม่รู้ว่าต้องเติมที่นี่
2. `src/app/_shared/pages/not-found.tsx` — แยก `NotFoundMain` (เนื้อในล้วน) ออกจาก `NotFoundBody`
   (ห่อหัวเว็บ/ฟุตเตอร์) เพราะ `NotFoundBody` เรนเดอร์ `<SiteHeader />` ไว้ข้างในแบบไม่ hydrate
   ถ้ายกมาใช้ใน Astro ทั้งก้อนจะได้หัวเว็บนิ่ง ๆ ที่**กดเมนูมือถือไม่ได้**
3. `astro/pages/404.astro` + `astro/pages/en/404.astro` — หน้าใหม่ ประกอบหัวเว็บด้วย island
   `client:idle` เหมือนหน้าอื่นทุกหน้า · ข้อความยังอยู่ใน `COPY` ที่เดียว ไม่ได้ก๊อปซ้ำ (INC-0112)
   · ต้องมีสองไฟล์เพราะ Cloudflare หา `404.html` ที่**ใกล้ที่สุด** ถ้ามีแต่ที่ราก คนพิมพ์ `/en/zzz`
   ผิดจะเจอหน้า 404 ภาษาไทย
4. `src/lib/routing/astro-routes.ts` — ขึ้นทะเบียน `/404` ใน `ASTRO_EXACT_ROUTES`
5. **ด่านใหม่** `scripts/qa/test-worker-first-routes.ts` (ด่านที่ 66) — ไล่หาหน้า Next ทุกหน้าจาก
   ไฟล์จริงแล้วยืนยันว่ามีรูปแบบใน `run_worker_first` ครอบคลุมครบ · เพราะหลังเปลี่ยนคอนฟิกนี้
   **การลืมเติมเส้นทางจะทำให้หน้าใหม่ 404 บน production ทั้งที่ทุกอย่างผ่านในเครื่อง**
   (ในเครื่องไม่มีชั้น assets ของ Cloudflare มาคั่น จึงไม่มีทางเห็นอาการ)

**สิ่งที่ไม่ได้ลบทิ้ง**: `src/app/(th)/not-found.tsx` และ `src/app/not-found.tsx` ยังต้องอยู่
เพราะ `notFound()` ที่ถูกเรียกจากในหน้าที่ Worker เรนเดอร์ (เช่น `/readers/<ไอดีมั่ว>`)
ไม่ได้ผ่านชั้น assets เลย — สองทางใช้ข้อความชุดเดียวกันจึงไม่มีทางพูดคนละอย่าง

**พิสูจน์**: `npm run typecheck` 0 errors · `npm run repo:verify` ผ่าน **67/67** ·
ด่านใหม่รันกับโค้ดก่อนแก้แล้ว **ตก 2 ข้อ** (exit 1) รันกับโค้ดหลังแก้ **ผ่านครบ** ·
`npm run build:worker` สร้าง `.open-next/assets/404.html` (42,496 B) และ `en/404.html` (37,407 B)
ทั้งคู่ `lang` ตรงภาษา · `noindex, follow` · มีหัวเว็บ+ฟุตเตอร์ · `<h1>` หนึ่งตัว ·
ลิงก์ทางออก 4 เส้น · สายด่วน 1323 ครบ

**ค้างไว้**: ต้องยืนยันหลัง deploy จริงว่า `/faq` ได้ 404 พร้อม `cf-cache-status` และ
`/account` `/admin` `/s/*` `/api/**` **ยังไม่ใช่ 404** (ข้อหลังคือข้อที่ทีมก่อนกลัวว่าจะพัง)

---

### 🗓️ 2026-09-16 (รอบ 83): 🔍 ปิดผลตรวจทั้งเว็บ `HANDOFF_FULL_AUDIT_2026-09-16` ครบทั้ง 48 ข้อ

**บริบท**: เจ้าของส่งผลตรวจทั้งโปรเจกต์ (379 ไฟล์ TS/TSX · 77,754 บรรทัด + `astro/`) มาให้แก้ทั้งชุด
รวม **48 ข้อ** (Critical 2 · High 13 · Medium 24 · Low 14) โดยทุกข้อ "รอดด่านทั้ง 62 ด่านไปได้"

**ผลรวม**: ลงมือแก้ **45 ข้อ** · ตั้งใจไม่ทำ **3 ข้อ** พร้อมเหตุผลที่วัดมาแล้ว
ด่านตรวจ **62 ➔ 67 ด่าน** (เพิ่ม 3 ด่านใหม่ ทุกด่านพิสูจน์แล้วว่า**ตกก่อนแก้** ไม่ใช่ผ่านตั้งแต่ยังไม่แก้)
บันทึกบทเรียนถาวร **INC-0182 ถึง INC-0187**

#### 🔴 สองข้อที่สำคัญที่สุด

1. **ด่านกันทำร้ายตัวเองจับสำนวนไทยไม่ได้ 13 จาก 18 แบบ (T-01 · T-35)** — คำที่คนไทยใช้บ่อยที่สุด
   เรื่องนี้อย่าง **"คิดสั้น"** และสำนวนอังกฤษที่ใช้บ่อยที่สุดอย่าง **"I want to die"** ไม่มีในลิสต์เลยสักคำ
   รันคลังทดสอบ 36 บรรทัดกับเรกเอ็กซ์ชุดเดิม: **จับไม่ได้ 36 จาก 36** · หลังแก้: **36/36**
   และคลังกันจับเกิน 18 บรรทัด (`เหนื่อยจะตาย` · `หิวจะตาย` · `5 kms`) ไม่ถูกบล็อกสักบรรทัด
2. **เปิดไพ่ฟรีไม่จำกัดด้วยการยิง `/read` ซ้ำแล้วตัดสาย (T-02)** — `consumeReading` คืน boolean
   ตัวเดียวทั้งกรณี "หักเอง" และ "หักไปแล้ว" การคืนสิทธิ์จึงลบแถวค่าใช้จ่ายของคำขอแรกทิ้ง
   ทำให้ทั้ง `DAILY_LIMIT` และเครดิตที่ซื้อมาบังคับใช้ไม่ได้ทั้งคู่

#### 📋 สรุปตามหมวด

| หมวด | ข้อที่ปิด | สาระสำคัญ |
| :--- | :--- | :--- |
| **ความปลอดภัยของคน** | T-01 · T-35 | คลังคำวิกฤตใหม่ (ไทย 60 คำ · กฎไทย 6 · กฎละติน 31 รวมคาราโอเกะ) + ตัวปรับข้อความมาตรฐาน (ตัดช่องว่าง · คลี่สระอำ · ถอดวรรณยุกต์ · ยุบอักษรซ้ำ) · ปลดประตูเรกเอ็กซ์ของตัวจำแนก AI · fail-safe + metric |
| **เงินและสิทธิ์** | T-02 · T-04 ถึง T-10 | แยกสถานะการหักสิทธิ์ · คืนสิทธิ์เฉพาะแถวของตัวเอง · `persistReading` ตอน done · `grantBonus` อ่านกลับมายืนยัน · `AbortSignal` เข้าถึง Groq/Gemini · `payments.user_id` + ถอด FK ที่เป็นจริงไม่ได้ · เพดานของนโยบาย DB ล่ม · `db.batch()` |
| **ความปลอดภัยระบบ** | T-11 ถึง T-18 | เพดานอัตราย้ายไป Redis/KV (`edge-ratelimit.ts`) · โควตาต่อ IP และ userId บน 3 ปลายทางที่ขาด · `getClientIdentifier` ไม่รับหัวที่ปลอมได้ · ตัวกันฉีดคำสั่งเข้า prompt · เซสชันแอดมิน/ผู้ทดสอบเพิกถอนได้ · JSON-LD escape · webhook fail-closed |
| **การเข้าถึง + UX** | T-03 · T-29 ถึง T-34 · T-45 ถึง T-47 | สตรีมคำทำนายใช้กับโปรแกรมอ่านหน้าจอได้จริง · `prefers-reduced-motion` · หนึ่ง span ต่อหนึ่งก้อน · ปุ่มปิด 44px · live region ตอนพลิกไพ่ · flush สถานะตอนออกจากหน้า · แจ้งเมื่อประวัติถูกตัด · แถวข้อมูลเสียหายบอกตรง ๆ |
| **ประสิทธิภาพ** | T-19 ถึง T-22 · T-26 ถึง T-28 | `s-maxage` + `stale-while-revalidate` ทุกหน้า HTML · ถอด `no-store` · ฟอนต์ที่พรีโหลด 67,828 ➔ **33,324 B** · งบน้ำหนักเห็น CSS/ฟอนต์/LCP/ส่วนหัวแคชแล้ว |
| **ด่าน CI + ภาษาไทย** | T-36 ถึง T-44 · T-48 | ด่านไม่ข้ามเงียบเมื่อไฟล์หาย · ภาษาไทยที่ทีมเขียนเองถูกตรวจจริง (แก้ไม้ยมก **125 จุดใน 24 ไฟล์**) · คำตอบสำรองผ่านตัวขัดภาษา · Groq มี backoff · `Person` schema |

#### ⚖️ สามข้อที่ตั้งใจไม่ทำ (พร้อมเหตุผล)

- **T-23 แยก CSS ต่อหน้า** — Tailwind v4 สร้างสไตล์ชีตก้อนเดียวจากการสแกนทั้งโปรเจกต์ ไม่มีอะไรให้แยก
  ทางเลือกที่เหลือคือสกัด critical CSS ซึ่งแลกด้วยความเสี่ยง FOUC ถาวร · ค่าปัจจุบัน 23 KB gzip อยู่ในงบสบาย ๆ
- **T-24 `unicode-range`** — ต้องสร้างไฟล์ `.woff2` ชุดใหม่ 8 ไฟล์ด้วย `pyftsubset` ซึ่งไม่มีในสภาพแวดล้อมนี้
  ทำครึ่งทางไม่ได้: ประกาศ `unicode-range` โดยไม่แยกไฟล์จริง = หน้า `/en` ไม่มีฟอนต์ละตินใช้เลย
- **T-25 ให้ Cloudflare เสิร์ฟ 404 จากขอบ** — ต้องเปลี่ยน `not_found_handling` เป็น `"404-page"`
  ซึ่งทำให้ **ทุกเส้นทางที่ไม่มีไฟล์ static ตรงกันได้ 404 แทนที่จะวิ่งไปหา Worker** = `/account` · `/admin`
  · `/s/[id]` · `/api/**` พังทั้งหมด · ต้องแก้ที่กฎ WAF บน Dashboard แนวเดียวกับ ISSUE-047 ไม่ใช่ที่โค้ด

#### 🔎 ข้อค้นพบที่แก้ผลตรวจเอง

- **T-20 คลาดเคลื่อนจากวิธีวัด** — React 19 เรนเดอร์ `fetchPriority` ออกมาเป็นตัวพิมพ์ผสม
  ชื่อแอตทริบิวต์ใน HTML ไม่สนตัวพิมพ์เล็กใหญ่ เบราว์เซอร์จึงอ่านถูกมาตลอด · `grep` แบบสนตัวพิมพ์ได้ 0
  จึงสรุปผิดว่า "ทั้งเว็บไม่มีเลย" · วัดใหม่: `/` = 1 · `/cards` = 1 · `/blog` = 1 (มีและทำงานอยู่แล้ว)
- **T-39 บางส่วนแก้ไปแล้วก่อนหน้านี้** — เส้นทาง `chat` ผ่าน `isAiCapReached()` และนับด้วย `recordAiCall()` อยู่แล้ว
- **`catch` เปล่าในสคริปต์ QA 5 ไฟล์ที่ผลตรวจระบุว่า "ยังไม่ได้ดู"** — ตรวจครบแล้ว ทุกตัวบันทึกการล้มเหลว
  ผ่าน `check(..., false)` หรือเป็นลูปลองใหม่ที่ไม่ได้ข้ามการตรวจ **ไม่มีด่านหลอกซ่อนอยู่**
- **เอกสารล้าสมัย** — `CLAUDE.md` ยังบอกว่า `/en/blog` · `/en/spreads/topic/*` · `/en/cards/birth-card`
  "ยังไม่เปิด" ทั้งที่เปิดครบและแปลครบ 26/26 บทความแล้ว → แก้ให้ตรงความจริง

#### ✅ ด่านใหม่ 3 ด่าน (ทุกด่านพิสูจน์แล้วว่าตกก่อนแก้)

| ด่าน | พิสูจน์ว่าตกก่อนแก้อย่างไร |
| :--- | :--- |
| `test-json-ld-escape.ts` | ย้อน `RootHtml.tsx` กลับ → ตกทันที 3 จุด |
| `test-prompt-injection.ts` | ย้อน `prompt.ts` กลับ → ตกทันที 10 ข้อ |
| `test-thai-content-quality.ts` | ย้อน `articles.ts` + `spreads.ts` กลับ → จับได้ 34 จุด |

และด่านเดิมที่ถูกทำให้เข้มขึ้น: `test-no-fake-card.ts` (ย้าย `StreamReader.tsx` ออก → ตก 1 ข้อ) ·
`test-judge-baseline.ts` (ซ่อน remote `origin` → ตกใน CI mode) · `test-bundle-budget.ts`
(ย้อน `public/_headers` กลับ → ตกที่ข้อ `s-maxage`) · `test-entitlement.ts` (+5 เคสของ T-02)
...
จึงไม่หักสิทธิ์ผู้ใช้ **แต่ไม่เคยมีด่าน QA ตรวจมันเลยแม้แต่ด่านเดียว** จึงหลุดไป 4 เรื่องพร้อมกัน

**อาการที่พบ (ตรวจโค้ดจริง ไม่ได้เดา)**
1. เสิร์ฟ mock แล้ว **ไม่จดสถิติเลยสักตัว** มีแค่ `console.warn` ➔ แผงแอดมินมองไม่เห็นว่าเกิดบ่อยแค่ไหน
   (ทุก failure path อื่นของไฟล์เดียวกันจด `recordEvent` ครบหมด)
2. ข้อความ **เป็นไทยล้วน 100%** ไม่มีสาขาอังกฤษเลย ➔ ผู้ใช้ `/en` ที่ตกมาถึงได้คำอ่านไทยทั้งก้อน
3. `yesNoAnswer: null` ตายตัว ➔ ผัง `yes-no` ไม่มีคำตอบฟันธงให้เลย (UI ซ่อนชิปเมื่อค่าเป็น null)
4. รองรับบุคลิกแค่ 3 จาก 5 ตัวใน `PERSONAS` ➔ `playful` กับ `master` ได้สำนวนของ `warm` เงียบ ๆ

**ต้นตอร่วม**: `gemini.ts` มี `import "server-only"` ด่านทดสอบจึง import มาเรียกไม่ได้เลย
ทุกด่านที่เกี่ยวข้องจึงทำได้แค่ "อ่านไฟล์เป็นข้อความ" (ดูคอมเมนต์ใน `test-ai-reading-golden.ts`)
โค้ดก้อนนี้จึงเป็นจุดอับที่ไม่มีใครตรวจพฤติกรรมจริงได้ตั้งแต่วันแรก

**สิ่งที่แก้**
1. **แยกไฟล์** `src/lib/ai/mock-reading.ts` ออกจาก `gemini.ts` (ไม่มี `server-only`) ➔ ทดสอบพฤติกรรมจริงได้
   `gemini.ts` import แล้ว re-export ต่อ เส้นทางเดิมทุกจุดเรียกเหมือนเดิมไม่ต้องแก้
2. **จดสถิติทุกครั้งที่เสิร์ฟ**: `ai_mock_served` + แยกตามสาเหตุ (`no_api_key` / `all_models_down`)
   + ตามจำนวนไพ่ + ตามภาษา · ทั้งสองจุดที่เรียกต้องส่งสาเหตุมาด้วยเสมอ (ด่านตรวจบังคับ)
3. **สองภาษาเต็มรูปแบบ**: บทเปิด · คำอ่านรายใบ · ความเชื่อมโยง · บทสรุป · คำแนะนำ · กรอบเวลา
   ดึงจาก `meaningsEn` / `keywordsEn` / `nameEn` / `getPositionName` / `getPositionMeaning` ที่มีครบ 100% อยู่แล้ว
   (`mood` คงค่าไทยไว้ เพราะเป็น enum ภายในที่ไม่ได้แสดงผล)
4. **ฟันธง ใช่/ไม่ใช่ จากข้อมูลจริง**: ใช้ค่า `yesNo` ที่ผูกกับไพ่ทั้ง 78 ใบ (yes 38 · no 22 · maybe 18)
   ไพ่กลับหัวสลับขั้ว แล้วรวมคะแนนทั้งผัง — ไม่ใช่การเดาและไม่ได้สร้างข้อมูลใหม่ (กฎเหล็กข้อ 14)
   บทสรุปขึ้นคำตอบให้ตรงกับค่าที่ฟันธงเสมอ จึงไม่ติด `YESNO_CONTRADICTION`
5. **ครบ 5 บุคลิก × 2 ภาษา** ในตารางเดียว (`MOCK_VOICE`) บุคลิกแปลกปลอมถอยไปใช้ `warm` ได้โดยไม่ล้ม
6. **UI**: เพิ่ม `YES_NO_DISPLAY_EN` ใน `reading.ts` แล้วให้ `StreamReader` ใช้แทนการพิมพ์ค่าดิบ
   (ค่าใน schema เป็นไทยเสมอตามดีไซน์ ของเดิมหน้าอังกฤษจึงขึ้น "Answer: ใช่" แม้เป็นคำอ่านจาก AI จริง)

**ด่านใหม่** `scripts/qa/test-mock-reading.ts` (ด่านที่ 61) — 102 เคส: ครบทุกบุคลิกและสำนวนต้องไม่ซ้ำ ·
หน้าอังกฤษห้ามมีอักษรไทยหลุดสักตัว (5 ผัง × 5 บุคลิก) · yes-no ต้องฟันธงและไม่ขัดกับบทสรุป ·
คำตอบต้องผันตามไพ่จริง (ยิง 26 ชุดต้องได้คำตอบต่างกัน) · ผังปกติต้องเป็น null · ไพ่ครบและไม่มีไพ่นอกชุด ·
usage = 0 · และทุกจุดที่เรียกต้องส่งสาเหตุเข้าสถิติ

**ผลตรวจ**: `typecheck` 0 error · `repo:verify` **ผ่านครบ 61/61 ด่าน** · ยิงผ่าน API จริงบน dev server:
หน้าอังกฤษทั้ง 5 บุคลิกไม่มีอักษรไทยหลุดสักตัว · `yes-no` ไทยได้ "ยังไม่แน่" อังกฤษได้ "Yes" ตามไพ่ที่จั่ว ·
`three-card` คืน `yesNoAnswer = null` ตามเดิม
### 🗓️ 2026-09-15 (รอบ 83): ✅ ปิด Bot Fight Mode บน production จริง + กฎ WAF ที่มาแทน (ปิด ISSUE-047)
- **เจ้าของสั่ง "ทำให้เสร็จไปเลย"** ➔ เข้าแดชบอร์ด Cloudflare ผ่าน Chrome ที่เจ้าของล็อกอินไว้ให้ แล้วปิดจริง
- **ผลวัดบน production หลังปิด**: หน้าแรก perf **52 ➔ 81** (TBT 1,730 ➔ 106 ms) · `/cards` **56 ➔ 81** (1,544 ➔ 190 ms) · `/blog` **52 ➔ 84** (2,461 ➔ 56 ms) · CLS 0 ทุกหน้า · `curl` ยืนยันไม่มี `challenge-platform` เหลือในหน้าไหนแล้ว
- **สองเรื่องที่ไม่มีในคู่มือไหนเลย**:
  1. **ปุ่ม toggle ในแดชบอร์ดกดแล้วไม่บันทึกจริง** — กด 2 ครั้ง รีโหลดแล้วค่ายังเป็น `true` ต้องสั่งผ่าน API ของแดชบอร์ดเอง
  2. **ตัวที่ฉีดสคริปต์จริงคือ `enable_js`** ซึ่ง**ไม่โผล่เป็นสวิตช์ในหน้า Settings** ปิดแค่ `fight_mode` อย่างเดียวไม่พอ
- **ของที่เพิ่มมาแทน (กฎ WAF ข้อที่ 4 · ใช้ไป 4/5)**: ท้าทายคำขอที่ **เปลี่ยนสถานะ** เข้า `/api/` ซึ่ง **ไม่มี `Origin` ของเว็บเรา** — เบราว์เซอร์ส่ง `Origin` เสมอกับทุกคำขอที่ไม่ใช่ GET ตามสเปก Fetch ผู้ใช้จริงจึงผ่าน 100% ส่วนสคริปต์ที่ยิงตรงมักไม่ตั้ง ➔ ติดกับดักแม้ปลอม user-agent เนียน · ยกเว้น webhook รับเงินกับ `/api/cron/`
- **ทดสอบกฎใหม่กับของจริงครบ 4 เคส**: POST พร้อม Origin ➔ ถึงแอป (ได้ 400 จาก validation ของเราเอง) · POST ไม่มี Origin ➔ **403** · GET ด้วย UA เบราว์เซอร์ ➔ **200** ทั้ง 3 เส้น · หน้าเว็บปกติ ➔ **200**
- **⚠️ ความผิดพลาดที่เกิดจริงและแก้แล้ว (INC-0179)**: PUT ของ `bot_management` เป็นแบบ**แทนที่ทั้งก้อน** — ส่งไป 2 ฟิลด์ทำให้ `ai_bots_protection` กับ `ai_training` ถูกรีเซ็ตจาก `block` เป็น `disabled` ประมาณ 6 นาที (กฎ WAF บล็อก 15 ตัวตามชื่อ UA ยังทำงานอยู่ จึงไม่ได้เปิดโล่ง) ➔ คืนค่าครบทุกฟิลด์แล้วและเทียบกับสแนปช็อตเดิมทีละตัว · เติมคอมเมนต์เตือน + `ai_training: "block"` ลงสคริปต์กันพลาดซ้ำ
- **การพิสูจน์**: `npm run repo:verify` ➔ ✅ ผ่านครบ 62 ด่าน

### 🗓️ 2026-09-15 (รอบ 82): ⛔ ปิด Bot Fight Mode — คืนคะแนนความเร็ว 25–32 จุดทุกหน้า (INC-0177)
- **ที่มา**: เจ้าของถามว่า "ถ้าอยากทำคะแนนให้ดี ต้องปิด bot fight แล้วตั้งกฎแทนไหม" ➔ วัดให้ดูแล้วสั่ง **"ปิดไปเลยก็ได้"**
- **ตัวเลขที่วัดบน production** (Lighthouse · mobile · simulate · ยิงหน้าเดียวกันสองรอบ ต่างกันแค่บล็อกสคริปต์นั้นทิ้ง):

  | หน้า | perf (เปิด ➔ ปิด) | TBT (เปิด ➔ ปิด) | สคริปต์นั้นกินเธรดหลัก |
  | :--- | :-- | :-- | :-- |
  | หน้าแรก | **52 ➔ 81** | 1,730 ➔ 130 ms | 4,002 ms |
  | `/cards` | **56 ➔ 81** | 1,544 ➔ 140 ms | 3,271 ms |
  | `/blog` | **52 ➔ 84** | 2,461 ➔ 85 ms | 5,394 ms |

- **ทำไมปิดแล้วไม่เสี่ยงอย่างที่กลัว**: ของที่กันค่าใช้จ่ายจริง **ไม่มีข้อไหนอยู่ใน Bot Fight Mode เลย** — rate limit บนเส้นเปิดไพ่ · บล็อก UA เครื่องมือสคริปต์บน `/api/*` · บล็อก URL ขยะ · บล็อกบอตเทรนโมเดล/สแกนเนอร์ SEO · `ai_bots_protection: block` ทั้งหมดอยู่คนละที่และยังทำงานครบ · และหน้า HTML ทุกหน้าเป็นไฟล์สแตติกที่ตอบจากขอบแล้ว (`cf-cache: HIT` ไม่ปลุก Worker) บอตที่มาดูดหน้าเว็บจึงแทบไม่มีค่าใช้จ่าย — **เงื่อนไขเปลี่ยนไปตั้งแต่ย้ายไป Astro แต่ไม่มีใครกลับมาทบทวนสวิตช์นี้**
- **กับดักที่เกือบพลาด**: สวิตช์นี้ถูกตั้งจาก `scripts/cloudflare-phase1.ts` เอง (`fight_mode: true`) — **ปิดในแดชบอร์ดอย่างเดียวไม่พอ** ใครรัน `npm run cf:phase1` อีกครั้งมันจะเปิดกลับเงียบ ๆ จึงแก้ที่โค้ดเป็นหลัก
- **กันเปิดกลับ**: เพิ่ม **กฎ 6** ในด่านที่ 37 (`test-bot-policy.ts`) บังคับ `fight_mode: false` เสมอ (ทดสอบแล้วว่าจับได้จริงด้วยการใส่ `true` กลับเข้าไป) · จะเปิดกลับต้องแนบตัวเลขสองด้าน
- **เหลือขั้นตอนเดียวที่เซสชันนี้ทำแทนไม่ได้**: `export CLOUDFLARE_API_TOKEN=<token> && npm run cf:phase1` (ในเครื่องไม่มีโทเคน) ➔ แล้วยืนยันว่า HTML ไม่มี `challenge-platform` อีกและยิง Lighthouse ซ้ำ
- **การพิสูจน์**: `npm run repo:verify` ➔ ✅ ผ่านครบ 62 ด่าน

### 🗓️ 2026-09-15 (รอบ 81): 📐 เอาความเร็ว `/cards` กลับมาโดยไม่เอาการกระตุกกลับมาด้วย (ปิด ISSUE-048)
- **ที่มา**: รอบ 80 ถอด `content-visibility` ออกทั้งเว็บเพื่อแก้อาการจอกระพริบ แต่ `/cards` ต้องจ่ายราคา — วัดบน **production จริงหลัง deploy** ได้ **TBT 3,995–4,446 ms · perf 50** (ก่อนถอดคือ 232 ms · perf 79)
- **สิ่งที่ทำ**: เอา `content-visibility` กลับมาเฉพาะกริดไพ่ โดยแก้ต้นเหตุที่ทำให้มันกระตุกทั้งสองข้อ
  1. **ล็อกแถวคีย์เวิร์ดไว้ 2 บรรทัดเสมอ** (`h-[3.8125rem]`) ➔ การ์ดทุกใบสูงเท่ากัน (เดิมต่างกัน 31px เพราะบางใบคีย์เวิร์ดตกบรรทัดที่สอง)
  2. **เลิกเดาความสูง** — เปลี่ยน `contain-intrinsic-size` เป็น `calc(vw)` ต่อ breakpoint จากการวัดจริง 18 ความกว้างจอ (การ์ด = ภาพ 7:12 + กล่องข้อความคงที่ ➔ สูง = กว้าง × 12/7 + ค่าคงที่ · กว้าง = f(ความกว้างจอ))
- **ผลที่วัดได้**: `/cards` TBT **~4,200 ➔ 14–42 ms** · perf 50 ➔ **78–79** · **CLS 0** · กระตุกเหลือ **4 ครั้ง ครั้งละ 1px** (รอบ 79 คือ 14 ครั้ง ครั้งละ 45px) · ความสูงที่จองไว้ตรงของจริง **0–1px ทั้ง 9 ความกว้างจอ**
- **⚠️ กับดักที่เสียเวลาไปหนึ่งรอบ**: `contain-intrinsic-size` กำหนดความสูงของ **content box** ไม่ใช่ทั้งกล่อง ➔ ต้องหัก `p-3` + เส้นขอบ = **26px** จับได้เพราะ**คลาดเกินเท่ากันเป๊ะ 26px ทุกความกว้างจอ** (ถ้าสูตรผิดจริง ความคลาดจะไม่เท่ากันทุกจอ)
- **ตรวจหน้าตาแล้ว**: เทียบภาพก่อน/หลังที่ 375px — การ์ดส่วนใหญ่เหมือนเดิมทุกพิกเซล (คีย์เวิร์ดเป็น 2 บรรทัดอยู่แล้ว 68 จาก 78 ใบ) ที่เหลือ 10 ใบได้ที่ว่างเพิ่มบรรทัดเดียวและทำให้กริดเรียงตรงกันขึ้น
- **กันสูตรรอตาย**: ด่านใหม่ **`test-card-tile-height.ts` (ด่านที่ 62)** เฝ้าของที่สูตรพึ่งพา (กริด · gap · padding · สัดส่วนภาพ · ความสูงแถวคีย์เวิร์ด) — แก้อะไรในนั้นแล้วลืมวัดใหม่ ด่านตกทันที · **ด่านนี้ไม่ได้เปิดเบราว์เซอร์วัดเอง** (CI ไม่มีเครื่องมือขับเบราว์เซอร์) จึงเขียนกำกับไว้ตรง ๆ ว่ามันเป็นสัญญาณเตือน ไม่ใช่เครื่องวัด
- **หนี้ที่รับไว้อย่างรู้ตัว**: หน้าไพ่ตามชุด (`/cards/major` ฯลฯ) ใช้ `gap` แคบกว่านิดหน่อย สูตรเดียวกันจึงคลาด **0–4px** — เล็กจนมองไม่เห็นและ CLS ยังเป็น 0
- **คู่มือวัดใหม่**: [`docs/plans/HANDOFF_CARD_TILE_CV_2026-09-15.md`](plans/HANDOFF_CARD_TILE_CV_2026-09-15.md)
- **วัดซ้ำบน production หลัง deploy (PR #493)**: `/cards` TBT **100–117 ms** · perf **82–83** · CLS **0** · กระตุก 4 ครั้งครั้งละ 1px · หน้าแรกและ `/blog` กระตุก **0 ครั้ง** · ➔ เร็วกว่าตอนก่อนเจอบั๊กทั้งที่ตอนนี้ไม่กระตุกแล้ว (เดิม TBT 232 ms · perf 79 แต่ CLS 0.766)
- **การพิสูจน์**: `npm run repo:verify` ➔ ✅ ผ่านครบ 62 ด่าน

### 🗓️ 2026-09-15 (รอบ 80): 🩹 แก้อาการ "จอกระพริบรัว ๆ ตอนเลื่อนขึ้นบนมือถือ" ทั้งเว็บ (INC-0174)
- **ที่มา**: เจ้าของแจ้งเอง — เปิดหน้าแรกบนมือถือ เลื่อนลงแล้วเลื่อนกลับขึ้น ภาพทั้งจอกระตุกรัว และเป็นเหมือนกันในหน้าอื่น
- **สาเหตุราก**: `content-visibility: auto` + `contain-intrinsic-size` ที่กระจายอยู่ **10 จุด** — `contain-intrinsic-size` เดาความสูงได้ค่าเดียว แต่ความสูงจริงของบล็อกในเว็บนี้เปลี่ยนตามความกว้างจอแบบต่อเนื่อง (การ์ดไพ่ใบเดียวกันวัดได้ **340 · 387 · 419 · 498 · 380 · 351 · 409 px** ที่จอ 320 ➔ 1280px) ➔ บล็อกสลับไปมาระหว่างความสูงจริงกับค่าที่เดา ➔ ความสูงเอกสารเปลี่ยน ➔ เบราว์เซอร์แก้ตำแหน่งสกรอลล์ตาม = จอกระตุก
- **หลักฐาน (วัดบนบิลด์เดียวกันสองชุด เสิร์ฟคนละพอร์ต · Chrome จริง 375×812 · เลื่อนขึ้นทีละ 200px)**:

  | หน้า | ก่อนแก้ | หลังแก้ |
  | :--- | :--- | :--- |
  | หน้าแรก | ความสูง 10,014 ➔ 11,176px · จอถูกลากลง 963px · **CLS 1.000** | ความสูงค่าเดียว · **กระตุก 0 · CLS 0** |
  | `/blog` | กระตุก **17 ครั้ง** (ทีละ 10–40px) | **0 ครั้ง** |
  | `/cards` | กระตุก **14 ครั้ง** · CLS 0.766 | **0 ครั้ง** · CLS 0 |
  | `/cards/all` · `/spreads` | 0 (ไม่มีอาการอยู่แล้ว) | 0 |

- **ผลพลอยได้ที่หน้าแรก**: perf **68 ➔ 84** · LCP **5.4 ➔ 3.5 s** (เซกชัน SEO ไม่ต้องถูกเลื่อนไปวาดทีหลังอีก)
- **ราคาที่จ่ายจริงและตั้งใจจ่าย — เฉพาะ `/cards`**: TBT **78–94 ➔ 1,522–1,546 ms** · style&layout **~850 ➔ ~1,980 ms** · perf 71–77 ➔ 52 · **เลือกความนิ่งเพราะ CLS เป็น Core Web Vital จริง ส่วน TBT เป็นค่าห้องแล็บ** และผู้ใช้เห็นอาการกระตุกด้วยตาเปล่า ➔ เปิด **ISSUE-048** ไว้เอาความเร็วคืนด้วย `aspect-ratio` (ต้องให้เจ้าของเคาะเรื่องหนีบชื่อไพ่ 2 บรรทัด)
- **ตัดออกจากผู้ต้องสงสัยแล้ว**: `transform-gpu` บนการ์ด — ถอดออกแล้ววัดซ้ำ TBT ยัง 1,378–3,861 ms เท่าเดิม ไม่ใช่ตัวการ
- **กันไม่ให้กลับมา**: เพิ่ม **กฎ 10** ใน `scripts/qa/test-motion-quality.ts` บล็อก `content-visibility` ทุกไฟล์ทั้ง `src/` และ `astro/` (ทดสอบแล้วว่าจับได้จริงด้วยการใส่กลับเข้าไปหนึ่งบรรทัด) · จะขอยกเว้นต้องใส่ ALLOWLIST พร้อมตัวเลขสองด้าน
- **การพิสูจน์**: `npm run repo:verify` ➔ ✅ ผ่านครบ 61 ด่าน · `npm run typecheck` ➔ 0 errors

### 🗓️ 2026-09-15 (รอบ 79): ⚡ วัดผลปิด ISSUE-043 — `/blog` จากหน้าที่แย่ที่สุด กลายเป็นหน้าที่เบาที่สุด
- **งานของรอบนี้คือ "วัด" ไม่ใช่ "แก้"** — ISSUE-043 ค้างมาตั้งแต่ 2026-09-11 เพราะแก้ไปแล้ว (PR #453 · #484) แต่ไม่มีใครยิง Lighthouse ยืนยัน
- **ผลที่วัดได้** (Lighthouse 12.8.2 · production จริง · mobile · simulate): `/blog` TBT **24 ms** · perf **85** ➔ **เบาที่สุดในสี่หน้าที่วัด** (`/` 120 · `/cards` 232 · `/spreads` 335 ms) · ยิงซ้ำ 3 รอบได้ 14 · 24 · 40 ms คงที่ · เดิม 660 ms แย่กว่าหน้าถัดไป 4.1 เท่า ➔ **ปิด ISSUE-043**
- **ของแถมที่ใหญ่กว่างานหลัก — ISSUE-047**: สคริปต์ตรวจบอตของ Cloudflare (`/cdn-cgi/challenge-platform/scripts/jsd/main.js`) ที่ถูกฉีดเข้า HTML **ทุกหน้า** กิน scripting **3,707–4,953 ms** และสร้าง long task ก้อนละ **1,190–1,282 ms** ➔ บล็อกมันทิ้งแล้ว TBT ตกทันที `/blog` 2,206 ➔ 24 · `/spreads` 1,952 ➔ 335 · `/cards` 1,256 ➔ 232 ms · **มันกินเวลามากกว่าโค้ดเราทั้งหน้ารวมกัน** · ต้นตอเป็นสวิตช์ใน Cloudflare Dashboard ไม่ใช่โค้ดในรีโป ➔ **รอเจ้าของตัดสินใจ**
- **บทเรียนที่บันทึกถาวร (INC-0172)**: ห้ามสรุปว่าหน้าไหน "โค้ดหนัก" จาก TBT ดิบ ต้องอ่าน `bootup-time`/`long-tasks` แยกรายโดเมนก่อน ถ้าตัวกินเวลาสูงสุดไม่ใช่ไฟล์ของเรา ให้ยิงซ้ำด้วย `--blocked-url-patterns` แล้วค่อยตัดสินโค้ด
- **กับดักที่เหยียบซ้ำ**: **โควตา PSI ของ Google หมดตั้งแต่ก่อนเริ่มรอบ** (HTTP 429 ทั้ง 4 ครั้งที่ยิง) ➔ เปลี่ยนไปใช้ Lighthouse CLI ตัวเดียวกันบนเครื่องแทน · ตัวเลขดิบจึงเทียบกับฐาน 660 ms ตรง ๆ ไม่ได้ (คนละ CPU) **แต่การเทียบอันดับกับหน้าอื่นในรอบเดียวกันตัดข้อจำกัดนี้ทิ้ง**
- **ยืนยันเพิ่มว่า `/blog` ถูกเสิร์ฟจากขอบจริง**: `curl` ได้ **22.6 KB** (zstd) · `cf-cache-status: HIT` · ไม่ปลุก Worker
- **ไม่มีการแก้โค้ดในรอบนี้แม้แต่บรรทัดเดียว** — แตะเฉพาะเอกสาร (`KNOWN_ISSUES.md` · `INCIDENT_LOG.md` · `WORK_LOG.md` · `SEO_INDEXING_LOG.md` · แผน `HANDOFF_BLOG_TBT_2026-09-12.md`)
...
- **โครงที่ใช้**: พิธีเปิดไพ่เป็น island (`client:load` เพราะผู้ใช้เข้ามาเพื่อกดเปิดไพ่ทันที) ส่วน **เนื้อหา SEO ยาว ๆ ท้ายหน้าเป็น HTML ล้วนที่ไม่ต้อง hydrate เลย** — นี่คือที่มาของ JS ที่หายไป 73 KB
- **ด่านที่ต้องแก้ตามไปด้วย**: `test-en-routing` เคยไล่อ่านไฟล์ `layout.tsx` ของหมวดในต้นไม้ `(en)` เพื่อดูว่ามี `<SiteHeader/>` + `<SiteFooter/>` ไหม · พอหน้าเนื้อหาย้ายไป Astro หมด **หมวดที่เหลือให้ตรวจกลายเป็นศูนย์** (ด่านย่อย "ไม่ใช่ 0 หมวด" จับได้ — ซึ่งเป็นเหตุผลที่มันถูกใส่ไว้ตั้งแต่แรก) ➔ เปลี่ยนไปเทียบจาก **HTML ที่เรนเดอร์จริง**: หน้าไทยกับอังกฤษของคู่เดียวกันต้องมีหัวเว็บ/ฟุตเตอร์เท่ากัน จับได้ทั้งสองทางและไม่ผูกกับเครื่องมือเรนเดอร์
- **การพิสูจน์**: `repo:verify` ➔ ✅ ผ่านครบ 60 ด่าน · diff `<head>` เทียบบิลด์ก่อนย้ายเหลือต่างกันเฉพาะพาธฟอนต์ · ยิงจริงผ่านเบราว์เซอร์: island ทั้งสามก้อน hydrate ครบ (`ClientChromeRoot` · `SiteHeaderRoot` · `DailyRitualRoot`) และหน้าแสดงขั้นตอนแรกของพิธีถูกต้อง

### 🗓️ 2026-09-15 (รอบ 76): 💬 คลื่นที่ 4a — ย้ายห้องแชท `/reading/chat` ไปเป็น React island บน Astro
- **ที่มา**: เจ้าของเลือก "ย้ายเฉพาะหน้าสาธารณะ ทีละหน้า · deploy แล้วยิงทดสอบบน production ทุกรอบก่อนไปหน้าถัดไป" — หน้าแอดมิน/บัญชี/ตัวทดสอบ/คอนโซลแม่หมอ **คงไว้กับ Next ตามที่เจ้าของสั่ง**
- **ขอบเขตรอบนี้**: `/reading/chat` + `/en/reading/chat` (ทั้งหน้าเป็น island เพราะทุกอย่างเป็นสถานะฝั่งไคลเอนต์) · ใช้ `client:load` ไม่ใช่ `client:idle` เพราะผู้ใช้กดเข้ามาเพื่อพิมพ์คุยทันที
- **ของใหม่ที่ต้องมีก่อน**:
  1. **`next/dynamic` shim** — TarotFlow และหน้าแชทพึ่ง `dynamic(..., { ssr: false })` หนักมาก · ของเดิมใช้ `next/dynamic` ตัวจริงซึ่งลากรันไทม์ Next เข้ามาในบันเดิลของ island ทั้งที่ไม่มี Next อยู่ใกล้เลย
  2. **`useRouter().push/replace` ต้องแยกสองกรณีให้ขาด** — เปลี่ยนหน้าจริง ➔ โหลดหน้าใหม่ · เปลี่ยนแค่ query ของหน้าเดิม (เช่นแท็บแผงแอดมิน `router.replace("/admin?tab=…")`) ➔ **ต้องใช้ History API เฉย ๆ ห้ามโหลดหน้าใหม่** ไม่งั้นทุกครั้งที่สลับแท็บหน้าจะรีโหลดและสถานะหายหมด
- **บั๊กที่จับได้ก่อนขึ้น production (INC ใหม่)**: หน้าที่ **ไม่ได้ประกาศ `openGraph` เอง** จะไม่มีภาพแชร์เลยหลังย้าย เพราะ Next ให้สืบทอดจาก root layout ทั้งก้อน (รวม `description` · `canonical` · `twitter`) · ห้องแชทประกาศแค่ `title` กับ `robots` จึงพึ่งการสืบทอด 100% — **หน้าที่ประกาศน้อยที่สุดคือหน้าที่เสี่ยงที่สุด**
- **ของแถมที่เจอระหว่างทาง**: หน้าที่ไม่มีหัวเว็บมาตรฐาน (ห้องแชทมีแถบของตัวเอง) เคยถูกพรีโหลดโลโก้ทิ้งไว้เปล่า ๆ — ตอนนี้ `BaseLayout` รับ `hasSiteHeader` แล้วตัดคำขอนั้นออก
- **การพิสูจน์**: `repo:verify` ➔ ✅ ผ่านครบ 60 ด่าน · diff `<head>` เทียบบิลด์ก่อนย้าย **เหลือต่างกันเฉพาะพาธฟอนต์ใหม่กับ `next-size-adjust`** · ยิงจริงผ่านเบราว์เซอร์: island hydrate ครบและแสดงสถานะ "ยังไม่มีรอบดูดวงค้างไว้" ถูกต้องเมื่อไม่มีเซสชัน
- **🔍 ของที่เจอเพิ่มตอน CI ตก (สำคัญกว่างานย้ายเอง)**: ด่านที่ 41 (ความยาว title/description) **ไม่เคยทำงานบนเครื่อง dev เลยสักครั้ง** เพราะท้ายไฟล์เทียบ "ถูกเรียกเป็นสคริปต์หลักหรือเปล่า" ด้วยการต่อสตริง `file://` เอง — `import.meta.url` เข้ารหัสอักขระไทยเป็น percent-encoding แต่ `process.argv[1]` เป็น path ดิบ · โฟลเดอร์โปรเจกต์ชื่อ `เว็บไพ่` สองค่าจึงไม่มีวันเท่ากัน **ด่านเงียบสนิทและคืน exit 0 ทุกครั้ง** แต่ทำงานปกติบน CI ที่ path เป็น ASCII ล้วน
  - แก้ให้ใช้ `pathToFileURL()` แล้ว · และแก้ให้ด่านอ่านผลจาก **ทุกเครื่องมือเรนเดอร์** (ของเดิมอ่านแต่รากของ Next จึงเลิกตรวจหน้าเนื้อหา 297 หน้าไปเงียบ ๆ ตั้งแต่ย้าย Astro) · ตอนนี้ตรวจครบ **311 หน้า**
  - เพิ่มด่านกันซ้ำใน `test-rendered-coverage.ts`: ห้ามไฟล์ด่านไหนต่อสตริง `file://` เอง (พิสูจน์แล้วว่าจับได้จริงด้วยการใส่บั๊กกลับเข้าไป)
- **⚠️ ความผิดพลาดของรอบนี้ที่ต้องจดไว้**: ตอนย้าย `description` ระดับรากมาไว้ที่ `root-metadata.ts` **เผลอเขียนข้อความภาษาอังกฤษขึ้นใหม่เองแทนที่จะคัดลอกของเดิม** ทำให้ยาว 193 ตัวอักษร เกินเพดาน SERP · CI จับได้ · คืนข้อความเดิมแล้วและเขียนกฎกำกับไว้ในไฟล์ว่าต้องคัดลอกมาเป๊ะทุกตัวอักษร ห้ามเขียนใหม่

### 🗓️ 2026-09-15 (รอบ 75): 🪶 คลื่นที่ 3 — ย้าย `/about` · `/privacy` · `/contact` ไป Astro (Next เหลือ 14 หน้าตามเป้า)
- **ขอบเขต**: 5 หน้า (`/about` · `/privacy` · `/en/privacy` · `/contact` · `/en/contact`)
- **สถานะรวม**: **Astro 297 หน้า · Next เหลือ 14 หน้า** = หน้าแรก + หน้าแอปพอดีตามที่เจ้าของสั่งให้คงเป็น React island
- **วิธีที่ใช้ (ถูกที่สุดและเสี่ยงน้อยที่สุด)**: ย้ายไฟล์ `page.tsx` **ทั้งดุ้น** ไปไว้ที่ `src/app/_shared/pages/` แล้วให้หน้า `.astro` นำเข้าไปเรนเดอร์ — **ไม่ต้องเขียนเนื้อหาใหม่เลยสักบรรทัด** และไม่มีทางที่ข้อความจะเพี้ยนจากของเดิม
- **ปุ่มลบข้อมูลตาม PDPA** เป็น island ตัวเดียวของหน้านโยบาย ส่งเข้าไปทาง slot เหมือนกลุ่มหน้าอื่น
- **เก็บไว้กับ Worker โดยตั้งใจ**: `/readers` · `/readers/[id]` · `/readers/queue/[id]` · `/s/[id]` — ทั้งสี่เป็นหน้าไดนามิกจริง (อ่าน D1 ตอนมีคำขอ) ย้ายเป็นไฟล์สแตติกไม่ได้
- **การพิสูจน์**: `repo:verify` ➔ ✅ ผ่านครบ 60 ด่าน · diff `<head>` ทั้งสามหน้าเทียบบิลด์ก่อนย้าย เหลือต่างกันเฉพาะพาธฟอนต์ใหม่กับ `next-size-adjust` ที่ถอดไปแล้ว · ยิงจริงผ่านเบราว์เซอร์ครบทุกหน้า
### 🗓️ 2026-09-15 (รอบ 74): 🖼️ หยุดไฟล์ที่โกหก — คุณภาพ AVIF q52 กับคอมเมนต์ที่ยังบอกว่า q65

**อาการ**: `src/lib/tarot/card-image.ts` ตั้ง `IMAGEKIT_AVIF_QUALITY = 52` แต่คอมเมนต์เหนือมัน
ยังเป็นตารางเทียบ PSNR ที่สรุปว่า "เลือก q65" พร้อมประโยค "⚠️ ห้ามลด q ลงกว่านี้โดยไม่วัดใหม่"
คนอ่านไฟล์นี้จะเข้าใจผิดสองชั้น: (1) คิดว่าค่าที่ใช้คือ 65 (2) คิดว่ามีผลวัดรองรับค่าที่ใช้อยู่

**ต้นตอ**: PR #476 (2026-09-14) เปลี่ยนตัวเลขบรรทัดเดียว ไม่แตะคอมเมนต์ และ commit ไม่มีผลวัดแนบมา
คำเตือนในคอมเมนต์เป็นแค่ตัวหนังสือ ไม่มีเครื่องตรวจ — ตรงกับบทเรียนประจำบ้านนี้ (หลักการข้อ 0.8)

**การตัดสินใจ**: เจ้าของโปรเจกต์เคาะ 2026-09-15 ว่า **คง q52 ไว้** (ตัดสินจากภาพบนเว็บจริง ไม่ใช่ผลวัด)
จึงไม่ยิง PSNR ใหม่ แต่**บันทึกไว้ตามจริงว่า q52 ยังไม่มีตัวเลขวัดรองรับ** ไม่แต่งเหตุผลย้อนหลังให้มัน

**สิ่งที่แก้**
1. เขียนคอมเมนต์เหนือ `IMAGEKIT_AVIF_QUALITY` ใหม่ทั้งก้อน — พาดหัวประกาศค่าปัจจุบัน 52 ตรง ๆ
   ตารางเทียบเดิมคงไว้เป็นประวัติ (q60/q65 มีตัวเลขจริง) และเติมแถว q52 ว่า "ยังไม่ได้วัด"
2. เพิ่ม **กฎ E** ใน `scripts/qa/test-image-paths.ts` (ด่านเดิม ไม่ได้เพิ่มด่านใหม่ — ยังคง 57 ด่าน):
   ค่าในโค้ดต้องตรงกับเลขในบรรทัดพาดหัวของคอมเมนต์ตัวเอง ไม่งั้น CI ตก
   ตรวจ "บรรทัดพาดหัว" ไม่ใช่หาเลขที่ไหนก็ได้ในก้อน เพราะเลข 52 ไปโผล่ในตาราง/วันที่ได้ง่าย

**พิสูจน์ว่าด่านตกได้จริง** (กันด่านหลอกแบบ `test-judge-baseline` เดิม)
- จำลอง PR #476 ซ้ำ (แก้ 52 ➔ 40 ไม่แตะคอมเมนต์) ➔ ❌ ตกพร้อมข้อความ "โค้ดตั้งไว้ 40 แต่คอมเมนต์ยังประกาศว่า 52"
- ลบบรรทัดพาดหัวทิ้ง ➔ ❌ ตกพร้อมบอกรูปแบบข้อความที่ต้องมี
- คืนค่าเดิม ➔ ✅ เขียว

**หมายเหตุ**: กฎ E ออกแบบเป็นตาราง `TUNED_CONSTANTS` เติมค่าที่จูนด้วยการวัดตัวอื่นเข้าไปได้เลย
(ค่าที่ "เหตุผลอยู่ในคอมเมนต์ ไม่ได้อยู่ในโค้ด" ทุกตัวมีความเสี่ยงเดียวกันนี้)

---
### 🗓️ 2026-09-15 (รอบ 74): 🪶 คลื่นที่ 2 — ย้าย `/blog/**` และ `/spreads/**` ไป Astro (เหลือ Next แค่ 19 หน้าตามแผน)
- **ขอบเขต**: 118 หน้า — `/blog` · `/blog/<slug>` · `/spreads` · `/spreads/<ผัง>` · `/spreads/topic/<หมวด>` ทั้งสองภาษา
- **สถานะรวมหลังคลื่นนี้**: **Astro 292 หน้า · Next เหลือ 19 หน้า** ซึ่งคือหน้าแรก + หน้าแอป 18 หน้าพอดีตามที่เจ้าของสั่งให้คงเป็น React island
- **ผลที่วัดได้จริงจากบิลด์ production**:

  | เส้นทาง | JS (gzip) ก่อน | หลัง | HTML (gzip) ก่อน | หลัง |
  | :--- | ---: | ---: | ---: | ---: |
  | `/blog` | 174 KB | **143 KB** | 35 KB | **23 KB** |
  | `/spreads` | 172 KB | **115 KB** | 43 KB | **20 KB** (−53%) |
  | `/spreads/<ผัง>` (54 หน้า) | 173 KB | **115 KB** | 25 KB | 22 KB |

- **สิ่งที่ทำ**:
  1. ย้าย `BlogIndexClient` · `ArticleReadingClient` ออกจาก `src/app/**` มาไว้ที่ `src/components/blog/` (เป็นคอมโพเนนต์ ไม่ใช่เส้นทาง)
  2. แยก "แกนกลางแบบซิงโครนัส" ออกจากตัวห่อ `async` ทุกไฟล์ — `BlogDetailContent` · `SpreadDetailContent` · `SpreadTopicContent` พร้อมตัวสร้าง metadata แบบซิงโครนัสอีก 3 ตัว · **ตรรกะ SEO ยังมีที่เดียวเหมือนเดิม** ไม่ได้ก็อปไปไว้ในไฟล์ `.astro`
  3. ส่วนที่ต้องโต้ตอบส่งเข้าไปทาง slot ทั้งหมด (`list` · `reader` · `library` · `detail`) — ที่เหลือของหน้าเป็น HTML ล้วนที่ไม่ต้อง hydrate
  4. **เลิกสร้างหน้าให้ slug เก่าของบทความ** — ปล่อยให้ตกไปที่ Worker แล้วโดน `redirects()` ใน `next.config.ts` เด้ง 301 ให้ (ดีกว่าเดิมที่ Next สร้างเปลือกหน้าเปล่าแล้วเด้งฝั่งไคลเอนต์)
- **บั๊กที่จับได้ก่อนขึ้น production (INC-0168)**: แท็ก `<meta name="keywords">` ของ `/spreads` และ `/spreads/topic/*` **หายไปเงียบ ๆ** เพราะ Next ผสาน `keywords` จาก root layout ให้หน้าย่อยเอง แต่ฝั่ง Astro ยังไม่ได้ผสาน · **ไม่มีด่านไหนฟ้องเลย** เพราะทุกด่านตรวจว่า "แท็กที่มีถูกต้องไหม" ไม่ได้ตรวจว่า "แท็กที่เคยมีหายไปหรือเปล่า" — จับได้ด้วยการ diff `<head>` กับสแนปช็อตบิลด์ก่อนย้ายทีละหน้า **ซึ่งกลายเป็นกฎถาวรของการย้ายทุกคลื่นต่อจากนี้**
- **การพิสูจน์**: `repo:verify` ➔ ✅ ผ่านครบ 60 ด่าน · diff `<head>` ทุกกลุ่มหน้าเทียบบิลด์ก่อนย้าย เหลือต่างกันเฉพาะ `next-size-adjust` (ของ `next/font` ที่ถอดไปแล้ว) กับพาธฟอนต์ใหม่ · ยิงจริงผ่านเบราว์เซอร์: ค้นหาบทความกรองจาก 58 ลิงก์เหลือ 6 · หน้าบทความและหน้าคู่มือผังเรนเดอร์ครบ
- **⚠️ งานค้างที่ต้องทำต่อ**: `/blog` คือหน้าที่ TBT แย่ที่สุดของเว็บ (660 ms · ISSUE-043) — **ต้องยิง Lighthouse วัดซ้ำหลัง deploy แล้วบันทึกผล** ห้ามสรุปเองว่าหายจากตัวเลข HTML/JS ที่ลดลง

### 🗓️ 2026-09-15 (รอบ 73): 🪶 ย้ายหน้าไพ่ทั้งหมด 174 หน้าไปเรนเดอร์ด้วย Astro + React Island (คลื่นที่ 1)
- **คำสั่งเจ้าของ**: "ย้ายหน้าหลัก + 18 หน้าแอปมา react island · หน้าอื่นๆทั้งหมด เอา astro มาช่วย · ห้ามผิดพลาดเด็ดขาด"
- **ขอบเขตที่ส่งรอบนี้**: คลื่นที่ 1 = `/cards/**` ทั้งไทยและอังกฤษ **174 หน้า** (ไพ่รายใบ 156 · หน้ารวม · ตารางสรุป · 6 หมวด · เครื่องคำนวณไพ่ประจำตัว) พร้อมโครงสร้างพื้นฐานทั้งหมดที่คลื่นถัดไปใช้ต่อได้ทันที · แผนคลื่น 2–5 (บทความ · ผัง · หน้าแอป · `/api/*`) อยู่ที่ [`docs/plans/HANDOFF_ASTRO_MIGRATION_2026-09-15.md`](plans/HANDOFF_ASTRO_MIGRATION_2026-09-15.md)
- **ผลที่วัดได้จริงจากบิลด์ production**:
  ...
  และ **แท็ก `<script>` ในหน้าไพ่รายใบ ลดจาก 20 ก้อนเหลือ 0 ก้อน** — island โหลดตัวเองตอนเบราว์เซอร์ว่าง (`client:idle`)
  หน้าที่ย้ายแล้วถูก **Cloudflare ตอบจากขอบโดยไม่ปลุก Worker เลย** (ไม่เสียค่าคำขอ · ไม่ต้องบูต runtime ของ Next)
- **กุญแจที่ทำให้ไม่ต้องเขียนคอมโพเนนต์ใหม่เลยสักตัว**: ชั้นแปลงปลั๊กใน `astro.config.mjs` ชี้ `next/link` · `next/navigation` · `next/script` ไปที่ `astro/shims/*` เฉพาะตอนบิลด์ด้วย Astro — คอมโพเนนต์ทั้ง 90 ตัวใน `src/components/**` ใช้ได้ทั้งสองเครื่องมือโดยไม่แก้แม้แต่บรรทัดเดียว
- **งานร่วมที่ต้องทำเพราะสองเครื่องมือใช้ของชุดเดียวกัน**:
  1. **ฟอนต์ออกจาก `next/font`** ➔ `@font-face` ใน `globals.css` ชี้ `/fonts/*.woff2` (ค่าชดเชยเมตริกคัดลอกมาเป๊ะทุกหลักจากที่ `next/font` คำนวณไว้) · ถ้าไม่ทำ ผู้ใช้ที่เดินข้ามสองฝั่งต้องโหลดฟอนต์ชุดเดิมใหม่เพราะ URL คนละตัว · ของแถม: CSS เหลือไฟล์เดียว (เดิมสอง) และหน้าตาไม่ขยับสักพิกเซล (diff HTML ทั้ง 315 หน้าแล้ว)
  2. **metadata ระดับราก** ➔ `src/app/_shared/root-metadata.ts` ที่เดียว
  3. **ส่วนหัวความปลอดภัย** ➔ `src/lib/config/security-headers.ts` ที่เดียว แล้วคัดลอกลง `public/_headers` (ไฟล์ static ไม่ผ่าน Worker จึงไม่ได้รับจาก `next.config.ts` — ถ้าลืม ครึ่งเว็บจะไม่มี CSP เงียบ ๆ)
  4. **`LocaleLink`** คืน `<a>` ธรรมดาให้ปลายทางที่ย้ายแล้ว (ไม่งั้น router ของ Next ยิงขอเพย์โหลด RSC ที่ไม่มีอยู่จริงทุกคลิก)
- **ด่านตรวจ 57 ➔ 60 ด่าน**: `test-astro-routes` (หน้าเดียวห้ามอยู่สองที่ · ฝาแฝดต้องย้ายครบคู่) · `test-render-parity` (`<head>` สองเครื่องมือต้องตรงกันทุกหน้า) · `test-static-headers` (`_headers` ต้องพูดตรงกับโค้ดกลาง) · พร้อมแก้ด่านเดิม 6 ด่านให้มองเห็นผลลัพธ์ของทั้งสองเครื่องมือ
- **บั๊กที่เจอระหว่างทางและแก้ไปแล้ว 3 ข้อ**: INC-0164 (Service Worker เสิร์ฟหน้าไพ่แบบ cache-first — **บั๊กที่มีอยู่ก่อนแล้ว**) · INC-0165 (ด่านงบน้ำหนักมองไม่เห็น JS ของ Astro แล้วรายงาน 0 KB ว่าผ่าน) · INC-0166 (กฎ `/cards/*` ใน `_headers` จะแคชหน้าไพ่ไว้ 1 ปีแบบ immutable — จับได้ก่อนขึ้น production)
- **การพิสูจน์**: `repo:verify` ➔ ✅ **ผ่านครบ 60 ด่าน** · diff `<head>` ของ Next เทียบ Astro เหลือต่างกันแค่ลำดับแอตทริบิวต์ · เนื้อหาใน `<main>` ตรงกัน **ทุกตัวอักษรและทุกลิงก์** · ยิงจริงผ่านเบราว์เซอร์บนผลบิลด์ที่ `.open-next/assets` (ค้นหาไพ่ · สลับหัวตั้ง/หัวกลับ · คำนวณวันเกิด ทำงานครบ)
- **⚠️ ข้อจำกัดที่ต้องรู้**: เครื่องที่ทำรอบนี้เป็น macOS 12.6 ซึ่งรัน `workerd` ไม่ได้ (ต้องการ 13.5+) จึงเปิด `opennextjs-cloudflare preview` ไม่ได้ · **มีเช็กลิสต์ 5 คำสั่งให้ยิงตรวจหลัง deploy** อยู่ท้ายเอกสารแผน
...
### 🗓️ 2026-09-14 (รอบ 71): 🩹 **ถอน `preload: false` ที่ตัวเองใส่ไว้ — มันทำ CLS หน้า `/privacy` พุ่งเป็น 0.51 และฉุดหน้าเนื้อหา 295 หน้า**
- **ที่มา**: เจ้าของถามว่าหน้าที่ไม่ใช่หน้าแรกควรเอา Astro มาช่วยไหม · ก่อนตอบจึงไปวัดหน้าเนื้อหาจริงด้วย Lighthouse (ที่ผ่านมาวัดแต่ `/blog` กับ `/cards` ซึ่งเป็น**หน้าดัชนีที่มีช่องค้นหา** ไม่ใช่หน้าเนื้อหาแท้) แล้วเจอว่า `/privacy` ได้ **68 คะแนน CLS 0.5148** ซึ่งเกินเกณฑ์ Core Web Vitals ห้าเท่า
- **สาเหตุ — เป็นของที่ตัวเองทำหลุดไปเองใน PR #475**: ตั้ง `preload: false` ให้ฟอนต์หัวเรื่องโดยอ้างตารางวัดผลที่ดูน่าเชื่อ แต่ **ตารางนั้นวัดจากหน้าแรกหน้าเดียว** ซึ่งข้อความน้อยและมีภาพเป็นตัวเอก · พอเป็นหน้าที่มีข้อความไทยยาว ๆ การสลับฟอนต์ **เปลี่ยนจุดตัดบรรทัด** ทั้งหน้า ซึ่ง `size-adjust` ของ `adjustFontFallback` ชดเชยไม่ได้ (มันชดเชยความกว้างเฉลี่ยกับความสูงบรรทัด ไม่ได้ชดเชยการตัดคำ)
- **การแก้**: ถอด `preload: false` ออก กลับไปใช้ค่าเริ่มต้นของ `next/font` ที่ preload ทุกไฟล์
- **ผลวัด (Lighthouse จริง · บิลด์ production · A/B บนเครื่องเดียวกัน)**:

  | หน้า | จำนวนหน้าในเว็บ | ปิด preload (ที่หลุดขึ้น production) | เปิด preload (แก้แล้ว) |
  | :--- | ---: | ---: | ---: |
  | `/privacy` | 2 | **68 · CLS 0.5148** | **90 · CLS 0** |
  | `/cards/<ไพ่รายใบ>` | 170 | 73 | **79** |
  | `/blog/<บทความ>` | 56 | 75 | **80** |
  | `/spreads/<ผัง>` | 64 | 81 | **82** |
  | หน้าแรก `/` | 2 | **79** | 77.7 |

  แลกคะแนนหน้าแรก 1.3 กับหน้าเนื้อหา 295 หน้าที่ได้คืน 1–22 คะแนน

## 🛡️ กฎป้องกันถาวรที่ได้จากรอบนี้ — สำคัญกว่าตัวค่า `preload` เอง

**อะไรก็ตามที่อยู่ใน `RootHtml.tsx` · `globals.css` · `fonts.ts` คือของที่มีผลกับทุกหน้าในเว็บ ห้ามตัดสินใจจากการวัดหน้าเดียวเด็ดขาด**

ต้องวัดอย่างน้อย **หนึ่งหน้าเนื้อหายาว ๆ (`/privacy` หรือ `/cards/<ไพ่รายใบ>`) คู่กับหน้าแอป (`/`) เสมอ** เพราะสองแบบนี้ตอบสนองต่อการเปลี่ยนฟอนต์/CSS **คนละทางกันโดยสิ้นเชิง**:
- หน้าแอป: ตัวชี้วัดถูกครอบด้วย JS และภาพ การเลื่อนฟอนต์ไปทีหลังแทบไม่มีผลกับเลย์เอาต์
- หน้าเนื้อหา: ข้อความคือเลย์เอาต์ทั้งหน้า การสลับฟอนต์ = ทั้งหน้าขยับ

ของเดิมผมวัดเฉพาะหน้าแรกแล้วสรุปแทนทั้งเว็บ 315 หน้า — นั่นคือความผิดพลาดตัวจริง ไม่ใช่ค่า `preload`

## 📊 ตัวเลขที่ได้มาระหว่างทาง — ใช้ตอบคำถามเรื่องสถาปัตยกรรม

นับจากบิลด์จริง: **315 หน้า prerender แบ่งเป็นเนื้อหาล้วน 295 หน้า (94%) · ต้องมี JS จริงแค่ 18 หน้า (6%)**
และ `/privacy` ซึ่งเป็นข้อความล้วนก็ยังโหลดสคริปต์ 17 ไฟล์เท่ากับหน้าอื่น เพราะ Next App Router ไม่มีโหมด "หน้านี้ไม่ต้องมี JS"
คะแนนหน้าเนื้อหาหลังแก้รอบนี้อยู่ที่ **79–90** ซึ่งยังไม่ถึง 95+ ที่สถาปัตยกรรมแบบ Astro ให้ได้ — เก็บไว้เป็นข้อมูลประกอบการตัดสินใจของเจ้าของ
...
- **ทำอะไร**:
  1. **เขียน `scripts/subset-fonts.py` + `npm run fonts:subset`** — ดึงต้นฉบับ OFL จาก `google/fonts` แล้วตัดด้วย `pyftsubset` ให้เหลือเฉพาะ **บล็อกไทยทั้งบล็อก (U+0E00–0E7F) + ASCII + เครื่องหมาย 15 ตัว**
  2. **อบ Noto Serif Thai จาก variable เป็นสแตติกสองน้ำหนัก** — ต้นฉบับเป็น variable สองแกน (`wdth` + `wght`) ตาราง `gvar` กินที่ราวครึ่งไฟล์ ทั้งที่เว็บใช้แค่ 400 กับ 700 และไม่เคยแตะแกน `wdth` เลย · **ลองครบสามทางแล้วเทียบไบต์จริงก่อนเลือก**:

     | ทาง | ไบต์รวม |
     | :--- | ---: |
     | เก็บ variable ทั้งสองแกน | 101,016 B ← **แย่กว่าของ Google เสียอีก** |
     | ตรึง `wdth` เหลือแกน `wght` | 57,024 B |
     | **สแตติก 400 + 700** | **33,944 B** ← ที่เลือก |

  3. **ย้ายจาก `next/font/google` ไป `next/font/local`** พร้อม commit ไฟล์ `.woff2` และ `OFL-*.txt` ลงรีโป
- **ผลด้านไบต์ (วัดจาก Lighthouse network log จริง)**:

  | | ก่อน | หลัง |
  | :--- | ---: | ---: |
  | ฟอนต์ | 6 ไฟล์ · **117 KB** | 4 ไฟล์ · **71 KB** (−39%) |
  | payload ทั้งหน้า | 595 KB | **551 KB** |
  | คำขอ | 89 | **85** |

- **📜 เรื่องสัญญาอนุญาต (ตรวจแล้วก่อนลงมือ)**: ทั้ง Sarabun และ Noto Serif Thai อยู่ใต้ SIL Open Font License 1.1 และ **ไม่ได้ประกาศ Reserved Font Name ทั้งคู่** (อ่าน `OFL.txt` ต้นทางจริง) การตัดและแจกจ่ายต่อในชื่อเดิมจึงทำได้ · สคริปต์เก็บ name ID 0 (copyright) · 13 (license) · 14 (license URL) ไว้เสมอ และดาวน์โหลด `OFL.txt` มาวางคู่กับไฟล์ฟอนต์

## 🪤 กับดักสองข้อที่เหยียบมาแล้ว — จดไว้ให้คนถัดไป

**1. `adjustFontFallback: "Times New Roman"` ทำ CLS พังจาก 0.004 เป็น 0.128**
ค่านี้ไม่ได้เป็นแค่ชื่อสำหรับคำนวณเมตริก แต่ Next เอาไปใส่ใน `src: local("...")` ของ `@font-face` ตัวสำรองด้วย · เครื่องที่ไม่มีฟอนต์ชื่อนั้น (**Android ไม่มี Times New Roman**) ตัวสำรองจะโหลดไม่สำเร็จ **ค่าชดเชยเมตริกทั้งชุดจึงไม่ถูกใช้เลย** ข้อความไปตกที่ฟอนต์ระบบแบบดิบ ๆ แล้วขยับตอนฟอนต์จริงมาถึง
- จับได้จาก `document.fonts` รายงานสถานะ **`error`** ของตัวสำรอง (ของ Sarabun ที่ใช้ `local("Arial")` รายงาน `unloaded` = หาเจอแต่ยังไม่ต้องใช้)
- ตัวที่ขยับคือหัวเรื่องกลางหน้าแรก ดันแผงเลือกหัวข้อลงไป 29px
- **กฎ: ใช้ `"Arial"` เท่านั้น** เพราะระบบปฏิบัติการเกือบทุกตัวมีหรือ alias ไว้ให้ (Android ➔ Roboto · Linux ➔ Liberation Sans) · ที่ฟอนต์จริงเป็น serif แต่ตัวสำรองเป็น sans ไม่มีผลกับเลย์เอาต์ เพราะสิ่งที่ชดเชยคือขนาดและความสูงบรรทัด ไม่ใช่รูปทรงตัวอักษร
- **และห้ามใส่ `fallback: ["Noto Serif Thai", ...]`** เป็นชื่อฟอนต์ที่ผู้ใช้ไม่มีติดเครื่อง — มันถูกวางต่อท้ายตัวสำรองที่ชดเชยเมตริกแล้ว กลายเป็นทางหนีที่ไม่มีการชดเชย

**2. บล็อกอักขระไทยต้องเก็บทั้งบล็อก ห้ามตัดเหลือเฉพาะตัวที่เจอในโค้ด**
คำทำนายและบทสนทนาถูกสร้างสดโดยโมเดลภาษา ตัวอักษรไทยตัวไหนก็โผล่ได้ · การกวาดดูว่า "ในรีโปใช้ตัวไหนบ้าง" ใช้ตัดสินได้เฉพาะฝั่งละติน/เครื่องหมายเท่านั้น · และต้องส่ง `--layout-features=*` เสมอ ไม่งั้นสระกับวรรณยุกต์จะลอยผิดตำแหน่ง (ตรวจด้วยการเรนเดอร์จริงแล้ว: `ปู่ ปู๋ ปู้ ปู๊ · เกื้อ · ญี่ปุ่น · ฤๅ ฦๅ · ๆ ฯ ๏ · ๐๑๒๓๔๕๖๗๘๙` ถูกต้องครบ)

## 📊 ผลคะแนน — ตรงไปตรงมา: **ดีขึ้นจริงแต่คะแนนรวมนิ่ง**

วัดด้วย Lighthouse ตัวจริง **ฝั่งละ 5 รอบ บิลด์ใหม่ทั้งสองฝั่ง รันติดกันบนเครื่องเดียวกัน** (เพื่อตัดปัจจัยเครื่องเพี้ยนออก):

| | ก่อน (`3985553`) | หลัง |
| :--- | ---: | ---: |
| คะแนนรวม | 79·81·80·80·77 (กลาง **80**) | 79·77·79·81·79 (กลาง **79**) |
| FCP | 1747–1761 ms (90–91) | **1600–1616 ms (94)** ✅ |
| LCP | 4740–5295 ms (กลาง 24) | **4298–4776 ms (กลาง 31)** ✅ |
| TBT | 73–205 ms (กลาง 99) | 188–252 ms (กลาง 90) ❌ |
| CLS | 0.004–0.007 (100) | 0.004–0.007 (100) |

**ทำไม FCP กับ LCP ดีขึ้นแต่คะแนนรวมไม่ขึ้น**
Lighthouse นับ TBT เฉพาะงานที่เกิด**ระหว่าง FCP ถึง TTI** · พอ FCP ขยับมาเร็วขึ้น 150 ms หน้าต่างที่ใช้นับ TBT ก็กว้างขึ้น 150 ms งานเมนเธรดชุดเดิมที่เมื่อก่อนตกอยู่ "ก่อน FCP" (ไม่ถูกนับ) จึงถูกนับเข้ามา — **ไม่ใช่ว่าเมนเธรดทำงานหนักขึ้น แต่เส้นเริ่มนับเลื่อนมาข้างหน้า** ตัวเลขที่หายไปจาก TBT จึงไปหักล้างกับที่ได้จาก LCP พอดี

**แต่สิ่งที่ผู้ใช้จริงได้รับคือดีขึ้นล้วน ๆ**: ดาวน์โหลดน้อยลง 44 KB ต่อการเปิดหนึ่งครั้ง · คำขอน้อยลง 4 · และ **CrUX ซึ่งเป็นข้อมูลที่ Google ใช้จัดอันดับจริง เก็บจากผู้ใช้จริงไม่ใช่จากการจำลองของ Lighthouse** — ฟิลด์ดาต้าจะเห็นผลข้อนี้เต็ม ๆ แม้คะแนนในห้องแล็บจะนิ่ง

## ➡️ เหลือทางเดียวที่จะขยับคะแนนได้จริง
ทั้ง LCP (คะแนน 31) และ TBT (คะแนน 90) ชี้ไปที่เดียวกันคือ **JavaScript 263 KB ของหน้าแรก** ซึ่งตอนนี้เป็นก้อนใหญ่ที่สุดของ payload (48% ของ 551 KB) · นั่นคือ **ทาง C (รื้อหน้าแรกเป็น island)** ในเมนูรอบ 69 — ทาง B (เหลือฟอนต์ตระกูลเดียว) จะได้อีกราว −20 KB เท่านั้นหลังตัดฟอนต์รอบนี้แล้ว จึงไม่คุ้มกับการเปลี่ยนบุคลิกงานพิมพ์ทั้งเว็บอีกต่อไป

### 🗓️ 2026-09-14 (รอบ 69): 📐 ตั้งเครื่องวัด Lighthouse จริงในเครื่อง · ปิด CLS · ล้างคำขอเสียเปล่า · และ**คำตอบว่าทำไม 100 เต็มยังไปไม่ถึง**
- **ที่มา**: เจ้าของสั่ง "ทำเลย อยากได้ 100" ต่อจากรอบ 68
- **สิ่งแรกที่ทำคือเลิกเดา — ตั้งเครื่องวัดให้ถูกก่อน**: ติดตั้ง `lighthouse` + Chromium ยิงใส่บิลด์ production ในเครื่อง (localhost ไม่ผ่าน proxy) ทุกการเปลี่ยนแปลงหลังจากนี้ถูกวัดด้วยคะแนน Lighthouse จริง ไม่ใช่ตัวเลขที่เก็บเองจาก PerformanceObserver
- **ของที่ลองแล้ว "แย่ลง" จึงถอดออก (จดไว้กันคนหลังมาลองซ้ำ)**:
  1. **`experimental.inlineCss: true`** — Lighthouse ขึ้นเองว่า "Render-blocking requests · Est savings of 250 ms" ทุกคู่มือแนะนำให้ฝัง CSS · **ลองแล้วคะแนนตกจาก 74 เหลือ 68 · FCP แย่ลงจาก 1326 เป็น 1575 ms** เพราะเมื่อ CSS แยกไฟล์ ตัวสแกนล่วงหน้าดึงมันขนานไปกับ HTML ที่ยังสตรีมอยู่ พอฝังลงไป HTML โตขึ้น ~20 KB gzip แล้วเบราว์เซอร์ต้องรอทั้งก้อน — ไบต์ที่เพิ่มแพงกว่ารอบเครือข่ายที่ประหยัดได้ · **บทเรียน: ช่อง "Est savings" บอกแค่ด้านที่ประหยัด ไม่ได้หักต้นทุนที่จ่ายเพิ่ม**
- **สิ่งที่แก้และเก็บไว้ (วัดแล้วดีขึ้นจริงทุกข้อ)**:
  1. **ปิด CLS ที่ต้นเหตุ** — `DailyCardStrip` ตั้งที่ว่างสำรองไว้ `h-[92px]` ตายตัว แต่ความสูงจริงคือ 106px (จอ ≥480px) · 138px (360–479px) · 162px (320px) แถบจึงดันเนื้อหาทั้งหน้าลง 32–70px ทุกครั้งที่ `/api/daily-card` ตอบ ➔ **CLS 0.084 ➔ 0.003–0.007 (คะแนน 93 ➔ 100)**
     - 🤝 **ข้อนี้ทับกับ PR #474 ซึ่ง merge เข้า main ระหว่างทาง และวิธีของเขาดีกว่า จึงใช้ของเขาแทนของผมทั้งก้อน**: ผมตรึงความสูงแยกสามเบรกพอยต์ (`h-[162px] min-[360px]:h-[138px] min-[480px]:h-[106px]`) ซึ่งยังผูกกับเลขที่วัดมาและพังได้ถ้าดีไซน์ขยับ · ส่วนเขาตรึง `h-[96px]` **ค่าเดียวทุกจอ** แล้วบังคับให้เนื้อหาพอดีเสมอด้วย `truncate` ที่ชื่อไพ่ + `flex-nowrap overflow-hidden` ที่แถวคีย์เวิร์ด พร้อมทำโครงร่างรอโหลด (skeleton) ให้หน้าตาตรงกับของจริง — เป็นคำตอบที่ถูกกว่าเพราะไม่ต้องรู้ตัวเลขความสูงล่วงหน้าเลย
     - สิ่งที่รอบนี้เติมให้แถบนั้นคือ `prefetch={false}` ซึ่ง #474 ไม่ได้ใส่ (ดูข้อถัดไป)
  2. **อุดคำขอเสียเปล่าที่รอบ 68 ทำหลุดมาเอง** — พอย้ายแถบยินยอมมาเรนเดอร์ฝั่งเซิร์ฟเวอร์ ลิงก์ "อ่านนโยบายความเป็นส่วนตัว" ในนั้นก็อยู่ในวิวพอร์ตตั้งแต่เฟรมแรกของ**ทุกหน้า** Next จึงพรีเฟตช์ `/privacy` ให้ **3 คำขอ 14 KB** ทุกครั้งที่มีคนเปิดเว็บ · ใส่ `prefetch={false}` ทั้งลิงก์นั้นและลิงก์ไพ่ประจำวัน (อีก 15 KB) ➔ **คำขอ 107 ➔ 89 · payload 667 ➔ 595 KB**
     - 📌 **กฎที่ได้จากข้อนี้**: อะไรก็ตามที่อยู่บน**ทุกหน้าตั้งแต่เฟรมแรก** (แถบยินยอม · หัวเว็บ · แถบไพ่ประจำวัน) ถ้ามี `<Link>` อยู่ข้างใน ต้องใส่ `prefetch={false}` เสมอ ไม่งั้นมันคือคำขอที่คูณด้วยจำนวนคนเปิดเว็บทั้งหมด
  3. **ลดไฟล์ favicon.ico จาก 11,160 เหลือ 2,778 ไบต์** — ข้างในมี PNG สี่ขนาด (16/32/48/64) ทั้งที่แท็บเบราว์เซอร์ใช้แค่ 16 กับ 32 (จอความละเอียดสูงใช้ `/icon.svg` ที่มีอยู่แล้ว) · ตัดสองเฟรมใหญ่ทิ้งด้วยการเขียนไฟล์ ICO ใหม่ตามสเปก แล้วยืนยันว่าอ่านกลับได้ครบทั้งสองเฟรม
  4. **เลิก `preload` ฟอนต์หัวเรื่อง** — `next/font` ใส่ `<link rel="preload">` ให้ทุกไฟล์เป็นค่าเริ่มต้น ฟอนต์สองตระกูล 6 ไฟล์ **117 KB จึงถูกดึงด้วยลำดับความสำคัญสูงสุดพร้อมกันตั้งแต่มิลลิวินาทีแรก** แย่งท่อกับ HTML/CSS/JS/ภาพ ทั้งที่ `display: "swap"` แปลว่ามันไม่ได้บล็อกการวาดหน้าเลย · **วัดสามทางเลือกแล้วเลือกทางกลาง** (ตารางเต็มอยู่ในคอมเมนต์ `src/app/_shared/fonts.ts`): ฟอนต์เนื้อความยัง preload · ฟอนต์หัวเรื่องไม่ preload ➔ **TBT 71–86 ➔ 90–99 · แลกกับ FCP 98 ➔ 90**
- **ผลรวม (Lighthouse จริง · บิลด์ production · รันฝั่งละ 3 ครั้ง · เครื่องเดียวกัน)**:

  | | ก่อน (`67307de`) | หลัง |
  | :--- | ---: | ---: |
  | **คะแนนรวม** | 74 · 72 · 76 (เฉลี่ย 74) | **77 · 80 · 73 (เฉลี่ย 77)** |
  | CLS | 0.084 (93) | **0.003–0.007 (100)** |
  | TBT | 231–371 (71–86) | **84–320 (76–99)** |
  | FCP | 1326–1334 (98) | 1759–1766 (90–91) |
  | LCP | 5157–5460 (19–24) | 5075–5195 (23–25) |
  | คำขอ · payload | 107 · 667 KB | **89 · 595 KB** |

  หน้าอื่นวัดด้วย Lighthouse เดียวกัน: `/blog` **84** · `/cards` **78**

## 🧱 ทำไม 100 เต็มยังไปไม่ถึง — คำตอบพร้อมเลข (อ่านก่อนลงมือรอบหน้า)

**ตัวถ่วงเดียวที่เหลือคือ LCP ซึ่งได้ 23–25 คะแนนจาก 100 และกินน้ำหนัก 25% = หายไป ~19 คะแนนจากข้อนี้ข้อเดียว**

สิ่งที่ต้องเข้าใจก่อน: Lighthouse/PSI ไม่ได้รายงาน LCP ที่วัดได้จริง แต่**จำลอง**ขึ้นมาใหม่จากกราฟการโหลด (Lantern)
- LCP ที่ **วัดได้จริง** บนบิลด์นี้คือ **176–192 ms** และเท่ากับ FCP เป๊ะ (ผลของรอบ 68 ยังอยู่ครบ)
- LCP ที่ **รายงาน** คือ ~5,100 ms เพราะสูตรคือ `(กราฟดีสุด + กราฟแย่สุด) ÷ 2` และ**กราฟแย่สุดนับทุกไฟล์ที่เริ่มโหลดก่อนจังหวะ LCP ว่าเป็นตัวบล็อกการวาดหน้าทั้งหมด** ซึ่งบนหน้านี้คือแทบทั้งหน้า

**ดังนั้น LCP ที่รายงาน ≈ ฟังก์ชันของ "ไบต์ทั้งหมดที่ถูกค้นพบตอนต้น"** ไม่ใช่ของอะไรอย่างอื่น และนี่คือบัญชีไบต์จริงของหน้าแรก (595 KB):

| ก้อน | ขนาด | ลดได้ไหม |
| :--- | ---: | :--- |
| React + Next.js runtime | **128 KB** | ❌ ไม่ได้ (Lighthouse เองบอกว่า 52 KB ในนั้นไม่ถูกใช้ แต่เป็นโค้ดเฟรมเวิร์ก) |
| ฟอนต์ไทยสองตระกูล | **117 KB** | 🟡 ได้ถ้ายอมแลก (ดูเมนูข้างล่าง) |
| โค้ดแอปของเรา | ~135 KB | 🟡 ได้ ถ้ารื้อหน้าแรกเป็น island |
| ภาพไพ่ | 113 KB (บน production เป็น AVIF เหลือ ~60 KB) | 🟢 บีบมาเกือบสุดแล้ว |
| HTML + CSS + ไอคอน | ~100 KB | 🟢 เพิ่งลดไปแล้วรอบนี้ |

**เลขชี้ขาด: LCP จะได้ 100 คะแนนก็ต่อเมื่อค่าที่รายงาน ≤ ~1.2 วินาที ซึ่งแปลว่าทั้งหน้าต้องเหลือราว 200 KB — แต่แค่ React runtime กับฟอนต์ไทยรวมกันก็ 245 KB แล้ว ก่อนจะนับโค้ดของเราสักบรรทัด**

### 🗳️ เมนูสามทางที่จะไปต่อได้ (ทุกข้อเป็นการตัดสินใจของเจ้าของ ไม่ใช่งานช่าง)

| ทาง | ทำอะไร | ได้คืน | ต้องแลกอะไร |
| :--- | :--- | ---: | :--- |
| **A · ตัดฟอนต์ให้เหลือเฉพาะตัวอักษรที่ใช้จริง** | subset ฟอนต์ไทยเองด้วย `pyftsubset` แทนการดึงชุดเต็มจาก Google (ไทยมีอักขระที่ใช้จริงไม่กี่ร้อยตัว) | −60 ถึง −80 KB · คาดว่า **+3 ถึง +5 คะแนน** | **ไม่ต้องแลกอะไรเลย หน้าตาเหมือนเดิมเป๊ะ** · ต้องเพิ่มขั้นตอนในสายพานบิลด์ · **แนะนำทำข้อนี้ก่อน** |
| **B · เหลือฟอนต์ตระกูลเดียว** | ใช้ Sarabun ทั้งหัวเรื่องและเนื้อความ เลิกใช้ Noto Serif Thai | −69 KB · คาดว่า **+3 ถึง +4 คะแนน** | บุคลิกงานพิมพ์ของเว็บเปลี่ยนทั้งเว็บ — เป็นการตัดสินใจเรื่องแบรนด์ |
| **C · รื้อหน้าแรกเป็น island** | ให้หน้าแรกเป็น HTML ฝั่งเซิร์ฟเวอร์ล้วน แล้วโหลด `TarotFlow` เมื่อผู้ใช้แตะหัวข้อเท่านั้น | −100 ถึง −135 KB · คาดว่า **+8 ถึง +12 คะแนน** | งานรื้อใหญ่ เสี่ยงสูง ต้องทดสอบทั้งสายพานเปิดไพ่ใหม่ทั้งหมด |

ต่อให้ทำครบทั้งสามทาง ประมาณการคือ **~90 คะแนน ไม่ใช่ 100** — เพราะพื้นที่ที่เหลือคือ React runtime 128 KB ที่ถอดไม่ได้ถ้ายังใช้ Next.js App Router
...
- **🔗 ความสัมพันธ์กับรอบ 67 (PR #472 ของ Antigravity AI)**: รอบนั้น merge เข้า main ระหว่างที่ PR นี้เปิดค้างอยู่ และสรุปเรื่อง preconnect **ตรงกัน** ว่าโฮสต์ AI ไม่ควรอยู่ตรงนั้นและควรเป็น CDN ภาพไพ่แทน · รอบนี้เก็บข้อสรุปเดียวกันไว้แต่แก้รายละเอียดสองจุด: (ก) อ่านโฮสต์จาก `getImageKitOrigin()` แทนการเขียน `https://ik.imagekit.io` ตายตัว เพื่อให้ตรงกับค่าที่ `card-image.ts` ใช้จริงเสมอ และไม่จองสายทิ้งไว้เมื่อยังไม่ได้ตั้ง ImageKit (ข) **ถอด `crossOrigin="anonymous"` ออก** เพราะ `<img>`/`<picture>` ของภาพไพ่ไม่ได้ตั้ง `crossorigin` จึงดึงแบบไม่ใช่ CORS — preconnect ที่เปิดสายแบบ CORS ไว้จะถูกนับเป็นคนละสาย เบราว์เซอร์ต้องเปิดใหม่อยู่ดี กลายเป็นเสียเปล่าสองต่อแทนที่จะช่วย
- **🔗 ความสัมพันธ์กับรอบ 66 (PR #471 ของ Antigravity AI)**: รอบนั้นลงมือกับหน้าเดียวกันโดยเล็งไปที่ TBT (ลด speculation prerender · เลื่อน gtag.js · ย้าย `AssetWarmup` ไปไว้เฉพาะหน้าแรก) รอบนี้ต่อยอดบนฐานนั้นและเล็งไปที่ **LCP** ซึ่งยังไม่มีใครแตะ · **จุดเดียวที่ทับกันคือ `AssetWarmup`** — รอบ 66 ย้ายมันไปไว้ในหน้าแรก ส่วนรอบนี้ **ลบทิ้งทั้งก้อน** เพราะหลักฐานชี้ว่ามันไม่มีประโยชน์แม้แต่บนหน้าแรก: `getCardWebpVariantSrc()` ที่มันเรียกใช้ **ไม่มีผู้เรียกอื่นเหลืออยู่เลยทั้งโปรเจกต์** ทุกเส้นทางที่วาดไพ่จริงไปทาง `<CardImage>` ➔ `<picture>` ซึ่งหยิบ AVIF เสมอ ไฟล์ WebP `w128` ที่มันดึงมาจึงไม่ถูกใช้สักไฟล์ (บั๊กชนิดเดียวกับ INC-0132 เป๊ะ) ซ้ำยังผิดขนาดอีกชั้นเพราะไพ่บนหน้าแรกแสดงที่ 90 CSS px บนจอ DPR 1.75 ขึ้นไป ซึ่งต้องการ 192–256 ไม่ใช่ 128
- **การสืบสวน (วัดจริงทุกตัวเลข ไม่ได้เดา)**: รัน Chromium จำลองมือถือ Moto G Power (412×823 · DPR 1.75 · เน็ต 1.6 Mbps RTT 150ms · CPU ช้าลง 4 เท่า) ยิงบิลด์ production ในเครื่อง
  1. **ชี้ตัวหน้าได้ก่อน** — รายงานไม่ได้บอก URL แต่ CLS ที่วัดได้ **0.0421 ตรงกับ 0.042 ในรายงานเป๊ะ** และภาพที่ถูกตำหนิ (`major-06` · `major-19` แสดงที่ 158×277 พิกเซลอุปกรณ์ = 90 CSS px × DPR 1.75) ตรงกับไพ่ในแถว "เลือกเรื่องที่คุณอยากรู้" ➔ **หน้าที่ถูกวัดคือหน้าแรก `/`**
  2. **ตัว LCP ไม่ใช่ภาพไพ่ แต่เป็นแถบขอความยินยอม PDPA** — `<p>` ในแบนเนอร์มีพื้นที่ **17,703 px²** ใหญ่กว่าภาพไพ่ใบแรก (13,463 px²) และเพราะแบนเนอร์เรนเดอร์ฝั่งไคลเอนต์ล้วน (`useState(false)` ➔ `useEffect`) มันจึงวาดก็ต่อเมื่อ React hydrate ทั้งหน้าเสร็จ · **A/B ยืนยัน**: ตั้ง localStorage ว่าเคยตอบแล้ว ➔ LCP ย้ายไปที่ภาพไพ่ทันทีและเร็วขึ้น 65%
  3. **preconnect สองบรรทัดยิงผิดโฮสต์มาตลอด** — `generativelanguage.googleapis.com` และ `api.groq.com` มีแต่ Worker ฝั่งเซิร์ฟเวอร์เท่านั้นที่ต่อ (`src/lib/ai/*`) เบราว์เซอร์คุยกับ `/api/...` ของโดเมนเราอย่างเดียว · ส่วน **ImageKit ซึ่งเป็นต้นทางของภาพไพ่ทุกใบกลับไม่มี preconnect เลย**
  4. **`AssetWarmup` ดาวน์โหลดทิ้ง 9 ไฟล์ทุกหน้า** — บั๊กชนิดเดียวกับ INC-0132: มันยิง `new Image()` ไปที่ WebP `w128` แต่ `<picture>` เลือก AVIF ทุกครั้ง ไฟล์ที่โหลดมาจึงไม่เคยถูกใช้เลยสักไฟล์ ซ้ำยังเรียก `img.decode()` กินเวลาเมนเธรด
  5. **ภาพใต้พับถูกตีตรา `fetchPriority="high"` ตายตัว** — `DailySpreadArt` (1,261px) กับ `ThreeCardSpreadArt` (1,272px) แย่งท่อเน็ตจากภาพ LCP ตัวจริง · และไพ่ในสไลด์ 4 ใบสั่ง `eager` ทั้งหมดทั้งที่มือถือเห็นจริงแค่ใบเดียวกับเศษของใบที่สอง
- **สิ่งที่แก้ (10 จุด)**:
  1. `ConsentBanner.tsx` — markup มากับ HTML เสมอ ซ่อน/แสดงด้วย CSS ล้วน (`[data-consent-banner]{display:none}` + `html[data-consent-ask]`) สถานะความยินยอมไม่อยู่ใน React state อีกต่อไป
  2. `RootHtml.tsx` — เพิ่มสคริปต์บรรทัดเดียวใน `<head>` ที่ตั้ง `data-consent-ask` ก่อนเฟรมแรก เฉพาะเครื่องที่ยังไม่เคยตัดสินใจ (ไม่มีกะพริบ ไม่มี hydration mismatch)
  3. `RootHtml.tsx` — ถอด preconnect/dns-prefetch ของโฮสต์ AI ทิ้ง แล้วใส่ของ CDN ภาพไพ่แทน (ไม่มี `crossOrigin` เพราะ `<img>` ของเราดึงแบบไม่ใช่ CORS)
  4. `card-image.ts` — เพิ่ม `getImageKitOrigin()` (preconnect รับได้เฉพาะ origin ไม่เอา path ของบัญชี)
  5. `card-image.ts` — แยก `CARD_AVIF_WIDTHS` ออกจาก `CARD_IMAGE_VARIANTS` แล้วเติมขั้น **192** (AVIF ไม่ต้องมีไฟล์จริงในรีโป ImageKit แปลงตอนร้องขอ) · **วัดจริงจาก ImageKit**: `major-06` w-256 = 23,214 B ➔ w-192 = 15,043 B (**−35%**) ส่วน WebP ยังใช้ชุดเดิมเป๊ะเพราะทุกขนาดต้องมีไฟล์บนดิสก์ (ด่านที่ 7 กฎ C)
  6. `QuickFortunePicker.tsx` — ไพ่ใบแรก `eager` + `fetchPriority="high"` · ใบที่เหลือเป็น `lazy`
  7. `TarotArtIcons.tsx` — ถอด `eager`/`fetchPriority="high"` ที่ฮาร์ดโค้ดไว้ใน `DailySpreadArt` และ `ThreeCardSpreadArt`
  8. ลบ `AssetWarmup.tsx` + `src/lib/utils/cache.ts` ทิ้งทั้งคู่ (ไม่เหลือผู้เรียกแล้ว)
  9. `CardsExplorer.tsx` · `CardGroupView.tsx` — `contain-intrinsic-size` 380px ➔ **406px** ตามที่วัดจริง (ต่ำกว่าจริง 26px ต่อแถว × 39 แถว = ลูกโซ่ forced reflow ตลอดการโหลด)
  10. `test-analytics-integrity.ts` — เปลี่ยนด่านเดิม "แบนเนอร์อ่านสถานะหลัง mount เท่านั้น" (ซึ่งเป็นตัวล็อกบั๊กนี้ไว้) เป็นด่านใหม่ 4 ข้อที่ล็อกทางแก้แทน
- **ผลการตรวจยืนยัน (วัดด้วยเครื่องมือเดียวกัน ก่อน/หลัง บนบิลด์ production จริง)**:

  > ฐานเทียบคือ `origin/main` ที่ `05b852b` (มี PR #471 อยู่แล้ว) บิลด์ใหม่ทั้งสองฝั่งด้วยเครื่องเดียวกัน
  > รันฝั่งละ 2–3 ครั้ง (ตัวเลขในตารางคือช่วงที่วัดได้จริง ไม่ได้เลือกรอบที่ดีที่สุด)

  | ตัวชี้วัดหน้าแรก | ก่อน (05b852b) | หลัง |
  | :--- | ---: | ---: |
  | FCP (ผู้ใช้ใหม่) | 484–624 ms | **284–312 ms** |
  | LCP (ผู้ใช้ใหม่) | 836–976 ms · `<p>` ของแบนเนอร์ · **รอ hydrate** | **284–312 ms** · ตัวเดียวกันแต่ **มากับ HTML** |
  | **ช่วงห่าง LCP − FCP** | **352–400 ms** (คือส่วนที่บนเน็ตจริงบวมเป็นหลายวินาที) | **0 ms** |
  | LCP (ผู้ใช้ที่เคยตอบแล้ว) | 476 ms (ภาพไพ่) | **416 ms** (ภาพไพ่) |
  | TBT (ประมาณจาก long task) | 285–294 ms | **264–309 ms** (ไม่ต่างอย่างมีนัย — ดูหัวข้อส่งต่อ) |
  | ไบต์ของภาพ LCP จาก ImageKit | 23,214 B | **15,043 B** (−35%) |

  **อ่านตารางนี้อย่างไร**: ช่องที่สำคัญคือ "ช่วงห่าง LCP − FCP" ไม่ใช่ตัวเลขดิบ · ในเครื่องที่เน็ตเร็ว
  และ CPU ช้าลงแค่ 4 เท่า hydrate ใช้เวลาแค่ ~350 ms ช่องว่างจึงดูเล็ก · แต่บนมือถือจริงที่ต้อง
  ดาวน์โหลด JS 229 KB ผ่านเน็ตช้าก่อนแล้วค่อย hydrate ช่องว่างเดียวกันนี้คือแปดวินาทีที่หายไป
  ...
  - ไม่มีหน้าไหนถอยหลัง (วัดบนบิลด์สุดท้าย): `/spreads` LCP 700ms · `/cards` 620ms · `/cards/major` 348ms · `/cards/major-06` 344ms · `/daily` 328ms · `/blog` 576ms (ทุกหน้า CLS 0 · TBT 109–170ms)
- **⚠️ สิ่งที่ยังไม่ถึง 100 คะแนน (ส่งต่อ)**: ตัวถ่วงที่เหลือคือ **TBT** ซึ่งกินน้ำหนักคะแนน 30% — มาจากการ hydrate หน้าแรกที่ยังใช้ JS 229 KB · การจะได้ 100 เต็มต้องลด JS ต่อหน้าลงอีกมาก (ดูแผน `HANDOFF_BUNDLE_DIET_2026-09-07.md` และ `HANDOFF_SMOOTH_FAST_2026-09-06.md`) · อีกข้อคือ **render-blocking CSS 820ms** ซึ่งแก้ได้ด้วย `experimental.inlineCss` แต่จะเพิ่ม HTML ทุกหน้า ~22 KB gzip = ทะลุงบ HTML ทุกเส้นทางทันที จึง **ยังไม่ทำ** ต้องให้เจ้าของเคาะว่ายอมขยับเพดานงบไหม
### 🗓️ 2026-09-14 (รอบ 68): 🎯 ขจัด CLS จาก DailyCardStrip เหลือ 0.000 และปรับการโหลด GTM สู่ระบบ Universal Idle/Interaction Deferral (ยกเลิก UA Sniffing เพื่อพัฒนาผล CrUX ของผู้ใช้จริง)
- **ที่มา**:
  1. การตรวจสอบร่วมเรื่อง `isSyntheticAudit` ใน #472: การดัก User-Agent ของ Lighthouse/PageSpeed เพื่อไม่โหลด `gtag.js` แม้จะทำให้ผลทดสอบแล็บได้ตัวเลขสวย แต่ไม่สะท้อนผล CrUX ในภาคสนามจริงที่ Google ใช้จัดอันดับ SEO
  2. CLS จาก `DailyCardStrip`: ตัวกล่องว่างสำรองเดิมตั้งไว้ `h-[92px]` แต่ความสูงจริงหลัง API ตอบกลับขยายตัวเป็น 106/138/162px เนื่องจากชื่อไพ่และคีย์เวิร์ดตัดขึ้น 2-3 บรรทัดบนจอมือถือ ดันเนื้อหาทั้งหน้าลง (CLS 0.042–0.072)
- **การดำเนินการ**:
  1. **ถอด `isSyntheticAudit` ใน `AnalyticsTracker.tsx`**:
     - ยกเลิกการดัก User-Agent ทั้งหมด 100% สู่ระบบโหลดแบบ Universal เสมอภาคสำหรับผู้ใช้จริงและบอท
     - โหลด `gtag.js` เมื่อเริ่มมีปฏิสัมพันธ์กับหน้าจอ (`scroll`, `touchstart`, `pointerdown`, `keydown`)
     - มี Fallback โหลดอัตโนมัติเมื่อเบราว์เซอร์อยู่ในสถานะ Idle ผ่าน `requestIdleCallback` (หลังโหลดหน้าเสร็จ 5–8 วินาที) ทำให้ FCP, LCP และ TBT ของผู้ใช้จริงไม่ถูกแย่ง CPU และข้อมูล CrUX ในภาคสนามเร็วขึ้นจริงอย่างโปร่งใส
  2. **ขจัด CLS ใน `DailyCardStrip.tsx` สู่ 0.000**:
     - ล็อคความสูงของ Skeleton และกล่องการ์ดจริงให้เท่ากันคงที่ที่ `h-[96px]` พอดีกับสัดส่วนเรขาคณิต
     - เพิ่ม Rich Skeleton ที่จำลองภาพไพ่ หัวข้อ ชื่อ และแท็กคีย์เวิร์ด ทำให้ไม่มีการกระตุกของสายตา
...
- **ที่มา**: ตามคำสั่งเจ้าของโปรเจกต์ "ทำสิ ถ้ามีอะไรต้องทำ คู่กันตลอดเพราะ ของเรา 2 ภาษา" เพื่อให้นโยบายทั้งสองภาษามีมาตรฐานทางกฎหมายเท่าเทียมกันทุกมิติ
- **การดำเนินการ**:
  1. **อัปเกรด `src/app/(th)/privacy/page.tsx`**:
     - **Section 1 (ข้อมูลที่เราเก็บ)**: เสริมรายละเอียดเรื่องการยืนยันตัวตนและการรักษาความปลอดภัยของบัญชีผ่าน OAuth (Google/LINE), การจัดเก็บค่าแฮชรหัสผ่านแบบเค็ม (PBKDF2/SHA-256) โดยไม่เก็บรหัสผ่านจริง, และคุกกี้ first-party `tarot_guest`
     - **Section 2 (คำมั่นสัญญาความปลอดภัย)**: ลบสัญลักษณ์ `•` ที่ซ้ำซ้อนกับ `list-disc` (แก้บั๊กแสดงผล Bullet ซ้อนสองชั้น), ขยายความคำมั่นไม่นำข้อมูลไปเทรน AI (Zero AI Training), ไม่ขาย/ให้เช่าข้อมูลแก่ Data Brokers, และไม่ติดตามข้ามเว็บ
     - **Section 3 (การจัดเก็บข้อมูล)**: ระบุมาตรฐานความปลอดภัยของ Cloudflare D1 ฐานข้อมูลกระจายศูนย์ พร้อมการเข้ารหัสทั้งขณะจัดเก็บ (Encryption at Rest) และขณะส่งข้อมูล (TLS 1.3 in Transit)
     - **Section 4 (สิทธิของคุณ)**: เสริมสิทธิที่จะไม่ถูกเลือกปฏิบัติ (Right to Non-Discrimination) ทัดเทียมมาตรฐานสากล
     - **Section 5 (การเปิดเผยเรื่อง AI)**: บันทึก **กฎเหล็กข้อ 14 นโยบายห้ามกุไพ่ปลอมเด็ดขาด (Zero Fabricated Cards Policy)** ยืนยันว่าทุกการทำนายทำงานบนสำรับ 1909 Rider-Waite 78 ใบแท้จริง มีกลไกป้องกันการมโนหรือสร้างไพ่ที่ไม่มีอยู่จริง เพื่อความเที่ยงธรรมสูงสุด
...
- **สิ่งที่แผนบอกผิด 4 ข้อ** (ทุกข้อวัดใหม่แล้ว ไม่ได้คัดลอกต่อ)
  | เรื่อง | แผนเขียนว่า | ของจริง | วัดด้วย |
  | :--- | :--- | :--- | :--- |
  | นโยบาย guest | 🟠 รอเจ้าของเคาะ | ✅ `GUEST_LIMIT=0` (09-09) · `DAILY_LIMIT=1` (09-12) บังคับใช้บน production แล้ว | ยิง `POST /api/reading/start` จริง ➔ `403 signup_required` |
  | ด่านตรวจ | 35/35 | **56/56** | `CHECKS.length` |
  | หน้า prerender | 364 | **311** (ไทย 164 · อังกฤษ 147) | นับไฟล์ `.html` หลังบิลด์สด |
  | น้ำหนัก JS | 212–336 KB · "งานใหญ่ที่สุดที่เหลือ" | **172–229 KB** ห่างเป้าแค่ 2–28 KB | `npm run test:budget` |
- **ตรวจงานที่อ้างว่าค้างทีละข้อด้วยการเปิดโค้ดดู** ไม่เชื่อสถานะในเอกสาร: Daily Digest ✅ เสร็จแล้ว (#451 · #452) · D-04 ✅ ต่อเข้า CHECKS แล้ว (#454) · D-05 ✅ บิลด์ได้ 0 warning · หน้าแอดมินรหัสแลกสิทธิ์ ✅ ใช้งานได้จริง · AI Judge baseline ✅ เก็บแล้ว 30 เคส overall 4.83 · ส่วนที่ยังค้างจริงคือ UI ค้นหา (ไม่มีหน้า `/search`) · Yes/No 78 หน้า (มีแค่ป้าย) · Omnichannel (ไม่มี `src/lib/social`) · `/en/cards/birth-card` (ยัง 404) · linter (ไม่มี `eslint.config.*` และไม่มีสคริปต์ `lint`)
- **ผลสรุปใหม่**: เหลือ **7 งานลงมือได้ทันที · 3 งานรอคนนอกทีม · 2 เรื่องรอเจ้าของเคาะ** (เดิมเขียนว่า 3 เรื่อง)
- **กฎป้องกันถาวร**: เพิ่ม `MASTER_PLAN` เข้า `TARGET_FILES` ของด่าน `test-docs-numbers.ts` (**ด่านเดิม ไม่เพิ่มจำนวนด่าน**) · และ**แก้ถ้อยคำในแผนให้เลขติดกับคำว่า "ด่าน"** เพราะทดสอบแล้วพบว่าลำพังการเอาไฟล์เข้าขอบเขต **ยังจับไม่ได้** ในเมื่อเดิมเขียนว่า "✅ 35/35 ผ่าน" โดยคำว่า "ด่าน" อยู่คนละคอลัมน์
  - พิสูจน์ 2 รอบ: ถ้อยคำเดิมทำพัง ➔ ด่านไม่จับ · ถ้อยคำใหม่ทำพัง ➔ **จับได้ทันที 2 จุดพร้อมเลขบรรทัด**
  - ไม่เหมาเอา `docs/plans/` ทั้งโฟลเดอร์ — แผนส่งต่อฉบับอื่นเป็นบันทึก ณ เวลานั้นโดยเจตนา ตัวเลขเก่าคือหลักฐาน ไม่ใช่บั๊ก

### 🗓️ 2026-09-14 (รอบ 60): 🧾 กวาดหนี้เอกสารทั้งแฟ้ม — เลข INC ซ้ำ 10 คู่ · งาน 2 PR หายจากบันทึก (INC-0145)

- **ที่มา**: เจ้าของสั่งให้ตรวจว่าไฟล์ `.md` ทั้งหมดอัปเดตครบหรือยัง ทั้งในเครื่องและบน GitHub
- **ผลตรวจฝั่ง git**: ✅ ตรงกันหมด — branch = `origin/main` = `c5a949a` · working tree สะอาด · `.md` ที่ track ไว้ 65 ไฟล์อยู่บน GitHub ครบ ไม่มีไฟล์ตกค้าง
- **ผลตรวจฝั่งเนื้อหา**: ❌ พบหนี้ 4 เรื่อง ซึ่งแก้ครบในรอบนี้แล้ว
  1. **งาน 2 PR หายจาก `WORK_LOG.md`** (ละเมิดกฎเหล็กข้อ 1) — จดคืนเป็น **รอบ 57b** (PR #453 · ลด TBT `/blog`) และ **รอบ 58b** (PR #455 · ช่องรหัสผ่าน + โลโก้ LINE) โดยอ่านจาก diff จริงของ commit ไม่ใช่เดาจากหัวข้อ · PR #453 เป็นตัวที่หนักกว่าเพราะ **ไม่ได้แตะไฟล์ `.md` เลยสักไฟล์**
...
> 📌 **บันทึกย้อนหลัง 2026-09-14** — PR นี้จดลง `INCIDENT_LOG.md` ไว้แล้วแต่ **ลืมจดลง `WORK_LOG.md`** ผิดกฎเหล็กข้อ 1 · จดคืนให้ครบตามลำดับเวลาจริง (merge หลังรอบ 58 ก่อนรอบ 59)

- **เป้าหมาย**: แก้หน้าต่างเข้าสู่ระบบที่ช่องกรอกสองช่องไม่เท่ากัน และไอคอน LINE ที่ตัวอักษรแตกจนอ่านไม่ออก
- **รายการที่แก้ไข**:
...
> 📌 **บันทึกย้อนหลัง 2026-09-14** — PR นี้ **ไม่ได้แตะไฟล์ `.md` เลยสักไฟล์** ทั้งที่กฎเหล็กข้อ 1 บังคับให้จด · จดคืนจาก diff จริงของ commit `2556d91` (merge หลังรอบ 57 ก่อนรอบ 58)

- **เป้าหมาย**: ลด Total Blocking Time ของหน้า `/blog` ที่ Lighthouse วัดได้ 660 ms เมื่อ 2026-09-11 — แย่กว่าหน้าอื่นในเว็บ 4.1 เท่า (ISSUE-043 · แผนเต็มที่ [`HANDOFF_BLOG_TBT_2026-09-12.md`](plans/HANDOFF_BLOG_TBT_2026-09-12.md))
- **รายการที่แก้ไข** (3 ไฟล์ · +73 / −14 บรรทัด):
  1. **ไอคอนท้องถิ่น** — `BlogIndexClient.tsx` เลิก `import { SearchTabIcon } from "@/components/ui/TarotArtIcons"` เปลี่ยนเป็น `SearchIcon` inline (วงกลม + เส้นด้ามจับ 2 path) ตัดการลากไฟล์ไอคอนศิลป์ทั้งก้อนเข้ามาในหน้านี้เพียงเพื่อแว่นขยายอันเดียว
  2. **ลด flight payload** — เพิ่มชนิด `BlogCardItem = Pick<ArticleSummary, …10 ฟิลด์>` แล้วให้ `blog-index.tsx` แมป `articles` เหลือเฉพาะฟิลด์ที่การ์ดเรียกใช้จริงก่อนส่งข้ามเส้นแบ่ง server ➔ client (กฎเดียวกับ INC-0103: ทุกฟิลด์ที่ส่งเป็น prop ให้ client component ถูก serialize ลง flight payload ของหน้านั้นเสมอ)
  3. **คลายข้อผูกของ helper** — `article-helpers.ts` เปลี่ยนพารามิเตอร์จาก `Article | ArticleSummary` เป็นชนิดเชิงโครงสร้าง 4 ตัว (`ArticleTitleLike` · `ArticleDescriptionLike` · `ArticleCategoryLike` · `ArticleAuthorLike`) เพื่อให้ตัดฟิลด์ตามข้อ 2 ได้โดยไม่ต้อง cast
  4. **`content-visibility: auto`** — การ์ดบทความแต่ละใบได้ `contentVisibility: "auto"` + `containIntrinsicSize: "auto 260px"` เลื่อนงานเรนเดอร์การ์ดนอกจอออกจากเธรดหลัก (ปิดหนี้ที่ `HANDOFF_SMOOTH_FAST` จดไว้ว่า "เขียนไว้แต่ไม่ได้ใช้")
  5. **ลำดับภาพ** — ภาพบทความเด่น `loading="eager"` + `fetchPriority="high"` · การ์ด 4 ใบแรก `eager` ที่เหลือ `lazy`
  6. **ตัดงาน filter ที่ไม่จำเป็น** — `useMemo` ของ `filtered` คืน `articles` ตรง ๆ ทันทีเมื่อยังไม่มีคำค้นและเลือกหมวด "ทั้งหมด" แทนที่จะไล่ `.filter()` ทุกรายการทุกครั้งที่เรนเดอร์
- **ผลการตรวจ**: Verified 100% Green (Collision · Typecheck · Cards · Spreads) ตามที่ commit บันทึกไว้
- **⚠️ สิ่งที่ยังค้าง**: **ยังไม่มีตัวเลข TBT หลังแก้** — ตอนนั้นโควตา PageSpeed Insights หมด จึงยังพิสูจน์ไม่ได้ว่า 660 ms ลดลงเท่าไร · **ISSUE-043 จึงยังไม่ปิด** เหลือขั้นตอนเดียวคือยิง Lighthouse ซ้ำแล้วอ่าน `bootup-time` เทียบของเดิม


### 🗓️ 2026-09-13 (รอบ 57): 🐛 cron ยังโดน 403 อยู่ — ตัวจริงคือ Bot Fight Mode ไม่ใช่กฎ WAF (รอบ 56 วินิจฉัยผิด)

> **เจ้าของตั้ง `CRON_SECRET` ฝั่ง GitHub เสร็จ ➔ กดรัน workflow จริง ➔ ยังได้ 403** · ครั้งนี้มีเนื้อหาที่ตอบกลับมาให้ดูเต็ม ๆ จึงเห็นสาเหตุจริง

**รอบ 56 วินิจฉัยผิดตรงไหน**: เห็น 403 แล้วไปเทียบกับกฎ WAF ข้อ 5 (บล็อก UA `curl`) ซึ่งมีอยู่จริงและเกี่ยวข้องจริง · ทดสอบจากคอนเทนเนอร์ที่ทำงานอยู่แล้วพบว่าเปลี่ยน UA ทำให้ได้ 200 จึงสรุปว่าแก้จบ — **แต่คอนเทนเนอร์ที่ทดสอบกับ GitHub runner อยู่คนละ IP คนละชั้นความน่าเชื่อถือในสายตา Cloudflare** ผลจากเครื่องหนึ่งจึงเอาไปตัดสินแทนอีกเครื่องไม่ได้

**สาเหตุจริง**: เนื้อหาที่ตอบกลับมาคือหน้า **managed challenge** ของ Cloudflare (`<title>Just a moment...</title>` · `cType: 'managed'` · เรียก `/cdn-cgi/challenge-platform/`) ซึ่งต้องมี JavaScript และคุกกี้ถึงจะผ่าน — **ตัวจับเวลาที่เป็น `curl` ไม่มีทางผ่านได้เลยไม่ว่าตั้ง User-Agent อะไร** เพราะ Cloudflare ตัดสินจาก **IP/ASN** ไม่ใช่ชื่อ UA และ IP ของ GitHub runner เป็นดาต้าเซ็นเตอร์เต็มตัว

**สิ่งที่แก้**: เปลี่ยนปลายทางจาก `https://seertarot.net/...` เป็น `https://tarot-web.bankjack10452.workers.dev/api/cron/daily-digest`

`*.workers.dev` เป็นโดเมนของ Cloudflare เอง **ไม่ได้อยู่ในโซน `seertarot.net`** การตั้งค่าระดับโซนทั้งหมด (Bot Fight Mode · WAF custom rules) จึงไม่มีผลกับเส้นนี้ · โค้ดที่รันคือ Worker ตัวเดียวกันเป๊ะ · **ลิงก์ในอีเมลยังชี้ `seertarot.net` เหมือนเดิม** เพราะสร้างจาก `SITE_ORIGIN` ในโค้ด ไม่ได้เอามาจาก host ของคำขอ

**ทำไมไม่แก้ที่ Cloudflare แทน**: จะต้องให้เจ้าของไปเพิ่ม/ปรับกฎที่ dashboard ซึ่ง (1) ต้องรอเจ้าของ (2) เป็นการเจาะรูในเกราะของโดเมนหลักที่มองไม่เห็นจากในรีโปเลย · การย้ายปลายทางแก้ได้ในโค้ด มีด่านตรวจกำกับ และไม่ลดความปลอดภัยของโดเมนหลักสักนิด

**ความปลอดภัยไม่ลดลง**: `/api/cron/daily-digest` เป็น fail-closed อยู่แล้ว — ไม่มี Bearer ที่ตรงกับ `CRON_SECRET` ได้ 401 ตัวเปล่าเสมอ ไม่ว่ายิงมาจากทางไหน

**ด่านกันเกิดซ้ำ** (ด่านที่ 51 → 41 ข้อย่อย): workflow ต้องยิงเข้า `*.workers.dev` และ**ห้าม**มี `https://seertarot.net/api/cron/daily-digest` ในไฟล์ — ตกทันทีถ้ามีใครเปลี่ยนกลับ

> **บทเรียน**: ผลการยิงทดสอบจาก "เครื่องของเรา" พิสูจน์ได้แค่ว่าเครื่องของเราผ่าน ไม่ได้พิสูจน์ว่าเครื่องที่จะยิงจริงผ่าน — เมื่อปลายทางมีชั้นที่ตัดสินจาก **IP** (bot protection · rate limit · geo) ต้องยิงทดสอบ**จากเครื่องตัวจริง**เท่านั้น · และ **403 ต้องอ่านเนื้อหาที่ตอบกลับมาเสมอ** ไม่ใช่ดูแค่ตัวเลข: รอบนี้คำตอบอยู่ในบรรทัดแรกของ body มาตั้งแต่ต้น

**ผลตรวจ**: `repo:verify` **ผ่านครบ 52/52 ด่าน** · ด่าน digest 41/41 · ยิงเข้า workers.dev จากคอนเทนเนอร์ได้ **401** (ถึง Worker จริง ไม่โดน challenge)

---

### 🗓️ 2026-09-13 (รอบ 56): 🐛 Daily Digest ที่เพิ่งปล่อยไป ส่งไม่ออกตลอดกาล — WAF ปัดตก `curl` ที่ชั้น edge

> **เจอตอนไปตรวจ production หลัง deploy รอบ 55** ไม่ใช่ตอนมีคนมาแจ้ง · ถ้าไม่ตรวจ จะรู้ตัวอีกทีตอนเจ้าของตั้ง `CRON_SECRET` เสร็จแล้วกดรันแล้วงงว่าทำไม 403

**อาการ**: ยิง `POST https://seertarot.net/api/cron/daily-digest` ได้ **403** ทั้งที่เพิ่ง deploy สำเร็จ · ยิง `/api/config/analytics` ซึ่งเป็นเส้นเดิมที่ใช้งานได้อยู่ ก็ได้ 403 เหมือนกัน แต่ `/daily` กับ `/robots.txt` ได้ 200 ปกติ ➔ แปลว่าไม่ใช่เส้นใหม่พัง แต่เป็นทั้งหมวด `/api/`

**สาเหตุราก**: กฎ WAF ข้อ 5 ของโซนนี้ (เขียนไว้ใน `docs/CLOUDFLARE_OPTIMIZATION_GUIDE.md:61` ตั้งแต่ก่อนหน้านี้) บล็อก User-Agent ที่เป็น `curl` / `python` / `Postman` / `Scrapy` ที่เส้น `/api/` **ตั้งแต่ชั้น edge ก่อนถึง Worker** · `.github/workflows/daily-digest.yml` ที่เขียนในรอบ 55 ใช้ `curl` โดยไม่ตั้ง User-Agent เอง ➔ ทุกคำขอจะถูกปัดตกด้วย 403 โดยไม่มีวันไปถึงโค้ดเลยสักครั้ง **ต่อให้ตั้ง `CRON_SECRET` ถูกทั้งสองฝั่งก็ไม่มีอีเมลออกสักฉบับ**

> ความผิดพลาดคือ**เขียน workflow ยิงเข้า production โดยไม่เคยยิงจริงดูสักครั้ง** — ตอนรอบ 55 ทดสอบครบทุกอย่างบน dev server ซึ่งไม่มี Cloudflare คั่นอยู่ จุดบอดจึงมองไม่เห็นทั้งรอบ

**หลักฐานที่วัดจาก production จริง**

| User-Agent | ผล |
| :--- | :-- |
| `curl/8.5.0` (ค่าเริ่มต้น) | **403** |
| `SeerTarot-Cron/1.0` | **200** |
| `Mozilla/5.0 (compatible; SeerTarotCron/1.0)` | **200** |

**สิ่งที่แก้**

- `.github/workflows/daily-digest.yml`: เติม `-A "SeerTarot-Cron/1.0"` พร้อมคอมเมนต์อธิบายว่าทำไมห้ามปล่อยเป็นค่าเริ่มต้น
- แยกข้อความ error ตามรหัสให้คนอ่าน log รู้ทันทีว่าต้องแก้ตรงไหน: **403 = โดน WAF ปัดตก (ไม่เกี่ยวกับความลับ)** · **401 = ค่าความลับสองฝั่งไม่ตรงกัน** · อื่น ๆ = ดูผลลัพธ์
- `docs/PENDING_SETUP.md`: เพิ่มตารางอ่านรหัสผลลัพธ์ + คำเตือนไม่ให้เผลอบล็อก UA ตัวนี้เวลาเพิ่มกฎ WAF ใหม่

**ด่านกันเกิดซ้ำ** (ขยายด่านที่ 51 เป็น 40 ข้อย่อย): workflow ต้องตั้ง User-Agent เอง และต้องแยกแยะ 403 ออกจาก 401 — สองข้อนี้ตกทันทีถ้ามีใครลบ `-A` ออก

> **บทเรียน**: ทดสอบบน dev server พิสูจน์ได้แค่ว่า "โค้ดถูก" ไม่ได้พิสูจน์ว่า "คำขอไปถึงโค้ด" — ชั้น edge (WAF · bot rules · rate limit) มองไม่เห็นจาก localhost เลยสักชั้น งานที่ยิงเข้า production จากภายนอกต้องยิงจริงดูอย่างน้อยหนึ่งครั้งเสมอ

**ผลตรวจ**: `repo:verify` **ผ่านครบ 52/52 ด่าน** · ด่าน digest 40/40
...
> **ที่มา**: เจ้าของถามว่า "ทำยังไงให้คนมาใช้เว็บเราและใช้ต่อ" — ตรวจของจริงแล้วพบว่า habit loop ที่เขียนไว้ตั้งแต่ `migrations/0008_daily_readings.sql` ไม่เคยทำงานเลยสักวัน

**สาเหตุราก 2 ชั้น — ทั้งสองชั้นเป็นบั๊กแบบ "โค้ดถูกต้อง 100% แต่ไม่มีใครเรียก"**

1. **ไม่เคยมีการบันทึก** — `recordDailyReading()` ถูกเรียกจากที่เดียวคือ `consumeReading()` และเรียกเฉพาะเมื่อ `spreadId === "daily"` ซึ่งเกิดได้จากผังในหน้าแรกเท่านั้น · แต่หน้า `/daily` (ประตูหลักของคำค้น "ดูดวงไพ่ยิปซีรายวัน") ใช้ `OneCardRitual` ที่รับ `spreadId` เข้ามาแล้ว **ไม่ใช้เลย** (`spreadId: _spreadId`) สุ่มไพ่ด้วย Web Crypto ในเบราว์เซอร์ล้วน ไม่เคยแตะเซิร์ฟเวอร์ ➔ ตาราง `daily_readings` ของคนกลุ่มนี้ว่างเปล่าตลอดกาล
2. **ไม่เคยมีการแสดงผล** — `dailyStreak` ถูกส่งครบทุกชั้นจนถึง `ClientEntitlement` ฝั่งเบราว์เซอร์แล้ว แต่ค้นทั้งเรโป `grep streak --include=*.tsx` = **0 ผลลัพธ์** ไม่มีหน้าจอไหนเอาขึ้นแสดงสักจุด

**สิ่งที่แก้**

- **ใหม่** `src/app/api/daily/checkin/route.ts` — เส้นเช็กอินไพ่ประจำวัน **ไม่หักโควตา ไม่แตะ `reading_usage`** (ไพ่ประจำวันยังฟรีเหมือนเดิมทุกประการ) หน้าที่เดียวคือประทับตรา "วันนี้มาแล้ว" แล้วคืนจำนวนวันต่อเนื่อง · มีด่าน `isRequestAuthorizedOrigin` เหมือน API อื่นทั้งเว็บ · ผู้เยี่ยมชมที่ยังไม่มีคุกกี้ (`gid = "anon"`) **ไม่บันทึก** เพราะจะกลายเป็นแถวกองกลางที่ทุกคนใช้ร่วมกันแล้วคืน streak ของคนอื่นให้คนที่เพิ่งเข้ามาครั้งแรก
- **ใหม่** `src/components/daily/DailyStreakRibbon.tsx` — ป้ายแสดงวันต่อเนื่อง (กฎเหล็กข้อ 2: ใช้ `✦` เท่านั้น ไม่มีอิโมจิการ์ตูน) · streak = 0 แล้วซ่อนตัวเอง ไม่ทวงให้ผู้ใช้รู้สึกผิด
- `DailyClient.tsx` — ยิงเช็กอินตอนผู้ใช้พลิกไพ่ **ก่อน** บันทึกประวัติ เพราะผู้ใช้ "เปิดไพ่วันนี้" ไปแล้วจริง ไม่ว่าสมุดบันทึกจะเขียนสำเร็จหรือไม่ (เส้นทาง `cardIdx < 0` ของกฎข้อ 14 return ออกกลางทาง) · เช็กอินล้มเหลว = แค่ไม่มีป้ายขึ้น ห้ามกระทบการอ่านคำทำนาย
- `EntitlementStatusCard.tsx` — หน้า `/account` แสดงวันต่อเนื่องด้วย ใช้ป้ายตัวเดียวกัน

**ด่านกันเกิดซ้ำ (ขยายด่านที่ 12 ของ `test-entitlement.ts` เป็น 12b — รวม 133 ข้อย่อย)**

ด่านเดิมข้อ 12 ตรวจว่า "`consumeReading` บันทึก streak ได้" ซึ่ง**ผ่านมาตลอด**ทั้งที่ระบบตายอยู่ — เพราะมันทดสอบฟังก์ชันที่ไม่มีใครเรียก · 12b จึงตรวจ **การต่อสาย** แทนตรรกะ: มีเส้นเช็กอินจริง · เส้นนั้นห้าม `import consumeReading` (กันวันหน้ามีคนทำให้ไพ่ประจำวันกินโควตา) · ต้องมีด่าน origin · `DailyClient.tsx` ต้องยิงเส้นนั้นจริง · และต้องมีไฟล์ `.tsx` ที่แสดง streak อย่างน้อย 1 ไฟล์ (ตอนนี้พบ 3)

> บทเรียน: **ด่านที่ทดสอบฟังก์ชันโดด ๆ ไม่เคยจับได้ว่าไม่มีใครเรียกฟังก์ชันนั้น** — ฟีเจอร์ที่ไม่มีใครเรียกและไม่มีใครเห็น เท่ากับฟีเจอร์ที่ไม่มีอยู่จริง

**ผลตรวจ**: `typecheck` 0 error · `repo:verify` **ผ่านครบ 50/50** · งบน้ำหนัก `/daily` **195 KB เท่าเดิม ไม่ขยับ** · ทดสอบ endpoint จริงบน dev: origin ปลอม 403 · GET 405 · anon คืน streak 0 ไม่เขียนแถว · ทดสอบเลขต่อเนื่องกับ D1 จริง: 4 วันติด = 4 · ยิงซ้ำวันเดิม 2 ครั้งยังคง 4 · ขาดช่วง 5 วัน = 0
### 🗓️ 2026-09-13 (รอบ 54): 🔍 ตรวจรายงานตรวจสอบภายนอก 4 ข้อทีละข้อ แล้วปิด 3 ข้อ (ISSUE-038 · 045 · 046)

> **คำสั่งเจ้าของ**: ส่งภาพรายงาน 4 ข้อมาถามว่า "ตรวจสอบจริงไหม" ➔ "แก้เลย"

#### 0. ผลการตรวจรายงาน — จริง 3 ข้อครึ่ง ผิดรายละเอียด 2 จุด

| ข้อ | คำกล่าวอ้าง | ผลตรวจ |
| :-- | :--- | :--- |
| 1 | จุดสถานะล่องหนใน `StreamReader.tsx:118-119` | ✅ **จริง 100%** บรรทัดตรงเป๊ะ |
| 2 | `TarotEncyclopediaModal` เปิดไม่ได้ทั้งเว็บ | ✅ **จริง** แต่บรรทัดที่อ้างผิด (จริงคือ 190 · 1791 ไม่ใช่ 178 · 1770) และ path ผิด (`encyclopedia/` ไม่ใช่ `home/`) |
| 3 | `/blog` TBT 660 ms | 🟡 **อ้างเอกสารเก่า ไม่ใช่การวัดใหม่** — ตัวเลขมาจาก `HANDOFF_BLOG_TBT_2026-09-12.md` (วัด 2026-09-11) · ยิง PSI ซ้ำแล้วโควตายังหมดจริง · หลังวันวัดมี PR แตะไฟล์นี้ 5 ตัว (#407 #423 #427 #429 #431) ตัวเลขจึงยังไม่ยืนยัน |
| 4 | ธงสิทธิ์ปิดค้างบน production | ✅ **จริง** ยืนยันด้วยการอ่าน KV production ตรง ๆ ได้ `{"value":false}` · แต่รายงาน**ตกไปหนึ่งข้อที่สำคัญ**: ด่านล็อกอินไม่ได้ถูกข้ามด้วย (`signin-gate.ts` ถูกวางนอกธงโดยตั้งใจตั้งแต่ INC-0111) |

#### 1. ISSUE-046 · จุดสถานะล่องหน — และของแถมที่ตาคนมองไม่เห็น

`StreamReader.tsx` แถบ "อ่านคำทำนายครบถ้วนแล้ว" ใช้ `bg-[#EBF3ED]` ทั้งกล่องและทั้งวงกลมข้างใน วงกลมจึงหายสนิท
**หลักฐานว่าเป็นการลอกผิด ไม่ใช่เจตนา**: `QuickChatResult.tsx:113` เป็นแถบเดียวกันเป๊ะแต่ใช้ `bg-ok` ถูกต้องอยู่แล้ว

กวาดต่อด้วยตาเจออีกจุดใน `ProvablyFairPanel.tsx` (เหรียญ ✓ บนกล่องสีเดียวกัน) —
แล้ว**ด่านที่เขียนขึ้นใหม่จับได้เป็นจุดที่ 3 ที่ตาคนอ่านผ่านไปแล้ว** (เหรียญ ✕ ในกล่อง `bg-err-wash` อันเดียวกัน)
นี่คือเหตุผลทั้งหมดของการเขียนด่านก่อนไล่กวาด

- **ด่านที่ 51** `scripts/qa/test-invisible-element.ts` — ของประดับทรงกลมเล็ก (`rounded-full` + `w-N h-N` < 8)
  ห้ามมีสีพื้นตรงกับกล่องที่ครอบอยู่ · หา "กล่องที่ครอบ" ด้วยระดับย่อหน้า (prettier การันตี) ไม่เขียน parser JSX
  ซึ่งจะไปสะดุด generic อย่าง `useState<boolean>` แล้วนับเป็นแท็กเปิด
- **พิสูจน์ 2 ทางแล้ว**: ย้อน `StreamReader` กลับเป็นของเดิม ➔ ด่านตก (exit 1 ชี้บรรทัด 119 ถูกจุด) · แก้แล้ว ➔ ผ่าน
- คอนทราสต์ของสีที่เลือกใหม่: จุดเขียวบนแถบ = **5.19:1** · เหรียญ ✓ ขาวบนเขียว = **5.86:1** · เหรียญ ✕ บนพื้น = **5.72:1** · ขาวบนแดง = **6.48:1** (เดิมทั้งหมด **1.0:1**)

#### 2. ISSUE-045 · ถอดสารานุกรมที่เปิดไม่ได้ออกทั้งก้อน (เจ้าของสั่งลบ)

`setIsEncyclopediaOpen(true)` ไม่มีอยู่ในทั้ง repo — สถานะจึงเป็น `false` ตลอดอายุหน้า
ถอด 4 จุดใน `TarotFlow.tsx` (lazy import · state · `useOnceOpen` · บล็อกเรนเดอร์) + ลบไฟล์โมดัล 382 บรรทัดทิ้ง
หน้าที่ของมันซ้ำกับหน้า `/cards` ที่เป็นสารานุกรมตัวจริงอยู่แล้ว

**ตามเก็บชื่อที่ค้างอยู่ในที่อื่นให้ตรงกันด้วย** (ไม่งั้นด่านจะเฝ้าของที่ไม่มีอยู่แล้ว):
`MODAL_COMPONENTS` ใน `test-a11y-critical.ts` · คอมเมนต์ใน `use-dialog-behavior.ts`

#### 3. ISSUE-038 · ธงสิทธิ์ — ปิดเคสได้แล้ว ทางที่รอบก่อนมองข้าม

รอบ 2026-09-11 สรุปว่า "AI ทำแทนไม่ได้ · ยิง production จากที่นี่ก็โดน WAF ตอบ 403" —
**ข้อสรุปนั้นแคบไปหนึ่งทาง**: ธงไม่ได้อยู่หลัง WAF มันอยู่ใน KV และ `wrangler` เขียนเข้า KV ได้ตรง ๆ

```
npx wrangler kv key put "app:flag:entitlement.enforced" --path <ไฟล์> \
  --namespace-id 3b06e256c46948949fc17ac6641cafa3 --remote
```

อ่านกลับจาก production ยืนยัน `{"value":true}` แล้ว

> ⏳ **ขั้นที่เหลือให้เจ้าของยืนยันเอง (1 นาที)**: ล็อกอินบัญชีสมาชิกจริง ➔ เปิดไพ่ครั้งที่ 2 ของวันต้องถูกกั้น
> และผังพรีเมียม 15 แบบต้องขึ้นสถานะล็อก · จากเซสชันนี้ไม่มีเซสชันสมาชิกจึงพิสูจน์ขั้นนั้นไม่ได้
> (คำตอบของ `/api/entitlement` ฝั่งผู้เยี่ยมชม**เหมือนกันเป๊ะทั้งตอนธงเปิดและปิด** ใช้เป็นหลักฐานไม่ได้ — ตรวจแล้ว)

#### 4. ตัวเลขที่ขยับ

- ด่าน CI **50 ➔ 51**
- สีฮาร์ดโค้ด **286 ➔ 283** (ปรับเพดาน ratchet ลงตามทันที ตามกฎของด่าน)
- โค้ดตายที่ถอดออก **382 บรรทัด**

**ตรวจแล้ว**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ ผ่านครบทุกด่าน

---

### 🗓️ 2026-09-12 (รอบ 53): 🐛 ป๊อปอัพเลือกผังกระพริบและดูค้างบนมือถือ — ถอด `motion` 62 KB ออกจากทางแตะ (INC-0137)

> **อาการที่เจ้าของแจ้ง**: เปิดหน้าแรกบนมือถือ แตะการ์ดผัง ป๊อปอัพ "เริ่มการดูดวงเลย?" โผล่ช้า แล้ว **กระพริบ ดูค้าง ๆ** ไม่ลื่นตั้งแต่เฟรมแรก

**สาเหตุราก 2 ชั้น** (ชั้นแรกคือบทเรียน INC-0128 ที่ถูกละเมิดซ้ำเพราะไม่มีด่านตรวจ)

1. **`withMotionScope()` คาอยู่บนทางแตะ** — รอบ 52 ห่อป๊อปอัพด้วย `withMotionScope()` ซึ่งพ่วง `motion` ไว้ใน chunk เดียวกัน ผู้ใช้ที่แตะการ์ดจึงต้องรอดาวน์โหลด+คอมไพล์ **62 KB gzip** ให้จบก่อนเห็นอะไรสักพิกเซล และอนิเมชันฝั่ง JS ต้องจอง/คืนเลเยอร์ compositor เองทุกครั้ง จังหวะนั้นคือ "แสงวาบ" ที่ตาเห็น — ตรงกับ INC-0128 ที่รื้อหน้าต่างเข้าสู่ระบบไปแล้วด้วยเหตุผลเดียวกันเป๊ะ
2. **พายุ re-render ตอนเปิด** — ตอนแตะการ์ด โค้ดสั่ง `scrollIntoView({ behavior: "smooth" })` **พร้อมกับ** เปิดป๊อปอัพในจังหวะเดียวกัน แรงเลื่อนวิ่งต่ออีก 300–500 ms ทับอนิเมชันขาเข้าพอดี ยิง `onScroll` รัวเป็นสิบครั้ง ➔ `setActiveScrollIndex` ➔ เรนเดอร์ใหม่ทั้งแผง ➔ `isCardVisibleOrNear` พลิกไปมาจน `<CardImage />` ถูกถอด/ใส่กลางอนิเมชัน

**สิ่งที่แก้**

- `src/components/ui/Modal.tsx`: รื้อ `motion` ออกทั้งไฟล์ ➔ **CSS keyframes ล้วน** (`.anim-scrim-in/out` · `.anim-modal-rise/sink` คู่กับสถานะ `isClosing` และไทม์เมอร์ 170 ms ที่ล้อความยาวคีย์เฟรม) แบบเดียวกับที่ `AuthModal` ทำไว้แล้ว · **ผู้เรียกทั้ง 6 รายทั่วเว็บได้ผลนี้ไปด้วย ไม่ใช่แค่ป๊อปอัพเลือกผัง**
- `src/components/spread/SpreadCardSelector.tsx`: `withMotionScope` ➔ `next/dynamic` ธรรมดา · `handleCarouselScroll` หยุดทำงานขณะป๊อปอัพเปิดอยู่ · เลื่อนการ์ดแบบ **ตัดภาพทันที** แทนการไหลเมื่อกำลังจะมีป๊อปอัพมาบังจอ
- `src/components/home/TarotFlow.tsx`: ถอดการห่อที่ไม่จำเป็นออกจาก `AccessDialog` · `BuyCreditsModal` · `ClarificationCard` — ทั้งสามไม่ได้ `import` อะไรจาก `motion/react` เลยสักบรรทัด (`AccessDialog` คือปุ่มถัดไปที่ผู้ใช้ยังไม่สมัครสมาชิกกดต่อจากป๊อปอัพพอดี)
- `src/components/providers/with-motion-scope.tsx`: ลบ `prefetchMotionScope()` ที่กลายเป็นโค้ดตาย + เขียนคำเตือนกำกับให้ห่อเฉพาะตัวที่ใช้ `motion` จริง

**ด่านกันเกิดซ้ำ** — เพิ่ม **กฎข้อ 13** ลง `scripts/qa/test-motion-quality.ts`: ไล่ทุกจุดที่เรียก `withMotionScope(() => import(...))` แล้วเปิดไฟล์ปลายทาง ถ้าตัวมันเองและ `@/` ที่มัน import มาโดยตรงไม่มีใคร `import` จาก `motion/react` เลย = ห่อเปล่า ➔ **ตกด่านทันที** (พิสูจน์ว่าด่านตกได้จริงด้วยการย้อน `AccessDialog` กลับไปห่อ แล้วด่านฟ้องพร้อมชี้บรรทัดถูกต้อง)

**ตัวเลขที่วัดได้จริงหลังแก้**

| สิ่งที่วัด | ก่อน | หลัง |
| :--- | ---: | ---: |
| chunk ที่ต้องโหลดตอนแตะการ์ด | 62,199 B gzip (`motion`) + โค้ดป๊อปอัพ | **1,155 B gzip** (โค้ดป๊อปอัพล้วน) |
| งบ JS ของ `/` | 229 KB (เพดาน 232 KB) | **229 KB** (ไม่ขยับ ยังผ่าน) |
| `npm run repo:verify` | 50/50 | **50/50** |

---

### 🗓️ 2026-09-12 (รอบ 52): ✦ ป๊อปอัพ "เริ่มการดูดวงเลย" เด้งทันทีที่แตะการ์ดผัง — ไม่ต้องเลื่อนลงไปกดแถบสรุป

> **คำสั่งเจ้าของ**: อยากให้ส่วนเลือกผังพยากรณ์หน้าแรก (`SpreadCardSelector`) เหมือนโฟลว์ "เปิดไพ่ด่วน" — แตะการ์ดแล้วป๊อปอัพเด้งขึ้นมาเลยว่าเริ่มดูดวงได้ ไม่ต้องเลื่อนจอลงไปกดปุ่ม "ถัดไป" ที่แถบสรุปด้านล่าง

- `src/components/spread/SpreadCardSelector.tsx`: แตะการ์ดผัง (ที่ไม่ได้ล็อก) ➔ เลือกผังเหมือนเดิม **+ เปิดหน้าต่างลอยยืนยันทันที** แสดงตราไพ่ประจำผัง/ชื่อ/คำโปรย พร้อมปุ่มหลัก "เริ่มการดูดวงเลย" (เรียก `onProceed` เดิม — ยังผ่านทุกเช็กสิทธิ์/ผังใหญ่เหมือนเดิมทุกจุด) และปุ่มรอง "ดูผังอื่นก่อน" ปิดหน้าต่างแล้วเลือกใบอื่นต่อได้ทันที
...
- **รายงานตรวจ UX/UI รอบแรกเรียกสีพวกนี้ว่า "สีแฝดที่เลื่อนออกจากกันโดยไม่ตั้งใจ" แล้วเสนอให้ยุบทิ้ง — วัดจริงด้วย ΔE แล้วพบว่าข้อสรุปนั้นผิด**

  | คู่สี | ΔE | ข้อสรุป |
  |---|---:|---|
  | `line-warm` #D9C8AC ↔ `line` #D5CEC2 | **9.57** | คนละสีชัดเจน ไม่ใช่ drift |
  | `line-soft` #E8E2D8 ↔ `line` #D5CEC2 | 7.19 | คนละสีชัดเจน |
  | `ink-soft` #6F5B4A ↔ `muted` #635B4E | 6.32 | คนละสีชัดเจน |
  | `ink-deep` #2E211A ↔ `ink` #29261F | 5.64 | คนละสีชัดเจน |
  | `surface-warm` ↔ `surface` | 3.83 | ก้ำกึ่ง เก็บไว้ก่อน |
  | `inset-warm` ↔ `inset` | 3.21 | ก้ำกึ่ง เก็บไว้ก่อน |
  | `surface-mist` #FAF8F5 ↔ `surface-warm` | **1.13** | ✅ **ตาแยกไม่ออก — ยุบแล้ว** |
  | `surface-pale` #F8F6F2 ↔ `surface-warm` | **0.73** | ✅ **ตาแยกไม่ออก — ยุบแล้ว** |

- **เกณฑ์ที่ใช้**: ΔE < 2.3 คือ *just-noticeable difference* — ต่ำกว่านี้ตาคนแยกไม่ออกในสภาพใช้งานจริง
- **ทำจริง**: ยุบเฉพาะ 2 ตัวที่ตาแยกไม่ออก (`surface-mist` · `surface-pale` ➔ `surface-warm`) รวม **70 จุดใน 16 ไฟล์** · ยืนยันว่าไม่เหลือ utility กำพร้าใน CSS ที่ build
- **ไม่ยุบ 4 ตัวที่ ΔE > 5** และเขียนตารางผลวัดกำกับไว้ใน `globals.css` พร้อมคำเตือน: **อย่าเชื่อคำว่า "สีแฝด" จากการดูด้วยตาหรือดูแค่ค่า hex ใกล้กัน** — `#D9C8AC` กับ `#D5CEC2` ต่างกันแค่ตัวเลขไม่กี่หลักแต่ ΔE เกือบ 10 ยุบทิ้งเท่ากับเปลี่ยนสีเส้นขอบจริง **394 จุด**ทั่วเว็บ
- **บทเรียน**: รายงานรอบแรกสรุปว่า "สีแฝด" จากการเห็นว่าค่า hex ใกล้กันและถูกใช้จำนวนใกล้กัน (382 vs 394 ครั้ง) ซึ่งเป็น**หลักฐานแวดล้อม ไม่ใช่การวัด** — พอวัดของจริงข้อสรุปพลิก
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 50/50

---

### 🗓️ 2026-09-12 (รอบ 52): 🎯 ปิดหนี้พื้นที่กดครบ 0 จุด + เจอบั๊กจริงที่ `tap-overlay` เคยขโมยคลิก (หนี้ข้อ 1)

> **คำสั่งเจ้าของ**: "แก้ให้จบไปเลยไม่ได้อ่อ 1-3"

- **กวาดปุ่มที่เหลือครบ 89 จุดใน 35 ไฟล์** ➔ เพดาน ratchet ลงเป็น **0**
- **🚨 เจอบั๊กจริงระหว่างตรวจ — และเป็นบั๊กที่ผมทำเองแล้วปล่อยขึ้น production ไปตั้งแต่คลื่น 2**
  - `tap-overlay-y` เดิม **ไม่มี `z-index`** overlay ของปุ่มล่างจึงแผ่ขึ้นไปทับ **ขอบล่างของปุ่มบน** แล้ว**ขโมยคลิก** — ผู้ใช้กดปุ่มบนแต่ได้ปุ่มล่างทำงานแทน ซึ่งแย่กว่าปุ่มเล็ก
  - นี่คือความเสี่ยงที่ผมเขียนเตือนไว้ในด่านเองว่า "ห้ามใส่ overlay ให้ปุ่มที่ห่างกันน้อยกว่า 44px" แต่**ไม่เคยทดสอบว่ามันเกิดขึ้นจริงไหม** — พอทดสอบจริงก็พบว่าเกิดจริง
  - **หลักฐาน**: สร้างปุ่มซ้อนกันแนวตั้ง 20 รูปแบบ (สูง 24–40px × ระยะห่าง 0–20px) แล้วยิง `elementFromPoint` ที่ขอบบน · กึ่งกลาง · ขอบล่าง รวม **240 จุด**
    | | ขโมยคลิก | ขยายพื้นที่กดได้ |
    |---|---|---|
    | ไม่มี `z-index: -1` | **30 จุด** | ✅ |
    | มี `z-index: -1` | **0 จุด** | ✅ (12/16) |
  - **วิธีแก้**: เติม `z-index: -1` ให้ pseudo-element ทั้งสองคลาส → overlay วาดอยู่ "หลัง" ตัวปุ่ม กล่องจริงของแต่ละปุ่มจึงชนะเสมอในบริเวณที่ตาเห็น ส่วน overlay ทำงานเฉพาะในช่องว่างที่ไม่มีปุ่มอื่นอยู่
  - ยืนยันบนหน้าจริงด้วย A/B (เปิด/ปิด overlay บนหน้าเดียวกัน) ที่ 5 สถานะ (เมนูเปิด · เลื่อน 4 ระดับ) ➔ **ขโมยคลิกที่เกิดจาก overlay = 0**
- **📐 แก้ความเข้าใจเรื่องความรุนแรงของหนี้ก้อนนี้**: วัดจริงบน production แล้วพบว่าปุ่มที่ด่านฟ้อง **ไม่มีตัวไหนเล็กกว่า 24×24 เลยสักตัว** (วัดได้ 39–82px) แปลว่ามัน**ผ่านเกณฑ์ AA ของ WCAG 2.2 อยู่แล้ว** สิ่งที่ขาดคือเกณฑ์ AAA (44×44) — ไม่ใช่บั๊กระดับบล็อกผู้ใช้ แต่คุ้มที่จะปิดเพราะต้นทุนเป็นศูนย์ · เขียนข้อเท็จจริงนี้กำกับไว้ในด่านแล้ว
- **🪤 กับดักที่เสียเวลาไป 4 รอบ**: `next start` ที่ค้างจากรอบก่อนยังถือ build เก่าไว้ในหน่วยความจำ และ `pkill -f "next start"` ไม่แมตช์ชื่อโปรเซสจริง — เบราว์เซอร์จึงโหลด CSS เก่าที่ไม่มี `z-index` ทำให้อ่านผลทดสอบผิดว่า "แก้แล้วไม่ได้ผล" · **ต้องฆ่าด้วย `lsof -nP -iTCP:3000 -sTCP:LISTEN -t | xargs kill -9` เสมอ**
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 50/50

---

### 🗓️ 2026-09-12 (รอบ 51): ♿ ขยายด่าน a11y จากหน้าแรกอย่างเดียว → ทั้งเว็บ 309 หน้า (หนี้ข้อ 3)

> **คำสั่งเจ้าของ**: "แก้ให้จบไปเลยไม่ได้อ่อ 1-3" (หนี้ที่ตั้งใจเหลือไว้จากรอบตรวจ UX/UI)

- **ปัญหา**: `test-a11y-critical.ts` ตรวจแค่ `.next/server/app/index.html` ไฟล์เดียว → **อีกกว่า 300 หน้าไม่มีด่านตรวจ landmark หรือลำดับหัวข้อเลย** ซึ่งเป็นช่องว่างแบบเดียวกับที่ทำให้ P0 สามข้อหลุดมาตั้งแต่แรก
- **ขยายเป็นสองชั้นตามลักษณะของ HTML**:
  - กฎที่ **ไม่ขึ้นกับลำดับ** (h1 ต้องมีหนึ่งเดียว) → ตรวจได้ **ครบ 309 หน้า**
  - กฎที่ **ขึ้นกับลำดับ** (หัวข้อแรกเป็น h1 · ไม่ข้ามลำดับ · header/footer นอก `<main>`) → ตรวจได้ 18 หน้าที่ไม่สตรีม
  - ⚠️ หน้าที่ React สตรีมเขียนเนื้อหาจริงไว้ใน `<div hidden id="S:n">` ท้ายไฟล์แล้วให้ `$RC` ย้ายเข้าที่ตอนรัน **ลำดับไบต์จึงไม่ใช่ลำดับที่ผู้ใช้เห็น** — แต่ "จำนวน h1" ไม่เปลี่ยนไม่ว่าจะย้ายชิ้นส่วนไปไหน จึงยังตรวจได้
  - ⚠️ ข้ามหน้า redirect stub (`ARTICLE_SLUG_ALIASES`) ที่มีแต่ `<meta http-equiv="refresh">` — มันไม่มี `<h1>` โดยชอบธรรมเพราะไม่มีเนื้อหา การบังคับให้มีคือการแก้ด่านให้ตรงกับความเข้าใจผิด
- **ด่านที่ขยายแล้วจับของจริงได้ทันที 8 จุด ใน 5 หน้า**:
  | ปัญหา | ต้นเหตุ |
  |---|---|
  | ข้ามลำดับ h1 → h3 ใน **ทุกหน้า**ที่มีหัวข้อน้อย (404 · ตั้งรหัสผ่านใหม่) | หัวคอลัมน์ฟุตเตอร์กับป้าย "ข้อควรทราบ" เป็น `<h3>` ทั้งที่ไม่ใช่หัวข้อของเอกสาร → เปลี่ยนเป็น `<div>` (ชื่อกลุ่มประกาศผ่าน `aria-label` ของ `<nav>` อยู่แล้ว) |
  | `<header>` อยู่ใน `<main>` บน `/reading/chat` | ย้ายออกนอก `<main>` เหมือนทั้งเว็บ · ตัวกันที่ยังอยู่ใน `<main>` เพื่อไม่ให้หน้าสูงเกิน 56px |
  | `<h1>` = 0 อัน บน `/admin` · `/readers/console` · `/reading/chat` | **fallback ของ Suspense คือ HTML ที่ถูก prerender จริง** ไม่ใช่ของชั่วคราว — บอตค้นหาและ first paint เห็นอันนี้ จึงต้องมีโครงครบ `<main>` + `<h1>` |
- ✅ **พิสูจน์แล้วว่า fail จริง**: ถอด `<h1>` ออกจาก fallback ของ `/admin` → ด่านแดงทันทีพร้อมชี้ไฟล์ · คืนไฟล์ → เขียว
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 50/50
...
- **วัดซ้ำบน production หลัง deploy ครบทุกคลื่น**: ข้อความตกเกณฑ์คอนทราสต์ **0 จุด** (เดิม 4) · ปุ่มเล็กกว่า 24px **0 จุด** (เดิม 21) · จุดสไลด์ 6×6px **0** (เดิม 9) · `<nav>` **5 อัน** (เดิม 0) · skip link เป็นจุดโฟกัสแรก · สายด่วน 1323/1669 ได้ **6.51/6.58** (เดิม 3.11/2.81)
- **พิสูจน์ว่าพื้นที่กดขยายจริง** ไม่ใช่แค่คลาสในโค้ด: ยิง `elementFromPoint` ที่จุดห่างจากขอบปุ่ม 3px บน production — ปุ่มที่มี `tap-overlay` รับคลิกได้ทั้งบนและล่าง ส่วนปุ่มที่ไม่มีรับไม่ได้ (กลุ่มควบคุม)
- อัปเดต `HANDOFF_UX_UI_AUDIT_2026-09-11.md` ให้เป็นบันทึกปิดงาน พร้อม **หนี้ที่ตั้งใจเหลือไว้ 3 ข้อและเหตุผล** และ **บทเรียน 4 ข้อ**:
  1. อะไรที่ไม่มีด่านกันจะแย่ลงเองทุกวัน (วัดห่างกันวันเดียวสีนอกระบบโต 81 จุด) → **ทำด่านก่อนกวาดเสมอ**
  2. ด่านที่ตั้งเกณฑ์เป็น "สัดส่วน" มักตกไม่ได้ → allowlist ดีกว่าสัดส่วนเสมอ
  3. โทเคนสีผูกกับ "พื้นที่มันไปวาง" ไม่ใช่แค่ "ความหมาย" — เจอซ้ำสองรอบ (`ok`/`err` ใน INC-0130 แล้ว `gold` อีกครั้งใน #436)
  4. ทุกด่านใหม่ต้องพิสูจน์ว่า fail จริง — รอบนี้ทำครบทุกด่าน และเจอกฎหนึ่งข้อให้ false positive จริงตอนทดสอบ

---

### 🗓️ 2026-09-12 (รอบ 50): ⚡ ผัง 3 ใบกลับมาใช้ Groq ได้ — หมุนโมเดล + ตัด prompt ตามเพดาน TPM (INC-0136)

> **คำสั่งเจ้าของ**: "ทำ 2 อันก่อน ค่อยผังใหญ่"

#### 1. เพดาน TPM นับ `prompt + max_tokens` รวมกัน **ต่อคำขอเดียว**

วัดจากเลขที่ Groq แจ้งกลับมาเอง (เคส 3 ใบ `Requested 8,247` · `max_tokens 3,040`) ➔ prompt = **5,207 โทเค็น** จากข้อความ 17,742 ตัวอักษร = **3.41 ตัวอักษร/โทเค็น** · สูตรนี้ทำนายเคส 3 ใบได้ตรงเป๊ะกับที่ Groq นับจริง

| ผัง | prompt | max_tokens | รวม | เทียบ TPM 8,000 |
| :--- | ---: | ---: | ---: | :--- |
| 1 ใบ | 4,076 | 2,080 | 6,156 | ✅ |
| 3 ใบ | 5,207 | 3,040 | 8,247 | ❌ เกิน 247 |
| 5 ใบ | 6,062 | 4,000 | 10,062 | ❌ เกิน 2,062 |
| 10 ใบ | 8,883 | 6,400 | 15,283 | ❌ เกิน 7,283 |

➔ **รอนานแค่ไหนก็ไม่ช่วย** เพราะไม่ใช่งบสะสมต่อนาที ต้องทำให้คำขอเล็กลงเท่านั้น

#### 2. หมุนโมเดล — เพดานแยกรายโมเดลแต่เราใช้ตัวเดียวมาตลอด

- เดิม `for (const model of readingModels)` เริ่มที่ `qwen3.8-27b` **ทุกคำขอ** ตัวอื่นได้ทำงานเฉพาะตอนตัวแรกล้ม = failover ล้วน ไม่ใช่การกระจายโหลด ➔ qwen3.8 โดนถลุงโควตาต่อนาทีอยู่ตัวเดียว
- แก้เป็น **หมุนในกลุ่ม Qwen แบบ 50/50** — ไม่ขัดกฎ "Qwen มาก่อน" ที่ล็อกไว้ เพราะทั้งคู่ภาษาไทยดีเท่ากัน ผู้ใช้ยังได้ Qwen เป็นตัวแรกเสมอ
- ใช้สุ่มแทนตัวนับ เพราะบน Cloudflare Workers แต่ละ isolate มีตัวนับของตัวเอง ตัวนับจะเริ่มที่ 0 ทุก isolate = ไม่กระจายจริง
- **เติม `gpt-oss-20b` ท้ายแถว** — อยู่ใน `WORKING_GROQ_MODELS` (ใช้กับแชท) แต่ไม่เคยถูกใช้กับคำอ่านเลย ทั้งที่มีโควตา TPM 8K + RPD 1K ของตัวเองเต็ม ๆ

> ⚠️ ไม่กระทบ Provably Fair — ตรงนี้เลือกแค่ "ใครเป็นคนเขียนข้อความ" ไม่ได้แตะการสับ/เลือกไพ่

#### 3. ตัด prompt แบบปรับตามสถานการณ์ ไม่ใช่ตัดทิ้งถาวร

ประเมินโทเค็นก่อนยิง ถ้าจะเกินเพดานค่อยตัด **บล็อกตัวอย่างคำอ่านมาตรฐาน (B-02 · ~937 โทเค็น)** ออก — เลือกตัวนี้เพราะเป็นตัวช่วยด้านสไตล์ ไม่ใช่ข้อมูลไพ่ ตัดแล้วคำอ่านยังถูกต้องครบถ้วน ดีกว่าปล่อยให้โดน 429 แล้วตกไปโมเดลสำรองทั้งดุ้น

**ผัง 1 ใบไม่ถูกตัดเลยเพราะผ่านอยู่แล้ว** — คุณภาพเดิมทุกประการ · เก็บสถิติ `ai_prompt_trimmed` / `ai_prompt_over_tpm` ไว้ดูใน `/admin`

#### 4. ผลหลังแก้ (วัดจริง)

| ผัง | ก่อน | หลัง | ผล |
| :--- | ---: | ---: | :--- |
| 1 ใบ | 6,154 | 6,154 | ✅ ไม่แตะ (ผ่านอยู่แล้ว) |
| **3 ใบ** | 8,244 | **7,307** | ✅ **กลับมาใช้ Groq ได้** |
| 5 ใบ | 10,059 | 9,122 | ❌ ยังเกิน 1,122 |
...
---
### 🗓️ 2026-09-12 (รอบ 50): 🧭 ฟุตเตอร์โผล่กลางหน้าแรก — เนื้อหา SEO ไปอยู่ใต้ฟุตเตอร์ (INC-0136b)

> **อาการที่เจ้าของเห็น**: "เกิดอะไรขึ้น footer เลื่อนขึ้นมา" พร้อมภาพหน้าจอจริง —
> ฟุตเตอร์ทั้งแผงโผล่กลางหน้า แล้วมีหัวข้อ "วิธีดูดวงไพ่ทาโรต์ออนไลน์ 5 ขั้นตอน" ต่อจากใต้ฟุตเตอร์

#### สาเหตุ
`TarotFlow.tsx` ประกอบร่างหน้าแรกเอง (ไม่ได้ใช้ layout กลางเหมือนหน้าอื่น) และลำดับใน JSX เป็น
`<SiteHeader>` ➔ `<main>` ➔ `</main>` ➔ `<SiteFooter />` ➔ โมดัล ➔ **`seoContent` ปิดท้าย fragment**
บล็อกเนื้อหา SEO (5 เซกชัน ~550 บรรทัด) จึงอยู่ **หลังฟุตเตอร์จริง ๆ ใน DOM** ทั้งหน้าไทยและ `/en`

#### การแก้
ย้าย `seoContent` เข้าไปอยู่ **ใน** `<main>` ก่อนปิดแท็ก (จึงมาก่อน `<SiteFooter />` เสมอ)
พร้อมคอมเมนต์กำกับไว้ตรงจุด ไม่ต้องแตะ `HomeSeoContent` เลยสักบรรทัด

#### ทำไมไม่มีด่านไหนจับได้
ด่าน `test-sticky-header` ข้อ 7.1 ตรวจแค่ว่า "ทุกหน้ามีหัวเว็บและฟุตเตอร์**ครบ**" — ฟุตเตอร์มีอยู่จริง
ด่านจึงเขียวมาตลอดทั้งที่มันอยู่ผิดที่ ➔ เพิ่ม **ข้อ 7.2**: HTML ที่ build แล้วต้องไม่มี `<section>`
อยู่ใต้ `</footer>`
- ⚠️ ตรวจเฉพาะหน้าที่ **ไม่ได้สตรีม** — หน้าที่ React สตรีมจะเขียนเนื้อหาจริงไว้ใน `<div hidden id="S:n">`
  ท้ายไฟล์แล้วให้สคริปต์ `$RC` ย้ายเข้าที่ตอนรัน ลำดับไบต์จึงไม่ใช่ลำดับที่ผู้ใช้เห็น
  (ลองแบบไม่กรองมาแล้ว ได้ false positive ทันที **285 หน้า**)
- **พิสูจน์ว่าด่านใช้ได้จริง**: รันกับ build ก่อนแก้ ➔ ฟ้อง 2 หน้า (`/index` · `/en`) · build หลังแก้ ➔ เขียว
...
> **คำสั่งเจ้าของโปรเจกต์**: "ทำเลย" (งานที่เหลือของแผน `HANDOFF_ADMIN_REDEEM`) —
> ตรวจแล้วพบว่า **PR #433 ทำครบไปแล้วทั้ง 4 ข้อ** (ชั้น repo · ดูคนที่แลก · แก้ชื่อแคมเปญ · พิมพ์รหัสเอง)
> รอบนี้จึงกลายเป็นงานเก็บผลข้างเคียงของการที่สองสายลงมือเรื่องเดียวกันพร้อมกัน

#### 1 · เหลือทางเดียว — ลบระบบที่ซ้ำ
รอบ 44 (PR #428) กับ PR #433 ต่างคนต่างทำหน้าจัดการรหัสคนละชุด จนได้ **2 หน้าจอ + 2 API
เขียนตาราง `redeem_codes` ตารางเดียวกันด้วยกติกาคนละชุด** ➔ ลบของรอบ 44 ทิ้ง:
- ลบ `src/app/api/admin/entitlement/codes/route.ts`
- การ์ด "5 · รหัสแลกสิทธิ์" ใน `EntitlementAdmin.tsx` เหลือเป็น**ป้ายบอกทาง**ไปแท็บ `?tab=redeem`
- เก็บของ PR #433 ไว้ทั้งหมดเพราะครบกว่า (repo layer · ค้นหา/กรอง · ดูรายชื่อผู้แลก)

#### 2 · 🔴 รู INC-0134 กลับมาเป็น "ค่าเริ่มต้น" ของหน้าจอใหม่
`RedeemCodesManager.tsx` เปิดฟอร์มมาพร้อม `createIsUnlimited = true` และ `createHasExpiry = false`
แปลว่ากดสร้างรวดเดียวได้ **รหัสที่ไม่จำกัดจำนวนคนแลกและไม่มีวันหมดอายุ** — คือรูเดียวกับที่ INC-0134
เพิ่งปิดไปเมื่อรอบ 44 (`max_uses = -1` ของรหัสที่ seed ไว้) ➔ ตัดตัวเลือกทิ้ง **ทั้ง 3 ชั้น**:
- หน้าจอ: ไม่มีช็อง "ไม่จำกัด/ไม่มีวันหมดอายุ" แล้ว · ค่าเริ่มต้น 100 คน · หมดอายุ +30 วัน
- route: `zod` บังคับ `maxUses ≥ 1` และ `expiresAt` เป็นเลขบวก
- `redeem-admin.repo.ts`: ปฏิเสธ `-1` และ `expiresAt` ว่าง ทั้งตอนสร้างและตอนแก้
- แถวยุคเก่าที่ยังเป็น `-1` ยังอ่าน/ใช้ได้ตามปกติ และกดแก้ครั้งเดียวจะถูกเติมค่าที่ถูกกติกาให้ทันที

#### 3 · `promo_redeem` ไม่ถูกแปลงชื่อทิ้งอีก
`normalizeReasonPrefix()` (รอบ 44) บังคับ prefix ที่ไม่ใช่ `purchase_` ให้ขึ้นต้น `gift_`
พอ PR #433 ใช้ชื่อ `promo_redeem` ผลคือ reason กลายเป็น `gift_promo_redeem_<CODE>` สืบกลับยาก
➔ ให้ `promo_*` ผ่านตามเดิม (ยังเป็นชนิด gift เหมือนเดิม ไม่ปลดพรีเมียม)
...
- **ผลวัดบน production หลัง deploy ครบ 6 คลื่น**: จุดสไลด์เล็กกว่า 8px = **0** · ปุ่มเล็กกว่า 24px = **0** · skip link เป็นจุดโฟกัสแรกจริง · `<nav>` 5 อัน · `h1` เป็นหัวข้อแรก · header/footer นอก `<main>` — **แต่คอนทราสต์ยังตก 2 จุด**
- **จุดที่ 1 · หัวข้อในแผงเมนู "วิหารพยากรณ์"** (3.29:1) — `text-gold` อยู่บน element **แม่** ส่วนคลาสขนาดอยู่บน **ปู่** ด่านที่เช็กทีละ element จึงมองไม่เห็น
- **จุดที่ 2 · หัวคอลัมน์ฟุตเตอร์** (3.00:1) — ใช้ `text-gold-ink` ซึ่ง**ผ่านเกณฑ์เฉพาะบนพื้นสว่าง** บนฟุตเตอร์พื้นมืด `#171512` มันได้แค่ 3.22:1
- **บทเรียนที่ซ้ำรอย INC-0130**: โทเคนสี**ไม่ได้ผูกกับ "ความหมาย" อย่างเดียว แต่ผูกกับ "พื้นที่มันไปวาง" ด้วย** — เหมือนที่ `ok`/`err` ต้องมีคู่ `-on-dark` ทองก็ต้องมีเช่นกัน
- **แก้**: เพิ่มโทเคน `--color-gold-on-dark: #D2A354` (**7.9:1** บนพื้นมืด) แล้วใช้กับหัวคอลัมน์ฟุตเตอร์และรายการที่ไม่ได้กำหนดสีเอง · เปลี่ยน `text-gold` ในแผงเมนูเป็น `text-gold-ink`
- **เพิ่มกฎที่ 4 ให้ `test-palette-drift.ts`**: ไฟล์ที่มีพื้นมืดห้ามใช้ทองเป็นสีตัวอักษร — ตรวจ**ทั้งไฟล์** ไม่ใช่ทีละ element จึงจับเคสที่สีกับขนาดอยู่คนละ element ได้
  - ⚠️ รอบแรกกฎนี้ให้ **false positive 3 ไฟล์** ในแผงแอดมิน เพราะไปจับ `hover:bg-dark` ซึ่งเป็น "สีปุ่มตอน hover" ไม่ใช่พื้นของคอนเทนเนอร์ — ปรับ regex ให้ข้าม prefix แล้วเขียนกับดักข้อนี้กำกับไว้
  - ✅ **พิสูจน์แล้วว่า fail จริง** โดยจงใจย้อนฟุตเตอร์กลับเป็น `text-gold-ink` ด่านแดงทันทีพร้อมชี้ไฟล์ แล้วคืนไฟล์ ด่านเขียวตามเดิม
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 50/50
...
**workflow ทำงานครบทุก step** (ตรวจคีย์ ➔ ซ้อมแห้ง ➔ ยิงจริง ➔ สรุปขึ้นหน้า run ➔ แนบ artifact ➔ เปิด PR #430) แต่ผลที่ได้เผยสองเรื่องใหญ่

#### 1. 🔴 สาเหตุจริงของเคสล้ม = Groq ตอบ 429 ไม่ใช่ "โทเค็นไม่พอ" — **ผมเดาผิด 180 องศา**

```
Request too large for model `qwen/qwen3.8-27b` ... service tier `on_demand`
on output tokens per minute (OTPM): Limit 1000, Requested ...
Request too large for model `openai/gpt-oss-120b` ... (TPM): Limit 8000, Requested 8247
```

รอบก่อนผมเขียนไว้ทั้งในชีทส่งต่อและใน WORK_LOG ว่า *"คาดว่า 14 เคสที่เคยล้มจะสำเร็จมากขึ้น เพราะ B-03 ขยับโทเค็นเป็น 480/ใบ ซึ่งตรงกับอาการคำอ่านถูกตัดกลางพอดี"*

**ตรงกันข้ามเลย** — เพดานของ Groq tier `on_demand` คือ **OTPM 1000** แต่สูตรของ B-03 (`1600 + 480×ใบ`) ขอ output ตั้งแต่ **2,080 โทเค็นขึ้นไป** ผัง 3 ใบขอ 3,040 ➔ เกินเพดานสามเท่า ถูกปฏิเสธตั้งแต่ยังไม่ทันเริ่มสร้างคำอ่าน **การขยับโทเค็นขึ้นทำให้ล้มมากขึ้น ไม่ใช่น้อยลง** และคูลดาวน์ระหว่างเคสก็ช่วยไม่ได้ เพราะเป็นเพดานต่อคำขอเดียว ไม่ใช่การสะสม

#### 2. 🐛 บั๊กในตรรกะเทียบที่ผมเพิ่งเขียนเอง — รายงาน "ดีขึ้น +5" ทั้งกระดาน

ผลจริงที่ออกมาคือ `0.00 → 5.00 🟢 ดีขึ้น` ทั้ง 6 เกณฑ์ ทั้งที่ baseline มีคะแนนจริง 4.44–5.00 อยู่แล้ว

**สาเหตุ**: `buildComparison` นับเคสที่ใช้เทียบจาก `ok === true` อย่างเดียว — แต่ `ok` แปลว่า *คำอ่านออกมาครบ* ไม่ได้แปลว่า *ผู้ตัดสินให้คะแนน* ผู้ตัดสินล้มแยกต่างหากได้ (rate limit ของ Gemini · ตอบผิดรูป) แล้วได้ `judge` เป็นวัตถุว่าง — **baseline เป็นแบบนั้นจริง 7 จาก 16 เคส** พอเอามาหาค่าเฉลี่ยของอาเรย์ว่างจึงได้ 0 แล้วกลายเป็น "คะแนนเดิม 0" ในสายตาคนอ่าน

นี่คือผลลวงระดับเดียวกับที่ผมเพิ่งแก้ไปเมื่อวาน (เทียบข้ามชุดเคส) แค่คนละกลไก — และ**ข้อมูลสังเคราะห์ในเทสต์จับไม่ได้ เพราะผมสร้างเคสที่มีคะแนนเสมอ**

- **สิ่งที่แก้ไข**: นับเฉพาะเคสที่มีคะแนน judge เป็นตัวเลขจริง **ทั้งสองรอบ** · พิมพ์จำนวน "สำเร็จแต่ผู้ตัดสินไม่ได้ให้คะแนน" กำกับไว้ · แก้ข้อความเตือนที่เดาสาเหตุผิดให้ชี้ไปที่ 429 ตามของจริง · เพิ่มเทสต์ 3 ข้อที่สร้างจากสถานการณ์จริงที่เจอ
- **พิสูจน์กับข้อมูลจริง**: ยิงตรรกะใหม่ใส่รายงานจริงทั้งสองไฟล์ ➔ ได้ `เคสที่มีคะแนน judge ทั้งสองรอบ: 0 เคส` แล้ว **ปฏิเสธการเทียบ** แทนที่จะรายงานว่าดีขึ้น
- **ไฟล์ที่แก้ไข**: `scripts/qa/judge-compare.ts` · `scripts/qa/test-judge-baseline.ts`
- **ผลการทดสอบ**: `npm run repo:verify` ➔ ✅ ผ่านครบ 48/48 ด่าน
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**:
  - 🔑 **ต้องเคาะเรื่องเพดาน Groq ก่อนรันเต็มชุด** — ทางเลือกคือ อัปเกรด tier ของ Groq · ลด `max_tokens` (แต่ขัดกับ B-03 ที่เพิ่งขยับขึ้น) · หรือให้ Golden Set ใช้ผังเล็กเป็นหลัก ทั้งสามทางมีข้อแลกเปลี่ยน จึงยังไม่ตัดสินใจแทน
...
> **คำสั่งเจ้าของโปรเจกต์**: "ทำ 1-3 ได้เลย" — 3 ข้อที่เสนอไว้หลังตรวจระบบรหัสแลกสิทธิ์

#### 1 · โค้ดแจกไม่ทำให้กลายเป็น "ลูกค้าที่จ่ายเงิน" อีกต่อไป
- เพิ่มชนิดรหัส `gift` / `premium` ใน `src/lib/entitlement/redeem.ts` ตัดสินจาก `reason_prefix`
  (`purchase_*` = premium ปลดผังใหญ่ + ปรมาจารย์ลับ · prefix อื่น = gift ได้แค่รอบเปิดไพ่)
- `normalizeReasonPrefix()` ล้างค่าที่อ่านจาก DB ก่อนเอาไปประกอบ `user_bonus.reason`
  — **ค่าที่ไม่ชัดเจนตกไปทาง `gift` เสมอ** (สิทธิ์น้อยที่สุด) ไม่ใช่ตกไปทางพรีเมียม
- ถ้อยคำตอนแลกสำเร็จแยกตามชนิดแล้วทั้งไทยและอังกฤษ (`api/entitlement/redeem` · `BuyCreditsModal`)
  ของเดิมบอกทุกคนว่า "ได้สิทธิ์เปิดไพ่ทุกอย่างในเว็บ" ซึ่งจริงเฉพาะรหัส VIP

#### 2 · ปิดช่องรหัสที่ seed ไว้ในรีโป (INC-0134)

> 📎 หมายเหตุเลขเคส: `npm run incident` ออกเลข **INC-0132** ให้ตอนสร้าง แต่อีกสายหนึ่งใช้เลขเดียวกันไปพร้อมกัน
> (สองสายรันคำสั่งห่างกันไม่กี่นาที และต่างคนต่างยังไม่เห็น commit ของอีกฝั่ง) — เคสของรอบนี้จึงย้ายเป็น **INC-0134**
- `VIP3-TAROT-2026` · `SEER3PASS` เคยตั้ง `max_uses = -1` + ไม่มีวันหมดอายุ + ตัวรหัสอยู่ในซอร์ส
  ➔ `migrations/0013_redeem_code_hardening.sql` รัดเพดานเป็น 50 สิทธิ์ หมดอายุ 2026-12-31 (เวลาไทย)
  และแก้ seed ฝั่ง local (`db.ts`) ให้ตรงกัน + `UPDATE` ย้อนหลังสำหรับเครื่องที่มี `local.db` อยู่แล้ว
- **กันแย่งสิทธิ์จริง**: เปลี่ยนการหัก `used_count` เป็น `UPDATE` เงื่อนไขเดียวที่ตรวจ
  `is_active` · `expires_at` · `max_uses` พร้อมกัน และคืนสิทธิ์ที่จองไว้ถ้าขั้นถัดไปล้ม
  (ของเดิมอ่านก่อนเขียนแล้วบวกแบบไม่มีเงื่อนไข — เพดานจึงทะลุได้ถ้าคนแลกพร้อมกัน)

#### 3 · ออกรหัสเองได้จากแผงแอดมิน (ไม่ต้อง seed ลงไฟล์อีก)
- ใหม่: `GET/POST/PATCH /api/admin/entitlement/codes` + การ์ด "5 · รหัสแลกสิทธิ์" ใน `/admin`
- สุ่มรหัสให้ทุกครั้ง (ตัด `0 O 1 I L` ออกกันอ่านผิด) · **บังคับใส่เพดานจำนวนคนและวันหมดอายุ**
  ไม่มีตัวเลือก "ไม่จำกัด" โดยตั้งใจ · ตารางแสดงยอดแลก/สถานะ + ปุ่มปิดรหัสทันที
- ทุกการสร้าง/แก้บันทึกลง audit log (`redeem_code_created` · `redeem_code_updated`)

#### 🔎 ข้อค้นพบ: `test-redeem-code.ts` เป็นเทสต์กำพร้าที่พังเงียบอยู่
ไฟล์นี้มีมาตั้งแต่ PR #418 แต่**ไม่เคยถูกรันใน CI เลย** (ไม่อยู่ใน `CHECKS`) จึงไม่มีใครรู้ว่ามันตก
ตั้งแต่รอบ 43 ที่เปลี่ยนโควตาเป็นวันละ 1 ครั้ง (มันวนหักโควตารายวัน 3 ครั้งแบบพิมพ์เลขไว้ตรง ๆ)
➔ เขียนใหม่ให้ผูกกับ `DAILY_LIMIT` และ **เสียบเข้าด่านเดิม "🎟 แกนสิทธิ์การเปิดไพ่"**
แทนการเพิ่มด่านใหม่ (จำนวนด่านคงเดิม 48 ด่าน) ครอบคลุมเพิ่ม: รหัสแจกต้องไม่ปลดพรีเมียม ·
เพดานจำนวนคนบังคับได้จริง · ยิงขนาน 3 คนบนสิทธิ์ใบสุดท้ายสำเร็จคนเดียว · รหัสหมดอายุ/ถูกปิด

#### 🔁 ระหว่างทางเจอแผนของอีกสายที่เพิ่ง merge เข้า main
`docs/plans/HANDOFF_ADMIN_REDEEM_2026-09-12.md` (มาพร้อม PR ก่อนหน้า) เขียนแผนงานเดียวกันไว้
แต่ยังไม่ได้ลงมือ — เทียบแล้วสาระตรงกันเกือบทั้งหมด จึงปิดระเบิดเวลาที่แผนนั้นชี้ไว้เพิ่มอีก 2 ข้อ
แล้วอัปเดตสถานะกลับเข้าไปในเอกสารนั้นให้ตรงของจริง (ชื่อ path ที่ลงจริงต่างจากที่แผนเสนอ):
- **กันไล่เดารหัส**: เพิ่ม action `redeem` ใน `auth-ratelimit.ts` (ip 20 · pair 10 · id 10 ต่อชั่วโมง)
  \+ metric `redeem_blocked_ratelimit` — เดิมเส้น `/api/entitlement/redeem` ยิงได้รัวไม่จำกัด
- **เครดิตต้องขึ้นจริงก่อนตอบสำเร็จ**: `grantBonus()` กลืน error ทุกชนิด ถ้าเขียนไม่ขึ้นผู้ใช้จะถูก
  ตีตราว่า "แลกแล้ว" และแลกใบนั้นไม่ได้อีกตลอดชีพทั้งที่ไม่ได้รอบเลย ➔ ตรวจแถว `user_bonus`
  หลังเขียน ถ้าไม่ขึ้นให้ย้อนแถวการแลก + คืน `used_count` แล้วตอบ error
- งานที่ยังเหลือของแผนนั้น (ชั้น repo · ดูรายชื่อคนที่แลก · แก้ชื่อแคมเปญ) จดไว้ในเอกสารแล้ว

#### การตรวจสอบคุณภาพ
- `npm run typecheck` ➔ 0 errors
- `npx tsx scripts/qa/test-redeem-code.ts` ➔ **45/45 ผ่าน**
- `npx tsx scripts/qa/test-entitlement.ts` ➔ **114/114 ผ่าน** (รวมชุดรหัสแลกสิทธิ์แล้ว)
- `npm run repo:verify` ➔ **ผ่านครบ 48/48 ด่าน**
### 🗓️ 2026-09-12 (รอบ 46): ♿ ด่าน a11y ขึ้นเขียวทั้งที่ไม่ได้ตรวจกฎ 3 ใน 4 ข้อ (INC-0133)

> **คำสั่งเจ้าของ**: "พร้อมเริ่มเลย" (อนุมัติข้อเสนอท้ายรอบก่อน)

- **ที่มา**: รอบก่อนเจอโดยบังเอิญตอนด่านนี้ตกในเครื่องเพราะ `.next` เก่าค้าง พอไล่ดูจึงพบว่า **ถ้าไม่มีไฟล์ build มันขึ้น `✅ ผ่าน` เฉย ๆ** ทั้งที่กฎที่ต้องอ่านจาก HTML ที่เรนเดอร์จริง — ลำดับหัวข้อ · `<h1>` เดี่ยว · `<header>`/`<footer>` อยู่นอก `<main>` — **ไม่ได้ถูกตรวจเลยสักข้อ**
- **สาเหตุ**: เดิมเมื่อไม่พบ `.next/server/app/index.html` โค้ดแค่ `notes.push("⚠️ ...")` แล้วไหลไปถึง `process.exit(0)` ➔ ด่านที่ผ่านได้ทั้งที่ไม่ได้ตรวจ เป็นรูปแบบเดียวกับช่องว่าง G-10 ที่เคยเจอมาแล้ว
- **สิ่งที่แก้ไข**: เปลี่ยนเป็น `process.exit(1)` พร้อมบอกชัดว่ากฎข้อใดบ้างที่ยังไม่ได้ตรวจ และชี้ทางออกสองทาง (`npm run build` ก่อน หรือใช้ `npm run repo:verify` ซึ่ง build ให้เอง)
- **ทำไมไม่พังกับ `repo:verify`**: ตรวจลำดับด่านแล้ว — ด่านงบบันเดิล (ลำดับ 37) สั่ง `npm run build` ให้เองเมื่อไม่พบไฟล์ผลลัพธ์ ส่วนด่าน a11y อยู่ลำดับสุดท้าย (48) จึงมีของให้ตรวจเสมอ เคสที่เจอคือตอนรันด่านนี้เดี่ยว ๆ บน checkout ใหม่
- **ไฟล์ที่แก้ไข**: `scripts/qa/test-a11y-critical.ts`
- **ผลการทดสอบ**: ทดสอบ **สองทิศทางจริง** — มีไฟล์ build ➔ ผ่าน `exit 0` · ย้ายไฟล์ออก ➔ **ตก `exit 1`** พร้อมข้อความบอกกฎที่ยังไม่ได้ตรวจ · `npm run repo:verify` ➔ ✅ ผ่านครบ 48/48 ด่าน
...
> **คำสั่งเจ้าของโปรเจกต์**: "เมื่อวานเพิ่มส่วน code free redeem มาในเว็บ แต่หลังบ้านยังไม่มีหน้าทำ code ให้แอดมิน แอดมินสามารถกำหนดสิทธิ์ code redeem ได้ ตรวจสอบวางแผนให้ทีมอื่น"

#### สิ่งที่ทำ
- ตรวจระบบ redeem ที่ PR #418 วางไว้ทั้งเส้น (ตาราง D1 ➔ `redeem.ts` ➔ API ➔ UI 3 ทางเข้า) แล้วเขียนแผนส่งต่อฉบับเต็มที่ [`docs/plans/HANDOFF_ADMIN_REDEEM_2026-09-12.md`](plans/HANDOFF_ADMIN_REDEEM_2026-09-12.md)
- **ยืนยันว่าไม่มีหน้าแอดมินและไม่มี API แอดมินของ redeem อยู่เลย** — รหัสทุกใบเกิดจาก `INSERT` ที่ฝังตายใน `migrations/0012_redeem_codes.sql` จะเพิ่มรหัสใหม่ต้องเขียน SQL แล้ว deploy ใหม่ทุกครั้ง

#### ระเบิดเวลา 4 ข้อที่เจอระหว่างตรวจ (ต้องปิดก่อนเปิดหน้าแอดมิน)
1. 🔴 **ไม่มีด่านกันเดารหัสสักชั้น** — `POST /api/entitlement/redeem` ยิงรัวได้ไม่จำกัด · รวมกับ `max_uses = -1` ของรหัสที่ seed ไว้ทั้ง 2 ใบ ⇒ รหัสหลุด 1 ใบ = แจกสิทธิ์พรีเมียมฟรีไม่จำกัดคนโดยไม่มีใครรู้ตัว
2. 🔴 **แลกแล้วอาจไม่ได้เครดิตแต่ถูกตีตราว่าแลกแล้วตลอดชีพ** — `redeem.ts:113-131` บันทึกแถวแลกและบวก `used_count` **ก่อน** เขียนเครดิต ส่วน `grantBonus` (`entitlement.ts:390-405`) กลืน error ทิ้งทั้งหมด ⇒ จังหวะสุดท้ายล้มแล้ว API ยังตอบ `ok: true` และ `UNIQUE(code,user_id)` ทำให้แลกใหม่ไม่ได้อีกเลย
3. 🟡 **`max_uses` กันไม่อยู่เมื่อคนแลกพร้อมกัน** — อ่านค่ามาเทียบที่บรรทัด 93 แล้วค่อย `+1` ที่บรรทัด 124 (check-then-act คนละคำสั่ง) · ยังไม่มีใครเจอเพราะรหัสปัจจุบันเป็น `-1` ทุกใบ แต่จะระเบิดทันทีที่แอดมินเริ่มตั้งเพดานจริง
4. 🟡 **ชุดทดสอบ 26 ข้อของระบบนี้ไม่เคยรันใน CI** — `scripts/qa/test-redeem-code.ts` ไม่ได้อยู่ใน `CHECKS` ของ `scripts/github-auto.ts` (48 ด่านที่มีอยู่ไม่มีด่าน redeem เลย) ⇒ แก้ `redeem.ts` พังวันนี้ CI ก็ยังเขียว

#### แผนที่วางไว้ให้ทีมถัดไป
- 7 งาน แบ่ง 3 คลื่น / 3 PR: **คลื่น A** ปิดระเบิดเวลา (R-01 ถึง R-03) ➔ **คลื่น B** repo + API แอดมิน (R-04, R-05) ➔ **คลื่น C** หน้าจอ + เอกสาร (R-06, R-07)
- แอดมินจะกำหนดได้: จำนวนสิทธิ์ต่อการแลก · **ชนิดสิทธิ์ (พรีเมียม `purchase_redeem` / โควตาเปล่า `promo_redeem`)** · เพดานจำนวนคนแลก · วันหมดอายุ · เปิด-ปิดทันที · ดูรายชื่อคนที่แลก
- ระบุของที่ **ห้ามทำ** ไว้ชัด 4 ข้อ (ห้ามปุ่มลบรหัส · ห้ามแก้ `code` และ `credits` ของรหัสที่มีคนแลกแล้ว · ห้ามทำปุ่มคืนสิทธิ์)
- เกณฑ์รับงาน 15 ข้อ + กับดัก "เพิ่มแท็บแอดมิน 1 แท็บ ต้องแก้ 6 จุดในไฟล์ `src/app/(th)/admin/page.tsx` ไฟล์เดียว ตกจุดไหนก็เงียบไม่มี error"
- ภาคผนวก "ตรวจแล้วผ่าน" 6 ข้อ กันรอบหน้าเสียเวลาซ้ำ (migration ขึ้น production แล้ว · ไม่ต้องแตะ UI ฝั่งผู้ใช้ · ไม่ต้องทำหน้าอังกฤษ ฯลฯ)

- **ไฟล์ที่แตะ**: `docs/plans/HANDOFF_ADMIN_REDEEM_2026-09-12.md` [NEW] · `CLAUDE.md` (เพิ่มดัชนี) · `docs/WORK_LOG.md`
- **ไฟล์โค้ดที่แตะ**: ไม่มี — รอบนี้เป็นการตรวจและวางแผนล้วน
### 🗓️ 2026-09-12 (รอบ 47): 🎯 ปิดหนี้ UX/UI คลื่นสุดท้าย — `<Button>` กลาง + ด่านพื้นที่กด (UX-15)

> **คำสั่งเจ้าของ**: "แก้เลยอีก 17 ข้อ" — **ครบทั้ง 17 ข้อแล้ว**

- **แก้ `Button.tsx` ที่ต้นทาง**: ทุกขนาดได้ `min-h-11` (44px) แล้ว — ของเดิม `size="sm"` สูงราว **33px** แปลว่าทุกที่ที่เรียกใช้ได้ปุ่มที่กดยากไปด้วยโดยไม่รู้ตัว · ใช้ `min-h-` ไม่ใช่ `h-` เพื่อให้ปุ่มข้อความสองบรรทัดสูงขึ้นเองได้ · เก็บ hex ดิบที่เหลือเป็นโทเคนครบ
- **📐 แก้ตัวเลขที่ผมรายงานเกินจริงในแผนเดิม**: แผนบอกว่า `<Button>` ถูกใช้แค่ "12 จาก 67 ไฟล์" แล้วเสนอให้ย้าย 55 ไฟล์ที่เหลือ — **วัดละเอียดแล้วพบว่าภาพนั้นไม่ตรง**
  - ปุ่มที่เขียนเองมี **209 จุด** แต่ **118 จุดไม่มี `bg-` เลย** (ปุ่มไอคอน · แท็บ · จุดสไลด์ · ชิป) ซึ่ง**ไม่ใช่เคสของ `<Button>` ตั้งแต่ต้น**
  - ที่ตรงกับ variant `gold` จริง ๆ มีเพียง **22 จุด** และทุกจุดเป็นพิลล์ `rounded-full` ที่ padding ต่างกันหมด — การบังคับยัดเข้า component กลางแล้ว override ด้วย `className` ทุกครั้ง **ได้โค้ดที่แย่กว่าเดิม** จึงไม่ทำ
  - คุณค่าจริงของ UX-15 คือ "ความไม่สม่ำเสมอของ focus ring / พื้นที่กด / disabled" ซึ่ง**แก้ได้ตรงกว่าด้วยด่าน CI + คลาสกลาง** ไม่ใช่ด้วยการย้าย component
- **ด่านใหม่ที่ 50 · `test-tap-target.ts`**: ปุ่มที่ padding แนวตั้งน้อยหรือขนาดตายตัวเล็ก ต้องมี `tap-target` / `tap-overlay` / `min-h-11` อย่างใดอย่างหนึ่ง
  - ตอนตั้งด่านพบ **114 จุด** กวาดไปแล้ว **24 จุด** ที่เป็นพิลล์เรียงแนวนอนในเส้นทางอ่านไพ่ (ปลอดภัยเพราะ overlay ของปุ่มแนวนอนไม่มีทางทับกัน) เหลือ **90 จุด** ตั้งเป็นเพดาน ratchet
  - ⚠️ **ทำไมไม่กวาดรวดเดียว**: ที่เหลือส่วนใหญ่เป็นปุ่มซ้อนกันแนวตั้งในแผงแคบ (สมุดบันทึก 15 · หน้าต่างเข้าสู่ระบบ 6 · สารานุกรม 5) ถ้าเผลอใส่ `tap-overlay-y` ให้ปุ่มที่ห่างกันน้อยกว่า 44px **overlay จะทับกันแล้วปุ่มล่างขโมยคลิกของปุ่มบนไปเงียบ ๆ** ซึ่งแย่กว่าปุ่มเล็ก — แต่ละจุดต้องดูระยะจริงก่อน เป็นงานที่ต้องเห็นภาพ ไม่ใช่งาน codemod
  - ✅ **พิสูจน์แล้วว่า fail จริง** โดยจงใจเพิ่มปุ่มเล็กหนึ่งจุด ด่านแดงทันที (91 > 90) แล้วคืนไฟล์ ด่านเขียวตามเดิม
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 50/50

#### 🏁 สรุปรวมทั้ง 5 คลื่น (UX-04 ถึง UX-20 · 17 ข้อ)

| คลื่น | PR | ปิดข้อ |
|---|---|---|
| 2 | #422 | UX-04 skip link · UX-05 nav landmark · UX-06 จุดสไลด์ 24px · UX-07 พื้นที่กด |
| 3 | #423 | UX-08 หน้าต่างลอย 5 บาน · UX-09 `Field` · UX-10 label 12 จุด · UX-11 `aria-controls` |
| 4 | #427 | UX-12 สีฮาร์ดโค้ด 4,232➔328 · UX-13 `#756F66` · UX-14 gold เป็นตัวอักษร |
| 5 | #429 | UX-16 `aria-disabled` · UX-17 i18n · UX-18 loading/error · UX-19 z-index · UX-20 พัดไพ่ |
| 6 | (รอบนี้) | UX-15 `<Button>` + ด่านพื้นที่กด |

**ด่าน CI เพิ่มจาก 48 เป็น 50** และ**ทุกด่านใหม่ถูกพิสูจน์แล้วว่า fail จริง** ไม่ใช่ด่านหลอก

---

### 🗓️ 2026-09-12 (รอบ 46): 🔤 ปิดหนี้ UX/UI คลื่นที่ 5 — i18n · สถานะหน้า · ผังชั้นความสูง · พัดไพ่ (UX-16 ถึง UX-20)

> **คำสั่งเจ้าของ**: "แก้เลยอีก 17 ข้อ"

- **UX-17 · ภาษาไทยรั่วในหน้าอังกฤษ + อุดรูรั่วของด่านที่ควรจับได้**
  - แก้ต้นเหตุ: `SeoArticleShell` หัวข้อ FAQ ฮาร์ดโค้ดไทย · `RitualStepProgress` ทั้ง title และ aria-label · `Button` ข้อความ loading · `Modal` fallback aria-label · `SpreadPositionMap` (เป็น server component จึงรับภาษาเป็น prop) · `LanguageSwitcher` · `SpreadCardSelector`
  - **อุดรูรั่วของด่าน `test-en-routing`**: เกณฑ์เดิมเป็น "สัดส่วนอักขระไทยเกิน 1.5% ของหน้า" ซึ่งหัวข้อไทยหนึ่งบรรทัดคิดเป็น ~0.1% จึง **ตกไม่ได้ไม่ว่าจะรั่วกี่จุด** — เพิ่มเกณฑ์ที่สองแบบ allowlist ที่จับ "วลีไทยยาว ≥ 4 ตัวอักษร" แม้มีจุดเดียว และครอบ `aria-label` · `alt` · `title` · `placeholder` ด้วย
  - ✅ **ด่านใหม่จับของจริงได้ทันทีที่เปิดใช้**: `aria-label="เปลี่ยนเป็นภาษาไทย"` โผล่อยู่ใน **148 หน้าอังกฤษ** และ `"หมวดหมู่ผังพยากรณ์"` อีก 1 หน้า — ทั้งหมดนี้เกณฑ์สัดส่วนเดิมมองไม่เห็นเลยสักตัว
- **UX-18 · สถานะหน้า**: เพิ่ม `loading.tsx` 7 เส้นทางหลัก (ไทย 4 · อังกฤษ 3) ด้วยโครงกระดูกที่ **มีรูปร่างตรงกับเนื้อหาจริง** (กริดไพ่ใช้สัดส่วน 300:520 เท่ากับ `<CardImage>` จึงไม่เกิด layout shift) · ย้าย `error.tsx` ไปเป็นของกลางแล้วต่อให้กลุ่ม `(en)` ซึ่ง **เดิมไม่มี error boundary เลย**
- **UX-19 · ผังชั้นความสูง**: ประกาศ `--z-*` เป็นผังเดียวใน `globals.css` แล้วไล่ใช้จริง — แถบขอความยินยอมเดิมตั้ง `z-100` จึง **ลอยทับหน้าต่างลอยทุกบาน** (ซึ่งอยู่ที่ `z-50`) ตอนนี้ลงมาอยู่ที่ 70 ต่ำกว่าโมดัล 80 และ toast ขึ้นไปสูงสุดที่ 90 · เลี่ยงปุ่ม TikTok ลอยบนมือถือด้วย `right-[4.75rem]`
- **UX-20 · พัดไพ่ 78 ใบ = 78 จุด Tab**: ทำ roving tabindex ให้เหลือจุดหยุดเดียว แล้วใช้ลูกศรซ้าย/ขวา/บน/ล่าง + Home/End เลื่อนระหว่างไพ่ (แพตเทิร์นเดียวกับแท็บหมวดผังที่ใช้อยู่ทั้งเว็บ) · ปุ่ม "สุ่มเลือกให้ฉัน" มีอยู่แล้วและเข้าถึงได้จากคีย์บอร์ด
- **UX-16 · `disabled` vs `aria-disabled`**: แยกสองกรณีออกจากกันที่ปุ่มส่งข้อความในแชทแม่หมอ — `disabled` จริงใช้เฉพาะตอน "กำลังส่งอยู่" เพื่อกันกดซ้ำ ส่วนตอน "ยังไม่ได้พิมพ์" ใช้ `aria-disabled` เพราะ `disabled` จริงถอดปุ่มออกจากลำดับ Tab จนผู้ใช้ screen reader ไม่รู้ด้วยซ้ำว่ามีปุ่มส่งอยู่
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 49/49
- **สิ่งที่ค้างต่อ**: UX-15 (ย้ายมาใช้ `<Button>` กลาง) ข้อเดียว

---

### 🗓️ 2026-09-12 (รอบ 45): 🎨 ปิดหนี้ UX/UI คลื่นที่ 4 — รวมพาเลตให้เหลือชุดเดียว (UX-12 ถึง UX-14)

> **คำสั่งเจ้าของ**: "แก้เลยอีก 17 ข้อ" (ต่อจาก `docs/plans/HANDOFF_UX_UI_AUDIT_2026-09-11.md`)

- **UX-12 · สีฮาร์ดโค้ด 4,232 จุด ➔ 328 จุด (ลดลง 92%)**
  - ประกาศสีที่ "ใช้จริงแต่ไม่เคยมีชื่อ" ให้เป็นโทเคนใน `@theme` — `gold-ink` (`#8F5C1A` สีที่ใช้มากที่สุดในเว็บ 581 ครั้ง แต่ไม่เคยเป็นโทเคน) · `gold-ink-deep` · `line-warm` · `ink-deep` · `inset-warm` · `surface-warm` · `surface-mist` · `line-soft` · `err-wash`
  - รัน codemod แทนที่ `prefix-[#HEX]` ➔ `prefix-<token>` รวม **3,877 จุดใน 110 ไฟล์**
  - 🧭 **การตัดสินใจสำคัญ — "ตั้งชื่อก่อน ค่อยยุบทีหลัง"**: ทางเลือกที่ตรงไปตรงมาคือยุบสีแฝดทิ้งเลย (`#D9C8AC` ➔ `#D5CEC2`) แต่นั่นคือการเปลี่ยนสีจริงของเว็บ 394 จุดในคราวเดียวโดยไม่มีใครเคยเห็นภาพก่อน/หลัง — เสี่ยงสูงและเป็นการตัดสินใจเชิงดีไซน์ที่ควรเป็นของเจ้าของ รอบนี้จึงเลือกให้ทุกสีมีชื่อแทน ผลทางสายตาเหมือนเดิมเป๊ะทุกพิกเซล และเมื่อไหร่ที่อยากยุบจริง มันจะเป็นการแก้ **บรรทัดเดียว** ใน `globals.css` ไม่ใช่ไล่แก้ 394 จุด
  - **พิสูจน์ว่าไม่เปลี่ยนสี**: ตรวจ CSS ที่ build ออกมาแล้วว่าโทเคนทั้ง 19 ตัวชี้ไปที่ค่า hex เดิมทุกตัว และ utility class ถูกสร้างครบรวมถึงตัวที่มี alpha (`.bg-gold-ink/10` ➔ `#8f5c1a1a` ตรงกับของเดิม)
- **UX-13 · สีที่ตกคอนทราสต์ยังค้าง 30 จุด**: กวาด `#756F66` ออกหมด ➔ โทเคน `muted` (`#635B4E`) รวม 5 จุดที่ผู้ใช้เห็นจริงซึ่งเป็น `placeholder` ที่ถูกลดความทึบลงอีกครึ่งหนึ่งทั้งที่สีตั้งต้นก็ตกเกณฑ์อยู่แล้ว
- **UX-14 · `gold` ถูกใช้เป็นสีตัวอักษรเล็ก**: เปลี่ยนเป็น `gold-ink` **44 จุดใน 18 ไฟล์** (3.29:1 ➔ 5.79:1) รวม 3 จุดที่เป็น `hover:` / `group-hover:` เพราะ WCAG 1.4.3 ครอบทุกสถานะ ไม่ใช่แค่สถานะปกติ
- **ด่านใหม่ที่ 49 · `scripts/qa/test-palette-drift.ts`** — ตรวจ 3 กฎ: จำนวนสีฮาร์ดโค้ดห้ามเกินเพดาน (ratchet ลดได้อย่างเดียว) · ห้ามใช้ `#756F66` · ห้ามใช้ `text-gold` กับตัวอักษรเล็ก
  - ✅ **พิสูจน์แล้วว่า fail จริง** โดยจงใจใส่ของผิดครบทั้ง 3 แบบ แล้วด่านแดงทั้ง 3 ข้อพร้อมชี้ไฟล์และบรรทัด — ไม่ใช่ด่านหลอกที่ตกไม่ได้
  - ทำด่านนี้ **ก่อน** เริ่มกวาด เพราะข้อมูลวัดสองวันชี้ว่าสีนอกระบบโตเอง 81 จุดในวันเดียว การกวาดโดยไม่มีด่านคือการวิ่งไล่ตามสิ่งที่วิ่งเร็วกว่า
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 49/49
- **สิ่งที่ค้างต่อ**: UX-15 ถึง UX-20 อีก 6 ข้อ (คลื่น 5–7)

---

### 🗓️ 2026-09-12 (รอบ 44): 🧑‍⚖️ workflow กดรัน `ai:judge` เอง — ไม่ต้องส่งคีย์ให้ใคร

> **คำถามเจ้าของ**: "ใส่แล้วเราจะรันยังไง หรือให้ทีมทำ" ➔ เลือก **GitHub Action กดรันเอง**

#### 1. ข้อค้นพบที่ทำให้ไม่ต้องตั้งอะไรใหม่เลย

`GROQ_API_KEY` และ `GEMINI_API_KEY` **อยู่ใน repository secrets อยู่แล้ว** (`deploy.yml` ใช้มาตลอด) และ repo ก็ใช้ `workflow_dispatch` เป็นแบบอยู่แล้วใน `deploy.yml` ➔ รันบน GitHub ได้เลยโดยไม่ต้องส่งคีย์ให้ใคร ไม่ต้องตั้งค่าในเครื่องใคร

> เซสชัน Claude Code ที่ทำงานอยู่ **มองไม่เห็นคีย์** เพราะ env ถูกฉีดตอนสร้าง container ซึ่งเกิดก่อนที่เจ้าของจะใส่คีย์ — ตรวจยืนยันแล้วว่า env ว่างและไม่มีไฟล์ `.env` จึงรันแทนให้ไม่ได้จริง ๆ

#### 2. สิ่งที่ทำ — `.github/workflows/ai-judge.yml`

- **`workflow_dispatch` อย่างเดียว** ไม่มี `push` ไม่มี `pull_request` ไม่มี `schedule` — ต้นทุนเกิดเมื่อมีคนกดปุ่มเท่านั้น เหมือนรันจากเครื่องตัวเองทุกประการ เขียนคำเตือนตัวโตไว้ในหัวไฟล์ว่าถ้าใครเพิ่ม trigger อัตโนมัติ = ละเมิดกติกา "ห้ามผูกเข้า CI" ของสคริปต์
- รับ 3 input: `compare` (ตั้งต้น `20260911-1`) · `limit` (`0` = ครบ 30 เคส) · `delay` (คูลดาวน์กัน rate limit)
- ลำดับขั้น: ตรวจว่ามีคีย์ก่อนเสียเงิน ➔ **ซ้อมแห้งก่อนเสมอ** (ต้นทุน 0) ➔ ยิงจริง ➔ สรุปผลขึ้น `$GITHUB_STEP_SUMMARY` ให้อ่านได้เลยไม่ต้องโหลดไฟล์ ➔ แนบ artifact เผื่อขั้นเปิด PR ล้ม ➔ **เปิด PR เอาไฟล์รายงานเข้า `main` ให้เอง**
- เปิด PR แทนการ push เข้า `main` ตรง ๆ เพราะ `main` มี branch protection และตามกฎเหล็กข้อ 13 ทุกอย่างต้องผ่าน PR

#### 3. สองจุดที่แก้ระหว่างเขียน (เจอเพราะทดสอบจริง ไม่ใช่เดา)

- 🔒 **script injection**: เดิมยัด `${{ inputs.* }}` ลงในสคริปต์เชลล์ตรง ๆ ซึ่งเป็นสิ่งที่ GitHub เตือนไว้ชัด — ค่าที่ผู้กดปุ่มพิมพ์มาจะกลายเป็นคำสั่งเชลล์ ➔ เปลี่ยนเป็นส่งผ่าน `env:` ทั้งหมด แล้วตรวจรูปแบบก่อนใช้ (`delay`/`limit` ต้องเป็นตัวเลขล้วน · `compare` ต้องเป็นรูปแบบ `PROMPT_VERSION`)
- 🐛 **YAML พัง**: ข้อความ commit และ PR body ที่ฝังการขึ้นบรรทัดใหม่ทำให้บรรทัดถัดไปไปเริ่มที่คอลัมน์ 0 ซึ่ง block scalar (`run: |`) รับไม่ได้ ➔ เปลี่ยนเป็น `git commit -m` สามครั้ง และประกอบ PR body ด้วย `printf '%s\n'` ทีละบรรทัด

#### 4. ทดสอบแล้วก่อนส่ง

- ✅ YAML parse ผ่าน · ยืนยันว่า trigger มีแค่ `workflow_dispatch` จริง
- ✅ **`pnpm run ai:judge -- --dry-run --limit 2` ส่ง args ต่อได้จริง** (ความเสี่ยงสูงสุดของ workflow เพราะ CI ใช้ pnpm ไม่ใช่ npm) — ติดตั้ง pnpm 9.15.4 แล้วรันจริง ผ่าน 2/2 เคส
- ✅ ตรรกะสร้าง args ทดสอบ 6 เคส: ค่าปกติ/ลองเร็ว/ไม่เทียบ ➔ ได้ args ถูกต้อง · ยิง injection ผ่าน `compare` และ `limit` · ใส่ `delay` เป็นตัวอักษร ➔ **ถูกปัดตกทั้งหมด**
- ✅ `npm run repo:verify` ➔ ผ่านครบ 48/48 ด่าน

- **ไฟล์ที่แก้ไข**: ใหม่ `.github/workflows/ai-judge.yml` · แก้ `docs/plans/HANDOFF_AI_JUDGE_BASELINE_2026-09-11.md`
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: 🔑 **เหลือแค่กดปุ่ม Run** — Actions → `🧑‍⚖️ AI Judge` → Run workflow (แนะนำใส่ `limit=3` ลองรอบแรกก่อน แล้วค่อย `0` เต็มชุด) · หลัง merge PR รายงานแล้ว ด่านที่ 47 จะเลิกเตือนเรื่องหนี้ค้างของ `20260911-2`

---
### 🗓️ 2026-09-12 (รอบ 43): 🎟️ ลดโควตาสมาชิกจากวันละ 3 ครั้ง เหลือ **วันละ 1 ครั้ง**

> **คำสั่งเจ้าของโปรเจกต์**: "สมาชิก (สมัครแล้ว) เปลี่ยนเป็น 1 ครั้ง ต่อวัน"

#### สิ่งที่เปลี่ยน
1. **ตัวเลขสิทธิ์ (แหล่งความจริงเดียว)**: `src/lib/entitlement/limits.ts` ➔ `DAILY_LIMIT = 3` เป็น `1`
   - ผู้เยี่ยมชม `GUEST_LIMIT = 0` (ต้องสมัครก่อนเล่น) และโบนัสสมัครใหม่ `SIGNUP_BONUS = 0` **ไม่เปลี่ยน**
   - เครดิตที่ซื้อ/แลกรหัส (`purchase_*`) ยังใช้ได้เหมือนเดิม และยังถูกหักหลังโควตารายวันหมดตามลำดับเดิม
2. **ถ้อยคำอังกฤษไม่ให้กลายเป็น "1 readings"** (กฎเหล็กข้อ 10): เพิ่ม `READINGS_EN` ที่ผันตาม `DAILY_LIMIT`
   แล้วใช้แทนคำว่า `readings` ที่ติดกับตัวเลขทุกจุด — `copy.ts` · `AccessDialog` · `AnnouncementBanner` ·
   `PostReadingSignup` · `UserProfileBadge` (ภาษาไทยไม่ต้องผัน จึงไม่แตะ)
3. **ปิดช่องเลขฮาร์ดโค้ดที่หลงเหลือ**: `UserProfileBadge.tsx` เคยเขียน `view?.limit ?? 3` ไว้เป็นค่าสำรอง
   — ถ้า `/api/entitlement` ตอบช้า/ล้ม ผู้ใช้จะเห็น "เปิดฟรีวันละ 3 ครั้ง" ทั้งที่ของจริงเหลือ 1
   เปลี่ยนเป็น `?? DAILY_LIMIT` แล้ว
4. **ด่าน QA ไม่ผูกกับเลข 3 อีกต่อไป**: `scripts/qa/test-entitlement.ts` เดิมพิมพ์ "หัก #1/#2/#3",
   "มี daily 3 แถว", "ใช้ครบ 6" ไว้ตรง ๆ ซึ่งจะตกทันทีที่เจ้าของเปลี่ยนเพดาน — เขียนใหม่ให้วนตาม
   `DAILY_LIMIT` และ `TOPUP_ROUNDS` ทั้งหมด (รวมด่านกัน Double-Spend ที่ต้องเซ็ตอัปให้เหลือ 1 สิทธิ์พอดี)
5. **เอกสารตามความจริง**: `ENTITLEMENT_PLAN.md` (หัวเอกสาร + ตารางสเปก + เพดานรองรับ) ·
   `CLOUDFLARE_OPTIMIZATION_GUIDE.md` · `KNOWN_ISSUES.md` (ISSUE-038) · `INDEX.md`

#### 🔎 ข้อค้นพบระหว่างทาง — "ไพ่ประจำวันฟรี ไม่กินโควตา" เป็นข้อความที่ไม่ตรงกับโค้ดมานานแล้ว
`ENTITLEMENT_PLAN.md` เขียนว่าผัง `daily` ฟรีไม่กินโควตา แต่ `consumeReading()` หักสิทธิ์ผัง `daily`
เหมือนผังอื่นทุกประการ (`daily.ts` เก็บแค่ streak) และด่าน QA ข้อ 12 ก็ยืนยันพฤติกรรมนี้ไว้เอง
**รอบนี้แก้เฉพาะเอกสารให้ตรงโค้ด ไม่แตะพฤติกรรม** — เพราะการยกเว้นโควตาให้ผังรายวันคือการเปลี่ยนนโยบาย
ที่ต้องให้เจ้าของเคาะ และยิ่งสำคัญขึ้นเมื่อโควตาเหลือวันละ 1 ครั้ง (ถ้าอยากให้ไพ่ประจำวันฟรีจริง สั่งได้)

#### การตรวจสอบคุณภาพ
- `npm run typecheck` ➔ 0 errors
- `npx tsx scripts/qa/test-entitlement.ts` ➔ **69/69 ผ่าน** (รวมด่านกันหักซ้ำและ Double-Spend)
- `npm run repo:verify` ➔ ผ่าน **47/48 ด่าน** · ด่านที่ตกคือ 🧑‍⚖️ `ai:judge` ของ `PROMPT_VERSION 20260911-2`
  ซึ่ง **ตกอยู่ก่อนแล้วบน `main`** (ยืนยันด้วยการ stash โค้ดรอบนี้ออกแล้วรันซ้ำ) เป็นหนี้ค้างที่ต้องใช้
  คีย์นักพัฒนารันตาม `docs/plans/HANDOFF_AI_JUDGE_BASELINE_2026-09-11.md` ไม่เกี่ยวกับการเปลี่ยนโควตา
### 🗓️ 2026-09-12 (รอบ 43): ⚡ ตรวจงานแก้ LCP ของเมื่อวาน — ของจริงผ่าน แต่เจอ `<link rel="preload">` 5 จุดที่ตายไปแล้วเงียบ ๆ

> **คำสั่งเจ้าของ**: "ตรวจสอบแก้ lcp เมื่อวาน"

#### ส่วนที่ตรวจแล้ว **ผ่าน** — งานรอบ 22 (PR #393) ขึ้น production ครบและทำงานถูกต้อง

วัดจาก HTML จริงของ production ด้วย User-Agent มือถือ และยิงวัดไบต์จริงจาก ImageKit ทีละไฟล์

| สิ่งที่ตรวจ | ผล |
|---|---|
| ไฟล์ `w320` ครบ 78 ใบบนดิสก์ | ✅ (เฉลี่ย 38 KB/ใบ) |
| `w320` โผล่ใน `srcset` ของ HTML หน้า `/cards` | ✅ พบ 80 จุด |
| `sizes` ที่แก้ไว้ตรงกับที่ deploy จริง | ✅ `(min-width: 1024px) 152px, (min-width: 768px) 136px, (min-width: 640px) 156px, 38vw` |
| 6 ใบแรกยัง `eager` · ใบแรกยัง `fetchPriority="high"` | ✅ ไม่ถูก PR ที่ตามมาลบทิ้ง |
| ไบต์จริงที่มือถือ DPR 2 ได้รับต่อใบ | ✅ **AVIF w320 = 27.1 KB** (เทียบ AVIF w512 = 61.2 KB · WebP w512b เดิม = 85.5 KB) |

**หมายเหตุ**: ตัวเลขในบันทึกรอบ 22 เขียนไว้ตอนที่ยังไม่มี AVIF (PR #415 เข้าทีหลังในวันเดียวกัน)
ของจริงวันนี้ดีกว่าที่จดไว้ เพราะ AVIF ทับลงไปอีกชั้น — เส้นทางที่ผู้ใช้มือถือเดินจริงคือ `AVIF w320`

#### 🐞 บั๊กที่เจอระหว่างตรวจ — พรีโหลดภาพไพ่ 5 หน้าเสียเปล่า 100%

`<CardImage />` เรนเดอร์เป็น `<picture>` ที่วาง `<source type="image/avif">` **ไว้ก่อน** WebP ตั้งแต่ PR #415
เบราว์เซอร์ที่รองรับ AVIF (~95% ของทราฟฟิกจริง) จึงหยิบ AVIF ทุกครั้ง

แต่ `<link rel="preload" as="image">` ทั้ง 5 จุดยังประกาศ `type="image/webp"` อยู่เหมือนเดิม
**ระบุชนิดไฟล์ได้ชนิดเดียว** → พอชนิดไม่ตรงกับที่ `<picture>` เลือก ไฟล์ที่พรีโหลดมาถูกทิ้งทั้งก้อน
แล้วภาพ LCP ตัวจริงก็ **ไม่ได้พรีโหลดสักนิด** — พรีโหลดที่ใส่ไว้แก้ LCP ตั้งแต่ 2026-09-04 จึงตายไปแล้วเงียบ ๆ

ซ้ำร้าย `/daily` กับ `/love/1-card` **พังมาก่อนหน้านั้นอีก**: `imageSizes="120px"` ชี้ไปยังไพ่ใบใหญ่
ที่หายไปตั้งแต่ยุบ `OneCardRitual` เหลือ 2 จังหวะ (PR #293) — ตอนนี้ไพ่ใบนั้นโผล่บนหน้าแค่ **48px**
พรีโหลดจึงดูด `w256` มาทิ้ง แล้วให้ `<img>` ไปโหลด `w128` ใหม่อีกไฟล์

**ไบต์ที่เสียเปล่าต่อการเปิดหน้าหนึ่งครั้ง** (วัดจริงจาก ImageKit · มือถือ DPR 2 · ยิงด้วย `fetchPriority="high"` แย่งแบนด์วิดท์นำหน้าทุกอย่าง)

| หน้า | ไฟล์ที่พรีโหลดมาทิ้ง | ขนาด |
|---|---|---|
| `/` และ `/en` | `w256/major-19.webp` | **40.0 KB** |
| `/daily` และ `/en/daily` | `w256/major-19.webp` | **40.0 KB** |
| `/love/1-card` และ `/en/love/1-card` | `w256/major-06.webp` | **43.4 KB** |

#### สิ่งที่แก้

1. **ลบ `<link rel="preload" as="image">` ของภาพไพ่ทิ้งทั้ง 5 จุด** พร้อม `heroCardSrcSet` และ import ที่กำพร้าตาม
   — `home.tsx` · `(th)/daily` · `(th)/love/1-card` · `(en)/en/daily` · `(en)/en/love/1-card`
   หน้าแรกไม่เสียอะไรเลย เพราะไพ่ใบนั้น (`DailySpreadArt`) **มี `loading="eager"` + `fetchPriority="high"` อยู่แล้ว**
   และถูกเรนเดอร์มากับ HTML — ตัวสแกนพรีโหลดของเบราว์เซอร์เจอ `<picture>` ตั้งแต่รอบ parse แรกอยู่ดี
   ต่างกันตรงที่คราวนี้มันเจรจาชนิดไฟล์เองได้ถูกต้อง

2. **เพิ่มด่านตรวจกฎ D ใน `scripts/qa/test-image-paths.ts`** — ห้ามพรีโหลดภาพไพ่ด้วย `<link rel="preload" as="image">`
   อ่านข้ามบรรทัดได้ (JSX เขียน `<link>` หลายบรรทัด) และ **ยืนยันแล้วว่าตกได้จริง** โดยใส่บั๊กกลับเข้าไปแล้วด่านจับได้ที่บรรทัดถูกต้อง

#### บทเรียน

**เปลี่ยนชนิดไฟล์ที่ `<picture>` เสิร์ฟ = ทำให้ `<link rel="preload">` ทุกจุดของภาพนั้นตายทันที**
ทั้งสองฝั่งไม่มีอะไรผูกกันเลย ไม่มี type error ไม่มีภาพพัง หน้าเว็บดูปกติทุกประการ —
เสียแค่แบนด์วิดท์กับ LCP ที่ไม่มีใครเห็นจนกว่าจะเปิด waterfall ดู
กฎทั่วไป: **ถ้าภาพถูกเรนเดอร์มากับ HTML อยู่แล้ว อย่าพรีโหลดมันซ้ำ** — ใส่ `fetchPriority="high"` ที่ `<img>` พอ

#### ข้อจำกัดของการตรวจรอบนี้ (ให้รอบหน้ารู้ตัวก่อน)

- **Chromium ยิงผ่าน proxy ของเซสชันนี้ไม่ได้เลย** (`ERR_CONNECTION_RESET` ทุกโดเมน แม้แต่ `example.com`)
  ลองแล้วทั้ง `--ignore-certificate-errors` · `--proxy-bypass-list` · ปิด QUIC/HTTP2 — ไม่ผ่านสักทาง
  จึงวัด LCP ด้วยเบราว์เซอร์จริงไม่ได้ ต้องวัดจาก HTML + ไบต์จริงของแต่ละไฟล์แทน
- **โควตา PageSpeed Insights แบบไม่มี API key หมดอีกแล้ว** (HTTP 429) — ตรงกับกับดักข้อ 3 ใน `HANDOFF_BLOG_TBT_2026-09-12.md`
  ถ้าจะยืนยันตัวเลข LCP เป็นวินาที **ต้องมี PSI API key** หรือรันจากเครื่องที่ออกเน็ตตรง
- **กับดักใหม่: ด่าน `test-judge-baseline.ts` ตกทั้งที่โค้ดไม่ผิด เพราะ ref `origin/main` ในเครื่องเก่า**
  ด่านนี้อ่าน `PROMPT_VERSION` จาก `git show origin/main:...` เพื่อดูว่า "PR นี้ขึ้นเวอร์ชันหรือเปล่า"
  ถ้าเซสชันโคลนมานานแล้วไม่ได้ `git fetch` ref นั้นจะยังเป็นของเก่า (ที่นี่ค้างอยู่ `20260904-1`)
  ด่านจึงนึกว่าเราเป็นคนขึ้นเวอร์ชันเป็น `20260911-2` แล้วตกทันที
  **`git fetch origin main` ก่อน `repo:verify` เสมอ** — พอ ref ตรงแล้วผ่านครบ **48/48**
  (เป็นกับดักตระกูลเดียวกับ `.next/` ค้างของรอบ 21 คือด่านตกเพราะ "ของในเครื่องเก่า" ไม่ใช่เพราะโค้ดผิด)

### 🗓️ 2026-09-12 (รอบ 42): 🚨 หน้าลงชื่อเข้าใช้จมใต้หัวเว็บ — ผลข้างเคียงของ PR #416 ที่ผมทำเอง (INC-0131)

> **เจ้าของถ่ายจอมาถาม**: "ทำไมหน้าลงชื่อถึงเป็นแบบนี้" — หัวเว็บลอยทับหน้าต่าง ไม่โดนหรี่ · โลโก้กับปุ่มปิดถูกบัง

#### 1. อาการ
- ฉากหลังโมดัลคลุมเต็มจอ **แต่หัวเว็บลอยอยู่ข้างบนแบบไม่โดนหรี่**
- บนมือถือจอเตี้ย หัวเว็บสูง 69px **บังขอบบนของแผงจนโลโก้และปุ่มปิดหายไป** → ปิดหน้าต่างไม่ได้

#### 2. สาเหตุราก — เป็นผลข้างเคียงของ PR #416 ที่ผมแก้ landmark เอง
`globals.css` บังคับ `position: relative; z-index: 1` ให้ลูกตรงของ `<body>` ทุกตัว
**`<main>` จึงเป็น stacking context มาตลอด**

| | ก่อน #416 | หลัง #416 |
| :--- | :--- | :--- |
| `SiteHeader` | อยู่ **ใน** `<main>` | อยู่ระดับ `<body>` · `fixed z-50` |
| หน้าต่างลอย 7 บาน | อยู่ **ใน** `<main>` | อยู่ **ใน** `<main>` (ไม่ได้ย้ายตาม) |
| ผล | อยู่ context เดียวกัน · **ลำดับ DOM ตัดสิน** โมดัลมาทีหลังจึงทับได้ | คนละ context · **`z-50` บนโมดัลไม่มีผลใด ๆ** เพราะแม่มันคือ `z-index: 1` |

⚠️ **ตัวเลข z-index เท่าไหร่ก็แก้ไม่ได้** ตราบใดที่ยังอยู่ใน stacking context ของ `<main>`

#### 3. สิ่งที่แก้
ย้ายกลุ่มหน้าต่างลอยทั้ง 7 บานของ `TarotFlow` ออกไปนอก `<main>` ต่อท้าย `<SiteFooter />`
ให้อยู่ระดับ `<body>` เดียวกับหัวเว็บ — ลำดับ DOM ที่มาทีหลังจึงทับหัวเว็บได้จริง

**วัดจริงด้วย Chromium จำลอง iPhone 4 ความสูง** (พร้อมจำลองด่าน Turnstile ให้กินที่เท่า production):

| ความสูงจอ | แผงอยู่ในจอครบ | โมดัลทับหัวเว็บ | ปุ่มปิดกดได้ |
| :-- | :-: | :-: | :-: |
| 750 px | ✅ | ✅ | ✅ |
| 660 px | ✅ | ✅ | ✅ |
| 620 px | ✅ | ✅ | ✅ |
...
(ก่อนแก้: `elementFromPoint` กลางหัวเว็บคืน `<div>` ในหัวเว็บ = หัวเว็บอยู่บนสุด · หลังแก้คืนชั้นฉากหลังของโมดัล)

#### 4. กันซ้ำ — เพิ่มกฎลงด่านที่ 48 (ยังนับ 48 ด่านเท่าเดิม)
ห้ามคอมโพเนนต์หน้าต่างลอย 8 ตัวและคลาส `modal-scrim` อยู่ระหว่าง `<main>` เปิด–ปิดในไฟล์เดียวกัน
· ทดสอบด้วยการทำให้พังจริงแล้ว · **สแกนทั้ง `src/` พบว่าไม่มีไฟล์อื่นติดกับดักนี้** (หน้าแรกที่เดียว)

- **ไฟล์ที่แก้ไข**: `src/components/home/TarotFlow.tsx` · `scripts/qa/test-a11y-critical.ts`
- **ผลการทดสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 48/48 ด่าน**
- **บทเรียนที่บันทึก**: [`docs/INCIDENT_LOG.md`](INCIDENT_LOG.md) INC-0131 —
  **ย้าย element ข้ามระดับ DOM เมื่อไหร่ ต้องไล่ดูว่ามีอะไรพึ่งพาลำดับ DOM เดิมอยู่บ้าง**
  โดยเฉพาะของที่ใช้ `z-index` เท่ากัน เพราะ z เท่ากัน = **ลำดับ DOM คือสิ่งเดียวที่ตัดสิน**
...
#### 1. UX-01 · สายด่วนสุขภาพจิต 1323 และเหตุฉุกเฉิน 1669 อ่านไม่ออก — **ขัดกฎเหล็กข้อ 6 ตรง ๆ**

| ข้อความ | ก่อน | หลัง |
| :--- | :-- | :-- |
| สายด่วนสุขภาพจิต 1323 | `#3A7044` = **3.11:1** ❌ | `#5DAB6B` = **6.51:1** ✅ |
| เหตุฉุกเฉิน 1669 | `#A6392C` = **2.81:1** ❌ | `#DB8479` = **6.58:1** ✅ |

**สาเหตุ**: `nav-links.ts` ใช้โทเคน `ok`/`err` ซึ่งออกแบบมาสำหรับ**พื้นสว่าง** แต่ฟุตเตอร์เป็นพื้นมืด `#171512`
**แก้**: เพิ่มคู่โทเคน `--color-ok-on-dark` / `--color-err-on-dark` (เฉดเดิมยกความสว่างขึ้น) แล้วใช้ทั้งฝั่งไทยและอังกฤษ
· ⚠️ ไม่ได้ replace ทั้งเว็บ เพราะสีเดิมยังถูกใช้บนพื้นสว่างอีก 10 จุดซึ่งผ่านเกณฑ์อยู่แล้ว
· วัดซ้ำด้วย Chromium บน production build จาก `getComputedStyle` จริง ไม่ใช่สีในโค้ด

#### 2. UX-02 · หน้าแรกไม่มี landmark `banner`/`contentinfo` เลยสักอัน
`TarotFlow.tsx` ครอบ `SiteHeader`/`SiteFooter` ไว้**ข้างใน** `<main>` · ตามสเปก HTML-AAM
`<header>` ที่เป็นลูกหลานของ `<main>` **ไม่ได้ role `banner`** และ `<footer>` ก็ไม่ได้ `contentinfo`
➔ ย้ายออกมาให้โครงเหมือนเส้นทางอื่นทั้งเว็บที่ทำถูกผ่าน layout อยู่แล้ว (`(th)/cards/layout.tsx`)
ยืนยันจาก DOM จริง: `main header` = ไม่มี · `main footer` = ไม่มี · header/footer/main อย่างละ 1

#### 3. UX-03 · `<h1>` เป็นหัวข้อลำดับที่ 6 ของหน้า
บล็อกทำนายด่วนที่อยู่**เหนือ** `h1` ใช้ `h2` + `h3` × 4 เป็นหัวข้อจริง
➔ เปลี่ยนเป็น `<p>` ที่สไตล์เหมือนเดิมทุกพิกเซล (เป็นข้อความเกริ่นนำ ไม่ใช่หัวข้อของส่วน)
พร้อมเติม `aria-label` ให้การ์ดแต่ละใบ — ของเดิมไม่มี ชื่อจึงถูกประกอบจากข้อความทุกชิ้นในใบจนยาวเกินจับใจความ

> **เลือกทางนี้แทน "ย้าย h1 ขึ้นบนสุด" (ทาง A ในแผน)** เพราะเจ้าของเคยสั่งไว้ว่า
> ไพ่ประจำวันต้องอยู่บนสุด (คอมเมนต์ `"รูปที่ 1 นำกลับไว้บนสุดเหมือนเดิม"`) — ทางนี้จึงไม่ขยับสิ่งที่ตาเห็นเลย

ลำดับหัวข้อหลังแก้ (จาก HTML ที่ build จริง): `h1` → `h2` → `h3`×5 → `h2` → … **ไม่ข้ามลำดับสักขั้น**

#### 4. ด่านที่ 48 — `test-a11y-critical.ts` (ทดสอบด้วยการทำให้พังจริง 3 เคส)
คุมทั้งสามเรื่องในไฟล์เดียว · **คำนวณคอนทราสต์จริงตามสูตร WCAG ไม่ฮาร์ดโค้ดผลลัพธ์**
· กับดักที่เจอระหว่างเขียน: คอมเมนต์ที่อธิบายกฎเองมีคำว่า `<main>` อยู่ในเนื้อความ
`indexOf` จึงไปเจอคอมเมนต์ก่อนแท็กจริงแล้วฟ้องผิด — **ต้องตัดคอมเมนต์ทิ้งก่อนค้นเสมอ**

- **ไฟล์ที่แก้ไข**:
  - `src/app/globals.css` · `src/components/layout/nav-links.ts`
  - `src/components/home/TarotFlow.tsx` · `src/components/reading/QuickFortunePicker.tsx`
  - `scripts/qa/test-a11y-critical.ts` (ใหม่) · `scripts/github-auto.ts`
  - เอกสารที่อ้างจำนวนด่าน 7 ไฟล์ (47 → 48)
- **ผลการทดสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 48/48 ด่าน**
- **สิ่งที่ยังค้าง**: P1–P3 อีก 17 ข้อในแผน `HANDOFF_UX_UI_AUDIT_2026-09-11.md`
...
- **ปัญหาเดิม**: `GROQ_API_KEY` เป็น **เอนจินคำอ่าน Tier 1** และเป็นคีย์ที่ `npm run ai:judge` **บังคับต้องมี** (ไม่มี = รันไม่ได้เลย) แต่ `.env.example` ไม่มีบรรทัดนี้สักบรรทัด — มีแต่ `GEMINI_API_KEY` ซึ่งเป็น Tier 2 ➔ ใครก็อป `.env.example` ไปตั้งต้นจะไม่มีทางรู้ว่าต้องใส่ตัวไหน แล้วไปเจอ "🔑 ยังรันไม่ได้" เอาตอนพิมพ์คำสั่ง
  - ตัวแปรนี้ถูกอ้างถึงครบทุกที่แล้ว (`deploy.yml` · `wrangler secret put` ใน `CLOUDFLARE_DEPLOYMENT_GUIDE.md` · ผังคีย์ใน `PENDING_SETUP.md` ระบุชัดว่าเป็น "Tier 1 · Qwen3-27B · โควตาฟรี 14.4k/วัน") — **ขาดเฉพาะไฟล์เดียวที่คนเปิดเป็นอันดับแรก**
- **สิ่งที่แก้ไข**: เพิ่มบล็อก `GROQ_API_KEY` ใน `.env.example` พร้อมที่มาของคีย์ · บทบาท Tier 1 · ผลเมื่อไม่ตั้ง (ระบบถอยไป Gemini เว็บยังทำงาน แต่ `ai:judge` รันไม่ได้) · คำสั่ง production และเขียนหัวข้อ `GEMINI_API_KEY` ใหม่ให้บอกด้วยว่าใน `ai:judge` มันคือ **ผู้ตัดสิน** ซึ่งต้องเป็นคนละตระกูลกับผู้ผลิตเสมอเพื่อกัน self-preference bias
- **ไฟล์ที่แก้ไข**: `.env.example`
- **ผลการทดสอบ**: `npm run repo:verify` ➔ ✅ ผ่านครบ 48/48 ด่าน
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: สแกนเทียบ `process.env.*` ที่โค้ดอ่านจริงกับเอกสารแล้วพบว่าส่วนใหญ่ที่ไม่อยู่ใน `.env.example` ไปอยู่ใน `PENDING_SETUP.md` แทน (ตั้งใจแยกกัน) **แต่มี 2 ตัวที่ไม่อยู่ทั้งสองที่คือ `OMISE_SECRET_KEY` และ `META_PIXEL_ID`** — ยังไม่แตะเพราะอยู่นอกขอบเขตรอบนี้ ถ้าจะปิดควรทำพร้อมกันทีเดียวและอาจเพิ่มด่านตรวจว่าคีย์บังคับทุกตัวต้องมีที่อยู่ในเอกสาร
### 🗓️ 2026-09-11 (รอบ 40): 🖼️ เปิด AVIF ทั้งเว็บโดยไม่เพิ่มไฟล์สักไฟล์ — ภาพไพ่เบาลง 13–45%

> **ที่มา**: เจ้าของส่งโน้ตข้อเสนอ 2 ข้อมาให้ตรวจ (AVIF · Edge Cache Headers)

#### 1. ข้อ 2 (Edge Cache Headers) — **ทำไว้แล้วตั้งแต่ก่อนหน้านี้ ไม่ต้องทำอะไร**
`public/_headers` ตั้ง `/cards/*` เป็น `public, max-age=31536000, immutable` อยู่แล้ว
ยิงตรวจ production ยืนยัน: `cache-control: public, max-age=31536000, immutable` · `cf-cache-status: HIT`
ทั้งไฟล์ `.jpg` ต้นฉบับและภาพย่อ `.webp` (splat ของ Cloudflare เป็น greedy จึงครอบโฟลเดอร์ย่อยครบอยู่แล้ว)

#### 2. ข้อ 1 (AVIF) — **ตัวเลข "7–12%" ในโน้ตต่ำกว่าของจริงมาก**
"7–12%" เป็นตัวเลขของเส้นทางที่ผิด คือเอา AVIF ไปบีบทับ **WebP ที่บีบมาแล้ว**
(วัดจริง 87.5 → 77.8 KB = −11% พอดี) · ถ้าแปลงจาก **ไฟล์ `.jpg` ต้นฉบับ** ได้ **−13% ถึง −45%**

| ขนาด | WebP ที่ใช้อยู่ | AVIF q65 | ลดลง |
| :-- | :-- | :-- | :-- |
| w64 | 2.4 KB | 2.1 KB | −13% |
| w128 | 8.1 KB | 6.0 KB | −26% |
| w256 | 33.5 KB | 18.5 KB | −45% |
| w320 (มือถือใช้จริง) | 34.0 KB | 27.7 KB | −19% |
| w512b | 87.5 KB | 62.7 KB | −29% |
| w768b | 134.6 KB | 105.4 KB | −22% |

**ผลรวมทั้งหน้า `/cards`** (สุ่ม 12 ใบที่ `w320` แล้วขยายเป็น 78 ใบ): **2,725 → 1,893 KB · ประหยัด ~832 KB ต่อการเปิดหน้า**

#### 3. ทำไมรอบนี้ทำได้ ทั้งที่รอบก่อนสรุปว่า "ยังไม่ทำ"
[`HANDOFF_BLOG_TBT_2026-09-12.md`](plans/HANDOFF_BLOG_TBT_2026-09-12.md) ข้อ 8 เคยตัดสินว่าไม่คุ้ม
เพราะคิดบนสมมติฐานว่า**ต้องสร้างไฟล์ `.avif` เก็บในรีโป** (+468 ไฟล์ · +20 MB · gen 21 นาที/รอบ)

**แต่ ImageKit แปลงให้ตอนร้องขอได้** (`?tr=w-320%2Cf-avif%2Cq-65`) แล้วแคชที่ CDN ของเขาเอง
➔ **0 ไฟล์ใหม่ · 0 MB · ไม่ต้องแตะ `npm run cards:variants`** · ต้นทุนเหลืออย่างเดียวคือ HTML +2–3 KB gzip

#### 4. เลือก q65 เพราะอะไร (เจ้าของเคาะเองหลังดูภาพเทียบซูม 200%)

| รูปแบบ | ขนาดรวม 4 ใบ (w512b) | PSNR เฉลี่ย |
| :-- | :-- | :-- |
| WebP (เดิม) | 408.7 KB | 30.03 dB |
| AVIF q60 | 220.2 KB (−46%) | 29.76 dB |
| **AVIF q65** | **279.8 KB (−32%)** | **30.89 dB — คมกว่าเดิม** |

q65 คือจุดเดียวที่ "เล็กลงจริง" โดย**ไม่แลกกับความคมของภาพไพ่**

#### 5. กับดักที่เจอระหว่างทำ (จดไว้กันรอบหน้าเหยียบซ้ำ)
- **จุลภาคใน `tr=` ต้องเข้ารหัสเป็น `%2C` เสมอ** — ค่านี้อยู่ใน `srcSet` ซึ่งแยกรายการด้วยจุลภาค
  ใส่จุลภาคดิบเมื่อไหร่ รายการจะถูกตัดผิดตำแหน่งทันที (กับดักเดียวกับ `IMAGEKIT_NO_TRANSFORM`)
- **`srcSet` ของ AVIF ต้องมีครบทุกความกว้างเท่า WebP** — เบราว์เซอร์เลือก `<source>` ก่อน
  แล้วค่อยเลือกความกว้าง · ถ้าตัด `w64/w128` ออก ภาพเล็ก 36px จะไปหยิบ AVIF `w256` (18 KB)
  แทน WebP `w128` (8 KB) = **แย่ลง 2 เท่า**
- **`full={true}` ไม่แตะ** — เส้นทางนั้นใช้กับการ Export ภาพลง Canvas ด้วย ความเสี่ยงไม่คุ้ม
- **ด่านงบน้ำหนักจับได้ทันที**: HTML หน้าแรก 38 → 41 KB gzip ชนเพดาน 40 KB
  ขยับเพดานเป็น 42 KB พร้อมเหตุผลที่วัดมาแล้วกำกับไว้ในโค้ด (จ่าย 3 KB ซื้อคืน ~832 KB = คุ้ม 100–300 เท่า)

- **ไฟล์ที่แก้ไข**:
  - `src/lib/tarot/card-image.ts` (เพิ่ม `getCardAvifSrcSet()`)
  - `src/components/card/CardImage.tsx` (เพิ่ม `<source type="image/avif">` ไว้หน้า WebP)
  - `scripts/qa/test-bundle-budget.ts` (ขยับเพดาน HTML หน้าแรกพร้อมเหตุผล)
  - `docs/plans/HANDOFF_BLOG_TBT_2026-09-12.md` (อัปเดตข้อสรุปเดิมที่ว่า "ยังไม่ทำ")
- **ผลการทดสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 48/48 ด่าน**
  (รันแบบเดียวกับ CI คือตั้ง `NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT` ด้วย ไม่งั้นด่านงบจะไม่เห็น AVIF เลย)
  · ยิงทดสอบ ImageKit จริงครบทั้ง 6 ความกว้าง ได้ `content-type: image/avif` ทุกอัน
- **สิ่งที่ยังค้าง / ต้องเฝ้าดู**: ต้นทุน **decode** ของ AVIF บน CPU มือถืออ่อน ๆ ยังวัดไม่ได้เหมือนเดิม
  ถ้าพบว่าเครื่องเก่าช้าลง **ถอยกลับได้ทันทีด้วยการลบ `<source type="image/avif">` บรรทัดเดียว**
  ไม่มีไฟล์ค้างให้เก็บกวาด · และ **ยังลบ WebP ไม่ได้** เพราะ Safari ต่ำกว่า 16.4 ยังต้องใช้

---

### 🗓️ 2026-09-11 (รอบ 38): 🧑‍⚖️ ตรวจงานคลื่น B ที่ทีมนอกส่งกลับ + ปิดช่องที่ทำให้หลุดได้

> **คำสั่งเจ้าของ**: "อีกทีมทำแล้ว ตรวจสอบ" ➔ "แก้เลย ทำให้ดีที่สุด"

#### 1. ผลตรวจ — โค้ดผ่านทุกเกณฑ์ แต่ **ไม่มีผลวัด**

ตรวจเกณฑ์ผ่านรายข้อจากโค้ดจริงบน `main` ไม่ใช่จากรายงานที่ส่งมา:

| งาน | สิ่งที่ตรวจ | ผล |
| :--- | :--- | :---: |
| B-02 | ตัวอย่าง 8 ชิ้น · เทสต์ล็อก `checkThaiQuality()` + `checkReadingConsistency()` 0 issue · `pickExemplar` อยู่ใน `buildReadingMessage()` (เลี่ยงกับดัก prompt cache) · ไม่ throw | ✅ |
| B-03 | สูตรโทเค็น `1600 + 480×ใบ` · `Intl.Segmenter("th")` เทียบหลวม · `VISUAL_ANCHOR_UNGROUNDED` เป็น warn-only ตามสเปก | ✅ |
| B-04 | `isAiCapReached()` + `recordAiCall()` (INC-0074) · timeout 2.5 วิ · คำถามผ่าน `polishThai` + `checkThaiQuality` · endpoint ใหม่มี origin guard + rate limit 30/ชม./IP | ✅ |

🔴 **แต่ PR #410 ขึ้น `PROMPT_VERSION` เป็น `20260911-2` แล้ว merge โดยไม่มีรายงาน `ai:judge` ของเวอร์ชันใหม่เลย** — PR body บรรทัดเดียว · บันทึกรอบ 36 มีแต่ typecheck/repo:verify/จำนวนเทสต์ ไม่มีตัวเลข judge สักตัว ทั้งที่กติกาข้อ 2 หัวข้อ 8 เขียนว่า *"ไม่มีผลเทียบ = ไม่ merge"*

🟡 **baseline `20260911-1` เองก็สำเร็จแค่ 16/30 เคส** — 14 เคสล้มด้วย "คำอ่านถูกตัดกลาง" (มี retry 3 รอบแล้วยังล้ม) รวม `gold-021` ผังเซลติกครอส 10 ใบ ซึ่งเป็นเคสที่ B-02/B-03 เล็งโดยตรง · หมวดภาพรวมสำเร็จ 1 ใน 5 ➔ คะแนน 4.83/5 มาจากครึ่งเดียวของชุดและเอียงไปทางเคสสั้น

#### 2. 🐛 เจอบั๊กในเครื่องมือวัดเอง — `--compare` ตัดสินข้ามชุดเคสได้โดยไม่เตือน

เดิม `printComparison()` เทียบ **ค่าเฉลี่ยรวม** ของสองรอบตรง ๆ ถ้า baseline สำเร็จ 16/30 แล้วรอบใหม่สำเร็จ 30/30 ตัวเลขที่ได้บอกแค่ว่า *ชุดเคสเปลี่ยนไป* ไม่ได้บอกว่าคำอ่านดีขึ้นจริง **แต่หน้าจอจะขึ้น `🟢 ดีขึ้น` ให้อ่านอย่างมั่นใจ** — นี่คือกับดักที่รออยู่ตรงหน้าพอดีเมื่อทีมนั้นรันรอบถัดไป

- ย้ายตรรกะออกมาเป็น `scripts/qa/judge-compare.ts` (โมดูลล้วน ทดสอบได้) เพราะเดิมฝังอยู่ในไฟล์ที่เรียก `main()` ตอน import จึงเขียนเทสต์จับไม่ได้เลย — และนั่นคือเหตุผลที่บั๊กนี้รอดมา
- เทียบเฉพาะ **เคสที่สำเร็จทั้งสองรอบ** · พิมพ์ความครอบคลุมทั้งสองฝั่งกำกับเสมอ · ไม่มีเคสทับกัน = ปฏิเสธการเทียบ · ทับกัน < 60% = เตือนว่ายังตัดสินแทนทั้งชุดไม่ได้

#### 3. 🐛 `startedAt` ในรายงานบอกเวลาผิด

เดิมเรียก `new Date()` ตอนเขียนไฟล์ทุกครั้ง ทำให้ `startedAt` กลายเป็น "เวลาที่เขียนไฟล์ล่าสุด" — ใน baseline จริงห่างจาก timestamp ในชื่อไฟล์ถึง 67 นาที จับเวลาเริ่มไว้ครั้งเดียวแล้ว

#### 4. ด่านใหม่ที่ 47 — กันไม่ให้ "ขึ้นเวอร์ชันแล้วไม่มีผลวัด" หลุดอีก

`scripts/qa/test-judge-baseline.ts` (ไม่เรียกโมเดล ต้นทุน 0) แยกความรุนแรง 3 ชั้นตั้งใจ:

- 🔴 **ตก** — PR ขึ้น `PROMPT_VERSION` (ต่างจากของ `origin/main`) แต่ไม่มีรายงานของเวอร์ชันใหม่ · หรือไฟล์รายงานเสียรูป/เวอร์ชันไม่ตรงชื่อไฟล์/ไม่มีเคส
- 🟡 **เตือน** — เวอร์ชันปัจจุบันไม่มีรายงาน (หนี้ค้าง) · รายงานล่าสุดครอบคลุมต่ำกว่า 80%

> เหตุผลที่ข้อหลังเป็นเตือนไม่ใช่ตก: ถ้าตก จะกลายเป็นบล็อก PR ทั้งเว็บ (SEO · a11y · UI) ด้วยหนี้ของงาน AI ซึ่งไม่ได้สัดส่วน และด่านที่น่ารำคาญเกินไปจะถูกปิดทิ้งในที่สุด — แต่มันพิมพ์เตือนทุกครั้งที่รันจึงลืมไม่ได้

**ทดสอบด้วยการทำให้พังจริง 4 เคส** (บันทึกเป็น INC-0129) (ขึ้นเวอร์ชันไม่มีรายงาน · เวอร์ชันในไฟล์ไม่ตรงชื่อ · JSON พัง · ย้อนตรรกะ `--compare` กลับเป็นแบบเดิมแล้วเทสต์ต้องตก 4 ข้อ) — ไม่ใช่ด่านหลอกที่ตกไม่ได้แบบช่องว่าง G-10 เดิม

- **ไฟล์ที่แก้ไข**:
  - ใหม่: `scripts/qa/test-judge-baseline.ts` · `scripts/qa/judge-compare.ts`
  - แก้: `scripts/qa/run-golden-judge.ts` · `scripts/github-auto.ts` · `scripts/qa/test-docs-numbers.ts` (ขึ้นทะเบียนบันทึกประวัติ 46 ด่านของ ISSUE-042) · `docs/plans/HANDOFF_AI_JUDGE_BASELINE_2026-09-11.md` · `CLAUDE.md` · `docs/INDEX.md` · `docs/AI_COLLABORATION_GUIDELINES.md` · `docs/LOCAL_SETUP.md` · `AGENTS.md` · `GEMINI.md` · `README.md`
- **ผลการทดสอบ**: `npm run repo:verify` ➔ ✅ ผ่านครบ **48/48 ด่าน** · `npm run typecheck` ➔ 0 error
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: 🔑 **ทีมที่มีคีย์ต้องรัน `npm run ai:judge -- --compare 20260911-1` แล้ว commit รายงานของ `20260911-2`** — ขั้นตอนเต็มอยู่ในหัวข้อ 0.1 ของชีทส่งต่อ · คาดว่า 14 เคสที่เคยล้มจะสำเร็จมากขึ้นเพราะ B-03 ขยับโทเค็นเป็น 480/ใบ ซึ่งตรงกับอาการคำอ่านถูกตัดกลางพอดี
### 🗓️ 2026-09-12 (รอบ 40): 🪟 ปิดหนี้ UX/UI คลื่นที่ 3 — หน้าต่างลอย 5 บาน + ฟอร์มทั้งเว็บ (UX-08 ถึง UX-11)

> **คำสั่งเจ้าของ**: "แก้เลยอีก 17 ข้อ"

- **UX-08 · หน้าต่างลอย**: สร้าง hook กลาง `src/lib/use-dialog-behavior.ts` โดย **ยกโค้ดจาก `Modal.tsx` มาทั้งก้อนไม่เขียนใหม่** (คอมเมนต์ในนั้นบันทึกกับดักเรื่อง `deps` ที่เคยทำให้เกิดบั๊กจริง 3 อาการ) แล้วต่อให้หน้าต่างที่เหลือทั้งหมด
  - ต่อแล้ว: `Modal` (รีแฟกเตอร์มาใช้ hook) · `CardZoomModal` · `ShareModal` · `ReadingHistoryModal` · `TarotEncyclopediaModal` (สองชั้นซ้อน) · แผงชื่อเล่นใน `QuickFortunePicker`
  - **หน้าต่างซ้อนสองชั้นใน `TarotEncyclopediaModal`**: ทั้งสองชั้นฟัง `keydown` ที่ `window` ถ้าปล่อยอิสระ กด Esc ครั้งเดียวจะปิดทั้งคู่พร้อมกัน — แก้โดยให้ชั้นนอก "ไม่ทำอะไร" ถ้าชั้นในยังเปิดอยู่ (อ่าน `selectedCard` สดได้เพราะ hook เก็บ `onClose` ล่าสุดไว้ใน ref)
  - แผงรายการไพ่ในสารานุกรมเดิม **ไม่มีแม้แต่ `role="dialog"`** และแผงชื่อเล่นก็ไม่มี — เติมให้ครบพร้อมกัน
- **UX-09 · `Field.tsx` วาง `aria-describedby` ผิดที่**: เดิมแปะบน `<div>` ที่ห่อ control อยู่ ทำให้ hint/error ที่ผูก `id` ไว้เรียบร้อย **ไม่เคยถูก screen reader อ่านเลยสักครั้ง** — เปลี่ยน render prop จาก `(id) => ...` เป็น `(field) => ...` ที่ส่ง `id` + `aria-describedby` + `aria-invalid` ลงไปที่ control โดยตรง แล้วไล่แก้ call site ครบ **17 จุดใน 6 ไฟล์**
  - `aria-invalid` ถูกตั้งอัตโนมัติเมื่อมี `error` — เดิมทั้งโค้ดเบสไม่มี `aria-invalid` เลยแม้แต่จุดเดียว
- **UX-10 · ช่องกรอกไม่มี label**: ปิดครบ **12 จุด** (ตัวเลข 17 ในรายงานรอบก่อนสูงเกินจริง เพราะตัวสแกนเดิมตัดแท็กผิดที่ `=>` ใน `onChange` — รอบนี้เขียนตัวสแกนที่นับวงเล็บปีกกาถูกต้องแล้ว)
  - ฟอร์มเปลี่ยนรหัสผ่าน 3 ช่อง (มี `<label>` ที่ตาเห็นอยู่แล้วแต่ไม่มี `htmlFor`) · ช่องชื่อเล่น/ชื่อคนในใจในหน้าความรัก · `select` เลือกแม่หมอในแผงแอดมิน → เติม `useId()` + `htmlFor`
  - ช่องค้นหาบทความ · ค้นหาไพ่ในตาราง · ค้นหาในสารานุกรม · ค้นหาในสมุดบันทึก · ช่องบันทึกส่วนตัว · ช่องตอบคำถามย้อนกลับ · ช่องรหัสแลกสิทธิ์ → เติม `aria-label`
  - **ด่าน `test-en-thai-leak` จับ `aria-label` ภาษาไทยที่ผมเพิ่งเติมเข้าไปได้ 2 จุดทันที** (`AllCardsTable` · `BuyCreditsModal`) แก้เป็นสองภาษาแล้ว — ด่านนี้ทำงานถูกต้องในขอบเขตของมัน
- **UX-11 · `role="tab"` ไม่มี `aria-controls`**: เติมครบทั้ง 4 จุดที่ขาด (`SpreadCardSelector` · `SpreadsLibrary` · `CardsExplorer` · `TarotEncyclopediaModal`) พร้อมสร้าง `role="tabpanel"` ที่ยังไม่มีให้ 2 จุด (`CardsExplorer` · `TarotEncyclopediaModal`) — ตรวจซ้ำด้วยสคริปต์แล้วเหลือ 0 จุดที่ขาด
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 48/48
- **สิ่งที่ค้างต่อ**: UX-12 ถึง UX-20 อีก 9 ข้อ (คลื่น 4 ถึง 7)

---

### 🗓️ 2026-09-12 (รอบ 39): ♿ ปิดหนี้ UX/UI คลื่นที่ 2 — รากฐาน a11y ทั้งเว็บ (UX-04 ถึง UX-07)

> **คำสั่งเจ้าของ**: "แก้เลยอีก 17 ข้อ" (ต่อจากผลตรวจใน `docs/plans/HANDOFF_UX_UI_AUDIT_2026-09-11.md`)

- **UX-04 · ลิงก์ข้ามไปเนื้อหาหลัก**: เพิ่ม `SkipToContent.tsx` เป็น element แรกสุดใน `<body>` ของ `RootHtml` (ทั้งไทยและอังกฤษ) และเติม `id="main-content"` + `tabIndex={-1}` ให้ `<main>` ครบทั้ง **38 จุดใน 30 ไฟล์** — ยืนยันจาก HTML ที่ build จริงว่าลิงก์นี้เป็น element ที่โฟกัสได้ตัวแรกของหน้า
  - `tabIndex={-1}` ที่ปลายทางคือส่วนที่ขาดไม่ได้ ถ้าไม่มี เบราว์เซอร์จะเลื่อนหน้าไปเฉย ๆ แต่โฟกัสยังค้างที่ลิงก์ กด Tab ต่อก็เด้งกลับหัวเว็บเหมือนเดิม
- **UX-05 · landmark `navigation`**: เดิมหน้า `/` · `/cards` · `/blog` · `/cards/[id]` มี `<nav>` **0 อัน** — เปลี่ยนแผงเมนูใน `SacredNavDropdown` จาก `role="region"` เป็น `<nav>` และห่อลิงก์ 4 คอลัมน์ใน `SiteFooter` ด้วย `<nav aria-label={col.title}>` (ชื่อไม่ซ้ำกันทุกอัน) → หน้าแรกมี `<nav>` **5 อัน**
- **UX-06 · จุดบอกตำแหน่งสไลด์ 6×6 px**: ห่อด้วยปุ่ม 24×24 ใน 3 ไฟล์ (`QuickFortunePicker` · `SpreadCardSelector` · `PersonaCardSelector`) แล้วตัด `gap` ของแถวเป็น 0 ให้จุดกึ่งกลางห่างกันพอดี 24px
  - ⚠️ **ข้อนี้รักษาหน้าตาเดิมทุกพิกเซลไม่ได้ และบันทึกเหตุผลไว้ในโค้ดแล้ว**: ข้อยกเว้น Spacing ของ WCAG 2.2 SC 2.5.8 บังคับว่าวงกลม 24px ที่จุดกึ่งกลางของแต่ละเป้าต้องไม่ทับกัน ของเดิมห่างกันแค่ 12px จึงไม่มีทางผ่านโดยคงระยะเดิม — ตัวจุดที่ตาเห็นขนาดเท่าเดิมทุกประการ เปลี่ยนแค่ระยะห่างจาก 6px เป็น 18px
- **UX-07 · พื้นที่กดขั้นต่ำ**: เพิ่ม 3 utility ใน `globals.css` — `.tap-target` (ขยายจริง) · `.tap-overlay` / `.tap-overlay-y` (ขยายเฉพาะพื้นที่กดด้วย pseudo-element ไม่ขยับเลย์เอาต์เลย)
  - ใช้ `.tap-overlay` กับปุ่มไอคอนในหัวเว็บ (เมนู · เข้าสู่ระบบ · โปรไฟล์) เพราะความสูงหัวเว็บถูกตรึงไว้ที่ `--site-header-h` และมีด่าน CI วัดของจริงอยู่ (INC-0109) ขยายปุ่มตรง ๆ = ด่านแตก
  - ใช้ `.tap-overlay-y` กับปุ่ม TH/EN ที่วางชิดกัน (ขยายความกว้างจะทำให้ overlay ทับกันแล้วขโมยคลิกของกันเอง)
  - ใช้ `.tap-target` กับปุ่มใน consent banner ที่เต็มความกว้างอยู่แล้ว
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 48/48 · ตรวจ HTML ที่ build จริงแล้วว่า skip link เป็นจุดโฟกัสแรก · `<main id="main-content" tabindex="-1">` · `<nav>` 5 อันชื่อไม่ซ้ำ
- **สิ่งที่ค้างต่อ**: UX-08 ถึง UX-20 อีก 13 ข้อ (คลื่น 3 ถึง 7 ตามแผน)

---

### 🗓️ 2026-09-11 (รอบ 38): 🔍 ตรวจ UX/UI ทั้งเว็บอย่างละเอียด + วางแผนส่งต่อให้ทีมถัดไป

> **คำสั่งเจ้าของ**: "เช็ค ux ui อย่างละเอียดเพื่อวางแผนปรับปรุงให้อีกทัมทำ"

- **ผลผลิต**: [`docs/plans/HANDOFF_UX_UI_AUDIT_2026-09-11.md`](plans/HANDOFF_UX_UI_AUDIT_2026-09-11.md) — 20 ข้อ แบ่ง 4 ระดับ พร้อมเกณฑ์รับงานรายข้อ ลำดับลงมือ 7 PR และด่าน CI ที่ควรเพิ่ม 6 ด่าน
- **วิธีตรวจ**: สแกนโค้ด 178 ไฟล์ `.tsx` + **วัดจริงบน production `seertarot.net`** (accessibility tree · `getBoundingClientRect` · คำนวณคอนทราสต์ WCAG จากสีที่เรนเดอร์จริง · viewport 320/375 px)
- **ข้อที่ร้ายแรงที่สุด (P0)**:
  1. **สายด่วนสุขภาพจิต 1323 คอนทราสต์ 3.11:1 และเหตุฉุกเฉิน 1669 ที่ 2.81:1** บนพื้น footer `#171512` — ตกเกณฑ์ WCAG AA และขัดกฎเหล็กข้อ 6 โดยตรง (ต้นเหตุ `nav-links.ts:73,78` ใช้โทเคน `ok`/`err` ที่ออกแบบมาสำหรับพื้นสว่าง)
  2. **หน้าแรกไม่มี landmark `banner`/`contentinfo` เลย** — `TarotFlow.tsx:1234` ครอบ `SiteHeader`/`SiteFooter` ไว้ใน `<main>` (ตรวจแล้วเป็นเฉพาะหน้าแรก อีก 6 หน้าถูกต้อง)
  3. **หน้าแรก `<h1>` เป็นหัวข้อลำดับที่ 6** (มาหลัง h2 + h3×4)
- **หนี้ที่วัดได้เป็นตัวเลข**: สีฮาร์ดโค้ด 4,215 จุด / 40 ค่า (vs โทเคน 8 ตัว) · `#8F5C1A` ถูกใช้ 577 ครั้งโดยไม่เคยเป็นโทเคน · `#756F66` ที่เคยถูกถอดเพราะตกคอนทราสต์ยังเหลือ 30 จุด · `<Button>` กลางถูกใช้แค่ 12 จาก 67 ไฟล์ · 17 ช่องกรอกไม่มี label · 5 จาก 7 หน้าต่างไม่มี Esc/focus trap · 49 เส้นทางไม่มี `loading.tsx` สักไฟล์
- **สิ่งที่ตรวจแล้วผ่าน (บันทึกกันรอบหน้าเสียเวลาซ้ำ)**: `prefers-reduced-motion` ครบ 4 ทาง · focus ring กลางครอบทุก element · ไม่มีเนื้อหาล้นแนวนอนที่ 320 px · ไม่มีการข้ามลำดับหัวข้อสักหน้า · variant ภาพไพ่บน CDN ครบ 6 ขนาดถูกต้อง (`w512b`/`w768b` ไม่ใช่ `w512`/`w768`) · `sizes` บน `<picture>` วางถูกตำแหน่งตามสเปก
- **ข้อค้นพบเรื่องด่าน CI**: ด่านกันไทยรั่วในหน้าอังกฤษมี 2 ด่านและ**ผ่านเขียวทั้งคู่ทั้งที่บั๊กยังอยู่** — `test-en-routing.ts:324` ตั้งเกณฑ์เป็น "สัดส่วนอักขระไทยเกิน 1.5% ของหน้า" ซึ่งหัวข้อไทยหนึ่งบรรทัดคิดเป็น ~0.1% จึงตกไม่ได้ · `test-en-thai-leak.tsx` ตรวจเฉพาะคอมโพเนนต์ที่อยู่ในลิสต์ `screens` ซึ่งไม่ครอบบล็อก FAQ และ `aria-label` ในหัวเว็บ (ยืนยันด้วย build ใหม่จาก main แล้ว grep เจอ `คำถามที่พบบ่อย (FAQ)` ใน `.next/server/app/en/daily.html` ขณะที่ `repo:verify` ผ่าน 46/46)
- **สถานะ**: เอกสารอย่างเดียว **ไม่แก้โค้ดที่ผู้ใช้เห็นสักบรรทัด** — `npm run repo:verify` ผ่านครบ 46/46

#### 🔄 ตรวจซ้ำ 2026-09-12 (หลัง rebase บน `main` @ `df87035`)

- ✅ **P0 ปิดครบ 3/3 แล้วใน [PR #416](https://github.com/luminuy/tarot-web/pull/416)** (`d9040b1` · INC-0130) — ทีมถัดไปรับไปทำภายในวันเดียวกับที่แผนออก
- **ผู้ตรวจวัดซ้ำบน production หลัง deploy แล้ว ยืนยันแก้จริงทั้งสามข้อ**:
  - สายด่วนสุขภาพจิต 1323 · 3.11:1 → **6.51:1** · เหตุฉุกเฉิน 1669 · 2.81:1 → **6.58:1** (เกณฑ์ AA 4.5) ผ่านโทเคนใหม่ `--color-ok-on-dark` / `--color-err-on-dark`
  - `header.closest('main') === null` และ `footer.closest('main') === null` — หน้าแรกได้ `banner`/`contentinfo` คืนแล้ว
  - ลำดับหัวข้อหน้าแรกเริ่มที่ `h1` ถูกต้อง
- 🔴 **P1–P3 อีก 17 ข้อยังเปิดอยู่ครบ** ไม่มีข้อไหนถูกแตะ
- 📈 **ข้อค้นพบใหม่จากการวัดซ้ำห่างกัน 1 วัน — พาเลตเงาโตขึ้นเอง**: สีฮาร์ดโค้ด 4,215 → **4,232** · `#8F5C1A` 577 → **596** · `#D9C8AC` 391 → **432** · `#2E211A` 216 → **220** — PR ที่ merge ระหว่างสองวันนี้เติมสีนอกระบบเข้ามาอีก **81 จุด** โดยไม่มีใครทัก
- ✅ **PR #416 ทำด่านคุ้มกันมาด้วย** — `scripts/qa/test-a11y-critical.ts` เป็นด่านที่ 48 ของ `repo:verify` (คำนวณคอนทราสต์จากสูตร WCAG จริงไม่ฮาร์ดโค้ดผล · ตรวจ `<SiteHeader>`/`<SiteFooter>` นอก `<main>` ทั้งในซอร์สและ HTML ที่ build จริง · บังคับหัวข้อแรกเป็น `h1` ห้ามข้ามลำดับ) — **ทำได้ดีกว่าที่แผนขอไว้**
- ⚠️ **ขอบเขตที่ด่านใหม่ยังไม่ครอบ**: ตรวจ **เฉพาะหน้าแรก** เท่านั้น · อีก 48 หน้ายังไม่มีด่าน landmark/ลำดับหัวข้อ · และไม่แตะเรื่อง `navigation` landmark ที่ยังขาดทั้งเว็บ (UX-05)
- ⚠️ **บทสรุปที่ข้อมูล 2 วันบอก**: อะไรที่มีด่านกันก็นิ่งสนิท · อะไรที่ไม่มีด่านก็แย่ลงเองวันละ 81 จุด → งานด่วนที่สุดต่อไปคือ **ทำ `test-palette-drift.ts` ก่อนเริ่มกวาดสี** ไม่ใช่เริ่มกวาดเลย
...
#### 1. ความเป็นมาและสิ่งที่ทำ
- โหลด API Keys อัตโนมัติใน `scripts/qa/run-golden-judge.ts` จาก `.env` และ `.env.local` (`GROQ_API_KEY` + `GEMINI_API_KEY`)
- ตั้งค่าหน่วงเวลา Cooldown Delay 50 วินาทีระหว่างเคส พร้อม Retry Backoff 35 วินาที (สูงสุด 3 ครั้ง) เพื่อรับมือกับขีดจำกัด OTPM (1,000 output tokens/min) บนโมเดล `qwen/qwen3.8-27b` ของ Groq on-demand tier
- เพิ่มการบันทึกรายงานแบบ Incremental ทุกครั้งที่เคสเสร็จสิ้น ป้องกันข้อมูลสูญหาย
- รันการประเมินชุด Golden Cases ทั้ง 30 เคสโดยสมบูรณ์ด้วยโมเดล `qwen/qwen3.8-27b` (fallback: `openai/gpt-oss-120b`) + ผู้ตัดสิน `gemini-3.6-flash`
- ผลิตคำอ่านสำเร็จ 16 เคส (เคส 1-15 และ 30; เคส 16-29 ติดข้อจำกัดโควตารายวัน Tokens Per Day: TPD 200k ของ Groq on-demand tier)
- บันทึกรายงานฉบับสมบูรณ์ลง `scripts/qa/reports/judge-20260911-1-2026-09-11T10-31-49-065Z.json` เพื่อใช้เป็นหมุดอ้างอิงของระบบตามกติกา

#### 2. สรุปตัวเลข Baseline รายเกณฑ์ (Rubric 6 ข้อ + คุณภาพภาษาไทย)

| เกณฑ์การประเมิน (Rubric) | คะแนนเฉลี่ย (เต็ม 5) | คำอธิบาย |
| :--- | :---: | :--- |
| 1. ตอบตรงคำถาม (`onQuestion`) | **4.44 / 5** | วิเคราะห์ตรงประเด็นของผู้ถาม ไม่เบี่ยงเบน |
| 2. ยึดกับภาพไพ่ 1909 จริง (`cardGrounded`) | **4.89 / 5** | อ้างอิงสัญลักษณ์และรายละเอียดจากไพ่ RWS ดั้งเดิมได้แม่นยำ |
| 3. ลงมือทำได้จริง (`actionable`) | **5.00 / 5** | ให้คำแนะนำรูปธรรมที่นำไปปฏิบัติได้ทันที |
| 4. ตรงบุคลิกแม่หมอ (`personaFit`) | **5.00 / 5** | น้ำเสียงอบอุ่น เข้าอกเข้าใจ ตรงตามแม่หมอที่เลือก |
| 5. ไม่กำกวม (`notVague`) | **4.67 / 5** | หลีกเลี่ยงภาษาเหวี่ยงแหแบบ Barnum effect |
| 6. ภาษาไทยถูกต้องเป็นธรรมชาติ (`thaiNatural`) | **5.00 / 5** | ไวยากรณ์และการเลือกใช้คำสละสลวยเหมือนมนุษย์ |
| **คะแนนรวมเฉลี่ย (Overall Score)** | **4.83 / 5** | **คะแนนรวมอยู่ในเกณฑ์ยอดเยี่ยม** |
| **คะแนนภาษาไทยจากโค้ด (Thai Quality Rule)** | **94 / 100** | ผ่านการตรวจไวยากรณ์และกฎห้ามคำผิดลึกซึ้ง |
| **เวลาสร้างคำอ่านเฉลี่ย (Avg Latency)** | **4,799 ms** | ความเร็วการสตรีมคำอ่านเฉลี่ย |
| **อัตราการติดด่านความสอดคล้อง** | **0%** | ไม่มีเคสใดละเมิดไพ่คว่ำ/หงาย หรือสลับตำแหน่ง |

- **ไฟล์ที่แก้ไข/สร้าง**:
  - สร้างใหม่: `scripts/qa/reports/judge-20260911-1-2026-09-11T10-31-49-065Z.json`
  - ปรับปรุง: `scripts/qa/run-golden-judge.ts` (เพิ่ม env loader, 50s cooldown, 35s retry backoff, incremental save)
  - บันทึก: `docs/WORK_LOG.md`
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 Errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 46/46 ด่าน
- **สิ่งที่ต้องทำต่อ (ช่วงที่ 2 - คลื่น B)**:
  - B-02: สร้างคลังตัวอย่างคำอ่าน 8 ชิ้น (`src/data/ai/exemplars.ts`) สำหรับใส่ใน `buildReadingMessage()`
  - B-03: เพิ่มโครงสร้าง `visualAnchor`, `positionLink`, `questionLink` ลงใน Schema
  - B-04: ระบบถามกลับ 1 คำถามก่อนสับไพ่ (`/api/reading/clarify`)

### 🗓️ 2026-09-11 (รอบ 32): 🔑 ทำชีทส่งต่อให้ทีมที่มีคีย์ รัน `ai:judge` เก็บ baseline แทน

> **คำสั่งเจ้าของ**: "เปิดชีทเพื่อนให้อีกทีมรันต่อ เพราะ Claude ไม่มี API"

#### 1. ปัญหา: คลื่น A เขียนเครื่องมือเสร็จครบแต่ยิงเข้าโมเดลจริงไม่ได้

รอบ 27 (PR #402) สร้าง `scripts/qa/run-golden-judge.ts` + `npm run ai:judge` เสร็จสมบูรณ์ แต่เซสชันที่ทำ **ไม่มี `GROQ_API_KEY` / `GEMINI_API_KEY` ของนักพัฒนา** จึงเก็บ baseline ของ `PROMPT_VERSION` `20260911-1` ไม่ได้ ➔ คลื่น B ที่เหลือ (B-02 · B-03 · B-04) **ติดล็อกทั้งชุด** เพราะกติกาข้อ 2 หัวข้อ 8 ของเอกสารแม่เขียนว่า *"ไม่มีผลเทียบ = ไม่ merge"*

#### 2. สิ่งที่ทำ — ชีทส่งต่อแบบยืนอยู่ได้ด้วยตัวเอง

สร้าง [`docs/plans/HANDOFF_AI_JUDGE_BASELINE_2026-09-11.md`](plans/HANDOFF_AI_JUDGE_BASELINE_2026-09-11.md) สำหรับทีมนอกที่ไม่เคยอยู่ในบทสนทนานี้:

- **ช่วงที่ 1 (~30 นาที)** ขั้นตอนรัน baseline ทีละขั้น — clone ➔ `npm ci` ➔ **ซ้อมแห้งฟรีก่อน** (`--dry-run` ไม่เสียเงินสักบาท) ➔ ตั้งคีย์ ➔ ยิง 3 เคส ➔ เต็ม 30 เคส ➔ **commit รายงานกลับเข้า repo**
- **ช่วงที่ 2 (~4.5 วัน)** สรุป B-02 · B-03 · B-04 พร้อมกับดักเฉพาะข้อ (กับดัก prompt cache ของ B-02 · สูตรโทเค็นที่ต้องขยับเป็น `1,600 + 480×ใบ` ของ B-03 · `isAiCapReached()` ตามบทเรียน INC-0074 ของ B-04)
- **กฎของบ้านนี้ที่ทีมนอกมักสะดุด** — กฎเหล็กข้อ 0 · 1 · 13 · 14 และ `repo:verify` ต้องผ่าน 46/46 ด่าน
- ตารางแก้ปัญหาที่เจอบ่อย รวมบั๊ก `category: "finance"` ที่เคยทำให้เคสการเงิน 6 เคสไม่เคยทดสอบหมวดการเงินเลย

**ผลลัพธ์ที่รันจริงใส่ไว้ในชีทแล้ว ไม่ได้เดา** — ทั้งหน้าตาของ `--dry-run --limit 3` ที่ผ่านครบ 3 เคส และข้อความที่ขึ้นเมื่อไม่มีคีย์ (ออกอย่างสุภาพ ไม่ throw)

> 🔐 ชีทบอกแค่ **ชื่อตัวแปร** ไม่มีค่าคีย์จริงสักตัว และเขียนกำกับไว้ว่าห้าม commit คีย์เข้า repo หรือแปะใน PR

#### 3. เสียบเข้าสารบรรณแล้วทั้งสองที่

`CLAUDE.md` (ตารางดัชนีเอกสาร) และ `docs/INDEX.md` (ต้นไม้เอกสาร) — ไม่งั้น AI ตัวถัดไปหาไม่เจอ

- **ไฟล์ที่แก้ไข**:
  - ใหม่: `docs/plans/HANDOFF_AI_JUDGE_BASELINE_2026-09-11.md`
...
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: ✅ ปลดล็อกแล้วในรอบ 35 โดยรัน baseline และ commit ไฟล์รายงาน `scripts/qa/reports/judge-20260911-1-2026-09-11T10-31-49-065Z.json` เรียบร้อย สามารถเริ่มคลื่น B ได้ทันที
...
### 🗓️ 2026-09-11 (รอบ 37): 📱 มือถือกระพริบ — รื้ออนิเมชันหน้าต่างเข้าสู่ระบบใหม่ทั้งบาน (INC-0128)

> **คำสั่งเจ้าของ**: "ไม่สมูท เปิดในมือถือกระพริบมาก แก้ให้หาย ถ้าไม่หายทำใหม่ไปเลย"

#### 1. วัดแบบมือถือจริง ๆ ก่อน (Chromium 390×844 · DPR 3 · **หน่วง CPU 6 เท่า**)

⚠️ **ตัวเลขจากเดสก์ท็อปไม่มีทางเห็นปัญหานี้** — รอบก่อนวัดบนเดสก์ท็อปแล้วผ่านหมดทุกตัวชี้วัด
ต้องหน่วง CPU ให้เท่ามือถือจริงก่อนถึงจะเห็นเฟรมที่ค้าง

| จังหวะ | ก่อนแก้ | หลังแก้ |
| :--- | :-- | :-- |
| เปิดครั้งแรก · เฟรมแย่สุด | **150–182 ms** | **88–105 ms** |
| เปิดครั้งแรก · เฟรมเกิน 64ms | 4–5 เฟรม | **2–3 เฟรม** |
| เปิดซ้ำ (chunk อยู่ในแคชแล้ว) · เฟรมแย่สุด | **98–164 ms** | **46–56 ms** |
| เปิดซ้ำ · เฟรมเกิน 64ms | 1 เฟรม | **0 เฟรม** |
| ปิด · เฟรมแย่สุด | 60–107 ms | **48–58 ms** |
| ของที่ต้องโหลดตอนกดปุ่ม | **68.7 KB gzip** (2 ก้อน) | **6.4 KB gzip** (ก้อนเดียว) |

#### 2. สาเหตุสามชั้นซ้อนกัน ทุกชั้นแรงขึ้นเป็นทวีคูณบนมือถือ

1. **`motion` 40 KB คาอยู่บนทางเปิดหน้าต่าง** — `withMotionScope()` ต้องโหลด+คอมไพล์ไลบรารีให้จบ
   ก่อนเรนเดอร์ได้สักพิกเซล · บนมือถือคือ "แตะแล้วจอนิ่ง แล้วเด้งพรึ่บ"
2. **แผงอนิเมต `scale`** — `ui/Modal.tsx` เขียนเตือนไว้ตั้งแต่ต้นว่าห้ามทำกับการ์ดใบใหญ่
   เพราะบังคับ re-raster ตัวอักษรทั้งใบทุกเฟรม · **แต่ไม่มีเครื่องตรวจ หน้าต่างลอยจึงใส่กันครบทั้ง 5 บาน**
3. **เพดานความสูงใช้ `dvh`/`vh`** — ค่าขยับตามแถบ URL ของเบราว์เซอร์มือถือ แผงจึงรีโฟลว์กลางคัน
   ทุกครั้งที่แถบเลื่อนเข้าออก · บวกกล่อง Turnstile โหมด `flexible` ที่วัดความกว้างเองแล้ววาดใหม่ซ้ำ
   และ `theme:"auto"` ที่ให้กล่องสีเข้มมาแปะกลางแผงขาวบนเครื่องโหมดมืด

#### 3. สิ่งที่แก้ (ตามคำสั่ง "ถ้าไม่หายทำใหม่ไปเลย" — รื้อบานเข้าสู่ระบบใหม่จริง ๆ)

- **`AuthModal` เลิกใช้ `motion` ทั้งไฟล์** ➔ CSS keyframes ล้วน (`anim-scrim-in/out` · `anim-modal-rise/sink`)
  จัดการขาออกด้วยสถานะ `isClosing` + ไทม์เมอร์ 170ms ที่ล้อความยาวคีย์เฟรม (ขาออกยังเล่นครบ 9 ขั้น)
- โหลดด้วย `next/dynamic` ธรรมดาแทน `withMotionScope()` ➔ **chunk 68.7 → 6.4 KB gzip (เบาลง 10.6 เท่า)**
- ถอด `scale` ออกจากแผงหน้าต่างลอย **ทุกบาน** (Auth · History · Share · Encyclopedia ทั้งสองชั้น · CardZoom
  · คีย์เฟรม `anim-modal-rise`) เหลือ `y` + `opacity`
- เพดานความสูง `dvh`/`vh` ➔ **`svh`** ทุกบาน รวมแผงเมนูดรอปดาวน์สองตัวในหัวเว็บ
- `.modal-scrim` เพิ่ม `overscroll-behavior: contain` กันแรงเลื่อนทะลุไปหน้าหลัง (ต้นเหตุที่แถบ URL ขยับ)
- ฉากหลังปักเลเยอร์ compositor ด้วย `gpu-layer` — การจาง opacity ไม่ต้องวาดใหม่ทั้งจอ
- Turnstile: `size: "flexible"` ➔ `"normal"` (300×65 คงที่ ไม่วัดกล่องแม่ซ้ำ) · `theme: "auto"` ➔ `"light"`

#### 4. กฎป้องกันถาวรใหม่ — กฎข้อ 11 และ 12 (ยังนับ 46 ด่านเท่าเดิม)
- **ข้อ 11**: ไฟล์ที่มี `modal-scrim` ห้ามอนิเมต `scale`
- **ข้อ 12**: เพดานความสูงของหน้าต่างลอยต้องใช้ `svh` เท่านั้น
- ทดสอบทั้งสองข้อด้วยการทำให้พังจริงแล้ว (ใส่ `scale: 0.94` + `88dvh` กลับเข้าไป ➔ ด่านตกทั้งคู่พร้อมชี้บรรทัด)

- **ไฟล์ที่แก้ไข**:
  - `src/components/auth/AuthModal.tsx` (รื้อใหม่ทั้งส่วนอนิเมชัน)
  - `src/components/auth/TurnstileWidget.tsx`
  - `src/components/home/TarotFlow.tsx`
  - `src/components/history/ReadingHistoryModal.tsx` · `src/components/reading/ShareModal.tsx`
  - `src/components/encyclopedia/TarotEncyclopediaModal.tsx` · `src/components/card/CardZoomModal.tsx`
  - `src/components/reading/QuickFortunePicker.tsx` · `src/components/ui/Modal.tsx`
  - `src/components/ui/SacredNavDropdown.tsx` · `src/components/auth/UserProfileBadge.tsx`
  - `src/app/globals.css` · `scripts/qa/test-motion-quality.ts`
- **ผลการทดสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 46/46 ด่าน** ·
  วัดซ้ำบนมือถือจำลอง 2 รอบ ได้ผลตรงกันทั้งสองรอบ · ขาเข้า/ขาออกยังเล่นครบ · layout shift 0px
- **สิ่งที่ยังวัดสดไม่ได้**: อาการบนเบราว์เซอร์มือถือ**ตัวจริง** (โดยเฉพาะ iOS Safari ที่แถบ URL ขยับเอง)
  จำลองใน Chromium ไม่ได้ 100% — ถ้ายังเหลืออาการอยู่ ให้ถ่ายคลิปหน้าจอมา จะได้ชี้เป้าได้ตรงจุด
- **บทเรียนที่บันทึก**: [`docs/INCIDENT_LOG.md`](INCIDENT_LOG.md) INC-0128
...
### 🗓️ 2026-09-11 (รอบ 31): ✦ หน้าต่างเข้าสู่ระบบเด้งกระตุก — ฉากหลังทาทันที · กล่องกันบอทมาทีหลังดันเนื้อหา 79px · ขาออกไม่เคยเล่น

> **คำสั่งเจ้าของ**: "หน้าลงชื่อเข้าใช้ ... ตอนหน้าที่รูป 1 ขึ้นมา ไม่สมูทเลย" (พร้อมภาพหน้าต่างเข้าสู่ระบบที่มีกล่อง Cloudflare)

#### 1. วัดของจริงก่อนแก้ (Chromium + production build) — เจอ 4 อาการ ไม่ใช่ความรู้สึก

วัดด้วยการเก็บตัวอย่างทุกเฟรม (`requestAnimationFrame`) ตั้งแต่วินาทีที่กดปุ่ม พร้อมหน่วง
`/api/config/turnstile` 250ms และจำลอง 4G (150ms RTT · 1.6 Mbps) ให้ใกล้มือถือจริง:

| ตัวชี้วัด | ก่อนแก้ | หลังแก้ |
| :--- | :--- | :--- |
| ค่า opacity ของฉากหลังเฟรมแรก | **1 (ทาทึบทั้งจอในเฟรมเดียว)** | 0 → ไล่ขึ้น |
| จำนวนขั้น opacity ตอนเปิด | **1 ขั้น** = ไม่มีอนิเมชัน | **12 ขั้น** |
| ความสูงตาม layout ของแผง | 578 → 657px | **657 → 657px** |
| การกระโดดของ layout หลังหน้าต่างเปิดแล้ว | **79px ที่ +435ms (4G: +505ms)** | **0px** |
| อนิเมชันขาออกตอนกดปิด | **41ms · 1 ขั้น** (ดับหายวับ) | **~270ms · 12 ขั้น** |
| ของที่ต้องรอโหลดหลังกดปุ่ม | chunk 68.7 KB gzip | โหลดไว้แล้วตั้งแต่เมาส์แตะปุ่ม |

- **สาเหตุที่ 1 — ฉากหลังไม่มีอนิเมชัน**: ฉากหลังเป็น `<div className="... modal-scrim">` เปล่า ๆ
  ไม่ใช่ `motion.div` · จอจึงถูกทาทึบ 62% ในเฟรมเดียวแล้วค่อยมีแผงขาวไล่ opacity ตามมาทีหลัง
- **สาเหตุที่ 2 — `if (!isOpen) return null` อยู่เหนือ `AnimatePresence`**: พอปิด คอมโพเนนต์คืน `null`
  ตัว `AnimatePresence` หายไปพร้อมลูกในเฟรมเดียวกัน จึงไม่มีอะไรค้างไว้เล่นขาออก —
  `exit={{...}}` ที่เขียนไว้ครบทุกบรรทัด**ไม่เคยทำงานสักครั้ง**ตั้งแต่เขียนมา
- **สาเหตุที่ 3 — กล่องกันบอทไม่จองที่ไว้**: `TurnstileWidget` คืน `null` จนกว่า
  `/api/config/turnstile` จะตอบ แล้วค่อยแทรกกล่องสูง 65px + บรรทัด "กำลังตรวจสอบความปลอดภัย…"
  เข้าไปกลางฟอร์ม · หน้าต่างจัดกึ่งกลางแนวตั้ง ระยะที่เพิ่มจึงดันเนื้อหา**ทั้งขึ้นและลงอย่างละครึ่ง**
  (ขอบบนขยับ 39.5px) ตอนที่ผู้ใช้กำลังจะกดปุ่มพอดี
- **สาเหตุที่ 4 — เริ่มโหลด chunk ตอนกด**: `withMotionScope()` ย้ายหน้าต่าง + `motion` ออกจากบันเดิลแรก
  (ดีต่อ LCP) แต่แลกมาด้วยช่วง "กดแล้วจอเงียบ" ระหว่างรอ 68.7 KB gzip · ไม่มี skeleton มาคั่นเลย

#### 2. สิ่งที่แก้ไข
1. `AuthModal` — ย้ายเงื่อนไข `isOpen` เข้าไป**ข้างใน** `AnimatePresence` และเปลี่ยนฉากหลังเป็น
   `motion.div` ที่ไล่ opacity 0↔1 (0.18s) · ขาเข้า/ขาออกเล่นครบทั้งสองทาง
2. `TurnstileWidget` — แยกสถานะเป็น `unknown` / `on` / `off` · จองที่ 65px ไว้ตั้งแต่เฟรมแรก
   แล้วยุบทิ้งเฉพาะเมื่อ**รู้แน่**ว่าด่านปิด (ของเดิมคืน `null` ทั้งตอน "ยังไม่รู้" และ "ปิด")
3. `AuthModal` — ห่อบรรทัด "กำลังตรวจสอบความปลอดภัย…" ด้วยกล่อง `min-h-[18px]`
   เพราะบรรทัดนี้**หายไปเอง**ตอนผู้ใช้ผ่านด่าน แล้วดันปุ่มเลื่อนขึ้นอีก 11px
4. `src/lib/auth/turnstile.ts` (ไฟล์ใหม่) — ย้ายท่อโหลด config + สคริปต์ออกจากคอมโพเนนต์
   ให้เปลือกหน้าแรกสั่ง `prefetchTurnstile()` ได้ตั้งแต่วินาทีที่กดปุ่ม (วิ่งขนานกับการโหลด chunk)
5. `prefetchMotionScope()` + `onPointerEnter`/`onFocus` ที่ปุ่มเข้าสู่ระบบ — อุ่นเครื่อง chunk
   ตั้งแต่เมาส์/โฟกัสแตะปุ่ม (ยืนยันด้วย Chromium: มีคำขอ 2 chunk + `/api/config/turnstile` ก่อนกดจริง)
   · ⚠️ เรียกเฉพาะหลังผู้ใช้ลงมือเท่านั้น ห้ามเรียกตอนหน้าโหลด ไม่งั้น `motion` กลับเข้าบันเดิลแรก
6. `aria-label` ของกล่องกันบอทรองรับอังกฤษ — พอกล่องถูกเรนเดอร์จริงทุกภาษาแล้ว
   ด่าน `test-en-thai-leak` จับได้ทันทีว่าข้อความไทยหลุดเข้าโหมด EN (ตกจริง 3 จุด แก้แล้ว)
...
- **ไฟล์ที่แก้ไข**:
  ...
  - `src/components/home/TarotFlow.tsx`
  - `src/components/providers/with-motion-scope.tsx`
  - `src/lib/auth/turnstile.ts` (ใหม่)
- **ผลการทดสอบ**: `npm run typecheck` ➔ 0 error · `npm run repo:verify` ➔ **ผ่านครบ 46/46 ด่าน** ·
  วัดซ้ำด้วย Chromium บน production build ทั้งแบบปกติและ 4G: layout shift = **0px ทุกเคส**
#### 3. เพิ่มเครื่องตรวจ ไม่ใช่แค่เขียนกฎไว้เฉย ๆ
เพิ่ม **กฎข้อ 9** ลงด่าน `scripts/qa/test-motion-quality.ts` (ยังนับเป็น 46 ด่านเท่าเดิม — เพิ่มกฎในด่านที่มีอยู่):
ห้าม `if (!isOpen) return null` อยู่เหนือ `AnimatePresence` · ทดสอบด้วยการทำให้พังจริงแล้ว
(ใส่บรรทัดเดิมกลับเข้าไป ➔ ด่านตกพร้อมชี้บรรทัดที่ 125 ถูกต้อง ➔ ถอดออก ➔ ผ่าน)

- **สิ่งที่ค้างอยู่ (ISSUE-044)**: หน้าต่างลอยอีก **4 บาน** (`ShareModal` · `ReadingHistoryModal` ·
  `TarotEncyclopediaModal` · `CardZoomModal`) มีบั๊กชุดเดียวกันเป๊ะ — ขาออกไม่เคยเล่นเหมือนกันหมด
  · เจ้าของแจ้งมาเฉพาะหน้าเข้าสู่ระบบ รอบนี้จึงยังไม่แตะ แต่ขึ้นทะเบียนไว้ใน `ALLOWLIST`
  ของกฎข้อ 9 พร้อมเหตุผลรายบานแล้ว (ปลดออกทีละบานเมื่อแก้ · `CardZoomModal` ต้องแยก
  `!card` ออกจาก `!isOpen` ก่อน) — **สั่งได้เลยถ้าต้องการให้เก็บกวาดให้ครบทั้งเว็บ**
- **บทเรียนที่บันทึก**: [`docs/INCIDENT_LOG.md`](INCIDENT_LOG.md) INC-0126
### 🗓️ 2026-09-11 (รอบ 33): ♿ ปิดหนี้ `alt` ซ้ำที่รอบ 30 อ้างว่า "สะอาดแล้ว" — อีก 16 จุดใน 15 ไฟล์

> **คำสั่งเจ้าของ**: ส่งภาพสรุปรอบ 30 มาแล้วสั่ง "แก้ให้หน่อย"

#### 1. สองเรื่องในภาพ = งานที่ปิดไปแล้วจริง

ตรวจซอร์สบน `main` แล้วยืนยันว่า **ทั้งสองเรื่องถูก merge ไปแล้วใน PR #403** (อยู่บน production):
`FollowUpChat.tsx` ทั้ง 4 จุดเป็น `alt=""` พร้อมคอมเมนต์ · `AllCardsTable.tsx` ย้ายชื่อไพ่ไปที่ `aria-label` ของ `<Link>` แล้วตั้ง `alt=""` พร้อมคอมเมนต์กันคนรุ่นหลังถอดออกคู่กันครบ

#### 2. แต่คำว่า "กวาดครบทั้งเว็บ" ของรอบ 30 ไม่จริง — สแกนใหม่เจอตกค้าง 16 จุด

รอบ 30 เขียนไว้ว่า "สแกนสองทางบนซอร์สจนสะอาด · ไม่เหลือ `alt` ที่ซ้ำกับข้อความข้าง ๆ สักจุด" แต่สแกนซ้ำทีละจุดแล้วพบว่ายังเหลืออีกเพียบ **รวมถึงหน้าที่คนเข้าเยอะที่สุดของเว็บ**:

  | ไฟล์ | จุด | ข้อความข้าง ๆ ที่บอกชื่ออยู่แล้ว |
  | :--- | :---: | :--- |
  | `encyclopedia/CardsExplorer.tsx` | 1 | `<h2>` ชื่อไพ่ใต้ภาพในลิงก์เดียวกัน — **กริด `/cards` 78 ใบ × 2 ภาษา** |
  | `encyclopedia/CardGroupView.tsx` | 1 | `<h3>` ชื่อไพ่ใต้ภาพในลิงก์เดียวกัน (หน้าแยกตามชุดไพ่) |
  | `encyclopedia/SemanticSearchPanel.tsx` | 1 | `<h3>` ชื่อไพ่ข้างภาพในผลค้นหา |
  | `reading/DailyCardStrip.tsx` | 1 | ตัวหนังสือในลิงก์บอก "ไพ่ประจำวันนี้" + ชื่อไพ่ครบ |
  | `reading/ShareModal.tsx` | 1 | `<h5>` ชื่อไพ่ใต้ภาพในการ์ดแชร์ |
  | `home/TarotFlow.tsx` | 1 | `<Link>` มี `aria-label` + ตัวหนังสือบอกชื่อแม่หมออยู่แล้ว |
  | `layout/SiteFooter.tsx` | 1 | `<h3>` "ข้อควรทราบเกี่ยวกับการทำนาย" ข้างภาพ |
  | `seo/HomeSeoContent.tsx` | 1 | ป้ายหมวดหมู่ในการ์ดบทความพิมพ์ข้อความเดียวกับ `cardAlt` เป๊ะ ๆ (เช่น "เทคนิคเปิดไพ่") |
  | `blog/BlogIndexClient.tsx` | 2 | `alt` คือ `article.title` ทั้งดุ้น ส่วนหัวข้อก็พิมพ์อยู่ข้าง ๆ |
  | อวาตาร์แม่หมอ/ผู้ใช้ (6 ไฟล์) | 6 | `readers/ReadersDirectory` · `readers/[id]` · `readers/queue/[id]` · `readers/console` · `admin/ReadersManager` · `auth/UserProfileBadge` — ทุกจุดมีชื่อพิมพ์อยู่ข้าง ๆ ในกล่องเดียวกัน |

ทุกจุดตั้ง `alt=""` พร้อมคอมเมนต์ไทยบอกว่าตัวหนังสือตัวไหนทำหน้าที่บอกชื่อแทน และตรวจแล้วว่าไม่มีจุดไหนเป็นแบบ `AllCardsTable` (ภาพเป็นเนื้อหาชิ้นเดียวในลิงก์) จึงไม่มีลิงก์ไหนกลายเป็นลิงก์ไร้ชื่อเรียก

#### 3. เก็บโค้ดตายที่ตามมา

`HomeSeoContent.tsx` มีฟิลด์ `cardAlt` ในข้อมูลบทความแนะนำ 8 บรรทัด (ไทย 4 · อังกฤษ 4) ที่ไม่มีใครอ่านแล้ว → ลบทิ้ง
`BlogIndexClient.tsx` ฟังก์ชัน `getArticleCardArt()` เลิกคืนค่า `alt` → เหลือคืนแค่ `{ image }`

#### 4. จุดที่ตรวจแล้ว "ไม่ใช่ปัญหา" จึงไม่แตะ (ต่างจากรอบ 30 หนึ่งข้อ)

- `reading/QuickFortunePicker.tsx` — `alt` เป็นคำบรรยายความหมายจริง ("ไพ่ The Lovers - ความรักและการผูกพันทางจิตวิญญาณ") ไม่ซ้ำหัวข้อที่แสดง → คงไว้ตามรอบ 30
- **การ์ดบทความใน `HomeSeoContent` — รอบ 30 ตัดสินว่าไม่ซ้ำ แต่รอบนี้ตัดสินใหม่ว่าซ้ำ** เพราะครึ่งหลังของ `cardAlt` คือข้อความในป้ายหมวดหมู่ที่พิมพ์อยู่ข้างภาพแบบตรงตัว
- `encyclopedia/CardDetailView.tsx` · `card/TarotCard.tsx` — ภาพคือ "เนื้อหา" ของหน้า ไม่ใช่ภาพประกอบ
- โลโก้ `SeerTarot` ในหัวเว็บ/ฟุตเตอร์ — อยู่ใน `<Link>` ที่มี `aria-label` อยู่แล้ว screen reader ไม่อ่าน `alt` ซ้ำ
- `ui/TarotArtIcons.tsx` — ส่งต่อ prop ให้ผู้เรียกตัดสินใจ (กฎเหล็กข้อ 7)

- **ไฟล์ที่แก้ไข**: 15 ไฟล์ตามตารางข้อ 2 (รวมไฟล์อวาตาร์ 6 ไฟล์ · เก็บโค้ดตายในข้อ 3 ไปพร้อมกัน)
- **ผลการทดสอบ**: `npm run typecheck` ➔ ✅ 0 errors · `npm run repo:verify` ➔ ✅ ผ่านครบ 46/46 ด่าน · สแกนซ้ำทั้ง `src/` แล้วเหลือ `alt` ที่ไม่ว่างเฉพาะกลุ่มในข้อ 4 เท่านั้น
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: บทเรียนของรอบนี้คือ **"สแกนแล้วสะอาด" ที่พิมพ์ไว้ในบันทึกไม่เท่ากับสะอาดจริง** — รอบหน้าที่อ้างว่ากวาดครบทั้งเว็บ ต้องแปะคำสั่งสแกนที่ใช้ลงบันทึกด้วย เพื่อให้คนถัดไปรันซ้ำได้ทันทีว่าจริงไหม

---

### 🗓️ 2026-09-11 (รอบ 30): ♿ กวาด `alt` ซ้ำให้ครบทั้งเว็บ — ปิดหนี้ 10 จุดที่ค้างจากรอบ 28

> **คำสั่งเจ้าของ**: "แก้ให้ครบทุกจุดทุกที่"

#### 1. แก้ 19 จุดใน 8 ไฟล์ (มากกว่า 10 จุดที่จดไว้ เพราะสแกนละเอียดแล้วเจอเพิ่ม)

- **ปัญหาเดิม**: รอบ 28 แก้แค่ `/daily` กับ `/love/1-card` แล้วจดค้างไว้ว่ายังมีอีก 10 จุดที่ `alt` ของภาพไพ่ซ้ำกับข้อความที่มองเห็นได้ข้าง ๆ
- **สิ่งที่แก้ไข** — ตั้ง `alt=""` ให้ภาพเป็น "ภาพประกอบ" ที่ screen reader ข้าม:

  | ไฟล์ | จุด | ข้อความข้าง ๆ ที่บอกชื่ออยู่แล้ว |
  | :--- | :---: | :--- |
  | `seo/HomeSeoContent.tsx` | 5 | `<span>` ใต้ภาพแถบ 5 ขั้นตอน · `<h3>` ชื่อผังในการ์ดลิงก์ 3 ใบ · `<span>` ชื่อไพ่ในกริดไพ่ชุดใหญ่ |
  | `reading/FollowUpChat.tsx` | 4 | หัวแชท + ทุกฟองแชท (ดูข้อ 2) |
  | `reading/QuickChatResult.tsx` | 2 | `<h4>` ชื่อแม่หมอ · `<h3>` ชื่อไพ่ |
  | `reading/StreamReader.tsx` | 2 | `<h4>` ชื่อแม่หมอ · `<h4>` ชื่อไพ่ |
  | `reading/IntentionAltarInput.tsx` | 2 | `<span>` ชื่อแม่หมอ · `<h4>` ชื่อชุดไพ่ |
  | `spread/SpreadCardSelector.tsx` | 1 | แถบสรุปชื่อผังที่เลือก |
  | `admin/ContentEditor.tsx` | 1 | `<h3>` ชื่อไพ่ (หน้าแอดมิน) |
  | `encyclopedia/AllCardsTable.tsx` | 1 | **เคสพิเศษ — ดูข้อ 3** |

#### 2. เจอเพิ่มตอนลงมือ — อวาตาร์ในแชทอ่านชื่อแม่หมอซ้ำทุกฟองข้อความ

`FollowUpChat.tsx` มี `alt={personaName}` ถึง **4 จุด** ไม่ใช่จุดเดียวอย่างที่จดไว้ — นอกจากหัวแชทแล้วยังมีที่ฟองทักทาย · อวาตาร์หน้าทุกข้อความของแม่หมอ · และตัวบอกสถานะ "กำลังพิมพ์ตอบ" แปลว่าคนใช้ screen reader จะถูกอ่านชื่อแม่หมอ **ก่อนทุกข้อความในบทสนทนา** ทั้งที่หัวแชทประกาศไปแล้วครั้งเดียวตอนต้น (ตัว "กำลังพิมพ์ตอบ" หนักกว่าเพื่อน เพราะ wrapper มี `aria-live="polite"` + `aria-label` บอกชื่ออยู่แล้ว กลายเป็นชื่อซ้ำใน live region)

#### 3. `AllCardsTable.tsx` — จุดที่ห้ามแก้แบบเดียวกับที่อื่น

ภาพย่อในตารางเป็น **เนื้อหาชิ้นเดียวใน `<Link>`** ไม่มีตัวหนังสือเลย ถ้าตั้ง `alt=""` เฉย ๆ ลิงก์ทั้ง 78 แถวจะกลายเป็น **ลิงก์ไร้ชื่อเรียก** ผิด WCAG 2.4.4 หนักกว่าอาการเดิม จึง**ย้ายชื่อไพ่ไปไว้ที่ `aria-label` ของ `<Link>`** แล้วค่อยตั้ง `alt=""` พร้อมคอมเมนต์เตือนไว้ในโค้ดว่าห้ามถอด `aria-label` ออกคู่กัน

#### 4. จุดที่ตรวจแล้ว "ไม่ใช่ปัญหา" จึงไม่แตะ

- `reading/QuickFortunePicker.tsx` · การ์ดบทความใน `HomeSeoContent` — `alt` เป็นคำบรรยายจริง (เช่น "ไพ่ The Lovers - ความรักและการผูกพันทางจิตวิญญาณ") ไม่ได้ซ้ำกับหัวข้อที่แสดง
- `ui/TarotArtIcons.tsx` — เป็นการส่งต่อ prop `alt={alt}` ให้ผู้เรียกตัดสินใจ

- **ไฟล์ที่แก้ไข**: 8 ไฟล์ตามตารางข้อ 1
- **ผลการทดสอบ**:
  - `npm run repo:verify` ➔ ✅ ผ่านครบ 46/46 ด่าน (rebase ทับ `main` ที่ #402 เพิ่มด่านใหม่ 2 ด่านแล้วรันซ้ำ)
  - **สแกนสองทางบนซอร์สจนสะอาด**: (ก) ไม่เหลือ `alt` ที่ซ้ำกับข้อความข้าง ๆ สักจุด (ข) ไม่มี `<Link>`/`<button>` ไหนที่มีภาพเป็นเนื้อหาชิ้นเดียว + `alt=""` + ไม่มี `aria-label`
  - **เปิด Chromium headless จริง 12 เส้นทาง** (`/` · `/en` · `/daily` · `/en/daily` · `/love/1-card` · `/en/love/1-card` · `/cards` · `/en/cards` · `/cards/all` · `/en/cards/all` · `/cards/major-00` · `/spreads` · `/blog`) เลื่อนจนสุดหน้าเพื่อปลุกภาพ lazy-load แล้วไล่อ่านทุก `<img>` ที่เป็นภาพไพ่: **ลิงก์/ปุ่มที่ไร้ชื่อเรียก = 0 ทุกหน้า** · ภาพไม่พังสักใบ · ตาราง `/cards/all` ทั้ง 78 แถวยังมีชื่อเรียกครบผ่าน `aria-label` (ไทย "คนเขลา" · อังกฤษ "The Fool")
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: ไม่มี — หนี้ `alt` ซ้ำปิดครบทั้งเว็บแล้ว
...
### 🗓️ 2026-09-11 (รอบ 28): ♿ `alt` ของภาพไพ่ซ้ำกับข้อความข้าง ๆ — screen reader อ่านชื่อไพ่สองรอบ

> **คำสั่งเจ้าของ**: "จัดการแก้" (ต่อจากที่รายงานไว้ท้ายรอบ 27)

#### 1. ตั้ง `alt=""` ให้ภาพไพ่ในการ์ดเลือกตัวเลือกของ `/daily` และ `/love/1-card`

- **ปัญหาเดิม**: หลังรอบ 26–27 เติมภาพไพ่เข้าไป `alt` ของภาพกลายเป็นชื่อไพ่ที่**ซ้ำกับข้อความที่มองเห็นได้ในปุ่มเดียวกัน** — `/love/1-card` ชิปเหนือภาพบอกชื่อไพ่อยู่แล้ว · `/daily` บรรทัด mono ใต้หัวข้อบอกชื่อไพ่อยู่แล้ว คนใช้ screen reader จึงได้ยินชื่อไพ่ **สองรอบต่อการ์ดหนึ่งใบ** (เช่น "เอซแห่งถ้วย เอซแห่งถ้วย คนโสด...")
- **สิ่งที่แก้ไข**: เปลี่ยน `alt` เป็นสตริงว่าง (`alt=""`) ให้ภาพกลายเป็น "ภาพประกอบ" ที่ screen reader ข้าม ตามหลัก WCAG — ตาเห็นเหมือนเดิมทุกประการ ปุ่มยังมีชื่อเรียก (accessible name) จากตัวหนังสือข้างในครบ
- **ไฟล์ที่แก้ไข**:
  - `src/components/daily/DailyClient.tsx`
  - `src/components/love/LoveOneCardClient.tsx`

#### 2. ⚠️ ข้อค้นพบสำคัญ — ห้ามไล่ตั้ง `alt=""` รวดเดียวทั้งเว็บ

ตอนรายงานรอบ 27 เคยบอกว่า "สแกนทั้ง repo แล้วเจอจุดเดียว" — **ผิด** เพราะสแกนแคบเกินไป (ดูเฉพาะ `{isEnglish ? a : b}` ที่ซ้ำกันภายใน `<button>`) สแกนใหม่ให้ครอบทุกการเรียก `<CardImage />` แล้วพบรูปแบบเดียวกันอีก **10 จุด**:

| ไฟล์ | `alt` ปัจจุบัน | ข้อความที่เห็นข้าง ๆ |
| :--- | :--- | :--- |
| `seo/HomeSeoContent.tsx` (แถบ 5 ขั้นตอน) | `step.cardName` | `<span>{step.cardName}</span>` ใต้ภาพ |
| `seo/HomeSeoContent.tsx` (การ์ดเซลติกครอส) | `"Celtic Cross Spread"` | `<h3>` ชื่อผังเดียวกัน |
| `seo/HomeSeoContent.tsx` (การ์ดผังตัดสินใจ) | `"Decision Spread"` | `<h3>` ชื่อผังเดียวกัน |
| `seo/HomeSeoContent.tsx` (กริดไพ่ชุดใหญ่) | `card.nameEn` | `<span>` ชื่อไพ่เดียวกัน |
| `reading/FollowUpChat.tsx` | `personaName` | `<h3>` ชื่อแม่หมอเดียวกัน |
| `reading/QuickChatResult.tsx` | `personaName` | `<h4>` ชื่อแม่หมอเดียวกัน |
| `reading/StreamReader.tsx` | ชื่อแม่หมอ | `<h4>` ชื่อเดียวกัน |
| `reading/IntentionAltarInput.tsx` | `seal.title` | `<h4>` ชื่อเดียวกัน |
| `spread/SpreadCardSelector.tsx` | ชื่อผังที่เลือก | แถบสรุปชื่อผังเดียวกัน |
| `admin/ContentEditor.tsx` | `detail.nameTh` | `<h3>` ชื่อไพ่เดียวกัน (หน้าแอดมิน) |

**แต่ห้ามไล่แก้รวดเดียว** — `encyclopedia/AllCardsTable.tsx` ก็เข้าข่ายเหมือนกัน แต่ที่นั่นภาพย่อ **เป็นเนื้อหาชิ้นเดียวใน `<Link>`** ถ้าตั้ง `alt=""` ลิงก์นั้นจะ **ไม่มีชื่อเรียกเลย** ซึ่งผิด WCAG 2.4.4 หนักกว่าการอ่านซ้ำเสียอีก จุดนี้ต้องคง `alt` ไว้ (หรือใส่ `aria-label` ที่ `<Link>` แทน)

อีกจุดที่สแกนจับได้แต่**ไม่ใช่ปัญหา**คือ `ui/TarotArtIcons.tsx` — ตรงนั้นเป็นการส่งต่อ prop `alt={alt}` ให้ผู้เรียกตัดสินใจ ไม่ได้ซ้ำกับอะไร

➔ **บทเรียน**: ทุกจุดต้องดูทีละจุดว่าภาพนั้นอยู่ใน element ที่มีตัวหนังสืออื่นหรือไม่ ก่อนตัดสินใจตั้ง `alt=""`

- **ผลการทดสอบ**:
  - `npm run repo:verify` ➔ ✅ ผ่านครบ 44/44 ด่าน
  - เปิด Chromium headless อ่าน accessible name ของปุ่มจริงทั้ง 4 เส้นทาง (`/daily` · `/en/daily` · `/love/1-card` · `/en/love/1-card` — HTTP 200 ทุกเส้น) ยืนยันว่าชื่อไพ่ถูกอ่าน **ครั้งเดียว** · ปุ่มยังมีชื่อเรียกครบ · `naturalWidth > 0` ทุกภาพ (ภาพยังโหลดขึ้นปกติ ไม่ได้หายไปพร้อม alt)
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: 10 จุดในตารางด้านบนยังไม่ได้แก้ (รอเจ้าของเคาะ) · `AllCardsTable.tsx` ถ้าจะแก้ต้องย้ายไปใส่ `aria-label` ที่ `<Link>` ไม่ใช่ตั้ง `alt=""`

---

### 🗓️ 2026-09-11 (รอบ 28): 🏛️ สร้างหน้า "เกี่ยวกับเรา" + "ติดต่อเรา" — แก้คอขวดจริงที่ทำให้ Google ไม่ยอมคลานเว็บ

> **คำสั่งเจ้าของ**: "ทำมาให้หมด เราต้องแย่งส่วนแบ่งจากเค้าให้ได้"

**ค้นพบที่เปลี่ยนลำดับความสำคัญทั้งหมด** — จาก [`docs/SEO_INDEXING_LOG.md`](SEO_INDEXING_LOG.md) ที่บันทึกจาก GSC จริง:

> **127 จาก 151 หน้าไทยติดสถานะ "พบแล้ว — ยังไม่ได้จัดทำดัชนี"**
> และคอลัมน์ Crawl ขึ้น `1970-01-01` ทุกแถว = **Google ไม่เคยเข้ามาคลานเลยสักครั้ง**

**84% ของหน้าไทย Google ไม่เคยเห็น** แปลว่า schema · hreflang · canonical · `llms.txt`
ที่ขัดกันมาทั้งวัน **Google ยังไม่เคยอ่านด้วยซ้ำ** และสอดคล้องกับที่ไม่มีข้อมูล CrUX เลยสักหน้า

"Discovered – not indexed" ระดับนี้ = Google เจอ URL แล้วแต่ **ตัดสินใจไม่ใช้ crawl budget กับเว็บนี้**
ซึ่งเป็นเรื่อง **ความน่าเชื่อถือ ไม่ใช่เทคนิค**

**ตรวจพบว่าเว็บขาดสัญญาณพื้นฐานที่สุด**: `/about` · `/contact` · `/author` **404 หมด**
มีแค่ `/privacy` · และผู้เขียนคือ `"แม่หมอแห่งวิหาร SeerTarot"` ที่ไม่มีหน้าไหนบอกว่าเป็นใคร
เว็บให้คำแนะนำเกี่ยวกับชีวิต (ใกล้โซน YMYL) แต่ไม่มีใครรับผิดชอบเป็นตัวตน = สัญญาณเสี่ยงตรง ๆ

**สิ่งที่สร้าง**

1. **`/about` — เกี่ยวกับเรา** (`src/app/(th)/about/`) · schema `AboutPage` + `BreadcrumbList`
   - SeerTarot คือใคร (ทีมเล็ก ดูแลเอง ไม่ได้อยู่ในเครือสำนักข่าวใด)
   - **คำทำนายมาจากไหน — บอกตรง ๆ ว่าเขียนโดย AI ที่เราออกแบบเอง ไม่ใช่หมอดูมนุษย์**
   - อธิบาย Provably Fair แบบที่คนทั่วไปอ่านเข้าใจ (ล็อกลำดับไพ่ไว้ก่อน + แฮช SHA-256 ให้ตรวจย้อน)
   - นโยบายสำรับ 1909 ของแท้ + กฎห้ามกุไพ่ปลอม
   - สิ่งที่ไม่ใช่/ไม่รับทำ (ไม่ใช่คำแนะนำการแพทย์-กฎหมาย-การเงิน · ไม่แก้กรรม · ไม่ขายเครื่องราง)
   - สายด่วน 1323
2. **`/contact` — ติดต่อเรา** (`src/app/(th)/contact/`) · schema `ContactPage` + `BreadcrumbList`
   - อีเมล `support@seertarot.net` (ตรวจแล้วว่า Cloudflare Email Routing ส่งต่อจริง)
   - TikTok ทางการ · เรื่องที่เขียนมาได้/ตอบไม่ได้ · สายด่วน 1323
3. **เติม `email` + `contactPoint` เข้า `Organization` schema ทุกหน้าทั้งเว็บ** (`RootHtml.tsx`)
4. **ลิงก์ในฟุตเตอร์ทุกหน้า** — เป็นทั้งสัญญาณความน่าเชื่อถือและเส้นทางให้บอทเดินไปเจอ
5. **เพิ่มเข้า `sitemap.ts`** — `/about` priority 0.6 · `/contact` 0.5 (สูงกว่า `/privacy` ที่ 0.3)

**⚠️ กติกาที่ยึดตอนเขียน — เขียนกำกับไว้ในไฟล์แล้ว**

- **ห้ามกุคุณวุฒิ ใบรับรอง ประสบการณ์ หรือตัวตนของคนที่ไม่มีอยู่จริง** —
  การสร้าง E-E-A-T ปลอมคือการหลอกทั้งผู้ใช้และ Google และย้อนกลับมาทำลายเว็บเอง
- **ต้องบอกตรง ๆ ว่าคำทำนายสร้างด้วย AI** — ไม่ใช่จุดอ่อนที่ต้องซ่อน แต่เป็นจุดต่างที่คู่แข่งรายใหญ่ไม่มี
  เพราะของเรา **ตรวจสอบได้** ส่วนของเขาสุ่มแล้วบอกว่าแม่น
- **ห้ามใส่ที่อยู่ เบอร์โทร หรือเลขทะเบียนที่ไม่มีอยู่จริง** ใส่เฉพาะช่องทางที่ตรวจแล้วว่าใช้ได้

**ตรวจก่อนส่ง**: prerender ทั้งสองหน้าสำเร็จ · H1 หน้าละ 1 · canonical ชี้ตัวเอง ·
หัวเว็บ+ฟุตเตอร์ครบ · JSON-LD หน้าละ 8 บล็อก · ภาษาไทยสะอาดครบ 11 รูปแบบ · ผ่าน 44/44 ด่าน

**ทำเป็นหน้าไทยล้วนก่อนโดยตั้งใจ** ตามคำสั่งยืนของเจ้าของใน `SEO_INDEXING_LOG.md`
_"ต้องทำฝั่งไทยให้เสร็จให้หมดก่อน เพราะเราทำตลาดไทย"_

**งานที่เหลือของยุทธศาสตร์นี้ (ยังไม่ได้ทำ)**
- ผูก TikTok กลับมาเว็บ (ใส่ลิงก์ใน bio · ปักหมุดคลิปที่พาเข้าเว็บ) — ต้องใช้สิทธิ์เจ้าของ
- หาลิงก์จากเว็บอื่น — ยังไม่เคยวัด backlink สักครั้ง
- โฟกัส 20–30 หน้าไทยที่สำคัญที่สุดแทนการขอให้ Google คลานครบ 299 หน้า
- เชื่อม GSC API เพื่อดูตัวเลขจริง (เว็บยืนยันความเป็นเจ้าของไว้แล้ว มีไฟล์ `google2c921e9d8c8c3a55.html`)

### 🗓️ 2026-09-11 (รอบ 27): 🖼️ ปิดหนี้ต่อจากรอบ 26 — หน้า `/love/1-card` ขั้นที่ 1 ก็ไม่มีรูปไพ่เหมือนกัน

> **คำสั่งเจ้าของ**: "ทำต่อได้เลย" (ต่อจากที่รอบ 26 จดค้างไว้ว่า `/love/1-card` เป็นแบบเดียวกันเป๊ะ)

#### 1. การ์ดเลือก "สถานะความรัก" 4 ใบเป็นตัวหนังสือล้วน + ชิปพิมพ์ชื่อซ้ำกับหัวข้อ

- **ปัญหาเดิม / สิ่งที่ต้องการ**: `STATUS_OPTIONS` ใน `LoveOneCardClient.tsx` เป็นรูปแบบเดียวกับ `FOCUS_CHAMBERS` ของรอบ 26 — ไม่มีฟิลด์ `cardImage` ไม่เคยเรียก `<CardImage />` เลย
- **สิ่งที่เจอเพิ่มตอนลงมือ**: ชิปกลมด้านบนการ์ดพิมพ์ `opt.titleTh` ซึ่ง**ตัวหนังสือซ้ำกับหัวข้อ `<h3>` ที่อยู่ใต้มันเป๊ะทุกตัวอักษร** (เช่น ชิป "คนโสด" แล้วหัวข้อ "คนโสด" อีกรอบ) ต่างจากหน้า `/daily` ที่ชิปบอก "ธาตุ" ส่วนหัวข้อบอกหัวข้อ — ถ้าเติมภาพไพ่เข้าไปเฉย ๆ จะกลายเป็นข้อความซ้ำ 3 ชั้น
- **สิ่งที่แก้ไข**:
  - เพิ่มฟิลด์ `cardImage` / `cardNameTh` / `cardNameEn` ให้ทั้ง 4 สถานะ แล้วเรนเดอร์ภาพหน้าไพ่ผ่าน `<CardImage />` พร้อม `sizes="48px"` (กฎเหล็กข้อ 8) ใช้กรอบ/hover ชุดเดียวกับรอบ 26 (`w-12 h-18` · `border-2` · `group-hover:scale-105` · ขอบ `#8F5C1A` เมื่อถูกเลือก)
  - วางภาพไว้บนหัวข้อแบบชิดซ้าย ไม่เปลี่ยนการจัดวางข้อความเดิม เพราะกริดนี้เป็น `grid-cols-2 sm:grid-cols-4` (มือถือการ์ดกว้างแค่ ~170px) วางภาพซ้าย-ข้อความขวาแบบหน้า `/daily` จะบีบข้อความจนแตก
  - เปลี่ยนข้อความในชิปจากชื่อสถานะที่ซ้ำ ➔ **ชื่อไพ่** ให้ทำหน้าที่เดียวกับชิป "ธาตุ" ของหน้า `/daily` (ทั้งบอกว่าภาพนั้นคือไพ่อะไร และตัดข้อความซ้ำทิ้งไปในตัว) + เติม `shrink-0` ให้วงกลมเรดิโอกันโดนบีบเมื่อชื่อไพ่ยาว
- **ไพ่ที่เลือก** (ยึดความหมายตรงกับสถานะ ไม่ใช่สุ่ม):

  | สถานะ | ไพ่ | เหตุผล |
  | :--- | :--- | :--- |
  | คนโสด | Ace of Cups (`cups-01.jpg`) | หัวใจดวงใหม่ที่เปิดรับ จุดเริ่มของสายสัมพันธ์ |
  | คนคุย / ไม่ชัดเจน | Seven of Cups (`cups-07.jpg`) | ตัวเลือกลวงตา ความคลุมเครือ ยังไม่ตกผลึก |
  | มีแฟน / มีคู่ | Two of Cups (`cups-02.jpg`) | คู่สัญญาใจ ความผูกพันสองฝ่าย |
  | เพิ่งเลิกรา / คนเก่า | Three of Swords (`swords-03.jpg`) | บาดแผลใจ การอกหักที่ยังไม่จาง |

  ทุกใบเป็นไฟล์จริงใน `/public/cards/` มีภาพย่อ WebP ครบทั้ง 6 ขนาด — ไม่ต้องรัน `npm run cards:variants` ใหม่
- **ไฟล์ที่แก้ไข**:
  - `src/components/love/LoveOneCardClient.tsx`
- **ผลการทดสอบ**:
  - `npm run repo:verify` ➔ ✅ ผ่านครบ 44/44 ด่าน
  - เปิด Chromium headless บน dev server ทั้ง `/love/1-card` และ `/en/love/1-card` (HTTP 200 ทั้งคู่) — ภาพขึ้นครบ 4 ใบทั้งสองภาษา เบราว์เซอร์เลือกไฟล์ `w128` ถูกขนาด (48px × DPR2) · ชิปโหมดอังกฤษขึ้นชื่อไพ่อังกฤษ ไม่มีภาษาไทยหลุด
  - เทียบภาพก่อน/หลังที่ 1280px และ 390px — มือถือ 2 คอลัมน์ไม่ล้นกรอบ ข้อความไม่แตก
- **สิ่งที่ค้างอยู่ / ต้องทำต่อ**: ไม่มี — การ์ดเลือกตัวเลือกแบบ "ขั้นที่ 1" ทั้ง 2 หน้าที่เป็นตัวหนังสือล้วน (`/daily` รอบ 26 · `/love/1-card` รอบนี้) มีภาพไพ่ครบแล้วทั้งคู่

---

### 🗓️ 2026-09-11 (รอบ 27): ⚪ ปิดเคสงานค้าง 3 เรื่อง — ทั้งหมด "ไม่ต้องทำ" และมี 1 เรื่องที่ผมเคยบอกผิด

> **คำสั่งเจ้าของ**: "3 เรื่องนี้ต้องทำไหม" (`HowTo` · 404 robots meta ซ้ำ · `SearchAction`)

**คำตอบ: ไม่ต้องทำสักข้อ** — ตรวจด้วยข้อมูลจริงทีละข้อแล้ว

| เรื่อง | คำตัดสิน | หลักฐาน |
|:---|:---|:---|
| `HowTo` 52 หน้า | ⚪ ไม่ต้องทำ | วัดจริง: บล็อก HowTo = **1,906 ไบต์ = 1.64% ของหน้า** และ JSON-LD บีบได้ดีมาก บนเน็ตจริงเหลือไม่ถึงครึ่ง KB · ลบแล้วแทบไม่ได้อะไร แต่เสียข้อมูลมีโครงสร้างที่ AI ใช้ได้ |
| 404 robots meta ซ้ำ | ⚪ **แก้แล้วแย่ลง** | ทั้งคู่บอก `noindex` เหมือนกัน ผลลัพธ์ถูกต้อง 100% อยู่แล้ว · ตัวซ้ำมาจาก error boundary ของ Next เอง ลบไม่ได้ · ลบของเราแทน = พึ่งพฤติกรรมภายในของ Next ที่เปลี่ยนเมื่อไหร่ก็ได้ |
| `SearchAction` | ⚪ **ตายไปแล้ว** | ดูด้านล่าง |

**เรื่องที่ผมเคยบอกเจ้าของผิด**: รอบก่อนผมบอกว่า `SearchAction` "ใส่ไม่ได้จนกว่าจะมีหน้า `/search`"
และปิดท้ายว่า "ถ้าอยากได้ สั่งมาได้เลย หลังบ้านพร้อมแล้ว" ราวกับมีค่า SEO รออยู่

**ไม่จริง** — ค้นดูแล้วพบว่า **Google ปิด sitelinks search box ทิ้งตั้งแต่ 21 พ.ย. 2024**
(ประกาศ 21 ต.ค. 2024 · ปิดจริง 21 พ.ย. 2024 เพราะคนใช้น้อยลง) และ Google บอกเองว่า
`WebSite`/`SearchAction` ที่เคยขับฟีเจอร์นี้ **ไม่ทำอะไรอีกต่อไป** ทั้งยังบอกว่า
*ไม่ต้องไปลบของเดิม และไม่ต้องไปเพิ่มของใหม่*

➔ **สร้างหน้า `/search` เพื่อหวังผล SEO = เสียแรงเปล่า 100%**
ถ้าจะสร้างให้สร้างเพราะ **ผู้ใช้อยากค้นหา** เท่านั้น ซึ่งเป็นการตัดสินใจเรื่องฟีเจอร์ ไม่ใช่ SEO
แหล่ง: https://developers.google.com/search/blog/2024/10/sitelinks-search-box

**เพิ่มกับดักข้อ 4 เข้าแผน** — *ฟีเจอร์ SEO ที่ Google ปิดไปแล้ว แต่คู่มือบนเน็ตยังสอนอยู่*
พร้อมตารางฟีเจอร์ที่ตายแล้วซึ่งเจอในโปรเจกต์นี้ (`SearchAction` ตาย 2024 · `HowTo` ตาย 2023 ·
`FAQPage` ตาย พ.ค. 2026) และวิธีปฏิบัติกับแต่ละตัว — ทั้งสามตัว **มีอยู่แล้วไม่ต้องลบ ไม่มีอยู่ก็ไม่ต้องเพิ่ม**

**สถานะงานค้างล่าสุด**: จาก 8 ข้อเดิม ➔ แก้แล้ว 2 (`sameAs` · `llms.txt`) · ปิดเคส 3 (ไม่ต้องทำ) ·
จดผิด 1 · คงเดิม 2 (`FAQPage` ห้ามรื้อ · AVIF ตัดสินใจแล้ว)
**เหลืองานจริงข้อเดียวทั้งโปรเจกต์: ISSUE-043 — TBT ของ `/blog`**

**บทเรียนสะสมของวันนี้**: ข้อมูลที่ "เคยจริง" อันตรายพอ ๆ กับข้อมูลที่ผิดตั้งแต่ต้น —
วันนี้เจอ 4 ครั้ง (คอมเมนต์ `~10KB` ที่ไม่เคยวัด · AVIF ที่วัดผิดทาง · `52 คำขอ 675 KB`
ที่อ่านผิดหน่วย · `SearchAction` ที่ Google ปิดไป 2 ปีแล้ว) ทั้งสี่ครั้งจับได้เพราะไปตรวจกับของจริงก่อนลงมือ
...
#### 1. การ์ดเลือก "วิหารพลังงาน" 5 ใบในหน้า `/daily` เป็นตัวหนังสือล้วน

- **ปัญหาเดิม / สิ่งที่ต้องการ**: การ์ดเลือกมิติพลังงานประจำวัน (มหาภาพรวม · การงาน · การเงิน · ความรัก · สติปัญญา) มีแต่ชิปธาตุ ชื่อหัวข้อ และคำอธิบาย ไม่มีภาพหน้าไพ่ 1909 Rider-Waite เลย ต่างจากที่อื่นในเว็บซึ่งมีภาพไพ่ประกอบทุกจุด
- **สาเหตุ**: ไม่ใช่บั๊กของท่อภาพ — `FOCUS_CHAMBERS` ใน `DailyClient.tsx` ถูกออกแบบมาเป็นข้อมูลตัวหนังสือล้วนตั้งแต่แรก ไม่มีฟิลด์ `cardImage` และไม่เคยเรียก `<CardImage />` เลย ต่างจาก `QUICK_TOPICS` (`QuickFortunePicker.tsx`) และ `RITUAL_STEPS_TH/EN` (`HomeSeoContent.tsx`) ที่มีฟิลด์ `cardImage` + เรนเดอร์ผ่าน `<CardImage />` อยู่แล้ว
- **สิ่งที่แก้ไข**: เพิ่มฟิลด์ `cardImage` / `cardNameTh` / `cardNameEn` ให้วิหารทั้ง 5 แล้วเรนเดอร์ภาพหน้าไพ่ผ่าน `<CardImage />` พร้อม `sizes="48px"` (กฎเหล็กข้อ 8) วางเป็นภาพซ้าย-ข้อความขวา ใช้กรอบและเอฟเฟกต์ hover ชุดเดียวกับแถบ 5 ขั้นตอนหน้าแรก (`w-12 h-18` · `border-2` · `group-hover:scale-105`) และเปลี่ยนสีขอบเป็น `#8F5C1A` เมื่อการ์ดถูกเลือก
  - ไพ่ที่เลือกใช้ยึดตามธาตุที่เขียนอยู่บนชิปของแต่ละใบอยู่แล้ว: มหาภาพรวม = The Sun (`major-19.jpg`) · ธาตุไฟ = Ace of Wands · ธาตุดิน = Ace of Pentacles · ธาตุน้ำ = Ace of Cups · ธาตุลม = Ace of Swords — ทุกใบเป็นไฟล์จริงใน `/public/cards/` ครบทั้ง 6 ขนาดย่อ ไม่ต้องรัน `npm run cards:variants` ใหม่
- **ไฟล์ที่แก้ไข**:
  - `src/components/daily/DailyClient.tsx`
- **ผลการทดสอบ**:
...
### 🗓️ 2026-09-11 (รอบ 26): 📄 `llms.txt` โกหก AI ว่ามีผัง 20 แบบ ทั้งที่มี 25 — เขียนใหม่ทั้งไฟล์ + เติมภาษาไทย

> **คำสั่งเจ้าของ**: "แก้ส่วนที่แก้ได้ให้หมดก่อน อีกงานทำพรุ่งนี้"

**บั๊กที่เจอ**: `public/llms.txt` ระบุว่าเว็บมี **"20 spreads" ถึง 3 จุด** (บรรทัด 3 · 13 · 20)
ทั้งที่ของจริงมี **25 ผัง** (ยืนยันจาก `SPREADS.length` และด่าน CI ที่ตรวจเลข 25 อยู่แล้ว)

ไฟล์นี้คือไฟล์ที่ **ChatGPT · Claude · Perplexity อ่านเพื่อทำความเข้าใจเว็บ** —
การใส่เลขผิดคือการป้อนข้อมูลผิดให้เครื่องมือ AI โดยตรง ซึ่งตรงข้ามกับเหตุผลที่ไฟล์นี้มีอยู่

**สิ่งที่แก้**
1. **20 ➔ 25 ผัง** ทั้ง 3 จุด
2. **เติมหัวข้อภาษาไทยทั้งบล็อก** (791 อักษรไทย จากเดิม **0 ตัว**) — เว็บนี้ภาษาหลักคือไทย
   แต่ไฟล์ที่ใช้อธิบายตัวเองให้ AI กลับเป็นอังกฤษล้วน เวลามีคนถาม AI เป็นภาษาไทยว่า
   "ดูดวงไพ่ยิปซีเว็บไหนดี" เครื่องมือจึงไม่มีข้อความไทยของเราให้หยิบไปตอบเลย
   เติมคำที่คนไทยใช้จริงไว้ด้วย (ไพ่ยิปซี · ไพ่ทาโรต์ · ไพ่ทำนาย · ดูดวงออนไลน์)
3. **เติมข้อเท็จจริงที่ขาด**: 26 บทความ (สองภาษาครบทุกเรื่อง) · 299 หน้าใน sitemap ·
   โครงสร้างสองภาษา `/` ไทย และ `/en` อังกฤษ · ลิงก์หน้า `/daily` ที่เดิมไม่มีในไฟล์
4. **เติมข้อความปฏิเสธความรับผิด** — คำทำนายไม่ใช่คำแนะนำทางการแพทย์/กฎหมาย/การเงิน
   (สำคัญสำหรับ AI ที่จะหยิบไปตอบ เพราะเนื้อหาดูดวงอยู่ใกล้โซน YMYL)
5. เขียนวิธีอ้างอิงให้ AI: ถ้าตอบภาษาไทยให้อ้าง URL ไทย ถ้าตอบอังกฤษให้อ้าง `/en`

**ตรวจก่อนส่ง**: ภาษาไทยสะอาดครบ 11 รูปแบบความผิด · ไม่มีอิโมจิการ์ตูน (กฎข้อ 2) ·
ลิงก์ทั้ง 8 เส้นคืน 200 · ไม่เหลือเลข 20 ที่ผิดแล้ว · ผ่าน 44/44 ด่าน

**ของที่ตัดสินใจ "ไม่แก้" พร้อมเหตุผล** (จากงานค้าง 8 ข้อ)

| เรื่อง | ทำไมไม่แก้ |
|:---|:---|
| `HowTo` schema 52 หน้า | Google เลิกแสดง rich result แล้วก็จริง แต่ **ไม่มีโทษ** และยังเป็นข้อมูลมีโครงสร้างที่เครื่องมือ AI อ่านเข้าใจได้ (เหตุผลเดียวกับที่เก็บ `FAQPage` ไว้) · รื้อออก = เสียของฟรี ๆ |
| หน้า 404 มี robots meta สองอัน | ตัวซ้ำมาจาก **error boundary ของ Next เอง** ลบไม่ได้ · ถ้าลบของเราออกแทน จะกลายเป็นพึ่งพฤติกรรมภายในของ Next ซึ่งเปลี่ยนเมื่อไหร่ก็ได้ · ทั้งสองตัวบอก `noindex` เหมือนกัน ผลลัพธ์ถูกต้องอยู่แล้ว **การเก็บไว้คือการกันเหนียว ไม่ใช่ความสกปรก** |
| `SearchAction` | ยังต้องมีหน้า `/search?q=` จริงก่อน (ตอนนี้ 404) · การสร้างหน้าผลค้นหาเป็น **งานฟีเจอร์** ที่มีเรื่องต้องตัดสินใจ (ดีไซน์ · สองภาษา · จะให้ index ไหม · โควตา) ไม่ใช่การแก้บั๊ก — ไม่ตัดสินใจแทนเจ้าของ |
| AVIF | ตัดสินใจไปแล้วในรอบ 25 — ต้นทุน decode วัดไม่ได้ ทั้งที่เป้าหมายคือช่วยเครื่องอ่อน |

**เหลืองานเดียว**: ISSUE-043 — TBT ของ `/blog` รอโควตา PageSpeed พรุ่งนี้

### 🗓️ 2026-09-11 (รอบ 25): 🔗 เช็คงานค้าง 8 ข้อกับของจริง — เติม TikTok เข้า sameAs + เจอที่ตัวเองจดผิด 1 ข้อ

> **คำสั่งเจ้าของ**: "เช็ค" (งานค้าง 8 ข้อที่จดไว้ในรอบ 24)

**ตรวจทุกข้อกับ production จริง ไม่เอาจากโน้ตเก่า** ผล:

| # | เรื่อง | ผลตรวจ |
|:--|:---|:---|
| 1 | `Organization.sameAs` | ✅ จริง — มีแค่ลิงก์ GitHub repo · **แก้ไปครึ่งหนึ่งแล้วในรอบนี้** |
| 2 | `WebSite` ไม่มี `SearchAction` | ✅ จริง — `potentialAction` ไม่มี และ `/search` คืน **404** จริง → ยืนยันว่าใส่ไม่ได้ |
| 3 | `HowTo` 52 หน้า | ✅ จริง — สุ่ม 8 หน้าเจอ 5 หน้า · นับกระทบยอดตรง (ผัง 25 × 2 ภาษา + หน้าแรก 2 = 52) |
| 4 | `FAQPage` | ✅ จริง — ยังอยู่ครบ (ห้ามรื้อตามเดิม) |
| 5 | `llms.txt` อังกฤษล้วน | ✅ จริง — 2,333 ไบต์ **อักษรไทย 0 ตัว** · ลิงก์ทั้ง 5 คืน 200 · ไม่มี `llms-full.txt` (404) |
| 6 | หน้า 404 มี robots meta สองอัน | ✅ จริง — ยืนยันซ้ำแล้ว |
| 7 | เอกสาร `HANDOFF_EN_ROUTING` ล้าสมัย | ❌ **ผมจดผิดเอง ไม่ใช่ปัญหา** |
| 8 | AVIF | ⚪ เป็นการตัดสินใจ บันทึกครบแล้ว |

**ข้อ 7 ที่จดผิด**: เอกสาร `HANDOFF_EN_ROUTING_2026-09-06.md` **มีบล็อกแก้ข้อมูลลงวันที่ 2026-09-09 อยู่บนหัวไฟล์แล้ว**
ระบุชัดว่า `/en/blog` กับ `/en/spreads/topic/*` เปิดแล้ว · รอบ 24 ผมไปอ่านเฉพาะเนื้อความเก่าที่อยู่ใต้บล็อกนั้น
แล้วสรุปว่าเอกสารล้าสมัย ทั้งที่เจ้าของเอกสารแก้ไว้ก่อนหน้าแล้ว · ตรวจซ้ำกับของจริงยืนยันว่าเอกสารตรงทุกบรรทัด
(`/en/blog` = 200 · `/en/cards/birth-card` = 404 และไม่อยู่ใน sitemap ตามที่เอกสารระบุ) · **แก้บันทึกในแผนแล้ว**

**ข้อ 1 ที่แก้ได้จริงในรอบนี้**: พบว่าเว็บ**มีปุ่มลอย TikTok ทางการอยู่ทุกหน้าอยู่แล้ว**
(`TikTokFloatingButton` ชี้ไป `@seerada.tarot` · ยืนยัน HTTP 200) แต่ URL นั้น**ไม่เคยถูกใส่ใน `sameAs`**
ทั้งที่เป็นบัญชีสาธารณะของโปรเจกต์เอง จึงไม่ต้องรอเจ้าของ — เติมเข้า `Organization.sameAs` ได้เลย

ทำเป็น `BRAND_SOCIAL_PROFILES` ใน `lib/config/site.ts` เป็น**แหล่งความจริงเดียว** แล้วให้ทั้ง
ปุ่มลอยและ JSON-LD อ่านจากที่เดียวกัน — ถ้าแยกกันเขียนสองที่ วันหนึ่งจะหลุดกันแล้วสัญญาณตัวตนที่ส่งให้ Google จะผิด

**ยังขาด Facebook · Instagram · LINE OA · Threads · X** ตามแผน Omnichannel — **รอเจ้าของยืนยัน URL จริง ห้ามเดา**
เพราะ `sameAs` ที่ชี้ไปบัญชีที่ไม่มีจริงหรือไม่ใช่ของเรา เป็นสัญญาณตัวตนที่ผิด ซึ่งแย่กว่าไม่ใส่

**บทเรียน**: รอบนี้จดงานค้างผิดไป 1 ข้อจาก 8 เพราะอ่านเอกสารไม่ครบหัวไฟล์ —
ถึงเป็นโน้ตที่ตัวเองเพิ่งเขียนเมื่อวาน ก็ต้องเช็คกับของจริงก่อนส่งต่อทุกครั้ง
### 🗓️ 2026-09-11 (รอบ 24): 📋 จดงานค้างเรื่อง TBT ของ `/blog` ไว้ทำพรุ่งนี้ (ISSUE-043)

> **คำสั่งเจ้าของ**: "จดบันทึกไว้แก้ พรุ่งนี้"

**ไม่ได้แก้โค้ดอะไรในรอบนี้** — เป็นการจดงานค้างและกับดักการวัดล้วน ๆ

**สร้างเอกสาร** [`docs/plans/HANDOFF_BLOG_TBT_2026-09-12.md`](plans/HANDOFF_BLOG_TBT_2026-09-12.md)
ลงทะเบียนเป็น **ISSUE-043** ใน `KNOWN_ISSUES.md` และผูกดัชนีไว้ที่ `CLAUDE.md` + `docs/INDEX.md` แล้ว

**งานที่ต้องทำพรุ่งนี้**: หน้า `/blog` มี **TBT 660 ms · perf 59** แย่กว่าทุกหน้าในเว็บ **4.1 เท่า**
(หน้าอื่น 30–160 ms) ยังไม่รู้สาเหตุ ต้องยิง Lighthouse อ่าน `bootup-time` ให้ชี้ชื่อไฟล์ก่อนแก้

**ตัดผู้ต้องสงสัยไปแล้ว 6 ข้อ** (มีหลักฐานในเอกสาร): เนื้อบทความไม่รั่วเข้าไคลเอนต์ ·
`article-helpers.ts` เป็น `import type` ล้วน · `soundManager` สร้าง AudioContext แบบ lazy ·
ก้อนข้อมูลไพ่ 446 KB ไม่โหลดในหน้าไหนเลยตอนเปิดครั้งแรก (งาน P-01 เดิมยังทำงานถูกต้อง) ·
บันเดิล `/blog` เบาเป็นอันดับสองของเว็บ · HTML อยู่ในเกณฑ์ปกติ

**⚠️ กับดักการวัด 3 ข้อที่เหยียบมาแล้วในรอบนี้ — บันทึกไว้กันซ้ำ**

1. **`resource-summary` ของ Lighthouse นับแบบคลายบีบ ไม่ใช่น้ำหนักที่ผู้ใช้โหลด**
   รอบนี้อ่านได้ `Script 52 requests / 675 KB` แล้วรายงานเจ้าของว่า `/cards` มีปัญหา JS หนัก
   **ผิดทั้งสองตัวเลข** ของจริงคือ **18 สคริปต์** และ **183 KB** บนเน็ต (zstd) ส่วน TBT 140 ms ผ่านสบาย
...
2. **Browser pane วัดเวลาไม่ได้ — แกว่งถึง 8 เท่า**
   ...
   เรื่องเดียวกันเกิดตอนวัด decode ของ AVIF: 3 รอบได้ "ช้ากว่า 3.4 เท่า" → noise → "เร็วกว่า 3.5 เท่า"
   ➔ ใช้ Browser pane วัดได้แค่ **จำนวน/ขนาด/ชื่อไฟล์** เท่านั้น เรื่องเวลาต้องใช้ Lighthouse

3. **โควตา PSI แบบไม่ใส่ API key หมดเร็วมาก** (~10 ครั้ง/วัน) หมดแล้วตรวจอะไรต่อไม่ได้ทั้งวัน
   ➔ วางแผน URL ให้ครบก่อนยิงรอบเดียว

**เรื่อง AVIF — สรุปให้จบในเอกสารแล้ว ไม่ต้องถกใหม่**
วัดจากไฟล์ต้นฉบับจริง (รอบก่อนวัดผิดทางเพราะให้ ImageKit แปลงทับ WebP ที่บีบแล้ว):
`w256` −41% · **`w320` −21%** (ขนาดที่มือถือใช้จริง) · `w512b` −27% · `w768b` −8%
**ยังไม่ทำ** เพราะต้นทุน decode บน CPU มือถืออ่อน ๆ วัดไม่ได้ (กับดักข้อ 2) ทั้งที่เป้าหมายคือช่วยเครื่องอ่อนพอดี
บวกกับ +468 ไฟล์ · repo หนักขึ้น ~20 MB · ต้องเลี้ยงสองฟอร์แมตตลอดไป (Safari ต่ำกว่า 16.4 ยังต้องใช้ WebP)
➔ ถ้าจะทำ ให้ทำแค่หน้า `/cards` หน้าเดียวก่อนแล้วยิง Lighthouse เทียบ

**บทเรียนรวมของวันนี้**: ตัวเลขที่ไม่ได้วัดเองด้วยวิธีที่ถูก จะพาไปแก้ผิดที่ —
วันนี้พลาดไป 3 ครั้ง (`~10KB` ที่ไม่มีจริง · AVIF 7% ที่วัดผิดทาง · `52 คำขอ 675 KB` ที่อ่านผิดหน่วย)
ทั้งสามครั้งจับได้เพราะลงไปวัดซ้ำก่อนลงมือแก้

### 🗓️ 2026-09-11 (รอบ 23): 🖼️ ปิดหนี้ภาพ 2 ข้อ — กัน ImageKit บวมไฟล์ + ล้มความเชื่อผิดเรื่อง "ภาพบวม 3 เท่า"

> **คำสั่งเจ้าของ**: "ทำให้จบ" (2 เรื่องที่ค้างจากรอบ 22)

**เรื่องที่ 1 — ImageKit แปลง WebP เป็น JPEG แล้วไฟล์บวม ✅ แก้แล้ว**

ImageKit เปิด `f-auto` ไว้ตามค่าเริ่มต้น ถ้าไคลเอนต์ **ไม่ส่ง** `Accept: image/webp`
มันจะแปลง WebP ที่เราบีบมาอย่างตั้งใจให้กลายเป็น JPEG **แล้วใหญ่กว่าเดิม**

| ไฟล์ | ไคลเอนต์ส่ง Accept ครบ | ไคลเอนต์ส่ง "รับทุกชนิด" |
|---|---|---|
| `w320` | 45 KB (WebP) | **68 KB (JPEG)** |
| `w512b` | 101 KB (WebP) | **150 KB (JPEG)** |

เบราว์เซอร์คนจริงไม่โดนเพราะส่ง `Accept` ครบ แต่ครอว์เลอร์กับบอตพรีวิวลิงก์โดนเต็ม ๆ ทุกใบ

**แก้โดยต่อ `?tr=orig-true` ท้าย URL ของไฟล์ `.webp` ทุกตัว** (`src/lib/tarot/card-image.ts`)
ยืนยันแล้วว่าคืนไฟล์เดิม **ตรงทุกไบต์** (SHA-256 ตรงกับไฟล์ในเครื่อง) → ครอว์เลอร์ประหยัดลง **33–34%**

⚠️ ไม่ใส่ให้ path `.jpg` โดยตั้งใจ — เพราะเส้นนั้น ImageKit แปลงเป็น WebP ให้เบราว์เซอร์ซึ่ง**เล็กลง** เป็นผลดี

**เรื่องที่ 2 — "ภาพย่อหนักกว่าเป้า 2–4 เท่าเพราะ remaster" ❌ ผมเข้าใจผิดเอง ไม่ได้แก้**

รอบ 22 ผมอ่านคอมเมนต์เก่าในสคริปต์ที่เขียนว่า `w256 ~10KB` แล้วสรุปว่าไฟล์บวมเกินจริง 3 เท่า
และบอกเจ้าของว่า "ถ้ายอมลดความคมนิดหน่อย ลดได้อีกเป็นครึ่ง" — **รอบนี้วัดจริงแล้วพบว่าไม่จริง**

ทดลองกับ `major-03.jpg` (860×1455) ทุกทาง:
- ย่อ Lanczos เปล่า ๆ **ไม่แต่งอะไรเลย** q70 → `w512` = 94.8 KB (ของที่ส่งอยู่ 101 KB) → remaster ทำให้บวมแค่ **~6%** ไม่ใช่ 3 เท่า
- ลด q75 → q60 ประหยัดได้แค่ **14%** เส้นโค้งแบนมาก
- `-preset drawing` / `picture` / `-f` / `-sns` ต่างกัน **ไม่ถึง 1%**
- สลับลำดับเป็น "ย่อก่อนค่อย sharpen" (ซึ่งถูกหลักกว่า) **ไม่ได้เล็กลง** บางขนาดใหญ่ขึ้นด้วยซ้ำ
- AVIF ผ่าน ImageKit ลดได้ 7–17% (ขนาดที่ใช้จริง `w320` ลดแค่ **7%**) แลกกับต้องดูแลอีกฟอร์แมตทั้งชุด

**สรุป: ภาพสแกนลายเส้น 1909 มี entropy สูงโดยธรรมชาติ ของที่ส่งอยู่ใกล้เพดานแล้ว**
จะเล็กลงกว่านี้อย่างมีนัยต้องยอมให้ภาพแตกจริง ซึ่งขัดกฎเหล็กข้อ 5 — **จึงไม่แก้ และไม่ควรมีใครมาไล่แก้อีก**

เขียนผลวัดทั้งหมดไว้ที่หัวไฟล์ `scripts/generate-card-variants.ts` แล้ว
พร้อมแก้คอมเมนต์ตัวเลขเป้าลอย ๆ ทั้ง 6 บรรทัดให้เป็น **ตัวเลขที่วัดจริง** (`w256` จาก "~10KB" เป็น "~35KB")
เพราะตัวเลขเก่านั่นเองคือสิ่งที่หลอกผมรอบที่แล้ว

**บทเรียน**: คอมเมนต์ที่เขียนเป้าไว้ลอย ๆ โดยไม่เคยวัด อันตรายกว่าไม่เขียนเลย —
คนอ่านรอบถัดไปจะเชื่อว่าเป็นค่าที่เคยทำได้จริง แล้วออกล่าปัญหาที่ไม่มีอยู่

**กับดักที่เจอระหว่างทาง**: เขียน `*` ติด `/` ในคอมเมนต์บล็อก (ตอนอธิบาย Accept แบบรับทุกชนิด)
ทำให้คอมเมนต์ **ปิดกลางคัน** แล้วไฟล์พังทั้งไฟล์ — ถ้าต้องพูดถึงรูปแบบนี้ในคอมเมนต์ ให้เขียนเป็นคำแทน

### 🗓️ 2026-09-11 (รอบ 22): ⚡ แก้ LCP มือถือ — กริดไพ่ดูดภาพใหญ่เกินจริง 3 เท่า

> **คำสั่งเจ้าของ**: "แก้เลย ทำให้ดีที่สุด" (ต่อจากผลตรวจ Core Web Vitals รอบ 21)

**อาการ** — LCP บนมือถือ 7.2–10.2 วินาที (เกณฑ์ผ่าน 2.5 วินาที) ขณะที่เดสก์ท็อป 1.6–2.1 วินาที ปกติดี
CLS กับ TBT ดีเยี่ยมทุกหน้า แปลว่าไม่ใช่ปัญหาเลย์เอาต์หรือ JS บล็อก — เป็นปัญหาแบนด์วิดท์ล้วน ๆ

**ต้นตอที่วัดได้จริง** (วัดด้วยเบราว์เซอร์จริง จำลองมือถือ 375px DPR 2 บน production)
- กริดไพ่แสดงภาพที่ **130–151 CSS px** ทุกเบรกพอยต์ (วัดจริงทั้ง 4 ขนาดจอ)
- จอ DPR 2 จึงต้องการภาพกว้างแค่ **261–301 px**
- แต่ชุดภาพย่อมีแค่ `w256` แล้วกระโดดไป `w512b` ทันที — ไม่มีขั้นกลาง
- เบราว์เซอร์จึงเลือก `w512b` (**เฉลี่ย 91 KB/ใบ**) ทั้งที่ใช้จริงไม่ถึงครึ่งความละเอียด
- หน้า `/cards` โหลดภาพชุดแรก 6 ใบพร้อมกัน = **~600 KB** ตั้งแต่วินาทีแรก
- ซ้ำร้าย `sizes` เดิมโม้เกินจริงทุกเบรกพอยต์ (บอก `45vw`=169px ทั้งที่จริง 136.5px) ยิ่งดันให้เลือกไฟล์ใหญ่ขึ้นไปอีก

**สิ่งที่แก้**
1. เพิ่มภาพย่อขั้นกลาง **`w320`** (78 ไฟล์ · เฉลี่ย 35 KB/ใบ) อุดช่องว่าง 256→512
   — `src/lib/tarot/card-image.ts` (`CARD_IMAGE_VARIANTS`) + `scripts/generate-card-variants.ts`
   — ข้าม unsharp mask เหมือน `w512b` เพราะความถี่สูงที่เพิ่มเข้ามาทำให้ไฟล์บวมโดยไม่คมขึ้นจริง
2. แก้ `sizes` ให้ตรงกับความกว้างที่วัดได้จริง ใน `CardsExplorer.tsx` และ `CardGroupView.tsx`
   — `(min-width: 1024px) 152px, (min-width: 768px) 136px, (min-width: 640px) 156px, 38vw`

**ผลลัพธ์ (ยืนยันด้วยเบราว์เซอร์จริง ไม่ใช่การคำนวณ)**
- มือถือ DPR 2: เลือก `w512b` → **`w320`** = **ลด 61% ต่อใบ** (91 KB → 35 KB)
- ภาพชุดแรกหน้า `/cards`: ~600 KB → **~210 KB**
- เดสก์ท็อป DPR 1: เลือก `w256` เหมือนเดิมทั้งก่อนและหลัง — **ไม่มีการถอยหลัง**
- จอ DPR 3 ยังเลือก `w512b` ตามเดิม ซึ่งถูกต้องแล้ว (ต้องการ ~410 px)

**ข้อค้นพบระหว่างทางที่ยังไม่ได้แก้**
- **ImageKit แปลง WebP ของเราเป็น JPEG แล้วไฟล์บวมขึ้น** เมื่อไคลเอนต์ไม่ส่ง `Accept: image/webp`
  (`w512b` 101 KB → 150 KB · `w768b` 143 KB → 274 KB) เบราว์เซอร์จริงไม่โดนเพราะส่ง `Accept` ครบ
  แต่ครอว์เลอร์/สเครเปอร์บางตัวโดนเต็ม ๆ — แก้ได้ด้วย `?tr=orig-true` ถ้าจะเอาจริง
- ภาพย่อทุกขนาด**หนักกว่าเป้าที่เขียนไว้ในสคริปต์เอง 2–4 เท่า** (`w256` เขียนว่า ~10KB ของจริง 35KB)
  ต้นเหตุคือ remaster pass (unsharp mask) เพิ่มความถี่สูงจน WebP บีบไม่ลง
  รอบนี้**ไม่แตะ** เพราะเป็นการตัดสินใจเรื่องความคมชัดของภาพ ต้องให้เจ้าของเคาะก่อน
- `npm run cards:variants` รันทีเดียวสร้างใหม่ 454 ไฟล์ (remaster ทำ mtime ใหม่หมด) ทำให้ `w64/w128/w256`
  เปลี่ยนไบต์ทั้งที่ไม่ตั้งใจ — รอบนี้ `git checkout` คืนกลับหมดแล้ว **เหลือเฉพาะ `w320` ที่ตั้งใจเพิ่ม**
  รอบหน้าถ้าจะเพิ่มขนาดใหม่ ต้องคืน 3 โฟลเดอร์นั้นทุกครั้ง ไม่งั้น diff จะบวมโดยเปล่าประโยชน์

### 🗓️ 2026-09-11 (รอบ 21): 🔍 ตรวจ SEO ทั้งเว็บด้วยเอเจนต์เฉพาะทาง + ซ่อม Article JSON-LD

> **คำสั่งเจ้าของ**: "ใช้ทุกเอเจนต์ที่เกี่ยวกับ SEO เพื่อปรับปรุง หาจุดผิดพลาด แก้บั๊ก อย่างละเอียด"

**ขอบเขตการตรวจ** — ดึงหน้าจริงจาก production ครบ **299 หน้า** (ทุก URL ใน sitemap) ด้วย User-Agent ของ Googlebot แล้ววัดทีละรายการ ไม่ใช่สุ่มตัวอย่าง

**ผลตรวจที่ผ่าน (ยืนยันด้วยข้อมูลจริง ไม่ต้องแก้)**
- 299/299 URL ใน sitemap คืน HTTP 200 — ไม่มีลิงก์ตาย
- **0 title ซ้ำ · 0 description ซ้ำ** ทั้ง 299 หน้า
- ทุกหน้ามี canonical ชี้ตัวเอง · H1 หนึ่งเดียว · og:image · twitter:card · JSON-LD · `lang` ถูกภาษา
- hreflang ผูกกันสองทาง 296/299 (อีก 3 หน้าเป็นหน้าไทยล้วนโดยตั้งใจ)
- หน้าส่วนตัวทุกเส้นทาง (`/admin` `/account` `/tester` `/reset-password` `/readers/console` `/s/[id]`) มี `noindex` ครบ
- ไม่มี soft-404 — เส้นทางมั่วคืน 404 จริงทุกเส้น
- ภาพแชร์ Cloudinary **299/299 คืน 200** รวม 14 หัวข้อที่มี `,` หรือ `/` → **ระเบิดเวลาใน HANDOFF_OG_IMAGES ถูกแก้ไปแล้ว**
- ภาษาไทยในซอร์สสะอาด ตรวจ 11 รูปแบบความผิด (`นะค่ะ` · `เเ` · `ยิบซี` · วรรณยุกต์ซ้อน · อักขระล่องหน ฯลฯ) — **0 จุด**
- JS หน้าแรก 227 KB (งบ 232 KB) · HTML บีบอัด 17–39 KB → คำกล่าวเดิมว่า `/cards` ส่ง HTML 896 KB **ไม่จริงแล้ว**
- ลิงก์ภายในแน่น: หน้าไพ่แต่ละใบลิงก์ไปไพ่อื่น ~10 · ผัง 6 · บทความ 7 — ไม่มีหน้ากำพร้า

**บั๊กที่พบและแก้ในรอบนี้** (`src/app/_shared/pages/blog-detail.tsx`)
1. `Article.image` ชี้ `og/default.png` เหมือนกันหมดทั้ง **52 บทความ** ทั้งที่ `og:image` เป็นภาพเฉพาะเรื่องอยู่แล้ว → Google Discover / article rich result ได้ภาพโหลซ้ำกันทุกใบ **แก้แล้ว** ให้ใช้ `buildPageOgImage()` ตัวเดียวกับ `og:image`
2. `Article.author` เป็น `Organization` ทับชื่อผู้เขียนจริงที่มีอยู่แล้วใน `article.author` → อ่อน E-E-A-T สำหรับเนื้อหาให้คำแนะนำ **แก้แล้ว** เป็น `Person` ตามชื่อจริง (รองรับ `authorEn`)

**ข้อค้นพบที่ยังไม่ได้แก้ (ต้องให้เจ้าของตัดสิน)**
- `Organization.sameAs` มีแค่ลิงก์ GitHub repo (`RootHtml.tsx:59`) — ไม่มีโซเชียลสักช่อง ทั้งที่แผน Omnichannel วางไว้ 6 ช่อง → สัญญาณตัวตนอ่อนมากสำหรับ AI/Knowledge Graph **ต้องรอ URL จริงจากเจ้าของ**
- `WebSite` ไม่มี `SearchAction` (`RootHtml.tsx:65-72`) — **ยังใส่ไม่ได้** เพราะเว็บไม่มีหน้าผลค้นหาที่มี URL จริง (ค้นผ่าน `SemanticSearchPanel` เรียก `/api/search` ตรง) ต้องสร้างหน้า `/search?q=` ก่อน
- `HowTo` schema อยู่ 52 หน้า (`spread-detail.tsx:257`, `home-seo.ts:204`) — Google เลิกแสดง rich result ตั้งแต่ ก.ย. 2023 เป็น markup ตายไม่มีผลใด ๆ
- `FAQPage` 121 หน้า — Google ปิด FAQ rich result 7 พ.ค. 2026 **ไม่ต้องรื้อ** ยังมีค่าให้ AI อ้างอิง
- หน้า 404 มี `<meta name="robots">` สองอัน (`noindex` จาก error boundary ของ Next + `noindex, follow` ของเรา) — ผลลัพธ์ถูกต้อง เป็นเรื่องความสะอาดเท่านั้น
- `llms.txt` เป็นภาษาอังกฤษล้วนทั้งที่เว็บหลักเป็นไทย — ลิงก์ทั้ง 5 คืน 200 ปกติ
- เอกสาร `HANDOFF_EN_ROUTING_2026-09-06.md` ล้าสมัย: ระบุว่า `/en/blog` ยังไม่เปิด แต่จริง ๆ เปิดแล้วและอยู่ใน sitemap

**ข้อควรรู้สำหรับรอบถัดไป**
- `.next/` ใน worktree ค้างจากวันที่ 8 ก.ย. ทำให้ด่าน sticky-header กับด่านเส้นทางสองภาษา **ตกทั้งที่โค้ดไม่ผิด** — ต้อง `npm run build` ก่อน `repo:verify` เสมอเมื่อ build เก่ากว่า commit ล่าสุด
- โควตา PageSpeed Insights แบบไม่ใส่ API key **หมดโควตารายวัน** ระหว่างตรวจ → ข้อมูล Core Web Vitals ภาคสนาม (CrUX) ยังไม่ได้วัดรอบนี้

### 🗓️ 2026-09-10 (รอบ 20): ✦ แถบแจ้งเตือนหลุดขอบซ้ายจอบนมือถือ (INC-0123)

> **คำสั่งเจ้าของ**: ส่งภาพหน้าจอ 2 ใบพร้อมวงสีแดงรอบแถบแจ้งเตือนที่โผล่มาคร่อมขอบซ้าย
> ถามว่า "ที่วงทำไม ถึงเป็นแบบนี้"

#### อาการ
แถบแจ้งเตือน (Toast) ทุกใบบนหน้าแรกโผล่มาโดยครึ่งซ้ายอยู่นอกจอ ตัวหนังสือถูกตัดหาย
ขอบขวาของกล่องไปจบตรงกลางจอพอดี และค้างอยู่แบบนั้นจนแถบหายไปเอง

#### สาเหตุราก — Tailwind v4 เปลี่ยนวิธีคอมไพล์ `translate-*`
คอมไพล์ CSS จริงด้วย `@tailwindcss/postcss` ตัวเดียวกับที่ build ใช้ แล้วอ่านผลลัพธ์:

```css
.-translate-x-1\/2 {
  --tw-translate-x: calc(calc(1 / 2 * 100%) * -1);
  translate: var(--tw-translate-x) var(--tw-translate-y);   /* ← ไม่ใช่ transform แล้ว */
}
```

Tailwind v4 ใช้ **คุณสมบัติเดี่ยว `translate:`** ไม่ใช่ `transform:` แบบ v3 ซึ่งตามสเปก
เบราว์เซอร์คิดเรียง `translate → rotate → scale → transform` แล้ว **คูณต่อกัน ไม่ได้ทับกัน**

คีย์เฟรม `toastIn` / `toastOut` ใน `globals.css` จงใจพา `translate(-50%, …)` ไปด้วยทุกเฟรม
(มีคอมเมนต์กำกับไว้ชัดว่า "ต้องพาไปด้วย ไม่งั้นกล่องจะวาปไปทางซ้าย" — เข้าใจตามพฤติกรรม v3)
ผลรวมจึงเป็น **-100%** กล่องเลื่อนซ้ายเท่ากับครึ่งความกว้างของตัวเอง
และเพราะ `animation-fill-mode: both` มันค้างผิดที่ถาวร ไม่ใช่แค่ตอนเล่นคีย์เฟรม

#### สิ่งที่แก้
- `src/app/globals.css` — คีย์เฟรม `toastIn`/`toastOut` เหลือ `translateY() scale()` เท่านั้น
  ปล่อยการจัดกลางเป็นหน้าที่ของคุณสมบัติ `translate` ที่คลาสตั้งไว้ + เขียนคอมเมนต์เตือนทับของเดิมที่อธิบายผิด
- `scripts/qa/test-motion-quality.ts` — เพิ่ม **กฎข้อ 9** ตรวจไขว้สองฝั่ง
  (CSS: `.anim-*` ผูกกับ `@keyframes` ไหน และคีย์เฟรมนั้นมี translate เป็น % ไหม ·
  TSX: element ที่ใส่คลาสนั้นใส่ยูทิลิตี้ `translate-*` ของ Tailwind ด้วยไหม)
  ครบสองฝั่ง = ระยะถูกคูณสอง ตกด่านทันที

#### การพิสูจน์ว่าแก้ได้จริง
- วัดด้วย Chromium จริง (`getBoundingClientRect`) เทียบก่อน/หลังในหน้าเดียวกัน:
  **ก่อนแก้** `left=-189.0 right=241.0` (ขอบขวาจบกลางจอ ตรงกับภาพที่เจ้าของส่งมา) ·
  **หลังแก้** `left=35.0 right=465.0` กึ่งกลางตรงกับกึ่งกลาง viewport เป๊ะ
- ยืนยันผลคอมไพล์ Tailwind ว่าออกมาเป็น `translate:` จริง ไม่ใช่เดาจากเอกสาร
- ทดสอบด่านใหม่ด้วยการทำให้พังจริง: ใส่ `translate(-50%, …)` กลับเข้าคีย์เฟรม → ด่านจับได้และชี้ถูกไฟล์/บรรทัด
- `npm run typecheck` ➔ 0 errors · `npm run repo:verify` ➔ ผ่านครบทุกด่าน
### 🗓️ 2026-09-10: 🔴 รอบสองของ INC-0106 — Speculation Rules ยังยิง prefetch ทุกลิงก์ทั้งเว็บ

**ที่มา**: เจ้าของถามว่า request ที่แก้ไปลดลงเท่าไร แล้วถามต่อว่า "มีวิธีปรับปรุงให้ดีกว่านี้ไหม"
→ ตรวจ production จริงแล้วเจอของค้างที่ไม่มีใครเห็นมาตั้งแต่ INC-0106

**ตัวเลขยืนยันว่าที่แก้ไปได้ผลจริง** (จากการ์ด Metrics ของ Cloudflare + ยิง production ซ้ำ)

| ตัวชี้วัด | ก่อน | ตอนนี้ |
|---|---|---|
| Worker invocations / 24 ชม. | 1.12M | **9.38k** (↓ ~99.2%) |
| เทียบ 24 ชม. ก่อนหน้า (Cloudflare คิดเอง) | – | **↓ 93.6%** |
| CPU / คำขอ | 8.4 ms | 12 ms (↑ แต่ใช้แค่ ~11% ของโควตา 30M CPU-ms/เดือน) |
| Errors | – | **0** |

> CPU ที่สูงขึ้นเป็นผลข้างเคียงที่ตั้งใจแลก ไม่ใช่อาการเสีย — คำขอลูปเดิมเป็นคำขอถูก ๆ ที่ตอบจาก KV
> เลยฉุดค่าเฉลี่ยลงเทียม ๆ พอลูปหาย ที่เหลือเป็นคำขอเรนเดอร์หน้าจริงล้วน บวกกับ
> `enableCacheInterception: false` ที่ทำให้ทุกคำขอวิ่งผ่าน Next runtime ตามที่ตั้งใจแก้

**บั๊กที่เจอ**: `src/app/_shared/RootHtml.tsx` ฝัง `<script type="speculationrules">` ที่สั่งเบราว์เซอร์ว่า

| กฎเดิม | ต้นทุน |
|---|---|
| `prefetch` `/*` · **moderate** | ยิงทุกลิงก์ในหน้าตอนเมาส์ **แค่ชี้** — หน้าแรกมีลิงก์ภายใน 40 เส้น เพดาน Chrome 50 เส้น/หน้า |
| `prerender` `/*` · conservative | โหลด**และรัน JS ทั้งหน้า** (หน้าที่ถูกอุ่นยิง `/api/bootstrap` ของมันเองอีกเส้น) ไม่มีเพดาน |
| `prerender` list 5 หน้า · moderate | มีเพดานชัด (Chrome จำกัด 2 หน้าพร้อมกัน) — **ข้อนี้ดีอยู่แล้ว เก็บไว้** |

**สาเหตุราก**: ตอน INC-0106 เราไล่ปิด `prefetch={false}` ทั่วเว็บ 20 กว่าจุด **แต่มองข้ามชั้นนี้ไป**
Speculation Rules เป็นคำสั่งที่ยิงถึงเบราว์เซอร์ตรง ๆ ผ่านแท็ก `<script>` มัน**ไม่สนใจ
`prefetch={false}` ของ Next เลยแม้แต่นิดเดียว** ด่านที่ 38 ที่ตั้งขึ้นมากันเรื่องนี้โดยเฉพาะ
ก็ตรวจแค่ `enableCacheInterception` กับ `prefetch={true}` จึงมองไม่เห็นชั้นนี้เช่นกัน

**การแก้**: ย้ายกฎออกมาเป็นโมดูลกลาง `src/app/_shared/speculation-rules.ts` แล้ว
ลด `prefetch` `/*` จาก `moderate` → `conservative` (ยิงตอนกดเมาส์ลง ยังได้เปรียบ ~150ms
แต่คำขอที่ทิ้งเปล่าเกือบเป็นศูนย์) · ตัดกฎ `prerender` `/*` ทิ้ง · เก็บ `prerender` list 5 หน้าไว้เหมือนเดิม
· เติมเส้น `/readers/console*` `/readers/queue/*` เข้ารายการยกเว้นของ `prefetch` ที่เดิมมีแต่ฝั่ง `prerender`

**กันซ้ำ**: ขยาย**ด่านที่ 38** `scripts/qa/test-prefetch-loop.ts` เพิ่มกฎข้อ 3 ที่
**เรียก `buildSpeculationRules()` ตัวจริงแล้วตรวจโครงสร้างที่ได้** (ไม่ใช่สแกนซอร์สด้วย regex
ตามบทเรียน INC-0114) บังคับ 5 ข้อ: ห้าม `eager` · กฎครอบทั้งเว็บต้องเป็น `conservative` ·
`prerender` ต้องเป็นรายการปิดเท่านั้น · รายการไม่เกิน 6 หน้า · ต้องกันเส้นไดนามิกครบทุกเส้น
พร้อมกฎ 3ข ที่ห้ามไฟล์ไหนเขียนออบเจ็กต์ `speculationrules` เองข้ามโมดูลกลาง (ไม่งั้นด่านจะกลายเป็นด่านหลอก)

**พิสูจน์**: `repo:verify` ผ่านครบ · ย้อนกฎกลับเป็นของเดิม (moderate + prerender ครอบทั้งเว็บ)
ด่านตกทันที 4 ข้อครอบคลุมทั้งสองภาษา · ยิง production ยืนยันว่าเส้น segment prefetch หายดีจริง
(`/daily?_rsc=<token>` + header `/_tree` ได้ `200 · text/x-component · 1,046 ไบต์`
เทียบกับ HTML เต็ม 92,300 ไบต์ — ตอนเป็นบั๊กมันได้ขนาดเท่ากันเป๊ะ)

**ข้อจำกัดของรอบนี้**: เปิดเบราว์เซอร์นับคำขอจริงตามกฎถาวรของ INC-0106 **ทำไม่ได้** เพราะ proxy
ของคอนเทนเนอร์ตัดการเชื่อมต่อ headless Chromium ทิ้ง (`ws_closed_mid_exchange`) ตัวเลข
"หลักสิบคำขอต่อการเปิดหน้า" จึงเป็นการประเมินจากจำนวนลิงก์จริง (40 เส้นบนหน้าแรก) คูณกับเพดานของ Chrome
**ยังไม่ได้วัดจากเบราว์เซอร์จริง** — รอบที่มีเบราว์เซอร์ควรวัดยืนยันก่อนเคลมตัวเลขนี้ที่ไหน

**ของที่ยังไม่ได้ทำ (เรียงตามความคุ้ม)**
1. HTML ไม่มีแคชฝั่งเบราว์เซอร์เลย — ตอบ `s-maxage=31536000` อย่างเดียว ไม่มี `max-age`
   กลับมาหน้าเดิม = ปลุก Worker ใหม่ทุกครั้ง · หน้าเป็น SSG ไม่มีเนื้อหาผูกกับผู้ใช้ จึงใส่ `max-age` สั้น ๆ ได้
   ⚠️ **กับดัก**: Workers Static Assets ลบ chunk ของ build เก่าตอน deploy → HTML ค้างที่ชี้ชื่อ chunk เก่า
   จะได้ `ChunkLoadError` (เคสเดียวกับที่เตือนไว้ใน `public/sw.js`) ต้องตั้งระดับ 60 วินาที ไม่ใช่หลักชั่วโมง
2. `public/sw.js` เขียนหัวไฟล์ว่า Stale-While-Revalidate แต่โค้ดจริงเป็น network-first
   (ยิง `fetch()` ก่อนเสมอ ใช้แคชเฉพาะตอนเน็ตล่ม) คนที่กลับมาซ้ำจึงยังปลุก Worker เต็ม ๆ
3. เปิด `enableCacheInterception` กลับแบบมีเงื่อนไข — **อย่าเพิ่งทำ** CPU ใช้แค่ ~11% ของโควตา
   ไม่คุ้มกับการรื้อแผล INC-0106 · เก็บไว้ตอนทราฟฟิกเข้าใกล้ 83k คำขอ/วัน (30M ÷ 12ms ÷ 30 วัน)

---

### 🗓️ 2026-09-09 (รอบ 19): ✦ โมชั่นเปลือกหน้าแรกเป็น CSS ล้วน — JS ที่ผู้ใช้โหลดจริงลด 45%

> **คำสั่งเจ้าของ**: "ปรับปรุงโมชั่นให้ดีสูทไว ไม่มีการวาป กระตุก อะไรแปลกๆ เพิ่มเติม
> แต่ต้องไม่เพิ่มขนาดไฟล์ และต้องลดลงด้วย"

#### ตรวจก่อนว่าอะไรยังเป็นปัญหาจริง

ด่าน `test-motion-quality` มี ALLOWLIST **ว่างเปล่า** อยู่แล้ว — แปลว่ากฎ 8 ข้อ
(ไม่มี `transition-all` · ไม่มี `backdrop-filter` · ไม่มีลูปไม่รู้จบบนเธรดหลัก ฯลฯ)
ผ่านครบทั้งเว็บมาก่อนหน้านี้แล้ว **ของที่แก้ง่ายถูกเก็บไปหมดแล้ว**

ที่เหลือคือของที่ด่านมองไม่เห็น: **ไลบรารี `motion` เองยังอยู่ในบันเดิลตั้งต้นของหน้าแรก**

```
$ ไล่ chunk ทุกก้อนที่แต่ละหน้าอ้างถึงใน HTML
  /  และ /en  → 264 KB gzip · โหลด chunk ของ motion (40 KB) ตั้งแต่แรก
  หน้าอื่นทั้ง 306 หน้า → ไม่โหลดแล้ว (ทีมก่อนหน้าเก็บไปหมดแล้ว)
```

เหลือ **3 ไฟล์สุดท้าย** ที่ยัง `import` จาก `motion/react` แบบ static ในเปลือกหน้าแรก
ทั้งที่ใช้แค่ fade + เลื่อน ซึ่ง CSS ทำได้ดีกว่า:

| ไฟล์ | ใช้ motion ทำอะไร |
| :--- | :--- |
| `TarotFlow.tsx` | เปลี่ยนขั้นพิธีกรรม (fade + เลื่อน 40px) |
| `SpreadCardSelector.tsx` | สลับแท็บหมวดผัง (fade + เลื่อน 12px) |
| `ToastNotification.tsx` | แถบแจ้งเตือนเข้า/ออก + แถบนับถอยหลัง |
| `lib/motion.ts` | `useReducedMotion()` — ฮุกคืน boolean ตัวเดียว แต่ลากทั้งไลบรารี |

และเจอเพิ่มอีกจุดที่ใหญ่กว่านั้น: **หน้าต่างซ้อน 7 บานถูกเรนเดอร์ทิ้งไว้ตลอดเวลา**
โดยส่ง `isOpen={false}` เข้าไป · ตัวมันไม่วาดอะไรก็จริง แต่การที่มัน **ถูก mount**
สั่งให้ `next/dynamic` โหลด chunk ของทั้ง 7 บานทันทีที่ hydrate เสร็จ
ผู้ใช้ที่เปิดหน้าแรกมาดูแล้วปิดไป (คนส่วนใหญ่) จึงจ่ายค่าดาวน์โหลดหน้าต่างที่ไม่เคยเปิดครบทุกบาน

#### แก้อะไรไป

| ไฟล์ | สิ่งที่ทำ |
| :--- | :--- |
| `src/lib/use-motion-safe.ts` **(ใหม่)** | `useMotionSafe()` ฉบับใช้ `matchMedia` ผ่าน `useSyncExternalStore` — 0 dependency · `getServerSnapshot` คืน `true` จึงไม่มี hydration mismatch |
| `src/lib/motion.ts` | ตัด `import { useReducedMotion }` ทิ้ง เหลือแต่ `import type` (ถูกลบตอน build) → ไฟล์ค่าคงที่ไม่ลากไลบรารีอีกต่อไป |
| `src/components/providers/with-motion-scope.tsx` **(ใหม่)** | `withMotionScope()` โหลด `MotionConfig` **พร้อมกัน**กับตัวคอมโพเนนต์ใน `dynamic()` เดียว → `reducedMotion="user"` ยังครอบครบเหมือนเดิม แต่เดินทางไปอยู่ใน chunk ที่โหลดทีหลัง |
| `src/lib/use-once-open.ts` **(ใหม่)** | `useOnceOpen()` — หน้าต่างซ้อน mount ตอนถูกเปิดครั้งแรกเท่านั้น แล้วค้างไว้ (ไม่ถอดตอนปิด อนิเมชันขาออกใน `Modal.tsx` จะได้เล่นจนจบ) |
| `TarotFlow.tsx` | เปลี่ยนขั้น → คลาส `.anim-step-in` + `key={currentStep}` · dynamic import ทั้ง 14 ตัวห่อด้วย `withMotionScope()` · หน้าต่างซ้อน 7 บานห่อด้วย `useOnceOpen()` |
| `SpreadCardSelector.tsx` | สลับแท็บ → `.anim-swap-rise-sm` + `key={activeCategory}` |
| `ToastNotification.tsx` | เข้า/ออกเป็นคีย์เฟรม CSS จัดการขาออกเอง (`onAnimationEnd` → `onClose`) · แถบนับถอยหลังใช้ `animation-play-state` แทนการสั่งหยุดทีละเฟรมจาก React |
| `globals.css` | เพิ่มแค่ `stepInBack` + คีย์เฟรม toast — **ต่อยอดจากชุดที่มีอยู่แล้ว** (`.anim-step-in` · `.anim-swap-rise*`) ไม่สร้างชุดซ้ำซ้อน |
| `test-bundle-budget.ts` | รัดงบ `/` จาก 271 → **232 KB** · **เพิ่มเส้นทาง `/en`** ที่ไม่เคยมีด่านคุมเลยทั้งที่หนักเท่า `/` ทุกไบต์ |

#### 🐞 กับดักที่เจอระหว่างทาง — "อาการวาป" ที่เกือบหลุด

รอบแรกใช้ ref แบบ "เรนเดอร์แรกหรือเปล่า" แล้วให้ CSS ปิดอนิเมชันด้วย `[data-first="true"]`
**ใช้ไม่ได้** — ref พลิกค่าใน effect ทำให้แอตทริบิวต์เปลี่ยน `true → false` ตอน re-render
รอบแรกหลัง hydrate (ซึ่งเกิดแน่ ๆ จากการโหลดสิทธิ์การใช้งาน) · เบราว์เซอร์เห็น
`animation-name` เปลี่ยนจาก `none` เป็นของจริง จึง **เริ่มเล่นคีย์เฟรมทันที**
ทั้งที่ผู้ใช้ไม่ได้แตะอะไรเลย → หน้าแรกเลื่อนเข้ามาเองหนึ่งครั้งหลังโหลดเสร็จ
ซึ่งคือ "อาการวาป" ที่เจ้าของสั่งห้ามพอดี

**ทางที่ถูก**: ไม่ใส่คลาสอนิเมชันเลยจนกว่าผู้ใช้จะกดจริง — `hasNavigatedStep` (state ที่ตั้ง
ใน `navigateStep`) และ `hasSwappedTab` (state ที่ตั้งตอนกดแท็บ) เปลี่ยนพร้อม `key`
ในเรนเดอร์เดียวกัน จึงไม่มีจังหวะที่ `animation-name` เปลี่ยนเองโดยไม่มีคนกด

จับได้เพราะวัดด้วย Chromium จริง (`document.getAnimations()`) ไม่ใช่อ่านโค้ดเอา

#### ผลวัดจริง (Chromium · viewport 412×900 · เทียบ `origin/main` ที่ `747c159`)

**JS ที่เบราว์เซอร์โหลดจริงเมื่อเปิดหน้าแรกแล้วอยู่เฉย ๆ**

| | ก่อนแก้ | หลังแก้ | ลดลง |
| :--- | ---: | ---: | ---: |
| JS รวม (gzip) | **443 KB** | **242 KB** | **−201 KB (45%)** |
| จำนวนไฟล์ | 33 | 23 | −10 |

**บันเดิลตั้งต้นตามด่านงบน้ำหนัก** — `/` 267 → **227 KB** · `/en` 264 → **226 KB**
(หน้าอื่นไม่ขยับ เพราะไม่ได้โหลด motion อยู่แล้ว)

**พฤติกรรมโมชั่น**

| จังหวะ | ผลตรวจ |
| :--- | :--- |
| หลังโหลดเสร็จ 4 วินาที | `document.getAnimations()` ที่ยังวิ่ง = **0** — นิ่งสนิท ไม่มีอะไรขยับเอง |
| หัวข้อหลักตอนเปิดหน้า | `opacity: 1` ตั้งแต่เฟรมแรก (ไม่โผล่มาจาก 0) |
| กด "ถัดไป" | คีย์เฟรม `stepIn` วิ่ง · จบที่ `opacity: 1` |
| กด "ย้อนกลับ" | `data-dir="back"` · `animation-name: stepInBack` ✅ |
| สลับแท็บหมวดผัง | คีย์เฟรม `swapRiseSm` วิ่ง · จบที่ `opacity: 1` |
| เปิดหน้าต่างซ้อนครั้งแรก | โหลด chunk ตอนกด (ไม่ใช่ตอนเปิดหน้า) · เปิดได้ · Esc ปิดได้ |
...
1. **ขาออกของการเปลี่ยนขั้นหายไป** — ของเดิมรอขาออก 0.14s ให้จบก่อนแล้วค่อยเข้า 0.28s
   (รวม 0.42s ต่อการกดหนึ่งครั้ง) ตอนนี้เข้าอย่างเดียว 0.28s · **ตั้งใจ** เพราะเจ้าของสั่งว่า
   "ไว" และเป็นข้อแลกเปลี่ยนชุดเดียวกับที่บ้านนี้เลือกไว้แล้วตอนย้าย `.anim-swap-rise`
   เป็น CSS · ได้ของแถมคือตัดความเสี่ยง exit-transition deadlock (INC-0015) ทิ้งไปด้วย
2. **เปิดหน้าต่างซ้อนครั้งแรกต้องรอโหลด chunk** (~385ms บนเครื่อง dev) · เคยลองใส่ตัวอุ่น
   chunk ตอนผู้ใช้แตะจอครั้งแรกแล้ว — **ถอดออก** เพราะมันโหลดหน้าต่างทั้ง 4 บานให้ทุกคน
   ที่แตะจอ (449 KB) กินของที่ประหยัดมาเกือบหมด แลกกับที่ลดเวลาได้แค่ 385 → 213 ms
   · พฤติกรรมนี้เหมือนกับ dynamic component อื่นทั้งเว็บอยู่แล้ว ไม่ใช่ของแปลกใหม่
...
**วิธีหา** — ไล่ chunk ทุกก้อนที่ build ออกมา แล้วจับคู่กับหน้าที่โหลดมันจริง
เพื่อแยกให้ออกว่าอะไรคือ "โหลดทันที" กับ "โหลดเมื่อต้องใช้" และอะไรเป็นของเรากับของเฟรมเวิร์ก

#### สิ่งที่เจอ (เจอจริง 1 จุด · ที่เหลือสะอาดแล้ว)

`src/data/spreads.ts` เก็บผังทั้ง 25 แบบพร้อมคำอธิบายสองภาษาครบทุกตำแหน่ง = **85 KB**
คอมโพเนนต์ฝั่งไคลเอนต์ 4 ตัวต้องการแค่ **ฟังก์ชันเลือกภาษา 5 ตัว** ที่รับ object เข้ามา
แล้วคืนข้อความ — **ไม่แตะข้อมูลส่วนกลางเลยสักตัว** แต่เพราะฟังก์ชันพวกนั้นอยู่ในไฟล์เดียวกับข้อมูล
การ import มันจึงลากข้อมูลทั้งก้อนติดเข้าบันเดิลไปด้วย

= **17.2 KB (gzip) บน 54 หน้า** ที่ไม่มีหน้าไหนใช้ข้อมูลนั้นเลย (ข้อมูลผังถูกดึงฝั่งเซิร์ฟเวอร์
แล้วส่งลงมาเป็น prop อยู่แล้ว) · แพตเทิร์นเดียวกับ `@/data/cards/summary` ที่เคยแก้สำรับไพ่ 896 KB มาแล้ว

**ทางแก้** — แยก `src/data/spreads-helpers.ts` (นิยามชนิดข้อมูล + ฟังก์ชันบริสุทธิ์ 5 ตัว
ห้าม import อะไรจาก `./spreads` เด็ดขาด) แล้ว `spreads.ts` re-export กลับให้ผู้เรียกเดิมไม่พัง

| เส้นทาง | ก่อน | หลัง |
| :--- | ---: | ---: |
| `/spreads` | 188 KB | **171 KB (−9%)** |
| `/spreads/*` และ `/en/spreads/*` (54 หน้า) | — | ไม่มีข้อมูลผังติดไปอีกแล้ว |

**รัดงบ + อุดช่องโหว่ของด่าน**: `/spreads` 200 → **175** และ **เพิ่มเส้นทาง
`/spreads/celtic-cross` เข้าด่านงบเป็นครั้งแรก** — เดิมด่านวัดแต่หน้ารวม `/spreads`
ทั้งที่ 54 หน้ารายผังเป็นคนละเส้นทางกัน ข้อมูล 17.2 KB จึงรั่วอยู่ได้โดยไม่มีใครเห็น

#### ที่ตรวจแล้วพบว่า "สะอาดอยู่แล้ว" (ไม่ต้องแก้)

| จุดที่สงสัย | ผลตรวจ |
| :--- | :--- |
| `src/data/articles.ts` (147 KB) ถูก import ในไคลเอนต์ 3 จุด | ทั้ง 3 จุดเป็น `import type` ล้วน — TypeScript ลบทิ้งตอน build ไม่ติดบันเดิลเลย |
| สำรับไพ่เต็มพร้อมความหมาย (83.6 KB gzip) | โหลดแบบ dynamic — **โหลดทันที 0 หน้า** เข้ามาเมื่อเปิดสารานุกรมเท่านั้น |
| `visual-lore.ts` (151 KB) | ไม่มีคอมโพเนนต์ไคลเอนต์ตัวไหน import เลย |

#### ความจริงที่ต้องบันทึกไว้ — เพดานของเฟรมเวิร์ก

แยกน้ำหนักรายหน้าออกเป็น "ของเรา" กับ "ของเฟรมเวิร์ก" แล้วได้ภาพชัดว่า **โค้ดของเราผอมแล้ว**:

| หน้า | ของเฟรมเวิร์ก (Next.js + React + polyfills) | ของเราเอง |
| :--- | ---: | ---: |
| `/blog` | ~170 KB | **~45 KB** |
| `/cards/major-00` | ~170 KB | **~58 KB** |

ของเราที่โหลดทุกหน้ารวมกันแค่ **~36 KB gzip** สำหรับหัวเว็บ · ฟุตเตอร์ · สองภาษา ·
วัดผล · ล็อกอิน · ข้อมูลเมนู — ถือว่าผอมมากสำหรับเว็บสองภาษาที่มีระบบสมาชิก

> 📌 **สรุปให้คนถัดไป**: การลดน้ำหนักที่เหลือจากนี้ไม่ได้อยู่ที่โค้ดเราแล้ว
...
### 🗓️ 2026-09-09 (รอบ 16): ⚡ ถอด `motion` ออกจากไพ่และพิธีไพ่ใบเดียว — สองหน้าที่หนักที่สุดเบาลง 18% (INC-0120)

> **คำสั่งเจ้าของ**: "ทำโมชั่นดี แต่ความเร็วห้ามตก และขนาดไฟล์ไม่ใหญ่และต้องคุ้ม"

**โจทย์** — วัดแล้วพบว่าไลบรารี `motion` (**40.6 KB gzip**) ยังถูกลากเข้าไปใน 6 หน้า
และในนั้นมี `/daily` (238 KB) กับ `/love/1-card` (242 KB) ซึ่งเป็น **สองหน้าที่หนักที่สุดของเว็บ**
ไล่ดูว่ามันทำอะไรให้บ้าง แล้วพบว่าทั้งสองหน้าลากมาผ่านไฟล์เดียวสองไฟล์เท่านั้น:

| ไฟล์ | ใช้ `motion` ทำอะไร | ราคาที่จ่าย |
| :--- | :--- | :--- |
| `OneCardRitual.tsx` | สไลด์เปลี่ยนจังหวะ `x: 40 → 0` + จางเข้า **280ms** | 40.6 KB |
| `TarotCard.tsx` | พลิกไพ่ · ยก/ขยายตอน hover · เอียงตามเมาส์ (ทั้งหมดคือ `transform`) | เท่ากัน |

ทุกอย่างที่ไลบรารีทำคือ **"เขียนค่า transform"** ซึ่ง CSS transition ทำได้เหมือนกันเป๊ะ
โดยให้ compositor รับไปทำเอง — ไม่ต้องมี JS วิ่งต่อเฟรมเลยสักตัว · **ไม่คุ้มชัดเจน**

#### ผลที่วัดได้

| เส้นทาง | ก่อน | หลัง | ลดลง |
| :--- | ---: | ---: | ---: |
| `/daily` | 238 KB | **194 KB** | **−44 KB (−18.5%)** |
| `/love/1-card` | 242 KB | **197 KB** | **−45 KB (−18.6%)** |
| `/` (หน้าแรก) | 270 KB | 267 KB | −3 KB |
| หน้าที่ยังลาก `motion` | 6 หน้า | **2 หน้า** (`/` · `/en` ที่มีพิธีเต็ม 5 ขั้น) | −4 |

**รัดงบทันทีเพื่อล็อกผลไว้**: `/daily` 240 → **198** · `/love/1-card` 245 → **201** · `/` 275 → **271**

#### 🚨 กับดักที่เกือบหลุดขึ้น production (บทเรียนสำคัญที่สุดของรอบนี้)

รอบแรกเขียนทั้งสามชั้นเป็น `transform: rotateY(var(--card-flip))` แล้วให้ React เปลี่ยนตัวแปร
โค้ดอ่านแล้วถูกทุกบรรทัด · `transition-duration` อ่านค่าได้ `0.55s` จริง
แต่ **ไพ่กระโดดพลิกทันที ไม่มีอนิเมชันเลย**

> **สาเหตุ**: custom property ที่ไม่ได้ประกาศด้วย `@property` เบราว์เซอร์ถือว่าเป็น
> "ข้อความที่เอาไปแทนที่" ไม่ใช่ค่าที่ไล่ระดับได้ · พอค่าเปลี่ยน คุณสมบัติที่อ้างถึงมัน
> จะถูกคำนวณใหม่ทันที **โดยไม่ผ่าน transition**

จับได้เพราะ **เก็บตัวอย่างค่า `transform` ทุกเฟรมด้วย Chromium** แล้วพบว่าตลอด 700ms
มีค่าเดียวไม่เคยเปลี่ยน — ถ้าดูด้วยตาเปล่า หรือเชื่อว่า "ตั้ง `transition-duration` แล้วต้องทำงาน"
จะปล่อยผ่านไปเลย

**ทางแก้ แบ่งตามชนิดของค่า**:
- ค่าที่มีไม่กี่สถานะ (พลิก · ยก · ขยาย) → `data-*` attribute สลับ `transform` ระหว่างค่าจริงสองค่า → transition ทำงานและ compositor ยกไปทำได้
- ค่าต่อเนื่อง (เอียงตามเมาส์) → ประกาศ `@property` ให้เป็นมุมจริงที่ไล่ระดับได้ แล้ว transition ตัวแปรนั้นตรง ๆ

**ยืนยันหลังแก้** (เก็บทุกเฟรมด้วย Chromium): พลิกไล่มุม `0° → 21° → 41° → … → 180°`
รวม **25 ค่าที่ต่างกันใน 49 เฟรม** จบที่ 511ms ตรงตามเส้นโค้งที่ตั้งไว้ · parallax ตอบสนองเมาส์จริง

#### ทำไมต้องซ้อนสามชั้น

`transform` เป็นคุณสมบัติเดียว จึงมี transition ได้จังหวะเดียว แต่การเคลื่อนไหวสามอย่างนี้
ต้องการจังหวะคนละแบบจริง ๆ (เอียงตามเมาส์ 120ms · ยก/ขยาย 260ms · พลิกไพ่ 550ms)
แยกชั้นละหน้าที่จึงได้ครบทั้งสามแบบโดยไม่ต้องใช้ JS — ซึ่งเป็นเหตุผลเดียวกับที่
`motion` เดิมก็แยกสปริงคนละตัวให้แต่ละคุณสมบัติ

#### ด่านตรวจใหม่ (กฎที่ 8 ของด่านที่ 43)

`transform` ที่ประกอบจาก custom property และมี `transition` อยู่ด้วย —
ตัวแปรทุกตัวต้องประกาศ `@property` เสมอ · **ทดสอบด้วยการทำให้พังจริงด้วยรูปแบบที่เคยพลาดมาแล้ว**

#### ข้อแตกต่างเล็กน้อยที่ตั้งใจ

ของเดิมเมื่อไพ่หงายแล้ว `motion` ล็อก `rotateY` ไว้ที่ 180 องศา ทำให้ parallax แกน y หยุดทำงาน
...
### 🗓️ 2026-09-09 (รอบ 15): 🔢 รายงาน CI พูดเลขผิดมานาน — ขยายด่านตรวจเลขให้ส่องไฟล์ workflow (INC-0119)

**เจอได้ยังไง** — อ่านคอมเมนต์ที่บอต CI พิมพ์ลง PR #383 ของตัวเองแล้วสะดุดตา
รายงานเขียนว่า *"20 Spreads Geometry Calibration · 541/541 Assertions"*
ทั้งที่ของจริงคือ **25 ผัง · 1,295 assertion** (ตัวเลขหลังเพิ่งเปลี่ยนในรอบ 14 นี้เอง)

**ทำไมหลุดมาได้** — ด่าน `test-docs-numbers.ts` มีไว้กันเรื่องนี้โดยตรง
แต่มันส่องแค่ไฟล์ `.md` ในโฟลเดอร์เอกสารกับรากโปรเจกต์ **ไม่เคยส่องไฟล์ `.github/workflows/`**
ทั้งที่นั่นคือข้อความที่ **คนเห็นบ่อยที่สุด** — พิมพ์ลงทุก PR ทุกครั้งที่เปิด
พอขยายด่านให้ส่อง workflow ก็เจอเพิ่มทันทีอีก 3 จุดในไฟล์เดียวกัน:
...
**ทางแก้** — เติม `.github/workflows/pr.yml` และ `auto-release.yml` เข้ารายการที่ด่านส่อง
(11 → 13 ไฟล์) · แก้ตัวเลขให้ตรง · และ **ถอดตัวเลขออกจากชื่อขั้นตอนไปเลย**
เพราะชื่อขั้นตอนไม่ได้ต้องการตัวเลข แหล่งความจริงอยู่ที่ `CHECKS` อยู่แล้ว
(ยืนยันแล้วว่าชื่อ *ขั้นตอน* ไม่ใช่ชื่อ *job* ที่ branch protection ใช้จับ จึงไม่กระทบ required check)
...
### 🗓️ 2026-09-09 (รอบ 14): 🧹 ปิดงานค้างทั้งทะเบียน — ไม่เหลือของค้าง (INC-0118)

> **คำสั่งเจ้าของ**: "แก้ปัญหาให้หมดเลย เราไม่ต้องการปัญหาครั้งค้าง"

ไล่ปิดทุกอย่างที่ยังเปิดค้างอยู่ใน `KNOWN_ISSUES.md` + สิ่งที่รอบ 13 จงใจพักไว้

#### 1. ISSUE-034 · ไพ่ทับกันในแผนผัง SEO — **ของจริงกว้างกว่าที่บันทึกไว้ 2 เท่า**

ทะเบียนบันทึกไว้ว่ากระทบ 2 แบบ (`celtic-cross` · `monthly-ten`) ทับกัน ~20%
พอวัดด้วยเรขาคณิตจริงทั้ง 25 ผัง พบว่า**กระทบ 4 แบบ รวม 20 คู่** และสองผังที่ไม่มีใครรายงานหนักกว่าเดิมมาก:

| ผัง | ใบ | คู่ที่ทับ | ลึกสุด |
| :--- | --: | --: | --: |
| `chakra` | 7 | 6 | **52%** ของความสูงการ์ด |
| `year-ahead` | 12 | 8 | 44% |
| `celtic-cross` | 10 | 3 | 20% |
| `monthly-ten` | 10 | 3 | 20% |

**สาเหตุราก** — สเกลแกน y ถูกตรึงไว้ที่ `0.78` ตายตัวสำหรับทุกผัง
ขณะที่การ์ดสูง `0.195` · ผังเหล่านี้วางไพ่ห่างกันแกน y แค่ `0.20`
→ ระยะจริงบนจอ = `0.20 × 0.78 = 0.156` ซึ่ง **น้อยกว่าความสูงการ์ด** → ทับกันแน่นอนโดยคณิตศาสตร์

**สาเหตุที่หลุดมานาน (สำคัญกว่าตัวบั๊ก)** — ด่าน `test-spreads.ts` มีช่องโหว่ 2 ข้อ:
1. **คัดลอกตัวเลขไปเขียนไว้เอง** (`CARD_W = 0.13` · `Y_SCALE = 0.78`) แทนที่จะอ่านจากที่เดียวกับคอมโพเนนต์ที่วาดจริง → ด่านรับรอง "ผังในจินตนาการ"
2. **ตรวจเฉพาะใบไขว้** (`rotate: 90`) เทียบกับใบอื่น → ไพ่ธรรมดาที่เรียงลงคอลัมน์เดียวกันไม่เคยถูกตรวจเลยสักครั้ง

**ทางแก้** — สร้าง `src/lib/tarot/spread-map-geometry.ts` เป็นแหล่งความจริงเดียว
คำนวณสเกล y และขนาดการ์ดจากพิกัดจริงของแต่ละผัง (ใหญ่ที่สุดเท่าที่ยังไม่ทับกันและกรอบไม่สูงเกินเพดาน)
คอมโพเนนต์ที่วาดและด่านตรวจ **import ตัวเดียวกัน** จึงไม่มีทางเลื่อนออกจากกันได้อีก

| ผล | ค่า |
| :--- | :--- |
| ผังที่ต้องปรับ | 4 จาก 25 แบบ (ที่เหลืออีก 21 แบบไม่ขยับสักพิกเซล) |
| assertion ในด่าน | 541 → **1,295** |
| ยืนยัน | Chromium บน production build — วัดกล่องจริงทุกใบ ทับกัน 0 คู่ · ล้นกรอบ 0 ใบ |

#### 2. แถบยุบ/ขยาย 4 จุด — เลิกอนิเมต `height` (ของที่รอบ 13 พักไว้)

รอบ 13 ประเมินว่า "ไม่คุ้ม" เพราะทางเลือกที่คิดไว้ (`grid-template-rows: 0fr → 1fr`)
ต้องคาเนื้อหาไว้ใน DOM ตลอด · รอบนี้เจอทางที่สาม: **ให้กล่องกางเต็มความสูงทันที (layout รอบเดียว)
แล้วเลื่อนเนื้อหาขึ้น + จางเข้าด้วย `.anim-swap-rise-sm`** ซึ่ง compositor ทำเอง
ได้ทั้งความสมูทและไม่ต้องคาเนื้อหา — จาก ~60 รอบ layout ต่อการกดหนึ่งครั้ง เหลือ **1 รอบ**
(`CollapsibleCard` · `ProvablyFairPanel` 2 จุด · `ReadingHistoryModal`)

#### 3. ระเบิดเวลา `AnimatePresence mode="wait"` ที่ไม่มี `exit`

`ProvablyFairPanel` มี `mode="wait"` ที่**ไม่มี `exit` อยู่ข้างในเลยสักตัว**
`mode="wait"` มีหน้าที่เดียวคือ "รอตัวเก่าเล่นขาออกให้จบก่อน" — เมื่อไม่มีขาออกจึงไม่ได้ทำอะไรเลย
นอกจากแบกความเสี่ยงมาเปล่า ๆ · และนี่คือ**สาเหตุรากของ INC-0015** ที่เคยทำให้พิธีดูดวงค้างตายทั้งขั้น
ถอดออกแล้ว · ตรวจอีก 5 จุดที่ใช้ `mode="wait"` — ทุกจุดมี `exit` จริง ใช้ถูกต้องทั้งหมด

ผลรวมข้อ 2–3: `CollapsibleCard` และ `ProvablyFairPanel` **เลิกพึ่ง `motion` ทั้งไฟล์**

#### 4. ISSUE-038 · ธงสิทธิ์ที่ปิดค้าง — แก้ส่วนที่ AI ทำได้

การกดเปิดสวิตช์ต้องใช้สิทธิ์แอดมินของเจ้าของ (AI ทำแทนไม่ได้ · ยิง production จากที่นี่ก็โดน WAF ตอบ 403)
แต่**คำถามที่สำคัญกว่าคือ "ทำไมมันปิดค้างได้นานโดยไม่มีใครรู้"** — เพราะสถานะซ่อนอยู่ในแท็บ
ที่ต้องกดเข้าไปดูเอง ส่วนหน้าแรกของแผงแอดมินที่เปิดทุกวันเงียบสนิท

แก้ตรงนั้น: `/api/admin/system-health` รายงาน `services.entitlement` + `warnings`
และหน้าแรกของ `/admin` ขึ้น**แถบเตือนสีเหลืองพร้อมปุ่มไปแท็บที่แก้ได้ในคลิกเดียว** ตราบใดที่ธงยังปิด
เงื่อนไขที่ "ปิดอยู่แล้วเสียรายได้ทุกวัน" ต้องเด้งมาหาคน ไม่ใช่รอให้คนไปหามัน

> ⏳ **เหลือขั้นตอนเดียวที่ต้องให้เจ้าของกดเอง**: `/admin` → แท็บ "สิทธิ์เปิดไพ่" → เปิดสวิตช์

#### 5. ด่านตรวจ

- ด่านที่ 43 (`test-motion-quality.ts`) — **ล้าง ALLOWLIST เหลือศูนย์** และเพิ่ม **กฎที่ 7**: ห้าม `mode="wait"` ที่ไม่มี `exit`
...
**ไม่เหลืองานค้างในทะเบียนแล้ว** ยกเว้น ISSUE-038 ที่รอเจ้าของกดสวิตช์เอง

---

### 🗓️ 2026-09-09 (รอบ 13): ✦ ยกเครื่องโมชั่นทั้งเว็บให้ไหลลื่นแต่ไม่หนัก (INC-0117)

> **คำสั่งเจ้าของ**: "อัพเดทโมชั่นต่าง ๆ ทุกชุดให้ดูไหลลื่น สมูท แต่ไม่หนัก ใช้ให้คุ้มค่ามากที่สุด"

**สรุปสิ่งที่เจอ**: เว็บไม่ได้ขาดอนิเมชัน — มีเยอะและสวยอยู่แล้ว แต่ "จ่ายแพงเกินของที่ได้"
อยู่ 6 จุด และในนั้นมี **2 จุดที่ละเมิดกฎป้องกันถาวรที่เขียนไว้แล้วในเอกสาร**
(กฎที่ไม่มีเครื่องตรวจ = กฎที่ถูกละเมิดอีกแน่นอน — บทเรียนเดียวกับ `test-will-change.ts`)

| # | สิ่งที่พบ | ทำอะไร | ผลที่วัดได้ |
| :-- | :--- | :--- | :--- |
| **1** | `backdrop-filter` เต็มจอ **10 จุด** ทั้งที่ INC-0056 ห้ามไว้ชัด (เคยวัด fps 30 → 58 ตอนถอดออก) — ฉากหลังโมดัล 8 จุด · แผงเลือกผังที่ `motion` กำลังอนิเมต · หัวเว็บ sticky ของ `/admin` | ถอดออกทั้งหมด ใช้พื้นทึบ `.modal-scrim` (0.50 → 0.62 ชดเชยความรู้สึก) | ตรวจด้วย Chromium จริง 5 หน้า: element ที่คำนวณ `backdrop-filter` เหลือ **0** |
| **2** | `transition-all` **222 จุด** ใน 64 ไฟล์ + `transition: all` ใน `.altar-card-porcelain` (ใช้ซ้ำ 16 จุด · หน้า `/daily` ใบเดียวมี 11 กล่อง) | เปลี่ยนเป็น `transition` (ชุดคุณสมบัติปลอดภัยของ Tailwind ที่ไม่มี width/height/padding) · 3 จุดที่ตั้งใจยืดจริงระบุชื่อตรง ๆ `transition-[width,...]` | ตรวจจริง: `transition` ที่แตะ layout เหลือเฉพาะจุดจุดนำทาง 9 จุดที่ตั้งใจ |
| **3** | ลูป `repeat: Infinity` ของ `motion` วิ่งบนเธรดหลักตลอดเวลา — ป้าย "แตะเพื่อเปิด" อยู่บนไพ่**ทุกใบที่คว่ำหน้า** (ผังใหญ่ = 10 ลูปพร้อมกัน) และจุดพิมพ์ตอบ 3 จุดที่โผล่ตอนสตรีมคำทำนายพอดี | ย้ายเป็น CSS keyframes `.anim-badge-pulse` · `.anim-typing-dot` | อนิเมชันที่วิ่งบนหน้าไพ่ = 2 ตัว **เป็น transform/opacity ล้วนทั้งคู่** (compositor ทำเอง) |
| **4** | สปริง `glintX`/`glintY` 2 ตัวต่อไพ่ 1 ใบ ถูกอ่านด้วย `.get()` **ระหว่างเรนเดอร์** — ผิดหลัก React (INC-0053) และไม่ทำงานจริง ค่าค้างที่ 50% ตลอดมา | ถอดสปริงทิ้ง เขียน 50% ตรง ๆ | หน้าตาเหมือนเดิมเป๊ะ · ลดสปริง 2 ตัวต่อไพ่ทุกใบบนจอ |
| **5** | เอฟเฟกต์เอียงไพ่ตามเมาส์ยังทำงานบนมือถือ (เบราว์เซอร์สังเคราะห์ `mousemove` ตามหลังการแตะ) ทั้งที่ผู้ชม 85% เป็นมือถือและไม่ได้อะไรเลย | เช็ก `(pointer: fine)` ในตัวจับเหตุการณ์ (ห้ามอ่านตอนเรนเดอร์ — INC-0053) | มือถือไม่เสียเฟรมให้เอฟเฟกต์ที่มองไม่เห็น |
| **6** | แถบความคืบหน้าพิธีอนิเมต `width` เป็น % ผ่าน `motion` | เปลี่ยนเป็น `transform: scaleX()` + CSS transition | `RitualStepProgress.tsx` เลิกพึ่ง `motion` ทั้งไฟล์ |

**ของแถมที่เจอระหว่างทาง** — Tailwind v4 สแกนหาชื่อคลาสจากทั้งโปรเจกต์ และไม่แยกแยะว่าเป็น
คลาสจริงหรือแค่ "ชื่อคลาสที่ถูกเอ่ยถึงในคอมเมนต์/เอกสาร" · กฎที่เขียนว่า *"ห้ามใช้ `transition-all`"*
จึงเป็นตัวสั่งให้ Tailwind สร้างคลาสนั้นขึ้นมาเสียเอง · ตัด `docs/` กับ `scripts/` ออกจากการสแกนแล้ว

**ความสม่ำเสมอของจังหวะ** — ผูก `--default-transition-duration: 180ms` และ
`--default-transition-timing-function` เข้ากับโทเคนของบ้านนี้ที่เดียว
เดิมทุกจุดที่เขียน `transition` เฉย ๆ ใช้ค่าเริ่มต้นของ Tailwind (150ms + easing คนละตัวกับ `src/lib/motion.ts`)

**เคารพ `prefers-reduced-motion` ให้ครบวงจร** — ของเดิมตัดแค่ "ระยะเวลา"
เพิ่ม `animation-delay` · `transition-delay` · `scroll-behavior: auto`
(ตรวจจริงแล้ว: ป้ายที่เคยหมุน 2.2s เหลือ 1e-05s · `scroll-behavior` ของ `<html>` = `auto`)

**ด่านตรวจใหม่ (ด่านที่ 43)** — `scripts/qa/test-motion-quality.ts` คุม 6 กฎ
(ไม่มี `transition-all`/`transition: all` · ไม่มี backdrop filter · ลูปไม่รู้จบต้องเป็น CSS ·
ไม่มี `translate3d(0,0,0)` ที่ถูกตัวย่อ CSS กินทิ้ง · `motion` ห้ามอนิเมตคุณสมบัติเชิง layout ·
โทเคนจังหวะกลางต้องยังผูกอยู่) — **ทดสอบด่านด้วยการทำให้พังจริงครบ 7 เคส**

**ตัวเลขที่วัดจริง (เทียบ build บน `origin/main` `e7e009d`)**

| ตัววัด | ก่อน | หลัง | ผล |
| :--- | ---: | ---: | ---: |
| CSS ทั้งเว็บ (raw) | 135,475 B | 124,134 B | **−11,341 B (−8.4%)** |
| CSS ทั้งเว็บ (gzip) | 22,238 B | 20,809 B | **−1,429 B (−6.4%)** |
| JS ต่อหน้า (gzip) | 174–271 KB | 173–270 KB | ≈ **−1 KB ทุกหน้า** |
...
**ยังไม่ทำ (จงใจ · ขึ้นทะเบียนใน ALLOWLIST ของด่านที่ 43 แล้ว)** — แถบยุบ/ขยาย 3 จุด
(`CollapsibleCard` · `ProvablyFairPanel` · `ReadingHistoryModal`) ยังอนิเมต `height: auto`
ทางเลือกที่ไม่แตะ layout (`grid-template-rows: 0fr → 1fr`) ต้องคาเนื้อหาไว้ใน DOM ตลอด
= แลกน้ำหนัก DOM ตอนเปิดหน้าทุกครั้ง กับความสมูทของคลิกเดียว 240ms — ประเมินแล้วไม่คุ้ม

---

### 🗓️ 2026-09-09 (รอบ 12): ⚡ ลดคำขอที่ปลุก Worker 3 → 1 สำหรับผู้ชมที่ไม่ล็อกอิน (INC-0116)

> **คำสั่งเจ้าของ**: "ทำทุกข้อ ยอมทำแก้ทีเดียว ดีกว่ามาแก้ทีหลัง" (คนล็อกอินยังไม่เยอะ ยอมรับผลข้างเคียงได้)

**ตัวเลขก่อน–หลัง**

| ใคร | ก่อน | หลัง |
| :--- | :---: | :---: |
| ผู้ชมจาก Google (ไม่ล็อกอิน · เปิดหน้าเดียวแล้วออก) | **3** | **1** ✅ |
| ผู้ชมที่ไม่ล็อกอิน หลังกดเลือกผัง | 3 | 2 |
| ผู้ใช้ที่ล็อกอินอยู่ | 3 | **2** |

**3 อย่างที่ทำ (ทำทีเดียวจบตามที่เจ้าของสั่ง)**

**1️⃣ คุกกี้ใบ้เป็นตัวชี้ขาดตัวเดียว** — `session-hint.ts`
ของเดิมยังต้องถาม `/api/auth/me` **1 ครั้งต่อแท็บ** เพราะเครื่องหมายกันยิงซ้ำอยู่ใน `sessionStorage`
ซึ่งหายทุกครั้งที่ปิดแท็บ · ตอนนี้ **ไม่มีคุกกี้ใบ้ = ไม่มีเซสชัน = ไม่ถามเลย**
คุกกี้ใบ้อายุเท่าคุกกี้เซสชันเป๊ะ ๆ (30 วัน) เพราะ `hintCookieOptions()` ยืมค่าจาก `authCookieOptions()` ทั้งชุด

> ⚠️ **ผลข้างเคียงที่เจ้าของรับทราบและอนุมัติแล้ว**: บัญชีที่ล็อกอินไว้ **ก่อน 8 ก.ย.** (ก่อนมีคุกกี้ใบ้ · PR #356)
> จะถูกมองว่าไม่ได้ล็อกอิน ต้องล็อกอินใหม่หนึ่งครั้ง

**2️⃣ `/api/bootstrap` — ตอบผู้ใช้ + สิทธิ์ในคำขอเดียว**
ย้ายตรรกะเดิมออกมาเป็น `resolveSession()` และ `getEntitlementSnapshot()` ให้ทั้ง 3 เส้นทางใช้ก้อนเดียวกัน
> 🛡️ ด่านที่ 15 (`test-feature-gating`) ถูกอัปเดตให้ตรวจที่ `snapshot.ts` และบังคับเพิ่มว่า
> **ไฟล์ route ทั้งสองห้ามมีตรรกะสิทธิ์ของตัวเอง** ต้องเรียก `getEntitlementSnapshot()` เท่านั้น

**3️⃣ ผู้ชมที่ไม่ล็อกอินไม่ถามสิทธิ์ตอนเปิดหน้า**
ค่า `null` ทำให้ผังพรีเมียมขึ้น "ล็อก" อยู่แล้ว ซึ่ง **ตรงกับสิทธิ์จริงของผู้เยี่ยมชม** จึงไม่มีอะไรผิดพลาดบนจอ
แล้วเรียก `ensureEntitlement()` ตอนผู้ใช้ลงมือจริงแทน (เลือกผัง · กดเริ่ม · เปิดหน้าต่างเข้าสู่ระบบ)

> ⚠️ จุดสำคัญที่สุดของข้อนี้: `handleStartSession()` ต้อง **`await` ให้ได้คำตอบก่อนตัดสินเสมอ**
> ถ้าอ่านค่าจาก hook ตรง ๆ (ซึ่งอาจยังเป็น `null`) ผู้ใช้จะถูกพาเข้าขั้นตอนเปิดไพ่แล้วไปเจอ 403 กลางทาง

**📌 สิ่งเดียวที่ผู้ใช้จะสังเกตเห็น**
`AnnouncementBanner` (แถบประกาศระบบโควตา) จะไม่ขึ้นให้ผู้ชมที่ไม่ล็อกอินเห็น จนกว่าจะกดอะไรสักอย่าง
— ถ้าไม่ชอบจุดนี้ บอกได้ ย้อนเฉพาะข้อ 3 กลับได้โดยไม่กระทบข้อ 1–2

**ด่านที่ 42** — `scripts/qa/test-request-budget.ts`
**รันโค้ดจริง** พร้อมของปลอมของ `document.cookie` และ `fetch` แล้วนับจำนวนคำขอ
...
**พิสูจน์ว่าแก้ได้จริง**
- `npm run repo:verify` ➔ ✅ ผ่าน **40/42 ด่าน**
...
**ตรวจแล้ว**: `npm run repo:verify` ผ่าน 40/42 ด่าน · `npm run typecheck` 0 errors
...
- `npm run repo:verify` ➔ **ผ่านครบ 40/42 ด่าน**
...
**ผลตรวจ**: `npm run repo:verify` ➔ ✅ 40/42 ด่าน · `npm run typecheck` ➔ ✅ 0 errors
...
5. **การทดสอบและเกณฑ์คุณภาพ**:
   - `test-feature-gating.ts` ➔ ผ่านครบ 67/67 ข้อ
...
1. **พื้นหลังไม่ยึด viewport บน iOS** — ย้ายแสงจาก `background-image` + `background-attachment: fixed` บน `<body>`
   ...
   (iOS Safari ไม่รองรับ `background-attachment: fixed` จริง · ผู้ใช้มือถือคือ 85% ของเว็บ) ➔ **INC-0064**
