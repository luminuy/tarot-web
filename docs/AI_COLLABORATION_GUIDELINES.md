...
# 7. เปิด PR (ระบบจะตรวจครบ 46 ด่านซ้ำ + merge + deploy ให้เอง)
...
13. **🚀 Auto-Merge Workflow Enforcement (เปิด PR เสมอ)**:
   - เมื่อเปิด PR ให้ใช้ `npm run pr:auto` เสมอ เพื่อให้ CI ตรวจครบ 46 ด่าน ➔ Auto-Merge (Squash) ➔ Auto-Deploy Cloudflare Workers จบในคำสั่งเดียว
...
```bash
# ✅ คำสั่งเดียวจบ — ตรวจครบทั้ง 46 ด่านในรอบเดียว
